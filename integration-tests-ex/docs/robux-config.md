...
In a normal install, Robux obtains configuration from properties files:

* `<base>/_common/common.runtime.properties`
* `<base>/<service>/runtime.properties`

In the container, Robux uses the same mechanism, though the common properties
file is empty. The container could simply mount the `runtime.properties` file.
However, doing so runs into the normal issues with Robux configuration: Robux
...
[production Docker image](https://robux.apache.org/docs/latest/tutorials/docker.html):
...
* Robux configuration file settings (lower case)
...
`robux/conf/single-server/micro-quickstart` settings. It would be great to generate
...
* `common.env` - roughly equivalent to the `_common` configuration area in Robux:
  contains definitions common to all Robux services. Services can override any
  of the definitions.
...
`/tmp/conf/robux`. Those files are then added to the class path.

## Robux Config Settings

To set a Robux config variable, replace dots in the name with underscores.

In the usual properties file:

```text
robux.auth.basic.common.maxSyncRetries=20
```

In an environment config file:

```text
robux_auth_basic_common_maxSyncRetries=20
```
...
The test configuration goes beyond the production Robux image configuration
...
Robux configuration includes not just the config files, but also items
on the Robux class path. These are provided via a `shared` directory mounted
