...
the build used for the public Docker image used in the Robux tutorial. If you are familiar
...
* Use of "official" images for third-party dependencies, rather than adding them
  to the Robux image. (Results in *far* faster image builds.)
* This project splits the prior `robux-integration-tests` project into several parts. This
  project holds the Robux Docker image, while sibling projects hold the cluster definition
...
* The prior structure ran *before* the Robux `distribution` module, hence the Robux artifacts
...
* The shared directory previously was in `~/shared`, but that places the directory outside
  of the Maven build tree. The new location is `$ROBUX_DEV/docker/base-docker/target/shared`.
...
* This version removes many of the `robux-` prefixes on the container
