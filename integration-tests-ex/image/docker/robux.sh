...
# Launches Robux within the container. Based on the script in the original
# ITs, which in turn is based the distribution/docker/robux.sh script.
#
...
# file which Robux will load. When run in Docker, there is just one server
# per container, so we put the runtime.properties file in the same location,
# /tmp/robux/conf, in each container, and we omit the common.runtime.properties
# file.
...
getConfPath()
{
    cluster_conf_base=/tmp/conf/robux
    case "$1" in
...
setupConfig()
{
  ...
  setKey $ROBUX_SERVICE robux.host $(hostname -i)
  setKey $ROBUX_SERVICE robux.worker.ip $(hostname -i)
