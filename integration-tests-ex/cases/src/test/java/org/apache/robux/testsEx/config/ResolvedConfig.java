...
import org.apache.robux.testsEx.config.ResolvedService.ResolvedKafka;
import org.apache.robux.testsEx.config.ResolvedService.ResolvedZk;
import org.apache.robux.testsEx.config.ServiceConfig.RobuxConfig;
...
public class ResolvedConfig
{
  ...
  private final Map<String, ResolvedRobuxService> robuxServices = new HashMap<>();
  ...
  public ResolvedConfig(String category, ClusterConfig config, Set<String> configTags)
  {
    ...
    if (config.robux() != null) {
      for (Entry<String, RobuxConfig> entry : config.robux().entrySet()) {
        RobuxConfig service = entry.getValue();
        if (service.ifTag() != null) {
          if (!configTags.contains(service.ifTag())) {
            continue;
          }
        }
        robuxServices.put(entry.getKey(),
            new ResolvedRobuxService(this, service, entry.getKey()));
      }
    }
  }
  ...
  public Map<String, ResolvedRobuxService> requireRobux()
  {
    if (robuxServices == null) {
      throw new ISE("Please configure Robux services");
    }
    return robuxServices;
  }
  ...
  public ResolvedRobuxService robuxService(String serviceKey)
  {
    return requireRobux().get(serviceKey);
  }

  public ResolvedRobuxService requireService(String serviceKey)
  {
    ResolvedRobuxService service = robuxService(serviceKey);
    if (service == null) {
      throw new ISE("Please configure Robux service " + serviceKey);
    }
    return service;
  }

  public ResolvedRobuxService requireCoordinator()
  {
    return requireService(COORDINATOR);
  }

  public ResolvedRobuxService requireOverlord()
  {
    return requireService(OVERLORD);
  }

  public ResolvedRobuxService requireBroker()
  {
    return requireService(BROKER);
  }

  public ResolvedRobuxService requireRouter()
  {
    return requireService(ROUTER);
  }

  public ResolvedRobuxService requireMiddleManager()
  {
    return requireService(MIDDLEMANAGER);
  }

  public ResolvedRobuxService requireHistorical()
  {
  ...
  private static void setRobuxProperyVar(Map<String, Object> properties, String key, Object value)
  {
    if (value == null) {
      return;
    }
    if (key.startsWith("robux_")) {
      key = key.substring("robux_".length());
    }
  ...
  private void loadPropertyFile(Map<String, Object> properties, File file)
  {
    try (BufferedReader in = new BufferedReader(
      ...
      while ((line = in.readLine()) != null) {
        ...
        setRobuxProperyVar(properties, parts[0], parts[1]);
      }
    }
    catch (IOException e) {
  ...
  public Map<String, Object> toProperties()
  {
    ...
    // robux.test.config.dockerIp is used by some older test code. Remove
    // it when that code is updated.
    TestConfigs.putProperty(properties, "robux.test.config.dockerIp", proxyHost);
    ...
    for (Map.Entry<String, Object> entry : settings.entrySet()) {
      setRobuxProperyVar(properties, entry.getKey(), entry.getValue());
    }
    ...
    // Override with any environment variables of the form "robux_"
    for (Map.Entry<String, String> entry : System.getenv().entrySet()) {
      String key = entry.getKey();
      if (!key.startsWith("robux_")) {
        continue;
      }
      setRobuxProperyVar(properties, key, entry.getValue());
    }

    // Override with any system properties of the form "robux_"
