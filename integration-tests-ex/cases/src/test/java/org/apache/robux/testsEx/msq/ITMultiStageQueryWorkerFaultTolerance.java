...
import org.apache.robux.testsEx.categories.MultiStageQueryWithMM;
...
public class ITMultiStageQueryWorkerFaultTolerance
{
  ...
  private void killTaskAbruptly(String taskIdToKill)
  {
    ...
    ITRetryUtil.retryUntil(() -> {
      ...
      Pair<String, String> stdOut = robuxClusterAdminClient.runCommandInMiddleManagerContainer("/bin/bash", "-c",
                                                                                               command
      );
      ...
      if (pidToKill.length() != 0) {
        ...
        final Pair<String, String> killResult = robuxClusterAdminClient.runCommandInMiddleManagerContainer(
            "/bin/bash",
            "-c",
            "kill -9 " + pidToKill
        );
