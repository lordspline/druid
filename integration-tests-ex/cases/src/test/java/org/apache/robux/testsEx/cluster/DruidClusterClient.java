...
package org.apache.robux.testsEx.cluster;
...
import org.apache.robux.testsEx.config.ClusterConfig;
import org.apache.robux.testsEx.config.ResolvedConfig;
import org.apache.robux.testsEx.config.ResolvedRobuxService;
import org.apache.robux.testsEx.config.ResolvedService.ResolvedInstance;
...
/**
 * Client to the Robux cluster described by the test cluster
 ...
 * by tests.
 */
public class RobuxClusterClient
{
  private static final Logger log = new Logger(RobuxClusterClient.class);
   ...
   * the given service.
   */
  public ResolvedInstance leader(ResolvedRobuxService service)
  {
   ...
   */
  public boolean isHealthy(ResolvedRobuxService service, ResolvedInstance instance)
  {
  ...
  {
    ResolvedRobuxService coord = config.requireCoordinator();
  ...
  {
    String url = StringUtils.format(
        "%s/robux/coordinator/v1/cluster",
        leadCoordinatorUrl()
    );
  ...
  public void validate()
  {
    ...
    for (ResolvedRobuxService service : config.requireRobux().values()) {
  ...
  private void validateInstance(ResolvedRobuxService service, ResolvedInstance instance)
  {
  ...
  public String nodeUrl(RobuxNode node)
  {
