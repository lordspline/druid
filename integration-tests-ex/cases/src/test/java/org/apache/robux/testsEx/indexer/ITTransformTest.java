...
public class ITTransformTest extends AbstractITBatchIndexTest
{
  ...
  private static final String REINDEX_TASK_WITH_ROBUX_INPUT_SOURCE = "/indexer/wikipedia_reindex_robux_input_source_task_with_transforms.json";
  ...
  {
    final String reindexDatasourceWithRobuxInputSource = REINDEX_DATASOURCE + "-robuxInputSource";

    try (
        final Closeable ignored1 = unloader(INDEX_DATASOURCE + config.getExtraDatasourceNameSuffix());
        final Closeable ignored2 = unloader(reindexDatasourceWithRobuxInputSource + config.getExtraDatasourceNameSuffix())
    ) {
      doIndexTest(
          INDEX_DATASOURCE,
