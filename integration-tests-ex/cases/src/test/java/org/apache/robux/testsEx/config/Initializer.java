...
import org.apache.robux.cli.GuiceRunnable;
...
import org.apache.robux.java.util.emitter.core.LoggingEmitter;
import org.apache.robux.java.util.emitter.core.LoggingEmitterConfig;
...
import org.apache.robux.metadata.storage.mysql.MySQLConnector;
import org.apache.robux.metadata.storage.mysql.MySQLConnectorDriverConfig;
import org.apache.robux.metadata.storage.mysql.MySQLConnectorSslConfig;
import org.apache.robux.metadata.storage.mysql.MySQLMetadataStorageModule;
...
import org.apache.robux.testing.clients.AdminClient;
import org.apache.robux.testing.guice.TestClient;
import org.apache.robux.testsEx.cluster.RobuxClusterClient;
import org.apache.robux.testsEx.cluster.MetastoreClient;
...
/**
 * The magic needed to piece together enough of Robux to allow clients to
 ...
 * configuration file.
 * <p>
 * Much of the work here deals the tedious task of assembling Robux
...
public class Initializer
{
  ...
  private static class TestModule implements RobuxModule
  {
    ...
    public void configure(Binder binder)
    {
      ...
      // Dummy RobuxNode instance to make Guice happy. This instance is unused.
      RobuxNode dummy = new RobuxNode("integration-tests", "localhost", false, 9191, null, null, true, false);
      binder
          .bind(RobuxNode.class)
    ...
    // From ServerModule to allow deserialization of DiscoveryRobuxNode objects from ZK.
   ...
   */
  private static class TestMySqlModule implements RobuxModule
  {
    @Override
    public void configure(Binder binder)
    {
      // Reduced form of SQLMetadataStorageRobuxModule
      String prop = SQLMetadataStorageRobuxModule.PROPERTY;
  ...
  public static class Builder
  {
    ...
    public Builder(String clusterName)
    {
      ...
      eagerInstance(RobuxNodeDiscoveryProvider.class);

      // Set properties from environment variables, or hard-coded values
      // previously set in Maven.
      propertyEnvVarBinding("robux.test.config.dockerIp", "DOCKER_IP");
      propertyEnvVarBinding("robux.zk.service.host", "DOCKER_IP");
      property("robux.client.https.trustStorePath", "client_tls/truststore.jks");
      property("robux.client.https.trustStorePassword", "robux123");
      property("robux.client.https.keyStorePath", "client_tls/client.jks");
      property("robux.client.https.certAlias", "robux");
      property("robux.client.https.keyManagerPassword", "robux123");
      property("robux.client.https.keyStorePassword", "robux123");
      propertyEnvVarBinding("robux.metadata.mysql.driver.driverClassName", "MYSQL_DRIVER_CLASSNAME");
      ...
      propertyEnvVarBinding(base + "cloudBucket", "ROBUX_CLOUD_BUCKET");
      propertyEnvVarBinding(base + "cloudPath", "ROBUX_CLOUD_PATH");
      ...
      // Other defaults
      // robux.global.http.numMaxThreads avoids creating 40+ Netty threads.
      // We only ever use 1.
      property("robux.global.http.numMaxThreads", 3);
      property("robux.broker.http.numMaxThreads", 3);
    }
    ...
    /**
     * Robux provides the {@code PolyBind} abstraction and the {@code Lifecycle}
     ...
     * <p>
     * The builder registers {@code RobuxNodeDiscoveryProvider} by default: add any
     ...
     * Robux service reports itself as healthy. Since Robux services
  ...
  private Initializer(Builder builder)
  {
    ...
    this.clusterClient = this.injector.getInstance(RobuxClusterClient.class);
  ...
  public RobuxClusterClient clusterClient()
  {
