...
public class DartControllerRegistry
{
  ...
  /**
   * Add a controller. Throws {@link RobuxException} if a controller with the same {@link Controller#queryId()} is
   * already registered.
   */
  public void register(ControllerHolder holder)
  {
    if (controllerMap.putIfAbsent(holder.getController().queryId(), holder) != null) {
      throw RobuxException.defensive("Controller[%s] already registered", holder.getController().queryId());
    }
  }
