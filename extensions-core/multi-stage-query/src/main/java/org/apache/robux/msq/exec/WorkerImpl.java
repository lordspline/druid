...
import org.apache.robux.msq.indexing.error.MSQWarningReportPublisher;
import org.apache.robux.msq.indexing.error.MSQWarningReportSimplePublisher;
import org.apache.robux.msq.indexing.error.MSQWarnings;
import org.apache.robux.msq.input.InputSlices;
import org.apache.robux.msq.input.stage.ReadablePartition;
...
import org.apache.robux.msq.kernel.controller.ControllerQueryKernelUtils;
import org.apache.robux.msq.kernel.worker.WorkerStageKernel;
import org.apache.robux.msq.kernel.worker.WorkerStagePhase;
import org.apache.robux.msq.shuffle.input.DurableStorageInputChannelFactory;
import org.apache.robux.msq.shuffle.input.MetaInputChannelFactory;
import org.apache.robux.msq.shuffle.input.WorkerInputChannelFactory;
import org.apache.robux.msq.shuffle.input.WorkerOrLocalInputChannelFactory;
import org.apache.robux.msq.shuffle.output.StageOutputHolder;
import org.apache.robux.msq.statistics.ClusterByStatisticsSnapshot;
import org.apache.robux.msq.statistics.PartialKeyStatisticsInformation;
import org.apache.robux.msq.util.DecoratedExecutorService;
...
import org.apache.robux.query.PrioritizedCallable;
import org.apache.robux.query.PrioritizedRunnable;
...
public class WorkerImpl implements Worker
{
  ...
  private final RobuxNode selfRobuxNode;
  ...
  private void reportQueryMetrics(boolean success, long time)
  {
    ...
    final Set<String> datasources = (Set<String>) queryMetricDimensions.get(RobuxMetrics.DATASOURCE);
    final Set<Interval> intervals = (Set<Interval>) queryMetricDimensions.get(RobuxMetrics.INTERVAL);
      ...
      for (final KernelHolder kernelHolder : kernelHolders.getAllKernelHolders()) {
        ...
        if (kernel.getPhase() == WorkerStagePhase.FAILED) {
          ...
          return Optional.of(
              MSQErrorReport.fromException(
                  id(),
                  MSQTasks.getHostFromSelfNode(selfRobuxNode),
  ...
  {
    Set<String> datasourceDim = (Set<String>) queryMetricDimensions.computeIfAbsent(RobuxMetrics.DATASOURCE, s -> new HashSet<>());
    Set<Interval> intervalDim = (Set<Interval>) queryMetricDimensions.computeIfAbsent(RobuxMetrics.INTERVAL, s -> new HashSet<>());
    ...
    return MetaInputChannelFactory.create(
        ...
        outputChannelMode -> {
          switch (outputChannelMode) {
            ...
            default:
              throw RobuxException.defensive("No handling for output channel mode[%s]", outputChannelMode);
          }
        }
    );
  }
   ...
   * {@link org.apache.robux.guice.RobuxProcessingModule}.
   */
