...
public class IndexerDataServerQueryHandler implements DataServerQueryHandler
{
  ...
  {
    final ServiceLocation serviceLocation = ServiceLocation.fromRobuxServerMetadata(requestDescriptor.getServerMetadata());
  ...
  {
    final Map<RobuxServerMetadata, Set<RichSegmentDescriptor>> serverVsSegmentsMap = new HashMap<>();
    ...
    for (RichSegmentDescriptor richSegmentDescriptor : richSegmentDescriptors) {
      ...
      if (segmentLoadInfo.getSegment().toDescriptor().equals(segmentDescriptorWithFullInterval)) {
        Set<RobuxServerMetadata> servers = segmentLoadInfo.getServers()
                                                          .stream()
                                                          .filter(robuxServerMetadata -> SegmentSource.REALTIME.getUsedServerTypes()
                                                                                                               .contains(
                                                                                                                   robuxServerMetadata.getType()))
        ...
        RobuxServerMetadata robuxServerMetadata = DataServerSelector.RANDOM.getSelectServerFunction().apply(servers);
        ...
        serverVsSegmentsMap.get(robuxServerMetadata)
    ...
    for (Map.Entry<RobuxServerMetadata, Set<RichSegmentDescriptor>> robuxServerMetadataSetEntry : serverVsSegmentsMap.entrySet()) {
   ...
   * <br>
   * See {@link  org.apache.robux.server.http.DataSourcesResource#isHandOffComplete(String, String, int, String)}
   */
