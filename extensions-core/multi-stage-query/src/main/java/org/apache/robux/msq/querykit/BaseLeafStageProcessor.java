...
import org.apache.robux.msq.exec.std.BasicStandardStageProcessor;
import org.apache.robux.msq.exec.std.ProcessorsAndChannels;
...
/**
 * Base class of frame processors that can read regular Robux segments, external data, *or* channels from
...
public abstract class BaseLeafStageProcessor extends BasicStandardStageProcessor
{
  ...
  {
    // BaseLeafStageProcessor is used for native Robux queries, where the following input cases can happen:
    ...
    final Function<List<SegmentMapFunction>, ProcessorManager<Object, Long>> processorManagerFn = segmentMapFnList -> {
      final SegmentMapFunction segmentMapFunction =
          CollectionUtils.getOnlyElement(segmentMapFnList, throwable -> RobuxException.defensive("Only one segment map function expected"));
