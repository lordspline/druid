...
import org.apache.robux.frame.processor.FrameProcessors;
...
public class SqlStatementResourceHelper
{
  ...
  public static void isMSQPayload(TaskPayloadResponse taskPayloadResponse, String queryId) throws RobuxException
  {
  ...
  )
  {
    ...
    if (totalPartitions == -1) {
      throw RobuxException.defensive("Expected partition count to be set for stage[%d]", finalStage);
    }
    if (totalWorkerCount == -1) {
      throw RobuxException.defensive("Expected worker count to be set for stage[%d]", finalStage);
    }
  ...
  )
  {
    ...
    return Optional.of(new SqlStatementResult(
        ...
        taskResponse.getStatus().getDuration(),
        null,
        RobuxException.fromFailure(new RobuxException.Failure(errorCode)
        {
          @Override
          protected RobuxException makeException(RobuxException.RobuxExceptionBuilder bob)
          {
            RobuxException ex = bob.forPersona(RobuxException.Persona.USER)
                                   .ofCategory(RobuxException.Category.UNCATEGORIZED)
                                   .build(errorMessage);
  ...
  private static Map<String, String> buildExceptionContext(MSQFault fault, ObjectMapper mapper)
  {
    try {
      ...
      throw RobuxException.defensive("Could not read MSQFault[%s] as a map: [%s]", fault, e.getMessage());
    }
  }
}
