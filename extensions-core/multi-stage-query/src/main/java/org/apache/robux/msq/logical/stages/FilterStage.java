...
import org.apache.robux.sql.calcite.rel.Projection;
...
class FilterStage extends ReadStage
{
  ...
  {
    if (stack.getNode() instanceof RobuxProject) {
      RobuxProject project = (RobuxProject) stack.getNode();
    ...
    if (stack.getNode() instanceof RobuxAggregate) {
      return new ProjectStage(
          this,
          virtualColumnRegistry,
          signature
