...
package org.apache.robux.msq.indexing.error;
...
public class MSQErrorReport
{
  ...
  /**
   * Returns a {@link RobuxException} "equivalent" of this instance. This is useful until such time as we can migrate
   * usages of this class to {@link RobuxException}.
   */
  public RobuxException toRobuxException()
  {
    final RobuxException robuxException =
        error.toRobuxException()
             .withContext("taskId", taskId);

    if (host != null) {
      robuxException.withContext("host", host);
    }

    if (stageNumber != null) {
      robuxException.withContext("stageNumber", stageNumber);
    }

    if (exceptionStackTrace != null) {
      robuxException.withContext("exceptionStackTrace", exceptionStackTrace);
    }

    return robuxException;
  }
   ...
   * types thrown by other Robux code.
   */
  ...
  )
  {
    ...
    while (cause != null) {
      ...
      } else if (cause.getClass().getPackage().getName().startsWith("org.apache.robux.query")) {
