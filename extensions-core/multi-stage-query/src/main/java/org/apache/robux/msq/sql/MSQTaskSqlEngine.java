...
import org.apache.robux.msq.util.ArrayIngestMode;
...
import org.apache.robux.sql.calcite.run.SqlEngines;
...
public class MSQTaskSqlEngine implements SqlEngine
{
  ...
  public static final Set<String> SYSTEM_CONTEXT_PARAMETERS =
      ImmutableSet.<String>builder()
                  ...
                  .add(RobuxSqlIngest.SQL_EXPORT_FILE_FORMAT)
  ...
  )
  {
    validateInsert(
        ...
        ? plannerContext.getPlannerToolbox()
                        .rootSchema()
                        .getNamedSchema(plannerContext.getPlannerToolbox().robuxSchemaName())
                        .getSchema()
  ...
  /**
   * Checks if the SELECT contains {@link RobuxSqlInsert#SQL_INSERT_SEGMENT_GRANULARITY} in the context. This is a
   * defensive cheeck because {@link org.apache.robux.sql.calcite.planner.RobuxPlanner} should have called the
   * {@link #validateContext}
   */
  private static void validateSelect(final PlannerContext plannerContext)
  {
    if (plannerContext.queryContext().containsKey(RobuxSqlInsert.SQL_INSERT_SEGMENT_GRANULARITY)) {
      throw RobuxException
          ...
          .build(
              ...
              + "by Robux for INSERT queries. If the user is seeing this exception, that means there's a bug in Robux "
  ...
  )
  {
    ...
    for (Entry<Integer, String> fieldMapping : fieldMappings) {
      ...
      final RelDataTypeField oldSqlTypeField =
          targetTable.getRowType(RobuxTypeSystem.TYPE_FACTORY).getField(columnName, true, false);

      if (!columnsExcludedFromTypeVerification.contains(columnName) && oldSqlTypeField != null) {
        final ColumnType oldRobuxType = Calcites.getColumnTypeForRelDataType(oldSqlTypeField.getType());
        final RelDataType newSqlType = rootRel.getRowType().getFieldList().get(columnIndex).getType();
        final ColumnType newRobuxType =
        ...
        if (newRobuxType.isArray() && oldRobuxType.is(ValueType.STRING)
            || (newRobuxType.is(ValueType.STRING) && oldRobuxType.isArray())) {
          ...
          if (newRobuxType.is(ValueType.STRING)
            ...
            messageBuilder.append(". Try setting arrayIngestMode to[array] to retain the SQL type[")
                          .append(newSqlType)
                          .append("]");
          } else if (newRobuxType.is(ValueType.ARRAY)
                     && oldRobuxType.is(ValueType.STRING)
          ...
          } else if (newRobuxType.is(ValueType.STRING) && oldRobuxType.is(ValueType.ARRAY)) {
          ...
          messageBuilder.append(". See https://robux.apache.org/docs/latest/querying/arrays#arrayingestmode "
  ...
  private static Granularity getSegmentGranularity(final PlannerContext plannerContext)
  {
    try {
      ...
      // This is a defensive check as the RobuxSqlInsert.SQL_INSERT_SEGMENT_GRANULARITY in the query context is
      // populated by Robux. If the user entered an incorrect granularity, that should have been flagged before reaching
      // here.
      throw RobuxException.forPersona(RobuxException.Persona.DEVELOPER)
                          .ofCategory(RobuxException.Category.DEFENSIVE)
                          .build(
                              e,
                              "[%s] is not a valid value for [%s]",
                              plannerContext.queryContext().get(RobuxSqlInsert.SQL_INSERT_SEGMENT_GRANULARITY),
                              RobuxSqlInsert.SQL_INSERT_SEGMENT_GRANULARITY
                          );
