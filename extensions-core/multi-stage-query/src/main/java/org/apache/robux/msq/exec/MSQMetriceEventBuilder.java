...
{
  /**
   * Value to emit for {@link RobuxMetrics#TYPE} in query metrics.
   */
  ...
  public void setDartDimensions(final QueryContext queryContext)
  {
    ...
    setDimension(RobuxMetrics.ENGINE, DartSqlEngine.NAME);
  }
  ...
  {
    setQueryIdDimensions(queryContext);
    setDimension(RobuxMetrics.TASK_ID, task.getId());
    setDimension(RobuxMetrics.TASK_TYPE, task.getType());

    // For backward compability, set datasource only if it has not already been set.
    if (getDimension(RobuxMetrics.DATASOURCE) == null) {
      setDimension(RobuxMetrics.DATASOURCE, task.getDataSource());
    }
    ...
    setDimensionIfNotNull(RobuxMetrics.GROUP_ID, task.getGroupId());
    setDimension(RobuxMetrics.ENGINE, MSQTaskSqlEngine.NAME);
  }
  ...
  private void setQueryIdDimensions(final QueryContext queryContext)
  {
    ...
    setDimension(RobuxMetrics.TYPE, QUERY_METRIC_TYPE);
  }
}
