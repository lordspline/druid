...
public class DartControllerContext implements ControllerContext
{
  ...
  {
    final List<RobuxServerMetadata> servers = serverView.getRobuxServerMetadatas();
    ...
    for (final RobuxServerMetadata server : servers) {
      if (server.getType() == ServerType.HISTORICAL) {
        workerIds.add(WorkerId.fromRobuxServerMetadata(server, queryId()).toString());
      }
    }
  ...
  {
    throw RobuxException.defensive("TaskLockType is not used with class[%s]", getClass().getName());
  }
}
