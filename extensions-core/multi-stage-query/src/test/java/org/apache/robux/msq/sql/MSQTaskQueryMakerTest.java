...
import static org.apache.robux.sql.calcite.BaseCalciteQueryTest.expressionVirtualColumn;
import static org.apache.robux.sql.calcite.table.RowSignatures.toRelDataType;
...
public class MSQTaskQueryMakerTest
{
  ...
  public void setUp() throws Exception
  {
    ...
    RobuxSecondaryModule.setupJackson(injector, objectMapper);
  ...
  public void testSimpleScanQuery() throws Exception
  {
    ...
    Query query = new Robuxs.ScanQueryBuilder().resultFormat(ScanQuery.ResultFormat.RESULT_FORMAT_COMPACTED_LIST)
                                               .eternityInterval()
                                               .dataSource(CalciteTests.DATASOURCE1)
                                               .columns(resultSignature.getColumnNames())
                                               .columnTypes(resultSignature.getColumnTypes())
                                               .build();
    RobuxQuery robuxQueryMock = buildRobuxQueryMock(query, resultSignature);
    // Act
    msqTaskQueryMaker = getMSQTaskQueryMaker();
    QueryResponse<Object[]> response = msqTaskQueryMaker.runQuery(robuxQueryMock);
    // Assert
  ...
  public void testJoinFailWithPolicyValidationOnLeftChild() throws Exception
  {
    ...
    Query queryLeftChildNoRestriction = new Robuxs.ScanQueryBuilder().resultFormat(ScanQuery.ResultFormat.RESULT_FORMAT_COMPACTED_LIST)
                                                                     .eternityInterval()
                                                                     .dataSource(joinDataSourceLeftChildNoRestriction)
                                                                     .columns(resultSignature.getColumnNames())
                                                                     .columnTypes(resultSignature.getColumnTypes())
                                                                     .build();
    RobuxQuery robuxQueryMock = buildRobuxQueryMock(queryLeftChildNoRestriction, resultSignature);
    // Act
    msqTaskQueryMaker = getMSQTaskQueryMaker();
    QueryResponse<Object[]> response = msqTaskQueryMaker.runQuery(robuxQueryMock);
    // Assert
  ...
  public void testJoinFailWithPolicyValidationOnRightChild() throws Exception
  {
    ...
    Query queryRightChildNoRestriction = new Robuxs.ScanQueryBuilder().resultFormat(ScanQuery.ResultFormat.RESULT_FORMAT_COMPACTED_LIST)
                                                                      .eternityInterval()
                                                                      .dataSource(joinDataSourceRightChildNoRestriction)
                                                                      .columns(resultSignature.getColumnNames())
                                                                      .columnTypes(resultSignature.getColumnTypes())
                                                                      .build();
    RobuxQuery robuxQueryMock = buildRobuxQueryMock(queryRightChildNoRestriction, resultSignature);
    // Act
    msqTaskQueryMaker = getMSQTaskQueryMaker();
    QueryResponse<Object[]> response = msqTaskQueryMaker.runQuery(robuxQueryMock);
    // Assert
  ...
  private static RobuxQuery buildRobuxQueryMock(Query query, RowSignature resultSignature)
  {
    RobuxQuery robuxQueryMock = Mockito.mock(RobuxQuery.class);
    when(robuxQueryMock.getQuery()).thenReturn(query);
    when(robuxQueryMock.getDataSource()).thenReturn(query.getDataSource());
    when(robuxQueryMock.getOutputRowSignature()).thenReturn(resultSignature);
    when(robuxQueryMock.getOutputRowType()).thenReturn(toRelDataType(resultSignature, JAVA_TYPE_FACTORY, false));
    return robuxQueryMock;
  }
  ...
  private MSQTaskQueryMaker getMSQTaskQueryMaker()
  {
    ...
    return new MSQTaskQueryMaker(
        ...
        new MSQTaskQueryKitSpecFactory(new RobuxProcessingConfig())
    );
  }
}
