...
import org.apache.robux.query.aggregation.datasketches.hll.HllSketchAggregatorFactory;
...
public abstract class HllSketchBaseSqlAggregator implements SqlAggregator
{
  ...
  )
  {
    ...
    final RobuxExpression columnArg = Expressions.toRobuxExpression(plannerContext, inputAccessor.getInputRowSignature(), columnRexNode);
    if (columnArg == null) {
      return null;
    }
    ...
    if (columnArg.isDirectColumnAccess()
      ...
      if (inputType == null) {
        throw new ISE(
            "Cannot translate sqlTypeName[%s] to Robux type for field[%s]",
