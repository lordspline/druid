...
import org.apache.robux.query.aggregation.AggregatorAndSize;
...
import org.apache.robux.query.aggregation.datasketches.theta.oldapi.OldSketchBuildAggregatorFactory;
import org.apache.robux.query.aggregation.datasketches.theta.oldapi.OldSketchMergeAggregatorFactory;
...
import org.apache.robux.segment.TestColumnSelectorFactory;
...
import org.apache.robux.segment.vector.TestVectorColumnSelectorFactory;
...
public class SketchAggregatorFactoryTest
{
  ...
  {
    Throwable exception = Assert.assertThrows(RobuxException.class, () -> AGGREGATOR_16384.factorize(metricFactory));
  ...
  {
    Throwable exception = Assert.assertThrows(RobuxException.class, () -> AGGREGATOR_16384.factorizeWithSize(metricFactory));
    Assert.assertEquals("Unsupported input [x] of type [COMPLEX<json>] for aggregator [COMPLEX<thetaSketchBuild>].", exception.getMessage());
  }
  ...
  {
    Throwable exception = Assert.assertThrows(RobuxException.class, () -> AGGREGATOR_16384.factorizeBuffered(metricFactory));
    Assert.assertEquals("Unsupported input [x] of type [COMPLEX<json>] for aggregator [COMPLEX<thetaSketchBuild>].", exception.getMessage());
  }
  ...
  {
    Throwable exception = Assert.assertThrows(RobuxException.class, () -> AGGREGATOR_16384.factorizeVector(vectorFactory));
    Assert.assertEquals("Unsupported input [x] of type [COMPLEX<json>] for aggregator [COMPLEX<thetaSketchBuild>].", exception.getMessage());
  }
