...
package org.apache.robux.query.aggregation.datasketches.theta.sql;
...
import org.apache.robux.query.aggregation.datasketches.theta.SketchEstimatePostAggregator;
import org.apache.robux.query.aggregation.datasketches.theta.SketchMergeAggregatorFactory;
import org.apache.robux.query.aggregation.datasketches.theta.SketchModule;
import org.apache.robux.query.aggregation.datasketches.theta.SketchSetPostAggregator;
import org.apache.robux.query.aggregation.datasketches.theta.sql.ThetaSketchSqlAggregatorTest.ThetaSketchComponentSupplier;
...
public class ThetaSketchSqlAggregatorTest extends BaseCalciteQueryTest
{
  ...
  public static class ThetaSketchComponentSupplier extends StandardComponentSupplier
  {
    ...
    {
      return RobuxModuleCollection.of(super.getCoreModule(), new SketchModule());
    }
  ...
  public void testAvgDailyCountDistinctThetaSketch()
  {
    ...
    testQuery(
        "SELECT\n"
        + "  AVG(u)\n"
        + "FROM (SELECT FLOOR(__time TO DAY), APPROX_COUNT_DISTINCT_DS_THETA(cnt) AS u FROM robux.foo GROUP BY 1)",
        ImmutableList.of(
  ...
  public void testApproxCountDistinctFunctionOnUnsupportedComplexColumn()
  {
    RobuxException robuxException = Assert.assertThrows(
        ...
        () -> testQuery(
            "SELECT APPROX_COUNT_DISTINCT_DS_THETA(double_first_added) FROM robux.wikipedia_first_last",
            ImmutableList.of(),
            ImmutableList.of()
        )
    );
  ...
  {
    final String sql = "SELECT DS_THETA(dim2) as y FROM robux.foo ORDER BY THETA_SKETCH_ESTIMATE(DS_THETA(dim2)) DESC LIMIT 10";
