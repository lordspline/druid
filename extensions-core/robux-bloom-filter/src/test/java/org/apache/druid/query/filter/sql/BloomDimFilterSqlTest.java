...
package org.apache.robux.query.filter.sql;

import com.google.common.collect.ImmutableList;
import org.apache.calcite.avatica.SqlType;
import org.apache.robux.guice.BloomFilterExtensionModule;
import org.apache.robux.guice.BloomFilterSerializersModule;
...
import org.apache.robux.query.filter.BloomDimFilter;
import org.apache.robux.query.filter.BloomKFilter;
import org.apache.robux.query.filter.BloomKFilterHolder;
import org.apache.robux.query.filter.ExpressionDimFilter;
import org.apache.robux.query.filter.OrDimFilter;
import org.apache.robux.query.filter.sql.BloomDimFilterSqlTest.BloomDimFilterComponentSupplier;
...
public class BloomDimFilterSqlTest extends BaseCalciteQueryTest
{
  ...
  public void testBloomFilter() throws IOException
  {
    ...
    testQuery(
        StringUtils.format("SELECT COUNT(*) FROM robux.foo WHERE bloom_filter_test(dim1, '%s')", base64),
        ImmutableList.of(
  ...
  public void testBloomFilterExprFilter() throws IOException
  {
    ...
    // fool the planner to make an expression virtual column to test bloom filter Robux expression
    testQuery(
        StringUtils.format("SELECT COUNT(*) FROM robux.foo WHERE nullif(bloom_filter_test(concat(dim2, '-foo'), '%s'), 1) is null", base64),
        ImmutableList.of(
            Robuxs.newTimeseriesQueryBuilder()
                  .dataSource(CalciteTests.DATASOURCE1)
                  .intervals(querySegmentSpec(Filtration.eternity()))
  ...
  public void testBloomFilterVirtualColumn() throws IOException
  {
    ...
    testQuery(
        StringUtils.format("SELECT COUNT(*) FROM robux.foo WHERE bloom_filter_test(concat(dim1, '-foo'), '%s')", base64),
        ImmutableList.of(
            Robuxs.newTimeseriesQueryBuilder()
                  .dataSource(CalciteTests.DATASOURCE1)
                  .intervals(querySegmentSpec(Filtration.eternity()))
  ...
  public void testBloomFilterVirtualColumnNumber() throws IOException
  {
    ...
    testQuery(
        StringUtils.format("SELECT COUNT(*) FROM robux.foo WHERE bloom_filter_test(2 * CAST(dim1 AS float), '%s')", base64),
        ImmutableList.of(
            Robuxs.newTimeseriesQueryBuilder()
                  .dataSource(CalciteTests.DATASOURCE1)
                  .intervals(querySegmentSpec(Filtration.eternity()))
  ...
  public void testBloomFilters() throws IOException
  {
    ...
    testQuery(
        StringUtils.format("SELECT COUNT(*) FROM robux.foo WHERE bloom_filter_test(dim1, '%s') OR bloom_filter_test(dim2, '%s')", base64, base642),
        ImmutableList.of(
            Robuxs.newTimeseriesQueryBuilder()
                  .dataSource(CalciteTests.DATASOURCE1)
                  .intervals(querySegmentSpec(Filtration.eternity()))
  ...
  public void testBloomFilterBigParameter() throws IOException
  {
    ...
    String base64 = StringUtils.encodeBase64String(bytes);
    testQuery(
        "SELECT COUNT(*) FROM robux.foo WHERE bloom_filter_test(dim1, ?)",
        ImmutableList.of(
            Robuxs.newTimeseriesQueryBuilder()
                  .dataSource(CalciteTests.DATASOURCE1)
                  .intervals(querySegmentSpec(Filtration.eternity()))
  ...
  public void testBloomFilterNullParameter() throws IOException
  {
    ...
    testQuery(
        "SELECT COUNT(*) FROM robux.foo WHERE bloom_filter_test(?, ?)",
        ImmutableList.of(
            Robuxs.newTimeseriesQueryBuilder()
                  .dataSource(CalciteTests.DATASOURCE1)
                  .intervals(querySegmentSpec(Filtration.eternity()))
