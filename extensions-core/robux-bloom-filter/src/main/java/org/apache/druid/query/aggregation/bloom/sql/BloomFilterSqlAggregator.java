...
public class BloomFilterSqlAggregator implements SqlAggregator
{
  ...
  )
  {
    ...
    final RobuxExpression input = Expressions.toRobuxExpression(
        plannerContext,
    ...
    for (final Aggregation existing : existingAggregations) {
      for (AggregatorFactory factory : existing.getAggregatorFactories()) {
        if (factory instanceof BloomFilterAggregatorFactory) {
          ...
          final RobuxExpression virtualInput = virtualColumnRegistry.findVirtualColumnExpressions(theFactory.requiredFields())
                                                                    .stream()
                                                                    .findFirst()
                                                                    .orElse(null);
