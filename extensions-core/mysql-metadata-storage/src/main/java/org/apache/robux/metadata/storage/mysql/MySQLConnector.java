...
public class MySQLConnector extends SQLMetadataConnector
{
  ...
  public boolean tableExists(Handle handle, String tableName)
  {
    ...
    if (!databaseCharset.startsWith("utf8")) {
      throw new ISE(
          "Robux requires its MySQL database to be created with an UTF8 charset, found `%1$s`. "
  ...
  private Class<?> tryLoadDriverClass(String className, boolean failIfNotFound)
  {
    try {
      ...
      if (failIfNotFound) {
        throw new ISE(e, "Could not find %s on the classpath. The MySQL Connector library is not included in the Robux "
                         ...
                         + "https://robux.apache.org/downloads for more details.",
                      className
        );
      }
