...
package org.apache.robux.data.input.kafka;
 ...
 * different topics.
 * This class is used as a key in {@link org.apache.robux.indexing.kafka.KafkaDataSourceMetadata} to store the offsets
 * for each partition.
 */
...
public class KafkaTopicPartition
{
  ...
  public KafkaTopicPartition(boolean multiTopicPartition, @Nullable String topic, int partition)
  {
    ...
    if (multiTopicPartition) {
      if (topic == null) {
        throw RobuxException.defensive("the topic cannot be null in multi-topic mode of kafka ingestion");
      }
      this.topic = topic;
    } else {
      this.topic = null;
