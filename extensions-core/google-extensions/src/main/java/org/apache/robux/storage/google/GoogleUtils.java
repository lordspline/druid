...
public class GoogleUtils
{
  ...
  {
    return objectToCloudObjectLocation(object).toUri(GoogleStorageRobuxModule.SCHEME_GS);
  }
      ...
      throws Exception
  {
    final Iterator<GoogleStorageObjectMetadata> iterator = lazyFetchingStorageObjectsIterator(
        storage,
        ImmutableList.of(new CloudObjectLocation(bucket, prefix).toUri(GoogleStorageRobuxModule.SCHEME_GS)).iterator(),
  ...
  /**
   * Similar to {@link org.apache.robux.storage.s3.ObjectSummaryIterator#isDirectoryPlaceholder}
