...
package org.apache.robux.storage.google.output;
...
import org.apache.robux.data.input.google.GoogleCloudStorageInputSource;
...
import org.apache.robux.storage.google.GoogleInputDataConfig;
import org.apache.robux.storage.google.GoogleStorage;
import org.apache.robux.storage.google.GoogleStorageRobuxModule;
...
public class GoogleExportStorageProvider implements ExportStorageProvider
{
  ...
  public StorageConnector createStorageConnector(File taskTempDir)
  {
    ...
    if (tempDirFile == null) {
      throw RobuxException.defensive("Couldn't find temporary directory for export.");
    }
    ...
    if (allowedExportPaths == null) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          ...
                          .build(
                              "The runtime property `robux.export.storage.google.allowedExportPaths` must be configured for GCS export.");
    }
  ...
  {
    final URI providedUri = new CloudObjectLocation(bucket, prefix).toUri(GoogleStorageRobuxModule.SCHEME_GS);
    ...
    throw RobuxException.forPersona(RobuxException.Persona.USER)
                        ...
                        .build("None of the allowed prefixes matched the input path [%s]. "
                               ...
                               + "The paths are controlled via the property `robux.export.storage.google.allowedExportPaths`.",
                               providedUri);
  }
  ...
  {
    return new CloudObjectLocation(bucket, prefix).toUri(GoogleStorageRobuxModule.SCHEME_GS).toString();
  }

  @Override
  public String getFilePathForManifest(String fileName)
  {
    return new CloudObjectLocation(bucket, JOINER.join(prefix, fileName)).toUri(GoogleStorageRobuxModule.SCHEME_GS).toString();
  }
}
