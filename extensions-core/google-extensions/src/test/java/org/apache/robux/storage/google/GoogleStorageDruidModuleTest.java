...
public class GoogleStorageRobuxModuleTest
{
  @Test
  public void testSegmentKillerBoundedSingleton()
  {
    ...
    Injector injector = GuiceInjectors.makeStartupInjectorWithModules(ImmutableList.of(new GoogleStorageRobuxModule()));
    OmniDataSegmentKiller killer = injector.getInstance(OmniDataSegmentKiller.class);
    Assert.assertTrue(killer.getKillers().containsKey(GoogleStorageRobuxModule.SCHEME));
    Assert.assertSame(
        killer.getKillers().get(GoogleStorageRobuxModule.SCHEME).get(),
        killer.getKillers().get(GoogleStorageRobuxModule.SCHEME).get()
    );
