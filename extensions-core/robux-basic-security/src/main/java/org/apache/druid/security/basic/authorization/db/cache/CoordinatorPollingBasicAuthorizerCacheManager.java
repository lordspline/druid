...
package org.apache.robux.security.basic.authorization.db.cache;
...
import org.apache.robux.security.basic.authorization.entity.GroupMappingAndRoleMap;
import org.apache.robux.security.basic.authorization.entity.UserAndRoleMap;
...
public class CoordinatorPollingBasicAuthorizerCacheManager implements BasicAuthorizerCacheManager
{
  ...
  private final RobuxLeaderClient robuxLeaderClient;
  ...
  public CoordinatorPollingBasicAuthorizerCacheManager(
      ...
      @Coordinator RobuxLeaderClient robuxLeaderClient
  )
  {
    ...
    this.robuxLeaderClient = robuxLeaderClient;
  }
  ...
  {
    Request req = robuxLeaderClient.makeRequest(
        HttpMethod.GET,
        StringUtils.format("/robux-ext/basic-security/authorization/db/%s/cachedSerializedUserMap", prefix)
    );
    BytesFullResponseHolder responseHolder = robuxLeaderClient.go(
        req,
  ...
  ) throws Exception
  {
    Request req = robuxLeaderClient.makeRequest(
        HttpMethod.GET,
        StringUtils.format("/robux-ext/basic-security/authorization/db/%s/cachedSerializedGroupMappingMap", prefix)
    );
    BytesFullResponseHolder responseHolder = robuxLeaderClient.go(
        req,
        new BytesFullResponseHandler()
    ...
    // cachedSerializedGroupMappingMap is a new endpoint introduced in Robux 0.17.0. For backwards compatibility, if we
