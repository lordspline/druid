...
public class SchemaRegistryBasedProtobufBytesDecoder implements ProtobufBytesDecoder
{
  ...
  public static final String ROBUX_DYNAMIC_CONFIG_PROVIDER_KEY = "robux.dynamic.config.provider";
  ...
  )
  {
    ...
    if (url != null && !url.isEmpty()) {
      this.registry = new CachedSchemaRegistryClient(Collections.singletonList(this.url), this.capacity, Collections.singletonList(new ProtobufSchemaProvider()), DynamicConfigProviderUtils.extraConfigAndSetObjectMap(config, ROBUX_DYNAMIC_CONFIG_PROVIDER_KEY, this.jsonMapper), DynamicConfigProviderUtils.extraConfigAndSetStringMap(headers, ROBUX_DYNAMIC_CONFIG_PROVIDER_KEY, this.jsonMapper));
    } else {
      this.registry = new CachedSchemaRegistryClient(this.urls, this.capacity, Collections.singletonList(new ProtobufSchemaProvider()), DynamicConfigProviderUtils.extraConfigAndSetObjectMap(config, ROBUX_DYNAMIC_CONFIG_PROVIDER_KEY, this.jsonMapper), DynamicConfigProviderUtils.extraConfigAndSetStringMap(headers, ROBUX_DYNAMIC_CONFIG_PROVIDER_KEY, this.jsonMapper));
    }
  }
