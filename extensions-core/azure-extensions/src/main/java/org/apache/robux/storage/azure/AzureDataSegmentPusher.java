...
public class AzureDataSegmentPusher implements DataSegmentPusher
{
  ...
  static final List<String> ALLOWED_PROPERTY_PREFIXES_FOR_HADOOP = ImmutableList.of("robux.azure");
  ...
  String getAzurePath(final DataSegment segment, final boolean useUniquePath)
  {
    ...
    return StringUtils.format("%s/%s", storageDir, AzureStorageRobuxModule.INDEX_ZIP_FILE_NAME);
  ...
  private Map<String, Object> makeLoadSpec(String prefix)
  {
    return ImmutableMap.of(
        "type",
        AzureStorageRobuxModule.SCHEME,
        "containerName",
        segmentConfig.getContainer(),
        "blobPath",
        prefix
