...
@ManageLifecycle
public class K8sRobuxNodeDiscoveryProvider extends RobuxNodeDiscoveryProvider
{
  private static final Logger LOGGER = new Logger(K8sRobuxNodeDiscoveryProvider.class);
  ...
  @Inject
  public K8sRobuxNodeDiscoveryProvider(
      PodInfo podInfo,
  ...
  public BooleanSupplier getForNode(RobuxNode node, NodeRole nodeRole)
  {
    return () -> k8sApiClient.listPods(
        ...
        K8sRobuxNodeAnnouncer.getLabelSelectorForNode(discoveryConfig, nodeRole, node),
        nodeRole
    ).getRobuxNodes().containsKey(node.getHostAndPortToUse());
  }

  @Override
  public RobuxNodeDiscovery getForNodeRole(NodeRole nodeType)
  {
  ...
  public void start()
  {
    ...
    try {
      ...
      // event occurences.
      listenerExecutor = Execs.scheduledSingleThreaded("K8sRobuxNodeDiscoveryProvider-ListenerExecutor");
  ...
  @VisibleForTesting
  static class NodeRoleWatcher implements RobuxNodeDiscovery
  {
    ...
    private void watch()
    {
      String labelSelector = K8sRobuxNodeAnnouncer.getLabelSelectorForNodeRole(discoveryConfig, nodeRole);
      ...
      while (lifecycleLock.awaitStarted(1, TimeUnit.MILLISECONDS)) {
        try {
          DiscoveryRobuxNodeList list = k8sApiClient.listPods(podInfo.getPodNamespace(), labelSelector, nodeRole);
          baseNodeRoleWatcher.resetNodes(list.getRobuxNodes());
    ...
    private void keepWatching(String labelSelector, String resourceVersion)
    {
      ...
      while (lifecycleLock.awaitStarted(1, TimeUnit.MILLISECONDS)) {
        try {
          ...
          try {
            while (iter.hasNext()) {
              Watch.Response<DiscoveryRobuxNodeAndResourceVersion> item = iter.next();
