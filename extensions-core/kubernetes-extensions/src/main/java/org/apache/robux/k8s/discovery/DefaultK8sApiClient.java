...
public class DefaultK8sApiClient implements K8sApiClient
{
  ...
  @Override
  public DiscoveryRobuxNodeList listPods(
      String podNamespace,
  ...
  )
  {
    try {
      ...
      Map<String, DiscoveryRobuxNode> allNodes = new HashMap();
      for (V1Pod podDef : podList.getItems()) {
        DiscoveryRobuxNode node = getDiscoveryRobuxNodeFromPodDef(nodeRole, podDef);
        allNodes.put(node.getRobuxNode().getHostAndPortToUse(), node);
      }
      return new DiscoveryRobuxNodeList(podList.getMetadata().getResourceVersion(), allNodes);
    }
    catch (ApiException ex) {
  ...
  private DiscoveryRobuxNode getDiscoveryRobuxNodeFromPodDef(NodeRole nodeRole, V1Pod podDef)
  {
    String jsonStr = podDef.getMetadata().getAnnotations().get(K8sRobuxNodeAnnouncer.getInfoAnnotation(nodeRole));
    try {
      return jsonMapper.readValue(jsonStr, DiscoveryRobuxNode.class);
    }
    catch (JsonProcessingException ex) {
      throw new RE(ex, "Failed to deserialize DiscoveryRobuxNode[%s]", jsonStr);
    }
  }
  ...
  public WatchResult watchPods(String namespace, String labelSelector, String lastKnownResourceVersion, NodeRole nodeRole)
  {
    try {
      ...
      {
        private Watch.Response<DiscoveryRobuxNodeAndResourceVersion> obj;
          ...
          try {
            while (watch.hasNext()) {
              ...
              if (item != null && item.type != null && !item.type.equals(WatchResult.BOOKMARK)) {
                DiscoveryRobuxNodeAndResourceVersion result = null;
                if (item.object != null) {
                  result = new DiscoveryRobuxNodeAndResourceVersion(
                    item.object.getMetadata().getResourceVersion(),
                    getDiscoveryRobuxNodeFromPodDef(nodeRole, item.object)
                  );
                } else {
