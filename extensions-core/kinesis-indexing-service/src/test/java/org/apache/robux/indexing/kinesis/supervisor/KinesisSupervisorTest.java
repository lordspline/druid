...
package org.apache.robux.indexing.kinesis.supervisor;
...
import org.apache.robux.data.input.kinesis.KinesisRecordEntity;
...
import org.apache.robux.indexing.common.TaskInfoProvider;
...
import org.apache.robux.indexing.kinesis.KinesisDataSourceMetadata;
import org.apache.robux.indexing.kinesis.KinesisIndexTask;
import org.apache.robux.indexing.kinesis.KinesisIndexTaskClientFactory;
import org.apache.robux.indexing.kinesis.KinesisIndexTaskIOConfig;
import org.apache.robux.indexing.kinesis.KinesisIndexTaskTuningConfig;
import org.apache.robux.indexing.kinesis.KinesisRecordSupplier;
import org.apache.robux.indexing.kinesis.KinesisSequenceNumber;
...
import org.apache.robux.indexing.overlord.TaskQueue;
...
import org.apache.robux.indexing.overlord.supervisor.SupervisorReport;
import org.apache.robux.indexing.overlord.supervisor.SupervisorStateManager;
import org.apache.robux.indexing.overlord.supervisor.SupervisorStateManagerConfig;
import org.apache.robux.indexing.overlord.supervisor.autoscaler.SupervisorTaskAutoScaler;
...
import org.apache.robux.indexing.seekablestream.SeekableStreamIndexTaskClient;
import org.apache.robux.indexing.seekablestream.SeekableStreamIndexTaskRunner;
import org.apache.robux.indexing.seekablestream.SeekableStreamIndexTaskTuningConfig;
import org.apache.robux.indexing.seekablestream.SeekableStreamStartSequenceNumbers;
import org.apache.robux.indexing.seekablestream.common.RecordSupplier;
import org.apache.robux.indexing.seekablestream.common.StreamPartition;
import org.apache.robux.indexing.seekablestream.supervisor.SeekableStreamSupervisorStateManager;
import org.apache.robux.indexing.seekablestream.supervisor.SeekableStreamSupervisorTuningConfig;
import org.apache.robux.indexing.seekablestream.supervisor.TaskReportData;
import org.apache.robux.indexing.seekablestream.supervisor.autoscaler.AutoScalerConfig;
import org.apache.robux.indexing.seekablestream.supervisor.autoscaler.LagBasedAutoScalerConfig;
...
import org.apache.robux.java.util.emitter.service.AlertBuilder;
...
public class KinesisSupervisorTest extends EasyMockSupport
{
  ...
  public void testRecordSupplier()
  {
    ...
    KinesisSupervisor supervisor = new KinesisSupervisor(
        ...
        new KinesisSupervisorSpec(
            ...
            new RobuxMonitorSchedulerConfig(),
  ...
  /**
   * Test generating the starting sequences from the partition data stored in robux_dataSource which contains the
  ...
  public void testBadMetadataOffsets() throws Exception
  {
    ...
    EasyMock.expect(taskStorage.getActiveTasksByDatasource(DATASOURCE)).andReturn(List.of()).anyTimes();
    EasyMock.expect(indexerMetadataStorageCoordinator.retrieveDataSourceMetadata(DATASOURCE)).andReturn(
        new KinesisDataSourceMetadata(
            new SeekableStreamStartSequenceNumbers<>(
                STREAM,
    ...
    Assert.assertEquals(
        "org.apache.robux.java.util.common.ISE",
  ...
  public void testDontKillTasksWithMismatchedType() throws Exception
  {
    ...
    List<Task> existingTasks = List.of(id2);

    EasyMock.expect(taskMaster.getTaskQueue()).andReturn(Optional.of(taskQueue)).anyTimes();
    EasyMock.expect(taskMaster.getTaskRunner()).andReturn(Optional.of(taskRunner)).anyTimes();
    EasyMock.expect(taskRunner.getRunningTasks()).andReturn(Collections.emptyList()).anyTimes();
    EasyMock.expect(taskQueue.getActiveTasksForDatasource(DATASOURCE)).andReturn(toMap(existingTasks)).anyTimes();
  ...
  public void testKillBadPartitionAssignment() throws Exception
  {
    ...
    List<Task> existingTasks = List.of(id1, id2, id3, id4);
  ...
  public void testRequeueAdoptedTaskWhenFailed() throws Exception
  {
    ...
    List<Task> existingTasks = List.of(id1);
    ...
    EasyMock.expect(taskQueue.getActiveTasksForDatasource(DATASOURCE))
            .andReturn(toMap(List.of(captured.getValue())))
            .anyTimes();
    EasyMock.expect(taskStorage.getStatus(iHaveFailed.getId()))
            .andReturn(Optional.of(TaskStatus.failure(iHaveFailed.getId(), "Dummy task status failure err message")));
  ...
  public void testDiscoverExistingPublishingTask() throws Exception
  {
    ...
    EasyMock.expect(taskRunner.getRunningTasks()).andReturn(workItems).anyTimes();
    EasyMock.expect(taskRunner.getTaskLocation(EasyMock.anyString())).andReturn(location).anyTimes();
    EasyMock.expect(taskQueue.getActiveTasksForDatasource(DATASOURCE)).andReturn(Map.of(task.getId(), task)).anyTimes();
    EasyMock.expect(taskStorage.getStatus("id1")).andReturn(Optional.of(TaskStatus.running("id1"))).anyTimes();
  ...
  public void testDiscoverExistingPublishingAndReadingTask() throws Exception
  {
    ...
    EasyMock.expect(taskMaster.getTaskRunner()).andReturn(Optional.of(taskRunner)).anyTimes();
    EasyMock.expect(taskRunner.getRunningTasks()).andReturn(workItems).anyTimes();
    EasyMock.expect(taskQueue.getActiveTasksForDatasource(DATASOURCE)).andReturn(toMap(List.of(id1, id2))).anyTimes();
    EasyMock.expect(taskStorage.getStatus("id1")).andReturn(Optional.of(TaskStatus.running("id1"))).anyTimes();
    EasyMock.expect(taskStorage.getStatus("id2")).andReturn(Optional.of(TaskStatus.running("id2"))).anyTimes();
    EasyMock.expect(taskStorage.getTask("id1")).andReturn(Optional.of(id1)).anyTimes();
    EasyMock.expect(taskStorage.getTask("id2")).andReturn(Optional.of(id2)).anyTimes();
  ...
  public void testStopGracefully() throws Exception
  {
    ...
    EasyMock.expect(taskQueue.getActiveTasksForDatasource(DATASOURCE))
            .andReturn(toMap(List.of(id1, id2, id3)))
            .anyTimes();
    EasyMock.expect(taskStorage.getStatus("id1")).andReturn(Optional.of(TaskStatus.running("id1"))).anyTimes();
    EasyMock.expect(taskStorage.getStatus("id2")).andReturn(Optional.of(TaskStatus.running("id2"))).anyTimes();
    EasyMock.expect(taskStorage.getStatus("id3")).andReturn(Optional.of(TaskStatus.running("id3"))).anyTimes();
      ...
      throws InterruptedException
  {
    ...
    List<Task> existingTasks = List.of(task);
  ...
  private List<Task> testShardMergePhaseTwo(List<Task> phaseOneTasks) throws Exception
  {
    ...
    EasyMock.expect(taskQueue.getActiveTasksForDatasource(DATASOURCE)).andReturn(toMap(postMergeTasks)).anyTimes();
