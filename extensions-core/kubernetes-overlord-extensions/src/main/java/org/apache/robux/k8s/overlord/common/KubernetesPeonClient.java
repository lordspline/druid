...
package org.apache.robux.k8s.overlord.common;
...
public class KubernetesPeonClient
{
  ...
  private List<Job> getPeonJobsWithoutOverlordNamespaceKeyLabels()
  {
    return clientApi.executeRequest(client -> client.batch()
                                                    ...
                                                    .jobs()
                                                    .inNamespace(namespace)
                                                    .withLabel(RobuxK8sConstants.LABEL_KEY)
                                                    .list()
                                                    .getItems());
  }
  ...
  private List<Job> getPeonJobsWithOverlordNamespaceKeyLabels()
  {
    return clientApi.executeRequest(client -> client.batch()
                                                    ...
                                                    .inNamespace(namespace)
                                                    .withLabel(RobuxK8sConstants.LABEL_KEY)
                                                    .withLabel(RobuxK8sConstants.OVERLORD_NAMESPACE_KEY, overlordNamespace)
                                                    .list()
                                                    .getItems());
  }
  ...
  Pod getPeonPodWithRetries(KubernetesClient client, String jobName, int quietTries, int maxTries)
  {
    try {
      ...
      throw RobuxException.defensive(e, "Error when looking for K8s pod with label[job-name=%s]", jobName);
    }
  }
   ...
   * These substrings, found in {@link RobuxK8sConstants#BLACKLISTED_PEON_POD_ERROR_MESSAGES},
  ...
  private boolean shouldRetryStartingPeonPod(Throwable e)
  {
    ...
    for (String blacklistedMessage : RobuxK8sConstants.BLACKLISTED_PEON_POD_ERROR_MESSAGES) {
