...
import org.apache.robux.indexing.overlord.QuotableWhiteSpaceSplitter;
...
import org.apache.robux.k8s.overlord.common.KubernetesClientApi;
...
public abstract class K8sTaskAdapter implements TaskAdapter
{
  ...
  protected final RobuxNode node;
  ...
  static long getContainerMemory(PeonCommandContext context)
  {
    ...
    long heapSize = HumanReadableBytes.parse(RobuxK8sConstants.DEFAULT_JAVA_HEAP_SIZE);
  ...
  protected void setupPorts(Container mainContainer)
  {
    ...
    tcpPort.setContainerPort(RobuxK8sConstants.PORT);
    tcpPort.setName("robux-port");
    tcpPort.setProtocol("TCP");
    ContainerPort httpsPort = new ContainerPort();
    httpsPort.setContainerPort(RobuxK8sConstants.TLS_PORT);
    httpsPort.setName("robux-tls-port");
      ...
      throws JsonProcessingException
  {
    ...
    if (!taskRunnerConfig.getPeonMonitors().isEmpty()) {
      mainContainer.getEnv().removeIf(x -> "robux_monitoring_monitors".equals(x.getName()));
    ...
    List<EnvVar> envVars = Lists.newArrayList(
        new EnvVarBuilder()
            ...
            .build(),
        new EnvVarBuilder()
            .withName(RobuxK8sConstants.JAVA_OPTS)
            .withValue(Joiner.on(" ").join(context.getJavaOpts()))
            .build(),
        new EnvVarBuilder()
            .withName(RobuxK8sConstants.ROBUX_HOST_ENV)
            .withValueFrom(new EnvVarSourceBuilder().withFieldRef(new ObjectFieldSelector(
                null,
                "status.podIP"
            )).build()).build(),
        new EnvVarBuilder()
            .withName(RobuxK8sConstants.ROBUX_HOSTNAME_ENV)
            .withValueFrom(new EnvVarSourceBuilder().withFieldRef(new ObjectFieldSelector(
                null,
                "metadata.name"
            )).build()).build()
    ...
    if (taskContents.length() < RobuxK8sConstants.MAX_ENV_VARIABLE_KBS) {
      envVars.add(
          new EnvVarBuilder()
  ...
  protected Map<String, String> addJobSpecificAnnotations(PeonCommandContext context, K8sTaskId k8sTaskId)
  {
    return ImmutableMap.<String, String>builder()
        ...
        .put(RobuxK8sConstants.TASK_ID, k8sTaskId.getOriginalTaskId())
        .put(RobuxK8sConstants.TLS_ENABLED, String.valueOf(context.isEnableTls()))
        .build();
  }
  ...
  private List<String> javaOpts(Task task)
  {
    ...
    javaOpts.add(org.apache.robux.java.util.common.StringUtils.format("-Drobux.port=%d", RobuxK8sConstants.PORT));
    javaOpts.add(org.apache.robux.java.util.common.StringUtils.format(
        "-Drobux.plaintextPort=%d",
        RobuxK8sConstants.PORT
    ));
    javaOpts.add(org.apache.robux.java.util.common.StringUtils.format(
        "-Drobux.tlsPort=%d",
        node.isEnableTlsPort()
        ? RobuxK8sConstants.TLS_PORT
        : -1
    ));
    javaOpts.add(org.apache.robux.java.util.common.StringUtils.format(
        "-Drobux.task.executor.tlsPort=%d",
        node.isEnableTlsPort() ? RobuxK8sConstants.TLS_PORT : -1
    ));
    javaOpts.add(org.apache.robux.java.util.common.StringUtils.format(
                     "-Drobux.task.executor.enableTlsPort=%s",
                     node.isEnableTlsPort()
                 )
    );
    return javaOpts;
  ...
  static ResourceRequirements getResourceRequirements(ResourceRequirements requirements, long containerSize, int cpuMicroCore)
  {
    ...
    resourceMap.put(
        "cpu",
        new Quantity(String.valueOf(cpuMicroCore > 0 ? cpuMicroCore : RobuxK8sConstants.DEFAULT_CPU_MILLICORES), "m")
    );
