...
import org.apache.robux.java.util.http.client.AbstractHttpClient;
...
public class KerberosHttpClient extends AbstractHttpClient
{
  ...
  )
  {
    try {
      ...
      if (RobuxKerberosUtil.needToSendCredentials(cookieManager.getCookieStore(), uri)) {
        ...
        RobuxKerberosUtil.authenticateIfRequired(internalClientPrincipal, internalClientKeytab);
        ...
        String challenge = currentUser.doAs(new PrivilegedExceptionAction<>()
        {
          ...
          {
            return RobuxKerberosUtil.kerberosChallenge(host);
          }
        });
      ...
      Futures.addCallback(internalFuture, new FutureCallback<>()
      {
        ...
        public void onSuccess(RetryResponseHolder<Final> result)
        {
          if (should_retry_on_unauthorized_response && result.shouldRetry()) {
            ...
            // remove Auth cookie
            RobuxKerberosUtil.removeAuthCookie(cookieManager.getCookieStore(), uri);
