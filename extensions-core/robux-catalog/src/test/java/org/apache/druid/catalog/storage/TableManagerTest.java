...
import org.apache.robux.catalog.CatalogTest;
...
public class TableManagerTest
{
  ...
  public void testList() throws DuplicateKeyException
  {
    ...
    assertEquals(TableId.ROBUX_SCHEMA, id.schema());
    assertEquals("table1", id.name());
    id = list.get(1);
    assertEquals(TableId.ROBUX_SCHEMA, id.schema());
    assertEquals("table2", id.name());

    List<String> names = manager.tableNamesInSchema(TableId.ROBUX_SCHEMA);
    assertEquals(2, names.size());

    names = manager.tableNamesInSchema(TableId.SYSTEM_SCHEMA);
    assertEquals(0, names.size());

    List<TableMetadata> details = manager.tablesInSchema(TableId.ROBUX_SCHEMA);
