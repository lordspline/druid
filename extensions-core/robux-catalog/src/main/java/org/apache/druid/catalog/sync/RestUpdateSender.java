...
/**
 * Sends updates to a set of Robux nodes provided by the given supplier.
...
public class RestUpdateSender implements Consumer<byte[]>
{
  ...
  private final Supplier<Iterable<RobuxNode>> destinationSupplier;
  ...
  public RestUpdateSender(
      ...
      final Supplier<Iterable<RobuxNode>> destinationSupplier,
  ...
  public void accept(byte[] serializedEntity)
  {
    ...
    for (RobuxNode node : destinationSupplier.get()) {
      futures.add(
          sender.send(
  ...
  private URL getListenerURL(RobuxNode robuxNode, String baseUrl)
  {
    try {
      return new URL(
          robuxNode.getServiceScheme(),
          robuxNode.getHost(),
          robuxNode.getPortToUse(),
          baseUrl
      );
    }
    catch (MalformedURLException mue) {
      String msg = StringUtils.format(callerName + ": Malformed url for RobuxNode [%s] and baseUrl [%s]", robuxNode, baseUrl);
      LOG.error(msg);
