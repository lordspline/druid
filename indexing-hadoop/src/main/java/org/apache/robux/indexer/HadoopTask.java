...
import org.apache.robux.guice.ExtensionsConfig;
import org.apache.robux.guice.ExtensionsLoader;
import org.apache.robux.guice.StartupInjectorBuilder;
import org.apache.robux.indexing.common.task.AbstractBatchIndexTask;
import org.apache.robux.indexing.common.task.Initialization;
...
public abstract class HadoopTask extends AbstractBatchIndexTask
{
  ...
  // Right now it just looks for `robux.*\.jar`
  ...
  protected static final Predicate<URL> IS_ROBUX_URL = new Predicate<>()
  {
    @Override
    public boolean apply(@Nullable URL input)
    {
      try {
        ...
        return fName.startsWith("robux") && fName.endsWith(".jar") && !fName.endsWith("selfcontained.jar");
      }
   ...
   *
   * 1. Non-Robux jars (see {@link #IS_ROBUX_URL}) found in the ClassLoader for HadoopIndexTask.class. This will
   ...
   * 3. Robux jars (see {@link #IS_ROBUX_URL}) found in the ClassLoader for HadoopIndexTask.class
   ...
   * predictive {@link #IS_ROBUX_URL} that captures all things which reference robux classes. This lead to a case where
   * the class loader isolation worked great for stock robux, but failed in many common use cases including extension
   ...
   * Such an approach combined with reasonable exclusions in org.apache.robux.cli.PullDependencies#exclusions tries to maintain
   * sanity in a ClassLoader where all jars (which are isolated by extension ClassLoaders in the Robux framework) are
                                             ...
                                             final List<String> defaultHadoopCoordinates)
  {
    ...
    // hadoop dependencies come before robux classes because some extensions depend on them
    ...
    final String hadoopContainerRobuxClasspathJars;
    ExtensionsConfig extnConfig = EXTENSIONS_LOADER.config();
    if (extnConfig.getHadoopContainerRobuxClasspath() == null) {
      hadoopContainerRobuxClasspathJars = Joiner.on(File.pathSeparator).join(jobURLs);

    } else {
      List<URL> hadoopContainerURLs = Lists.newArrayList(
          ExtensionsLoader.getURLsForClasspath(extnConfig.getHadoopContainerRobuxClasspath())
      );
      ...
      hadoopContainerRobuxClasspathJars = Joiner.on(File.pathSeparator)
                                                .join(hadoopContainerURLs);
    }

    log.info("Hadoop Container Robux Classpath is set to [%s]", hadoopContainerRobuxClasspathJars);
    System.setProperty("robux.hadoop.internal.classpath", hadoopContainerRobuxClasspathJars);
