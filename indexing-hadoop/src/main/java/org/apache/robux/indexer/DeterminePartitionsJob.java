...
import org.apache.robux.collections.CombiningIterable;
...
import org.apache.robux.indexer.partitions.SingleDimensionPartitionsSpec;
...
public class DeterminePartitionsJob implements Jobby
{
  ...
  private static final Joiner TAB_JOINER = HadoopRobuxIndexerConfig.TAB_JOINER;
  private static final Joiner COMMA_JOINER = Joiner.on(",").useForNull("");

  private final HadoopRobuxIndexerConfig config;
  ...
  DeterminePartitionsJob(
      HadoopRobuxIndexerConfig config
  )
  {
    this.config = config;
  }
    ...
    try {
      ...
      if (!partitionsSpec.isAssumeGrouped()) {
        ...
        try {
          if (!groupByJob.waitForCompletion(true)) {
            ...
            failureCause = Utils.getFailureMessage(groupByJob, HadoopRobuxIndexerConfig.JSON_MAPPER);
            return false;
          }
        }
      ...
      try {
        if (!dimSelectionJob.waitForCompletion(true)) {
          ...
          failureCause = Utils.getFailureMessage(dimSelectionJob, HadoopRobuxIndexerConfig.JSON_MAPPER);
          return false;
        }
      }
      catch (IOException ioe) {
      ...
      for (Interval segmentGranularity : config.getSegmentGranularIntervals()) {
        ...
        if (Utils.exists(dimSelectionJob, fileSystem, partitionInfoPath)) {
          List<ShardSpec> specs = HadoopRobuxIndexerConfig.JSON_MAPPER.readValue(
  ...
  public Map<String, Object> getStats()
  {
    ...
    try {
      ...
      return TaskMetricsUtils.makeIngestionRowMetrics(
          jobCounters.findCounter(HadoopRobuxIndexerConfig.IndexJobCounters.ROWS_PROCESSED_COUNTER).getValue(),
          jobCounters.findCounter(HadoopRobuxIndexerConfig.IndexJobCounters.ROWS_PROCESSED_WITH_ERRORS_COUNTER)
                     .getValue(),
          jobCounters.findCounter(HadoopRobuxIndexerConfig.IndexJobCounters.ROWS_UNPARSEABLE_COUNTER).getValue(),
          jobCounters.findCounter(HadoopRobuxIndexerConfig.IndexJobCounters.ROWS_THROWN_AWAY_COUNTER).getValue()
      );
    }
  ...
  private static DeterminePartitionsJobSampler createSampler(HadoopRobuxIndexerConfig config)
  {
  ...
  public static class DeterminePartitionsGroupByMapper extends HadoopRobuxIndexerMapper<BytesWritable, NullWritable>
  {
    @Nullable
    ...
    ) throws IOException, InterruptedException
    {
      ...
      final byte[] groupKeyBytes = HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsBytes(groupKey);
      ...
      context.getCounter(HadoopRobuxIndexerConfig.IndexJobCounters.ROWS_PROCESSED_COUNTER).increment(1);
    }
  }
      ...
      extends Mapper<BytesWritable, NullWritable, BytesWritable, Text>
  {
    ...
    protected void setup(Context context)
    {
      final HadoopRobuxIndexerConfig config = HadoopRobuxIndexerConfig.fromConfiguration(context.getConfiguration());
    ...
    {
      final List<Object> timeAndDims = HadoopRobuxIndexerConfig.JSON_MAPPER.readValue(key.getBytes(), List.class);
  ...
  public static class DeterminePartitionsDimSelectionAssumeGroupedMapper
      extends HadoopRobuxIndexerMapper<BytesWritable, Text>
  {
    private DeterminePartitionsDimSelectionMapperHelper helper;
    private DeterminePartitionsJobSampler sampler;
  ...
  static class DeterminePartitionsDimSelectionMapperHelper
  {
    ...
    DeterminePartitionsDimSelectionMapperHelper(HadoopRobuxIndexerConfig config)
    {
      this.config = config;
  ...
  {
    @Nullable
    protected volatile HadoopRobuxIndexerConfig config = null;
  ...
  public static class DeterminePartitionsDimSelectionReducer extends DeterminePartitionsDimSelectionBaseReducer
  {
        ...
        throws IOException
    {
      ...
      for (ShardSpec shardSpec : chosenShardSpecs) {
        log.info("  %s", HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsString(shardSpec));
      }
  ...
  public static class DimValueCount
  {
    ...
    {
      try {
        String jsonValue = HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsString(this);
    ...
    {
      try {
        return HadoopRobuxIndexerConfig.JSON_MAPPER.readValue(text.toString(), DimValueCount.class);
      }
      catch (IOException e) {
      ...
      throws IOException, InterruptedException
  {
    ...
    ).getBytes(HadoopRobuxIndexerConfig.JAVA_NATIVE_CHARSET);
