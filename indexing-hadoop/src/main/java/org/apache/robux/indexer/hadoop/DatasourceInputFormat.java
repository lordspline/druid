...
public class DatasourceInputFormat extends InputFormat<NullWritable, InputRow>
{
  ...
  private static final String CONF_DATASOURCES = "robux.datasource.input.datasources";
  private static final String CONF_SCHEMA = "robux.datasource.input.schema";
  private static final String CONF_SEGMENTS = "robux.datasource.input.segments";
  private static final String CONF_MAX_SPLIT_SIZE = "robux.datasource.input.split.max.size";
  ...
  public static List<String> getDataSources(final Configuration conf) throws IOException
  {
    ...
    return HadoopRobuxIndexerConfig.JSON_MAPPER.readValue(
        currentDatasources,
        new TypeReference<>() {}
    );
  }
      ...
      throws IOException
  {
    ...
    final DatasourceIngestionSpec spec = HadoopRobuxIndexerConfig.JSON_MAPPER.readValue(
        specString,
  ...
  ) throws IOException
  {
    ...
    conf.set(CONF_DATASOURCES, HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsString(newDataSources));

    conf.set(
        StringUtils.format("%s.%s", CONF_SCHEMA, spec.getDataSource()),
        HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsString(spec)
    );

    conf.set(
        StringUtils.format("%s.%s", CONF_SEGMENTS, spec.getDataSource()),
        HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsString(segments)
    );
