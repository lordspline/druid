...
public class DetermineHashedPartitionsJob implements Jobby
{
  ...
  public boolean run()
  {
    try {
      ...
      if (config.getInputIntervals().isEmpty()) {
        ...
        List<Interval> intervals = HadoopRobuxIndexerConfig.JSON_MAPPER.readValue(
      ...
      for (Interval segmentGranularity : config.getSegmentGranularIntervals()) {
        ...
        if (Utils.exists(groupByJob, fileSystem, partitionInfoPath)) {
          final Long numRows = HadoopRobuxIndexerConfig.JSON_MAPPER.readValue(
  ...
  public static class DetermineCardinalityMapper extends HadoopRobuxIndexerMapper<LongWritable, BytesWritable>
  {
    ...
    ) throws IOException
    {
      ...
      hyperLogLogs
          .get(interval)
          .add(hashFunction.hashBytes(HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsBytes(groupKey)).asBytes());
      ...
      extends Reducer<LongWritable, BytesWritable, NullWritable, NullWritable>
  {
    ...
    @Nullable
    protected HadoopRobuxIndexerConfig config = null;
    private boolean determineIntervals;
        ...
        throws IOException, InterruptedException
    {
      ...
      if (determineIntervals) {
        ...
        try {
          HadoopRobuxIndexerConfig.JSON_MAPPER.writerFor(
      ...
      extends Partitioner<LongWritable, BytesWritable> implements Configurable
  {
    ...
    {
      this.config = config;
      HadoopRobuxIndexerConfig hadoopConfig = HadoopRobuxIndexerConfig.fromConfiguration(config);
