...
public class HadoopRobuxIndexerMapperTest
{
  ...
  {
    final HadoopRobuxIndexerConfig config = new HadoopRobuxIndexerConfig(
    ...
    hadoopConfig.set(
        HadoopRobuxIndexerConfig.CONFIG_PROPERTY,
        HadoopRobuxIndexerConfig.JSON_MAPPER.writeValueAsString(config)
    );
  ...
  public void testHadoopyStringParserWithTransformSpec() throws Exception
  {
    ...
    final Mapper.Context mapContext = EasyMock.mock(Mapper.Context.class);
    EasyMock.expect(mapContext.getConfiguration()).andReturn(hadoopConfig).once();
    EasyMock.expect(mapContext.getCounter(HadoopRobuxIndexerConfig.IndexJobCounters.ROWS_THROWN_AWAY_COUNTER))
  ...
  private static void assertRowListEquals(final List<Map<String, Object>> expected, final List<InputRow> actual)
  {
    Assert.assertEquals(
        expected,
        actual.stream().map(HadoopRobuxIndexerMapperTest::rowToMap).collect(Collectors.toList())
    );
  }
  ...
  public static class MyMapper extends HadoopRobuxIndexerMapper
  {
