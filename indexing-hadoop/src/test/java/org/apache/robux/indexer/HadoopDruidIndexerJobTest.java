...
public class HadoopRobuxIndexerJobTest
{
  @Test
  public void test_run()
  {
    HadoopRobuxIndexerConfig config = mock(HadoopRobuxIndexerConfig.class);
    ...
    try (MockedStatic<JobHelper> jobHelperMock = Mockito.mockStatic(JobHelper.class)) {
      try (final MockedStatic<IndexGeneratorJob> indexGeneratorJobMock = Mockito.mockStatic(IndexGeneratorJob.class)) {
        ...
        HadoopRobuxIndexerJob target = new HadoopRobuxIndexerJob(config, handler);
        target.run();
