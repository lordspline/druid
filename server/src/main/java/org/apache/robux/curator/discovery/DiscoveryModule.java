...
import org.apache.robux.guice.KeyHolder;
...
import org.apache.robux.server.initialization.CuratorDiscoveryConfig;
...
/**
 * The DiscoveryModule allows for the registration of Keys of RobuxNode objects, which it intends to be
...
public class DiscoveryModule implements Module
{
  ...
  private static final String INTERNAL_DISCOVERY_PROP = "robux.discovery.type";
  ...
  /**
   * Requests that the un-annotated RobuxNode instance be injected and published as part of the lifecycle.
   * 
   * That is, this module will announce the RobuxNode instance returned by
   * injector.getInstance(Key.get(RobuxNode.class)) automatically.
  ...
  /**
   * Requests that the annotated RobuxNode instance be injected and published as part of the lifecycle.
   * 
   * That is, this module will announce the RobuxNode instance returned by
   * injector.getInstance(Key.get(RobuxNode.class, annotation)) automatically.
   * Announcement will happen in the ANNOUNCEMENTS stage of the Lifecycle
   *
   * @param annotation The annotation instance to use in finding the RobuxNode instance, usually a Named annotation
   */
   ...
   *
   * @param binder the Binder to register with
   * @param annotation The annotation class to use in finding the RobuxNode instance
   */
  ...
  /**
   * Requests that the keyed RobuxNode instance be injected and published as part of the lifecycle.
   * 
   * That is, this module will announce the RobuxNode instance returned by
   * injector.getInstance(Key.get(RobuxNode.class, annotation)) automatically.
   * Announcement will happen in the ANNOUNCEMENTS stage of the Lifecycle
   *
   * @param binder the Binder to register with
   * @param key The key to use in finding the RobuxNode instance
   */
  public static void registerKey(Binder binder, Key<RobuxNode> key)
  {
    RobuxBinders.discoveryAnnouncementBinder(binder).addBinding().toInstance(new KeyHolder<>(key));
  ...
  {
    JsonConfigProvider.bind(binder, "robux.discovery.curator", CuratorDiscoveryConfig.class);
    ...
    RobuxBinders.discoveryAnnouncementBinder(binder);
    ...
    PolyBind.createChoiceWithDefault(binder, INTERNAL_DISCOVERY_PROP, Key.get(RobuxNodeAnnouncer.class), CURATOR_KEY);

    PolyBind.createChoiceWithDefault(
        binder,
        INTERNAL_DISCOVERY_PROP,
        Key.get(RobuxNodeDiscoveryProvider.class),
        CURATOR_KEY
    );
    ...
    PolyBind.createChoiceWithDefault(
        binder,
        INTERNAL_DISCOVERY_PROP,
        Key.get(RobuxLeaderSelector.class, Coordinator.class),
        CURATOR_KEY
    );
  ...
  public CuratorServiceAnnouncer getServiceAnnouncer(
      ...
      final Set<KeyHolder<RobuxNode>> nodesToAnnounce,
  ...
  ) throws Exception
  {
    lifecycle.addMaybeStartHandler(
        ...
        {
          private volatile List<RobuxNode> nodes = null;
          ...
          public void start()
          {
            if (nodes == null) {
              nodes = new ArrayList<>();
              for (KeyHolder<RobuxNode> holder : nodesToAnnounce) {
                nodes.add(injector.getInstance(holder.getKey()));
              }
            }

            for (RobuxNode node : nodes) {
  ...
  private static class RobuxLeaderSelectorProvider implements Provider<RobuxLeaderSelector>
  {
    @Inject
    ...
    RobuxLeaderSelectorProvider(Function<ZkPathsConfig, String> latchPathFn)
    {
    ...
    {
      return new CuratorRobuxLeaderSelector(
