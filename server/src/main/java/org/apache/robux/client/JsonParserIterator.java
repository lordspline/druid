...
public class JsonParserIterator<T> implements CloseableIterator<T>
{
  ...
  )
  {
    ...
    this.url = url;
    if (query != null) {
      this.timeoutAt = query.context().getLong(DirectRobuxClient.QUERY_FAIL_TIME, -1L);
   ...
   * - The QueryException from {@link DirectRobuxClient} is converted to a more specific type of QueryException
