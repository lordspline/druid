...
import org.apache.robux.curator.inventory.CuratorInventoryManager;
import org.apache.robux.curator.inventory.CuratorInventoryManagerStrategy;
import org.apache.robux.curator.inventory.InventoryManagerConfig;
...
public class BatchServerInventoryView implements ServerInventoryView, FilteredServerInventoryView
{
  ...
  private final CuratorInventoryManager<RobuxServer, Set<DataSegment>> inventoryManager;
  ...
  )
  {
    this.inventoryManager = new CuratorInventoryManager<>(
        ...
        {
          @Override
          public RobuxServer deserializeContainer(byte[] bytes)
          {
            try {
              return jsonMapper.readValue(bytes, RobuxServer.class);
            }
            catch (IOException e) {
          ...
          @Override
          public void newContainer(RobuxServer container)
          {
          ...
          @Override
          public void deadContainer(RobuxServer deadContainer)
          {
          ...
          @Override
          public RobuxServer updateContainer(RobuxServer oldContainer, RobuxServer newContainer)
          {
            return newContainer.addDataSegments(oldContainer);
          }

          @Override
          public RobuxServer addInventory(
              final RobuxServer container,
              String inventoryKey,
          ...
          @Override
          public RobuxServer updateInventory(RobuxServer container, String inventoryKey, Set<DataSegment> inventory)
          {
            return updateInnerInventory(container, inventoryKey, inventory);
          }

          @Override
          public RobuxServer removeInventory(final RobuxServer container, String inventoryKey)
          {
  ...
  protected void addSingleInventory(final RobuxServer container, final DataSegment inventory)
  {
  ...
  void removeSingleInventory(RobuxServer container, SegmentId segmentId)
  {
  ...
  private boolean doRemoveSingleInventory(RobuxServer container, SegmentId segmentId)
  {
  ...
  {
    try {
      RobuxServer server = getInventoryValue(serverKey);
  ...
  protected RobuxServer addInnerInventory(
      final RobuxServer container,
      String inventoryKey,
      final Set<DataSegment> inventory
  )
  ...
  private Set<DataSegment> filterInventory(final RobuxServer container, Set<DataSegment> inventory)
  {
    Predicate<Pair<RobuxServerMetadata, DataSegment>> predicate = Predicates.or(
        defaultFilter,
        ...
        Iterables.transform(
            Iterables.filter(
                Iterables.transform(
                    ...
                    {
                      @Override
                      public Pair<RobuxServerMetadata, DataSegment> apply(DataSegment input)
                      {
            ...
            new Function<Pair<RobuxServerMetadata, DataSegment>, DataSegment>()
            {
              @Override
              public DataSegment apply(Pair<RobuxServerMetadata, DataSegment> input)
              {
  ...
  protected RobuxServer updateInnerInventory(RobuxServer container, String inventoryKey, Set<DataSegment> inventory)
  {
    Set<DataSegment> filteredInventory = filterInventory(container, inventory);
  ...
  protected RobuxServer removeInnerInventory(final RobuxServer container, String inventoryKey)
  {
  ...
  public void registerSegmentCallback(
      ...
      final Predicate<Pair<RobuxServerMetadata, DataSegment>> filter
  )
  {
