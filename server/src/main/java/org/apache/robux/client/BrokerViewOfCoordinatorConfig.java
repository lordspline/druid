...
public class BrokerViewOfCoordinatorConfig implements HistoricalFilter
{
  ...
  @Override
  public Int2ObjectRBTreeMap<Set<QueryableRobuxServer>> getQueryableServers(
      Int2ObjectRBTreeMap<Set<QueryableRobuxServer>> historicalServers,
      CloneQueryMode mode
  )
  {
    ...
    final Int2ObjectRBTreeMap<Set<QueryableRobuxServer>> filteredHistoricals = new Int2ObjectRBTreeMap<>();
    for (int priority : historicalServers.keySet()) {
      Set<QueryableRobuxServer> servers = historicalServers.get(priority);
  ...
  private synchronized Set<String> getCurrentServersToIgnore(CloneQueryMode cloneQueryMode)
  {
    switch (cloneQueryMode) {
      ...
      case INCLUDECLONES:
        ...
        return Set.of();
      default:
        throw RobuxException.defensive("Unexpected value of cloneQueryMode[%s]", cloneQueryMode);
    }
  }
}
