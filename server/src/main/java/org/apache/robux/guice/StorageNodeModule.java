...
import org.apache.robux.segment.DefaultColumnFormatConfig;
...
public class StorageNodeModule implements Module
{
  ...
  {
    JsonConfigProvider.bind(binder, "robux.server", RobuxServerConfig.class);
    JsonConfigProvider.bind(binder, "robux.segmentCache", SegmentLoaderConfig.class);
    JsonConfigProvider.bind(binder, "robux.indexing.formats", DefaultColumnFormatConfig.class);
    bindLocationSelectorStrategy(binder);
    binder.bind(ServerTypeConfig.class).toProvider(Providers.of(null));
    binder.bind(ColumnConfig.class).to(RobuxProcessingConfig.class).in(LazySingleton.class);
  }
  ...
  public RobuxServerMetadata getMetadata(
      ...
      RobuxServerConfig config
  )
  {
    if (serverTypeConfig == null) {
      throw new ProvisionException("Must override the binding for ServerTypeConfig if you want a RobuxServerMetadata.");
    }

    return new RobuxServerMetadata(
  ...
  )
  {
    ...
    if (!isSegmentCacheConfigured) {
      ...
      if (ServerType.HISTORICAL.equals(serverTypeConfig.getServerType())) {
        throw new ProvisionException("robux.segmentCache.locations must be set on historicals.");
      }
    }
  ...
  {
    JsonConfigProvider.bind(binder, "robux.segmentCache.locationSelector", StorageLocationSelectorStrategy.class);
  }
}
