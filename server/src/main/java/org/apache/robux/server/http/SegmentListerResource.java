...
import org.apache.robux.client.HttpServerInventoryView;
...
import org.apache.robux.server.coordination.DataSegmentChangeResponse;
import org.apache.robux.server.coordination.SegmentLoadDropHandler;
import org.apache.robux.server.coordinator.loading.HttpLoadQueuePeon;
...
/**
 * Endpoints exposed here are to be used only for robux internal management of segments by Coordinators, Brokers etc.
 */
@Path("/robux-internal/v1/segments")
...
public class SegmentListerResource
{
   ...
   *
   * (1) Client sends first request /robux/internal/v1/segments?counter=-1&timeout=<timeout>
   * Server responds with list of segments currently served and a <counter,hash> pair.
   *
   * (2) Client sends subsequent requests /robux/internal/v1/segments?counter=<counter>&hash=<hash>&timeout=<timeout>
