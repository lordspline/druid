...
import org.apache.robux.java.util.common.guava.SequenceWrapper;
...
/**
 * Class that helps a Robux server (broker, historical, etc) manage the lifecycle of a query that it is handling. It
...
public class QueryLifecycle
{
   ...
   * @return results
   * @throws RobuxException if the given authorizationResult deny access, which indicates a bug
   */
   ...
   * @throws RobuxException if the current state is not NEW, which indicates a bug
   */
  ...
  /**
   * Returns {@link AuthorizationResult} based on {@code ROBUX_AUTHENTICATION_RESULT} in the given request, base query
   ...
   * @throws RobuxException        if the current state is not INITIALIZED, which indicates a bug
   */
  ...
  )
  {
    ...
    if (!state.equals(State.AUTHORIZED)) {
      throw RobuxException.defensive("Unexpected state [%s], expecting [%s].", state, State.AUTHORIZED);
    }
  }
   ...
   * @throws RobuxException if the current state is not AUTHORIZED, which indicates a bug
   */
  public <T> QueryResponse<T> execute()
  {
    transition(State.AUTHORIZED, State.EXECUTING);

    final ResponseContext responseContext = DirectRobuxClient.makeResponseContextForQuery();
  ...
  )
  {
    ...
    try {
      ...
      QueryMetrics queryMetrics = RobuxMetrics.makeRequestMetrics(
          queryMetricsFactory,
          toolChest,
          baseQuery,
          StringUtils.nullToEmptyNonRobuxDataString(remoteAddress)
      );
  ...
  {
    if (state != from) {
      throw RobuxException.defensive("Cannot transition from[%s] to[%s], current state[%s].", from, to, state);
    }
   ...
   * Stack traces are logged if {@link QueryContext#isDebug()}, or if the {@link RobuxException.Persona} is
   * {@link RobuxException.Persona#DEVELOPER} or {@link RobuxException.Persona#OPERATOR}. The idea is that other
  ...
  public static boolean shouldLogStackTrace(final Throwable e, final QueryContext queryContext)
  {
    if (queryContext.isDebug() || e.getMessage() == null) {
      return true;
    } else if (e instanceof RobuxException) {
      final RobuxException.Persona persona = ((RobuxException) e).getTargetPersona();
      return persona == RobuxException.Persona.OPERATOR || persona == RobuxException.Persona.DEVELOPER;
    } else {
      return false;
    }
  }
