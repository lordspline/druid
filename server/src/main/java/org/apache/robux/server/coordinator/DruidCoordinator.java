...
import org.apache.robux.indexer.CompactionEngine;
...
import org.apache.robux.server.compaction.CompactionRunSimulator;
...
import org.apache.robux.server.coordinator.duty.CloneHistoricals;
...
import org.apache.robux.server.coordinator.duty.CoordinatorDuty;
import org.apache.robux.server.coordinator.duty.CoordinatorDutyGroup;
import org.apache.robux.server.coordinator.duty.DutyGroupStatus;
import org.apache.robux.server.coordinator.duty.KillAuditLog;
import org.apache.robux.server.coordinator.duty.KillCompactionConfig;
import org.apache.robux.server.coordinator.duty.KillDatasourceMetadata;
import org.apache.robux.server.coordinator.duty.KillRules;
import org.apache.robux.server.coordinator.duty.KillStalePendingSegments;
import org.apache.robux.server.coordinator.duty.KillSupervisors;
import org.apache.robux.server.coordinator.duty.KillUnreferencedSegmentSchema;
import org.apache.robux.server.coordinator.duty.KillUnusedSegments;
import org.apache.robux.server.coordinator.duty.MarkEternityTombstonesAsUnused;
import org.apache.robux.server.coordinator.duty.MarkOvershadowedSegmentsAsUnused;
import org.apache.robux.server.coordinator.duty.MetadataAction;
import org.apache.robux.server.coordinator.duty.PrepareBalancerAndLoadQueues;
import org.apache.robux.server.coordinator.duty.RunRules;
import org.apache.robux.server.coordinator.duty.UnloadUnusedSegments;
...
import org.apache.robux.server.coordinator.stats.CoordinatorStat;
 ...
 */
@ManageLifecycle
public class RobuxCoordinator
{
  private static final EmittingLogger log = new EmittingLogger(RobuxCoordinator.class);

  private final Object lock = new Object();
  private final RobuxCoordinatorConfig config;
  ...
  private final RobuxLeaderSelector coordLeaderSelector;
  ...
  public RobuxCoordinator(
      ...
      @Coordinator RobuxLeaderSelector coordLeaderSelector,
  ...
  public Map<String, Double> getDatasourceToLoadStatus()
  {
    ...
    for (ImmutableRobuxDataSource dataSource : snapshot.getDataSourcesWithAllUsedSegments()) {
      ...
      for (RobuxServer robuxServer : serverInventoryView.getInventory()) {
        final RobuxDataSource loadedView = robuxServer.getDataSource(dataSource.getName());
        if (loadedView != null) {
          // This does not use segments.removeAll(loadedView.getSegments()) for performance reasons.
          // Please see https://github.com/apache/robux/pull/5632 for more info.
    ...
    synchronized (lock) {
      ...
      coordLeaderSelector.registerListener(
          new RobuxLeaderSelector.Listener()
          {
            ...
            {
              RobuxCoordinator.this.becomeLeader();
            }

            @Override
            public void stopBeingLeader()
            {
              RobuxCoordinator.this.stopBeingLeader();
            }
          }
      );
    }
  ...
  private void becomeLeader()
  {
    synchronized (lock) {
      ...
      log.warn(
          ...
          + " Use API '/robux/coordinator/v1/duties' to get current status.",
    ...
    public void run()
    {
      try {
        ...
        if (metadataManager.isStarted() && serverInventoryView.isStarted()) {
          ...
          final RobuxCoordinatorRuntimeParams params = RobuxCoordinatorRuntimeParams
              .builder()
  ...
  // Duties that read/update the state in the RobuxCoordinator class
