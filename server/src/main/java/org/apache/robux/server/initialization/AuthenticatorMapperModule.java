...
public class AuthenticatorMapperModule implements RobuxModule
{
  private static final String AUTHENTICATOR_PROPERTIES_FORMAT_STRING = "robux.auth.authenticator.%s";
  ...
  private static class AuthenticatorMapperProvider implements Provider<AuthenticatorMapper>
  {
    ...
    public AuthenticatorMapper get()
    {
      ...
      for (String authenticatorName : authenticators) {
        ...
        String nameProperty = StringUtils.format("robux.auth.authenticator.%s.name", authenticatorName);
