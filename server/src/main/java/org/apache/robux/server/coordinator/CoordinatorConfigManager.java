 ...
 * and {@link RobuxCompactionConfig}.
 */
public class CoordinatorConfigManager
{
  ...
  public RobuxCompactionConfig getCurrentCompactionConfig()
  {
    RobuxCompactionConfig config = jacksonConfigManager.watch(
   ...
   * config store. This method is also compatible with pre-0.22.0 versions of Robux.
   *
  ...
  public ConfigManager.SetResult getAndUpdateCompactionConfig(
      UnaryOperator<RobuxCompactionConfig> operator,
      AuditInfo auditInfo
  )
  {
    ...
    // persisted by older versions of Robux which didn't have fields such as 'granularitySpec'.
    ...
    RobuxCompactionConfig current = convertBytesToCompactionConfig(currentBytes);
    RobuxCompactionConfig updated = operator.apply(current);
  ...
  public RobuxCompactionConfig convertBytesToCompactionConfig(byte[] bytes)
  {
  ...
  {
    UnaryOperator<RobuxCompactionConfig> operator = current -> {
  ...
  {
    UnaryOperator<RobuxCompactionConfig> operator = current -> current.withClusterConfig(config);
    return updateConfigHelper(operator, auditInfo);
  }
  ...
  {
    UnaryOperator<RobuxCompactionConfig> callable = current -> current.withDatasourceConfig(config);
  ...
  {
    final RobuxCompactionConfig current = getCurrentCompactionConfig();
  ...
  {
    UnaryOperator<RobuxCompactionConfig> callable = current -> {
  ...
  )
  {
    ...
    try {
      ...
      for (AuditEntry audit : auditEntries) {
        RobuxCompactionConfig compactionConfig = convertBytesToCompactionConfig(
  ...
  private boolean updateConfigHelper(
      UnaryOperator<RobuxCompactionConfig> configUpdateOperator,
      AuditInfo auditInfo
  )
  {
    int attemps = 0;
