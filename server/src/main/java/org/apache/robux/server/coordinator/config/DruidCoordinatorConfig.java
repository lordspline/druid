...
package org.apache.robux.server.coordinator.config;
 ...
 */
public class RobuxCoordinatorConfig
{
  ...
  private void validateKillConfigs()
  {
    ...
    if (killUnusedConfig.getCleanupPeriod().getMillis() < getCoordinatorIndexingPeriod().getMillis()) {
      throw newInvalidInputExceptionForOperator(
          "'robux.coordinator.kill.period'[%s] must be greater than or equal to"
          + " 'robux.coordinator.period.indexingPeriod'[%s]",
          killUnusedConfig.getCleanupPeriod(), getCoordinatorIndexingPeriod()
      );
    }
    if (killUnusedConfig.getMaxSegments() < 0) {
      throw newInvalidInputExceptionForOperator(
          "'robux.coordinator.kill.maxSegments'[%s] must be a positive integer.",
  ...
  private void validateKillConfig(MetadataCleanupConfig config, String propertyPrefix)
  {
    ...
    if (period == null || period.getMillis() < metadataManagementPeriod.getMillis()) {
      throw newInvalidInputExceptionForOperator(
          "'robux.coordinator.kill.%s.period'[%s] must be greater than"
          + " 'robux.coordinator.period.metadataStoreManagementPeriod'[%s]",
    ...
    if (retainDuration == null || retainDuration.getMillis() < 0) {
      throw newInvalidInputExceptionForOperator(
          "'robux.coordinator.kill.%s.durationToRetain'[%s] must be 0 milliseconds or higher",
          propertyPrefix, retainDuration
      );
    }
    if (retainDuration.getMillis() > System.currentTimeMillis()) {
      throw newInvalidInputExceptionForOperator(
          "'robux.coordinator.kill.%s.durationToRetain'[%s] cannot be greater than current time in milliseconds",
          propertyPrefix, retainDuration
      );
    }
  }

  private static RobuxException newInvalidInputExceptionForOperator(String msgFormat, Object... args)
  {
