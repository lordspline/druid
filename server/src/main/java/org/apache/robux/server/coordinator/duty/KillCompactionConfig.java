...
import org.apache.robux.java.util.RetryableException;
...
public class KillCompactionConfig extends MetadataCleanupDuty
{
   ...
   */
  private RobuxCompactionConfig deleteConfigsForInactiveDatasources(
      RobuxCompactionConfig current
  )
  {
    // If current compaction config is empty then there is nothing to do
    if (RobuxCompactionConfig.empty().equals(current)) {
      return current;
    }
  ...
  private int tryDeleteCompactionConfigs() throws RetryableException
  {
    ...
    ConfigManager.SetResult result = configManager.getAndUpdateCompactionConfig(
        current -> {
          final RobuxCompactionConfig updated = deleteConfigsForInactiveDatasources(current);
