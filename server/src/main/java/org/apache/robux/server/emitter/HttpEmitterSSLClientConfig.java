...
/**
 * This is kept separate from {@link org.apache.robux.java.util.emitter.core.HttpEmitterConfig} because {@link PasswordProvider}
 * is currently located in robux-api. The java-util module which contains HttpEmitterConfig cannot import
 * PasswordProvider because this would introduce a circular dependence between java-util and robux-api.
 *
 * PasswordProvider could be moved to java-util, but PasswordProvider is annotated with
 * {@link org.apache.robux.guice.annotations.ExtensionPoint}, which would also have to be moved.
 *
 * It would be easier to resolve these issues and merge the TLS-related config with HttpEmitterConfig once
 * https://github.com/apache/robux/issues/4312 is resolved, so the TLS config is kept separate for now.
 */
