...
public class CoordinatorDynamicConfigSyncer
{
  ...
  private final RobuxNodeDiscoveryProvider robuxNodeDiscovery;
  ...
  )
  {
    ...
    this.robuxNodeDiscovery = robuxNodeDiscoveryProvider;
  ...
  void broadcastConfigToBrokers()
  {
    ...
    for (DiscoveryRobuxNode broker : getKnownBrokers()) {
   ...
   * param.
   */
  private void pushConfigToBroker(DiscoveryRobuxNode broker)
  {
    final Stopwatch stopwatch = Stopwatch.createStarted();
    ...
    try {
      ...
      emitStat(
          ...
          RowKey.with(Dimension.SERVER, broker.getRobuxNode().getHostAndPortToUse()).build(),
          1
      );
    }
    emitStat(
  ...
  /**
   * Returns a collection of {@link DiscoveryRobuxNode} for all brokers currently known to the robuxNodeDiscovery.
   */
  private Collection<DiscoveryRobuxNode> getKnownBrokers()
  {
    return robuxNodeDiscovery.getForNodeRole(NodeRole.BROKER)
                             .getAllNodes();
  }
  ...
  /**
   * Utility method to convert {@link DiscoveryRobuxNode} to a {@link ServiceLocation}
   */
  @Nullable
  private static ServiceLocation convertDiscoveryNodeToServiceLocation(DiscoveryRobuxNode discoveryRobuxNode)
  {
