...
/**
 */
@Path("/robux/coordinator/v1/servers")
@ResourceFilters(StateResourceFilter.class)
public class ServersResource
{
  private static Map<String, Object> makeSimpleServer(RobuxServer input)
  {
  ...
  private static Map<String, Object> makeFullServer(RobuxServer server)
  {
    return new ImmutableMap.Builder<String, Object>()
   ...
   * segments returned from {@link RobuxServer#iterateAllSegments()} are unique. This is not a part of the {@link
   * RobuxServer} API to not let abuse this map (like trying to get() from it).
   */
  private static Map<SegmentId, DataSegment> createLazySegmentsMap(RobuxServer server)
  {
  ...
  public Response getClusterServers(@QueryParam("full") String full, @QueryParam("simple") String simple)
  {
    ...
    return builder
        .entity(Collections2.transform(serverInventoryView.getInventory(), RobuxServer::getHost))
        .build();
  }
  ...
  {
    RobuxServer server = serverInventoryView.getInventoryValue(serverName);
    if (server == null) {
