...
/**
 * A Helper class that uses RobuxNodeDiscovery to discover lookup nodes and tiers.
 */
public class LookupNodeDiscovery
{
  private final RobuxNodeDiscovery robuxNodeDiscovery;

  LookupNodeDiscovery(RobuxNodeDiscoveryProvider robuxNodeDiscoveryProvider)
  {
    this.robuxNodeDiscovery = robuxNodeDiscoveryProvider.getForService(LookupNodeService.DISCOVERY_SERVICE_KEY);
  }
  ...
  public Collection<HostAndPortWithScheme> getNodesInTier(String tier)
  {
    return Collections2.transform(
        Collections2.filter(
            robuxNodeDiscovery.getAllNodes(),
            node -> {
              if (node == null) {
                return false;
              }
        ...
        input -> HostAndPortWithScheme.fromString(
            input.getRobuxNode().getServiceScheme(),
            input.getRobuxNode().getHostAndPortToUse()
        )
    );
  }
  ...
  public Set<String> getAllTiers()
  {
    ...
    robuxNodeDiscovery.getAllNodes().forEach(
        node -> {
