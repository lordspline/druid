 ...
 * For example {@link org.apache.robux.client.HttpServerInventoryView} uses it to keep segment state in sync with data
 * nodes which expose the segment state via HTTP endpoint in {@link
 * org.apache.robux.server.http.SegmentListerResource#getSegments}.
 */
      ...
      try {
        ...
        if (executor.isShutdown()) {
          ...
          log.warn(
              ...
              + " If the issue persists, try restarting this Robux service.",
              logIdentity
          );
        }
      }
  ...
  private void markServerUnstableAndAlert(Throwable throwable, String action)
  {
    ...
    if (sinceUnstable.hasElapsed(maxUnstableDuration)) {
      String alertMessage = StringUtils.format(
          "%s. Try restarting the Robux process on server[%s].",
