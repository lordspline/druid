...
public class AllowOptionsResourceFilter implements Filter
{
      ...
      throws IOException, ServletException
  {
    ...
    // Robux itself doesn't explictly handle OPTIONS requests, no resource handler will authorize such requests.
    // so this filter catches all OPTIONS requests and authorizes them.
    if (HttpMethod.OPTIONS.equals(httpReq.getMethod())) {
      if (httpReq.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT) == null) {
      ...
      httpReq.setAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED, true);
    }
