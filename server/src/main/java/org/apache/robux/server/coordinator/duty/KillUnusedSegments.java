...
import org.apache.robux.collections.CircularList;
 ...
 *
 * @see org.apache.robux.indexing.common.task.KillUnusedSegmentsTask for details
...
public class KillUnusedSegments implements CoordinatorDuty
{
  ...
  )
  {
    ...
    if (this.ignoreDurationToRetain) {
      log.info(
          "robux.coordinator.kill.durationToRetain[%s] will be ignored when discovering segments to kill "
          + "because robux.coordinator.kill.ignoreDurationToRetain is set to true.",
          durationToRetain
      );
    }
  ...
  @Override
  public RobuxCoordinatorRuntimeParams run(final RobuxCoordinatorRuntimeParams params)
  {
    if (canDutyRun()) {
  ...
  private RobuxCoordinatorRuntimeParams runInternal(final RobuxCoordinatorRuntimeParams params)
  {
