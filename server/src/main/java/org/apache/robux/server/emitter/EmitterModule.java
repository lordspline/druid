...
public class EmitterModule implements Module
{
  ...
  private static final String EMITTER_PROPERTY = "robux.emitter";
  ...
  public void configure(Binder binder)
  {
    ...
    extraServiceDimensions
        .addBinding("version")
        .toInstance(StringUtils.nullToEmptyNonRobuxDataString(version)); // Version is null during `mvn test`.
  }

  @Provides
  @ManageLifecycle
  public ServiceEmitter getServiceEmitter(
      @Self Supplier<RobuxNode> configSupplier,
      Emitter emitter,
      @ExtraServiceDimensions Map<String, String> extraServiceDimensions
  )
  {
    final RobuxNode config = configSupplier.get();
