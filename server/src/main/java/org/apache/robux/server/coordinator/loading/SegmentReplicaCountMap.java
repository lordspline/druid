...
public class SegmentReplicaCountMap
{
  ...
  static SegmentReplicaCountMap create(RobuxCluster cluster)
  {
  ...
  private void initReplicaCounts(RobuxCluster cluster)
  {
    ...
    cluster.getBrokers().forEach(broker -> {
      final ImmutableRobuxServer server = broker.getServer();
    ...
    cluster.getRealtimes().forEach(realtime -> {
      final ImmutableRobuxServer server = realtime.getServer();
      for (DataSegment segment : server.iterateAllSegments()) {
        computeIfAbsent(segment.getId(), server.getTier())
            .incrementLoadedOnNonHistoricalServer();
      }
