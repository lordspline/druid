...
import org.apache.robux.client.ServerViewUtil;
...
import org.apache.robux.query.LocatedSegmentDescriptor;
...
public class ClientInfoResource
{
  ...
  private Set<String> getAllDataSources()
  {
    return serverInventoryView
        .getInventory()
        .stream()
        .flatMap(server -> server.getDataSources().stream().map(RobuxDataSource::getName))
  ...
  private Set<DataSegment> getAllSegmentsForDataSource(String dataSourceName)
  {
    return serverInventoryView
        ...
        .stream()
        .flatMap(server -> {
          RobuxDataSource dataSource = server.getDataSource(dataSourceName);
          if (dataSource == null) {
            return Stream.empty();
          }
