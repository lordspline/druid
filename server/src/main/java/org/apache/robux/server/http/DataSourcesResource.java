...
import org.apache.robux.audit.AuditEntry;
...
import org.apache.robux.guice.annotations.PublicApi;
...
import org.apache.robux.server.coordinator.rules.LoadRule;
import org.apache.robux.server.coordinator.rules.Rule;
import org.apache.robux.server.http.security.DatasourceResourceFilter;
 ...
 *
 */
@Path("/robux/coordinator/v1/datasources")
public class DataSourcesResource
{
  ...
  )
  {
    ...
    final Set<ImmutableRobuxDataSource> datasources =
    ...
    if (full != null) {
      ...
      entity = datasources.stream().map(ImmutableRobuxDataSource::getName).collect(Collectors.toList());
    }
  ...
  {
    final ImmutableRobuxDataSource dataSource = getQueryableDataSource(dataSourceName);
      ...
      if (rootCause instanceof HttpResponseException) {
        ...
        if (status.getCode() == 404) {
          ...
          return ServletResourceUtils.buildErrorResponseFrom(
              RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                            .ofCategory(RobuxException.Category.NOT_FOUND)
                            .build(errorMessage)
          );
        }
      }
  ...
  )
  {
    ...
    final ImmutableRobuxDataSource immutableRobuxDataSource = snapshot.getDataSource(dataSourceName);
    if (immutableRobuxDataSource == null) {
      return logAndCreateDataSourceNotFoundResponse(dataSourceName);
    }
  ...
  {
    ImmutableRobuxDataSource dataSource = getQueryableDataSource(dataSourceName);
    if (dataSource == null) {
      return logAndCreateDataSourceNotFoundResponse(dataSourceName);
    }
  ...
  )
  {
    ...
    if (segmentId == null) {
      return Response.status(Response.Status.BAD_REQUEST).entity(
          org.apache.robux.java.util.common.StringUtils.format(
  ...
  public Response getTiersWhereSegmentsAreServed(@PathParam("dataSourceName") String dataSourceName)
  {
    ...
    for (RobuxServer robuxServer : serverInventoryView.getInventory()) {
      if (robuxServer.getDataSource(dataSourceName) != null) {
        retVal.add(robuxServer.getTier());
      }
    }
  ...
  @Nullable
  private ImmutableRobuxDataSource getQueryableDataSource(final String dataSourceName)
  {
    List<RobuxDataSource> dataSources = serverInventoryView
        .getInventory()
        .stream()
    ...
    // RobuxDataSource objects (belonging to different servers) are independently, concurrently mutable objects.
    ...
    for (RobuxDataSource dataSource : dataSources) {
    ...
    return new ImmutableRobuxDataSource(dataSourceName, Collections.emptyMap(), segmentMap);
  }
  ...
  private Pair<DataSegment, Set<String>> getServersWhereSegmentIsServed(SegmentId segmentId)
  {
    ...
    for (RobuxServer robuxServer : serverInventoryView.getInventory()) {
      DataSegment currSegment = robuxServer.getSegment(segmentId);
      if (currSegment != null) {
        theSegment = currSegment;
        servers.add(robuxServer.getHost());
      }
    }
  ...
  private Map<String, Object> makeSimpleDatasource(ImmutableRobuxDataSource input)
  {
  ...
  private Map<String, Map<String, Object>> getSimpleDatasource(String dataSourceName)
  {
    ...
    String tier;
    for (RobuxServer robuxServer : serverInventoryView.getInventory()) {
      RobuxDataSource robuxDataSource = robuxServer.getDataSource(dataSourceName);
      tier = robuxServer.getTier();

      if (robuxDataSource == null) {
        continue;
      }
      ...
      for (DataSegment dataSegment : robuxDataSource.getSegments()) {
        // tier segments stats
      ...
      if (tierStats == null) {
        ...
        tiers.put(robuxServer.getTier(), tierStats);
      }
  ...
  static boolean isSegmentLoaded(Iterable<ImmutableSegmentLoadInfo> servedSegments, SegmentDescriptor descriptor)
  {
    for (ImmutableSegmentLoadInfo segmentLoadInfo : servedSegments) {
      if (segmentLoadInfo.getSegment().getInterval().contains(descriptor.getInterval())
          ...
          && Iterables.any(
          segmentLoadInfo.getServers(), RobuxServerMetadata::isSegmentReplicationTarget
      )) {
        return true;
      }
    }
