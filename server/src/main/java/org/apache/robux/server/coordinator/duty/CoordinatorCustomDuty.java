 ...
 * so that users can register their own duties without modifying Core Robux classes.
 *
 ...
 * Module's {@link RobuxModule#getJacksonModules()}. Once these steps are done, user will be able to load their
 * custom coordinator duty using the following properties:
 * robux.coordinator.dutyGroups=[<GROUP_NAME>]
 * robux.coordinator.<GROUP_NAME>.duties=[<DUTY_NAME_MATCHING_JSON_TYPE_NAME>]
 * robux.coordinator.<GROUP_NAME>.duty.<DUTY_NAME_MATCHING_JSON_TYPE_NAME>.<SOME_CONFIG_1>=100
 * robux.coordinator.<GROUP_NAME>.duty.<DUTY_NAME_MATCHING_JSON_TYPE_NAME>.<SOME_CONFIG_2>=200
 * robux.coordinator.<GROUP_NAME>.period="P1D"
 *
 * The duties can be grouped into multiple groups as per the elements in list robux.coordinator.dutyGroups.
 ...
 *
 * Note that custom duty does not implement CoordinatorDuty directly as existing Core Robux Coordinator Duties
