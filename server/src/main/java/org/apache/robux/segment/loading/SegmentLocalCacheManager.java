...
package org.apache.robux.segment.loading;
...
import org.apache.robux.segment.SegmentLazyLoadFailCallback;
...
public class SegmentLocalCacheManager implements SegmentCacheManager
{
   ...
   * </ul>
   *
   * @throws RobuxException if none of the configurations are set, and the info directory cannot be determined.
   */
  private File getEffectiveInfoDir()
  {
    ...
    if (config.getInfoDir() != null) {
      ...
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
          ...
          .build("Could not determine infoDir. Make sure 'robux.segmentCache.infoDir' "
                 + "or 'robux.segmentCache.locations' is set correctly.");
    }
    return infoDir;
  }
   ...
   * Because each time, Robux will delete the 'downloadStartMarker' file after pulling and unzip the segments from DeepStorage.
   ...
   * 1. robux don't have the write access to this location, most likely the administrator doesn't config it correctly<p>
   * 2. disk failure, robux can't read/write to this disk anymore
   * <p>
    ...
    synchronized (lock) {
      try {
        ...
        for (StorageLocation location : locations) {
          ...
          if (localStorageDir.exists()) {
            // Robux creates folders of the form dataSource/interval/version/partitionNum.
