...
public class SegmentMetadataQuerySegmentWalker implements QuerySegmentWalker
{
  ...
  public SegmentMetadataQuerySegmentWalker(
      ...
      final RobuxHttpClientConfig httpClientConfig,
  ...
  )
  {
    ...
    for (Pair<SegmentDescriptor, SegmentLoadInfo> segmentAndServer : segmentAndServers) {
      final RobuxServerMetadata robuxServerMetadata = segmentAndServer.rhs.pickOne();

      if (robuxServerMetadata == null) {
        log.makeAlert(
        ...
        serverSegments.computeIfAbsent(robuxServerMetadata.getName(), s -> new ArrayList<>()).add(segmentAndServer.lhs);
      }
    }
