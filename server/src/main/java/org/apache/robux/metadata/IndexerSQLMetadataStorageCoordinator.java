...
import org.apache.robux.metadata.segment.SegmentMetadataReadTransaction;
...
public class IndexerSQLMetadataStorageCoordinator implements IndexerMetadataStorageCoordinator
{
  ...
  )
  {
    ...
    // have difficulty with large unique keys (see https://github.com/apache/robux/issues/2319)
  ...
  private <T> T inReadOnlyTransaction(Function<SqlSegmentsMetadataQuery, T> sqlQuery)
  {
    try {
      ...
      if (rootCause instanceof RobuxException) {
        throw (RobuxException) rootCause;
      } else {
        throw t;
      }
    }
