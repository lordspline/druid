...
import org.apache.robux.metadata.SqlSegmentsMetadataQuery;
...
public class HeapMemorySegmentMetadataCache implements SegmentMetadataCache
{
  ...
  public void becomeLeader()
  {
    synchronized (cacheStateLock) {
      if (isEnabled()) {
        if (currentCacheState == CacheState.STOPPED) {
          throw RobuxException.defensive("Cache has not been started yet");
   ...
   *
   * @throws RobuxException if the cache is disabled, stopped or not leader.
   */
  private void verifyCacheIsUsableAndAwaitSyncIf(boolean shouldWait)
  {
    if (!isEnabled()) {
      throw RobuxException.defensive("Segment metadata cache is not enabled.");
    }
    ...
    synchronized (cacheStateLock) {
      ...
      while (waitCondition.get() && totalWaitTime.millisElapsed() <= timeoutMillis) {
        try {
          ...
          throw RobuxException.defensive(e, "Interrupted while waiting for cache to be ready");
        }
        catch (Exception e) {
          log.error(e, "Error while waiting for cache to be ready");
          throw RobuxException.defensive(e, "Error while waiting for cache to be ready");
        }
      }
