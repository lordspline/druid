    ...
    implements MetadataStorageActionHandler<EntryType, StatusType, LogType, LockType>
{
  ...
  )
  {
    try {
      getConnector().retryWithHandle(
          ...
          this::isTransientRobuxException
      );
    }
   ...
   * exception thrown in a {@link RobuxException}. When used in a HandleCallback,
  ...
  )
  {
    try {
      ...
      throw wrapInRobuxException(entryId, t);
    }
  }
  ...
  private boolean isTransientRobuxException(Throwable t)
  {
    if (t instanceof CallbackFailedException) {
      return isTransientRobuxException(t.getCause());
    } else if (t instanceof RobuxException) {
      return Boolean.parseBoolean(((RobuxException) t).getContextValue(CONTEXT_KEY_IS_TRANSIENT));
    } else {
  ...
  /**
   * Wraps the given error in a user-friendly RobuxException.
   */
  private RobuxException wrapInRobuxException(String taskId, Throwable t)
  {
    if (isStatementException(t) && getEntry(taskId).isPresent()) {
      ...
      return RobuxException.forPersona(RobuxException.Persona.OPERATOR)
