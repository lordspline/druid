...
/**
 * Interface for supporting Overlord and Coordinator Leader Elections in TaskMaster and RobuxCoordinator
 * which expect appropriate implementation available in guice annotated with @IndexingService and @Coordinator
 * respectively.
 *
 * Usage is as follows:
 * On lifecycle start:
 *  robuxLeaderSelector.registerListener(myListener);
 *
 * On lifecycle stop:
 *  robuxLeaderSelector.unregisterListener();
 */
public interface RobuxLeaderSelector
{
  /**
   ...
   * term when they started. RobuxCoordinator class uses it to do intermittent checks and stop the activity
   * as needed.
   */
  int localTerm();
