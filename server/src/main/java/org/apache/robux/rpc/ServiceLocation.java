...
public class ServiceLocation
{
  ...
  /**
   * Create a service location based on a {@link RobuxNode}, without a base path.
   */
  public static ServiceLocation fromRobuxNode(final RobuxNode robuxNode)
  {
    return new ServiceLocation(robuxNode.getHost(), robuxNode.getPlaintextPort(), robuxNode.getTlsPort(), "");
  }
  ...
  /**
   * Create a service location based on a {@link RobuxServerMetadata}.
   *
   * @throws IllegalArgumentException if the server metadata cannot be mapped to a service location.
   */
  public static ServiceLocation fromRobuxServerMetadata(final RobuxServerMetadata robuxServerMetadata)
  {
    final String host = getHostFromString(
        Preconditions.checkNotNull(
            robuxServerMetadata.getHost(),
            "Host was null for robux server metadata[%s]",
            robuxServerMetadata
        )
    );
    int plaintextPort = getPortFromString(robuxServerMetadata.getHostAndPort());
    int tlsPort = getPortFromString(robuxServerMetadata.getHostAndTlsPort());
