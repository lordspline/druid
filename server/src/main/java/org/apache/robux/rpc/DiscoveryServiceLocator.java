...
/**
 * A {@link ServiceLocator} that uses {@link RobuxNodeDiscovery}.
 */
public class DiscoveryServiceLocator implements ServiceLocator
{
  ...
  private final RobuxNodeDiscovery.Listener listener;
  ...
  @GuardedBy("this")
  private RobuxNodeDiscovery discovery = null;

  public DiscoveryServiceLocator(final RobuxNodeDiscoveryProvider discoveryProvider, final NodeRole nodeRole)
  {
  ...
  private class Listener implements RobuxNodeDiscovery.Listener
  {
    @Override
    public void nodesAdded(final Collection<DiscoveryRobuxNode> nodes)
    {
      synchronized (DiscoveryServiceLocator.this) {
        for (final DiscoveryRobuxNode node : nodes) {
          locations.add(ServiceLocation.fromRobuxNode(node.getRobuxNode()));
        }
      }
    }
    ...
    public void nodesRemoved(final Collection<DiscoveryRobuxNode> nodes)
    {
      synchronized (DiscoveryServiceLocator.this) {
        for (final DiscoveryRobuxNode node : nodes) {
          locations.remove(ServiceLocation.fromRobuxNode(node.getRobuxNode()));
        }
      }
    }
