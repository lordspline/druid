...
import org.apache.robux.segment.join.NoopDataSource;
...
public class CachingClusteredClientCacheKeyManagerTest extends EasyMockSupport
{
  ...
  private SegmentServerSelector makeServerSelector(boolean isHistorical, int partitionNumber)
  {
    ...
    QueryableRobuxServer queryableRobuxServer = mock(QueryableRobuxServer.class);
    RobuxServer server = mock(RobuxServer.class);
    ...
    expect(serverSelector.pick(query, CloneQueryMode.EXCLUDECLONES)).andReturn(queryableRobuxServer).anyTimes();
    expect(queryableRobuxServer.getServer()).andReturn(server).anyTimes();
    expect(serverSelector.getSegment()).andReturn(segment).anyTimes();
    replay(serverSelector, queryableRobuxServer, server);
