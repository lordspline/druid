...
import org.apache.robux.client.selector.ConnectionCountServerSelectorStrategy;
...
public class DirectRobuxClientTest
{
  @ClassRule
  ...
  private DirectRobuxClient client;
  private QueryableRobuxServer queryableRobuxServer;
  ...
  public void setup()
  {
    ...
    queryableRobuxServer = new QueryableRobuxServer(
        new RobuxServer(
            "test1",
            "localhost",
            null,
    ...
    serverSelector.addServerAndUpdateSegment(queryableRobuxServer, serverSelector.getSegment());
  }
  ...
  {
    final URL url = new URL(StringUtils.format("http://%s/robux/v2/", hostName));
    ...
    DirectRobuxClient client2 = new DirectRobuxClient(
        conglomerateRule.getConglomerate(),
        QueryRunnerTestHelper.NOOP_QUERYWATCHER,
        new DefaultObjectMapper(),
        httpClient,
    ...
    QueryableRobuxServer queryableRobuxServer2 = new QueryableRobuxServer(
        new RobuxServer(
            "test1",
            "localhost",
            null,
    ...
    serverSelector.addServerAndUpdateSegment(queryableRobuxServer2, serverSelector.getSegment());

    TimeBoundaryQuery query = Robuxs.newTimeBoundaryQueryBuilder().dataSource("test").build();
    query = query.withOverriddenContext(ImmutableMap.of(DirectRobuxClient.QUERY_FAIL_TIME, Long.MAX_VALUE));
    ...
    Assert.assertEquals(serverSelector.pick(null, CloneQueryMode.EXCLUDECLONES), queryableRobuxServer2);
  ...
  public void testQueryTimeoutBeforeFuture() throws IOException, InterruptedException
  {
    ...
    query = query.withOverriddenContext(
        ImmutableMap.of(DirectRobuxClient.QUERY_FAIL_TIME, System.currentTimeMillis() + 250, "queryId", queryId)
    );
    ...
    Assert.assertEquals("url[http://localhost:8080/robux/v2/] timed out", actualException.getMessage());
    Assert.assertEquals(hostName, actualException.getHost());
    EasyMock.verify(httpClient);
  }
  ...
  public void testQueryTimeoutFromFuture()
  {
    ...
    TimeBoundaryQuery query = Robuxs.newTimeBoundaryQueryBuilder().dataSource("test").build();
    query = query.withOverriddenContext(
        ImmutableMap.of(DirectRobuxClient.QUERY_FAIL_TIME, System.currentTimeMillis() + 500, "queryId", queryId)
    );
