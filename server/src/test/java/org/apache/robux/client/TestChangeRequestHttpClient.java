...
public class TestChangeRequestHttpClient<R> implements HttpClient
{
  ...
  public void completeNextRequestWith(RobuxException robuxException)
  {
    results.add(new ResultHolder<>(null, null, robuxException));
  }
  ...
  private HttpResponse buildErrorResponse(RobuxException robuxException)
  {
    HttpResponse httpResponse = new DefaultHttpResponse(
        HttpVersion.HTTP_1_1,
        HttpResponseStatus.valueOf(robuxException.getStatusCode())
    );
    httpResponse.setContent(ChannelBuffers.buffer(0));

    ErrorResponse errorResponse = robuxException.toErrorResponse();
    try {
  ...
  private static class ResultHolder<R>
  {
    ...
    final RobuxException serverError;

    ResultHolder(
        Supplier<R> supplier,
        RuntimeException clientError,
        RobuxException serverError
    )
    {
