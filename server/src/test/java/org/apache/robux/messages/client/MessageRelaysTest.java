...
import org.apache.robux.messages.MessageBatch;
...
public class MessageRelaysTest
{
  ...
  private static final RobuxNode OUTBOX_NODE = new RobuxNode("service", "host", false, 80, -1, true, false);
  private static final DiscoveryRobuxNode OUTBOX_DISCO_NODE = new DiscoveryRobuxNode(
      new RobuxNode("service", "host", false, 80, -1, true, false),
      NodeRole.HISTORICAL,
  ...
  private static class TestMessageListener implements MessageListener<String>
  {
    ...
    @Override
    public synchronized void serverAdded(RobuxNode node)
    {
      adds++;
    }
    ...
    @Override
    public synchronized void serverRemoved(RobuxNode node)
    {
      removes++;
    }
  ...
  /**
   * Implementation of {@link RobuxNodeDiscovery} that allows firing listeners on command.
   */
