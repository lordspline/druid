...
public class BroadcastDistributionRuleTest
{
  ...
  public void testSegmentIsBroadcastToAllTiers()
  {
    ...
    RobuxCluster cluster = RobuxCluster.builder().add(serverT11).add(serverT21).build();
    RobuxCoordinatorRuntimeParams params = makeParamsWithUsedSegments(cluster, wikiSegment);
  ...
  public void testSegmentIsNotBroadcastToServerIfAlreadyLoaded()
  {
    ...
    RobuxCluster cluster = RobuxCluster.builder().add(serverT11).add(serverT12).build();
    RobuxCoordinatorRuntimeParams params = makeParamsWithUsedSegments(cluster, wikiSegment);
  ...
  public void testSegmentIsNotBroadcastToDecommissioningServer()
  {
    ...
    RobuxCluster cluster = RobuxCluster.builder()
                                       .add(activeServer)
  ...
  public void testSegmentIsBroadcastToAllServerTypes()
  {
    final ServerHolder broker = new ServerHolder(
        create10gbServer(ServerType.BROKER, "broker_tier").toImmutableRobuxServer(),
        new TestLoadQueuePeon()
    );
    final ServerHolder indexer = new ServerHolder(
        create10gbServer(ServerType.INDEXER_EXECUTOR, TIER_2).toImmutableRobuxServer(),
        new TestLoadQueuePeon()
    );
  ...
  public void testReasonForBroadcastFailure()
  {
    ...
    final ServerHolder serverWithNoDiskSpace = new ServerHolder(
        new RobuxServer("server1", "server1", null, 0L, ServerType.HISTORICAL, TIER_1, 0)
    ...
    final ServerHolder serverWithFullQueue = new ServerHolder(
        create10gbServer(ServerType.HISTORICAL, TIER_1).toImmutableRobuxServer(),
  ...
  private RobuxCoordinatorRuntimeParams makeParamsWithUsedSegments(
  ...
  {
    RobuxServer server = create10gbServer(ServerType.HISTORICAL, tier);
    for (DataSegment segment : segments) {
      server.addDataSegment(segment);
    }
    return new ServerHolder(server.toImmutableRobuxServer(), new TestLoadQueuePeon());
  }
  ...
  private ServerHolder createDecommissioningHistorical(String tier, DataSegment... segments)
  {
    ...
    return new ServerHolder(server.toImmutableRobuxServer(), new TestLoadQueuePeon(), true);
  }

  private RobuxServer create10gbServer(ServerType type, String tier)
  {
    final String name = "server_" + serverId++;
    return new RobuxServer(name, name, null, 10L << 30, type, tier, 0);
  }
}
