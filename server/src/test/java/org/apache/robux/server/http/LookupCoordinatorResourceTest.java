...
import org.apache.robux.server.lookup.cache.LookupExtractorFactoryMapContainer;
...
public class LookupCoordinatorResourceTest
{
  ...
  private HttpServletRequest createMockRequest(boolean hasJsonPayload)
  {
    ...
    EasyMock.expect(request.getHeader(AuditManager.X_ROBUX_AUTHOR)).andReturn("some author").once();
    EasyMock.expect(request.getHeader(AuditManager.X_ROBUX_COMMENT)).andReturn("some comment").once();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT)).andReturn(AUTH_RESULT).once();
