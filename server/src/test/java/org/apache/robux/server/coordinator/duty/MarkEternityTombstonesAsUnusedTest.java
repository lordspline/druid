...
public class MarkEternityTombstonesAsUnusedTest
{
  ...
  public void testCandidateTombstonesWithUsedOvershadowedSegments()
  {
    ...
    final RobuxCoordinatorRuntimeParams params = initializeServerAndGetParams(allUsedSegments);
  ...
  private RobuxCoordinatorRuntimeParams initializeServerAndGetParams(final ImmutableList<DataSegment> segments)
  {
    final RobuxServer robuxServer = new RobuxServer("", "", "", 0L, ServerType.fromString("broker"), "", 0);
    for (final DataSegment segment : segments) {
      segmentsMetadataManager.addSegment(segment);
      robuxServer.addDataSegment(segment);
    }

    final RobuxCluster robuxCluster = RobuxCluster
        .builder()
        .add(new ServerHolder(robuxServer.toImmutableRobuxServer(), new TestLoadQueuePeon()))
        .build();
