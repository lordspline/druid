...
public class DataSourceCompactionConfigHistoryTest
{
  ...
  public void testAddDatasourceConfigShouldAddToHistory()
  {
    ...
    wikiAuditHistory.add(
        RobuxCompactionConfig.empty().withDatasourceConfig(wikiCompactionConfig),
        auditInfo,
        auditTime
    );
  ...
  public void testAddDeleteDatasourceConfigShouldAddBothToHistory()
  {
    ...
    wikiAuditHistory.add(RobuxCompactionConfig.empty(), auditInfo, auditTime.plusHours(2));
  ...
  public void testAddDeleteAnotherDatasourceConfigShouldNotAddToHistory()
  {
    ...
    wikiAuditHistory.add(
        RobuxCompactionConfig.empty().withDatasourceConfig(koalaCompactionConfig),
        auditInfo,
        DateTimes.nowUtc()
    );
    wikiAuditHistory.add(RobuxCompactionConfig.empty(), auditInfo, DateTimes.nowUtc());
  ...
  public void testAddModifyDatasourceConfigShouldAddBothToHistory()
  {
    ...
    final DataSourceCompactionConfig updatedWikiConfig
        = InlineSchemaDataSourceCompactionConfig.builder()
                                                ...
                                                .build();
    wikiAuditHistory.add(
        RobuxCompactionConfig.empty().withDatasourceConfig(updatedWikiConfig),
        auditInfo,
        auditTime.plusHours(3)
    );
  ...
  {
    final RobuxCompactionConfig originalConfig
        = RobuxCompactionConfig.empty().withDatasourceConfig(wikiCompactionConfig);

    wikiAuditHistory.add(originalConfig, auditInfo, DateTimes.nowUtc());

    final RobuxCompactionConfig updatedConfig = originalConfig.withClusterConfig(
