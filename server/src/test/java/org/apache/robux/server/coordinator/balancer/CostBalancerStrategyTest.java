...
public class CostBalancerStrategyTest
{
  ...
  public void testComputePlacementCost()
  {
    ...
    List<RobuxServer> historicals = IntStream.range(0, 3)
    ...
    final List<ServerHolder> serverHolders = historicals.stream().map(
        server -> new ServerHolder(server.toImmutableRobuxServer(), new TestLoadQueuePeon())
    ).collect(Collectors.toList());
  ...
  public void testGetStats()
  {
    final ServerHolder serverA = new ServerHolder(
        createHistorical().toImmutableRobuxServer(),
  ...
  public void testFindServerAfterExecutorShutdownThrowsException()
  {
    ...
    ServerHolder serverA = new ServerHolder(createHistorical().toImmutableRobuxServer(), peon);
    ServerHolder serverB = new ServerHolder(createHistorical().toImmutableRobuxServer(), peon);
  ...
  private RobuxServer createHistorical()
  {
    String serverName = "hist_" + uniqueServerId++;
    return new RobuxServer(serverName, serverName, null, 10L << 30, ServerType.HISTORICAL, "hot", 1);
  }
