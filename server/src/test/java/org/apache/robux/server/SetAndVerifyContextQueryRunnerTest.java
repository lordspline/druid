...
public class SetAndVerifyContextQueryRunnerTest
{
  ...
  {
    Query<ScanResultValue> query = new Robuxs.ScanQueryBuilder()
        .dataSource("foo")
    ...
    Assert.assertTrue(
        System.currentTimeMillis() > transformed.context().getLong(DirectRobuxClient.QUERY_FAIL_TIME)
    );
  }
  ...
  public void testTimeoutDefaultTooBigAndOverflows()
  {
    ...
    Assert.assertEquals(Long.MAX_VALUE, (long) transformed.context().getLong(DirectRobuxClient.QUERY_FAIL_TIME));
  }
