...
import org.apache.robux.segment.FrameBasedInlineSegmentWrangler;
import org.apache.robux.segment.InlineSegmentWrangler;
import org.apache.robux.segment.MapSegmentWrangler;
 ...
 *
 * Note that since SpecificSegmentsQuerySegmentWalker in the robux-sql module also uses ClientQuerySegmentWalker, it's
...
public class ClientQuerySegmentWalkerTest
{
  ...
  public void testTimeseriesOnTable()
  {
    final TimeseriesQuery query =
        (TimeseriesQuery) Robuxs.newTimeseriesQueryBuilder()
                                .dataSource(FOO)
  ...
  {
    ScanQuery subquery = new Robuxs.ScanQueryBuilder().dataSource(MULTI)
                                                      .columns("s", "n")
                                                      .eternityInterval()
  ...
  @Test // Regression test for bug fixed in https://github.com/apache/robux/pull/15300
  ...
  public void testMetricsWithMaxSubqueryRowsEnabled()
  {
    ...
    for (ServiceMetricEvent event : emitter.getMetricEvents(ClientQuerySegmentWalker.ROWS_COUNT_METRIC)) {
      ...
      if (ClientQuerySegmentWalker.ROWS_COUNT_METRIC.equals(map.get("metric"))) {
        ...
        Assert.assertEquals(DUMMY_QUERY_ID, map.get(RobuxMetrics.ID));
        Assert.assertEquals("1.1", map.get(RobuxMetrics.SUBQUERY_ID));
  ...
  {
    TimeseriesQuery subQuery1 = (TimeseriesQuery) Robuxs.newTimeseriesQueryBuilder()
        .dataSource(FOO)
        .granularity(Granularities.ALL)
        .intervals(Collections.singletonList(INTERVAL))
        .aggregators(new LongSumAggregatorFactory("sum", "n"))
        ...
        .build()
        .withId(DUMMY_QUERY_ID);
    TimeseriesQuery subQuery2 = (TimeseriesQuery) Robuxs.newTimeseriesQueryBuilder()
        .dataSource(BAR)
        .granularity(Granularities.ALL)
        .intervals(Collections.singletonList(INTERVAL))
        .aggregators(new LongSumAggregatorFactory("sum", "n"))
        ...
        .build()
        .withId(DUMMY_QUERY_ID);
    final Query query = Robuxs.newScanQueryBuilder()
        .columns("sum")
   ...
   * {@link org.apache.robux.query.QueryToolChest#resultsAsArrays}.
   */
  ...
  private static class ExpectedQuery
  {
    ...
    ExpectedQuery(Query<?> query, ClusterOrLocal how)
    {
      ...
      contextBuilder.put(DirectRobuxClient.QUERY_FAIL_TIME, 0L)
