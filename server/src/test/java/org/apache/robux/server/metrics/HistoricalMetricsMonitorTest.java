...
{
  private RobuxServerConfig robuxServerConfig;
  ...
  public void setUp()
  {
    robuxServerConfig = EasyMock.createStrictMock(RobuxServerConfig.class);
  ...
  public void testSimple()
  {
    ...
    EasyMock.expect(robuxServerConfig.getMaxSize()).andReturn(maxSize).once();
    EasyMock.expect(segmentLoadDropMgr.getSegmentsToDelete()).andReturn(ImmutableList.of(dataSegment)).once();
    EasyMock.expect(robuxServerConfig.getTier()).andReturn(tier).once();
    EasyMock.expect(robuxServerConfig.getPriority()).andReturn(priority).once();
    EasyMock.expect(segmentManager.getDataSourceSizes()).andReturn(ImmutableMap.of(dataSource, size));
    EasyMock.expect(robuxServerConfig.getTier()).andReturn(tier).once();
    EasyMock.expect(robuxServerConfig.getPriority()).andReturn(priority).once();
    EasyMock.expect(robuxServerConfig.getMaxSize()).andReturn(maxSize).times(2);
    ...
    EasyMock.replay(robuxServerConfig, segmentManager, segmentLoadDropMgr);
    monitor.doMonitor(serviceEmitter);
    EasyMock.verify(robuxServerConfig, segmentManager, segmentLoadDropMgr);
