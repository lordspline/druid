...
import org.apache.robux.server.initialization.jetty.RobuxGuiceContainer;
...
public class WebserverTestUtils
{
      ...
      throws IOException
  {
    ...
    IoCComponentProviderFactory ioc = new GuiceComponentProviderFactory(resourceConfig, injector)
    {
      ...
      {
        return RobuxGuiceContainer.populateScopeMap(super.createScopeMap());
      }
    };
      ...
      SingletonTypeInjectableProvider<Context, HttpServletRequest>
  {
    ...
    static HttpServletRequest createMockRequest()
    {
      ...
      EasyMock.expect(mockRequest.getAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED)).andReturn(null).anyTimes();
      EasyMock.expect(mockRequest.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT))
