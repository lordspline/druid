...
package org.apache.robux.server.coordinator.rules;
...
import org.apache.robux.server.coordinator.balancer.CachingCostBalancerStrategy;
import org.apache.robux.server.coordinator.balancer.ClusterCostCache;
...
import org.apache.robux.server.coordinator.loading.StrategicSegmentAssigner;
...
public class LoadRuleTest
{
  ...
  public void testLoadRuleAssignsSegments()
  {
    ...
    CoordinatorRunStats stats = runRuleAndGetStats(rule, segment, robuxCluster);
  ...
  private CoordinatorRunStats runRuleAndGetStats(LoadRule rule, DataSegment segment, RobuxCluster cluster)
  {
  ...
  private RobuxCoordinatorRuntimeParams makeCoordinatorRuntimeParams(
  ...
  public void testLoadPrimaryAssignDoesNotOverAssign()
  {
    ...
    final DataSegment segment = createDataSegment(TestDataSource.WIKI);
    CoordinatorRunStats firstRunStats = runRuleAndGetStats(rule, segment, robuxCluster);
    ...
    CoordinatorRunStats secondRunStats = runRuleAndGetStats(rule, segment, robuxCluster);
  ...
  public void testOverAssignForTimedOutSegments()
  {
    ...
    CoordinatorRunStats statsAfterLoadPrimary = runRuleAndGetStats(rule, segment, robuxCluster);
  ...
  public void testLoadUsedSegmentsForAllSegmentGranularityAndCachingCostBalancerStrategy()
  {
    ...
    CoordinatorRunStats stats = runRuleAndGetStats(
        rule,
        segments.get(1),
        makeCoordinatorRuntimeParams(robuxCluster, segments.toArray(new DataSegment[0]))
    );
    Assert.assertEquals(1L, stats.getSegmentStat(Stats.Segments.ASSIGNED, Tier.T1, TestDataSource.WIKI));
  }
  ...
  public void testMaxLoadingQueueSize()
  {
    ...
    RobuxCluster robuxCluster = RobuxCluster
        ...
        .addTier(
            Tier.T1,
            new ServerHolder(
                createRobuxServer(Tier.T1).toImmutableRobuxServer(),
  ...
  public void testDropDuringDecommissioning()
  {
    ...
    RobuxCoordinatorRuntimeParams params = makeCoordinatorRuntimeParams(robuxCluster, segment1, segment2);
  ...
  public void testExtraReplicasAreDroppedFromDecommissioningServer()
  {
    ...
    CoordinatorRunStats stats = runRuleAndGetStats(
        rule,
        segment1,
        makeCoordinatorRuntimeParams(robuxCluster, segment1)
    );
  ...
  private RobuxServer createRobuxServer(String tier)
  {
    final String serverName = "hist_" + tier + "_" + serverId.incrementAndGet();
    return new RobuxServer(serverName, serverName, null, 10L << 30, ServerType.HISTORICAL, tier, 0);
  }

  private ServerHolder createServer(String tier, DataSegment... segments)
  {
    final RobuxServer server = createRobuxServer(tier);
    for (DataSegment segment : segments) {
      server.addDataSegment(segment);
    }

    return new ServerHolder(
        server.toImmutableRobuxServer(),
