...
import org.apache.robux.server.coordinator.config.KillUnusedSegmentsConfig;
...
public class RobuxCoordinatorConfigTest
{
  ...
  public void testCoordinatorRunConfigDefaultValues()
  {
    ...
    final CoordinatorRunConfig config = deserializeFrom(props, "robux.coordinator", CoordinatorRunConfig.class);
  ...
  {
    final Properties props = new Properties();
    props.setProperty("robux.coordinator.startDelay", "PT10M");
    props.setProperty("robux.coordinator.period", "PT30S");
    final CoordinatorRunConfig config = deserializeFrom(props, "robux.coordinator", CoordinatorRunConfig.class);
  ...
  public void testCoordinatorPeriodConfigDefaultValues()
  {
    ...
    final CoordinatorPeriodConfig config
        = deserializeFrom(props, "robux.coordinator.period", CoordinatorPeriodConfig.class);
  ...
  {
    final Properties props = new Properties();
    props.setProperty("robux.coordinator.period.indexingPeriod", "PT1M");
    props.setProperty("robux.coordinator.period.metadataStoreManagementPeriod", "PT3M");
    final CoordinatorPeriodConfig config
        = deserializeFrom(props, "robux.coordinator.period", CoordinatorPeriodConfig.class);
  ...
  public void testLoadQueuePeonConfigDefaultValues()
  {
    ...
    final HttpLoadQueuePeonConfig config
        = deserializeFrom(props, "robux.coordinator.loadqueuepeon.http", HttpLoadQueuePeonConfig.class);
  ...
  {
    final Properties props = new Properties();
    props.setProperty("robux.coordinator.loadqueuepeon.http.repeatDelay", "PT20M");
    props.setProperty("robux.coordinator.loadqueuepeon.http.hostTimeout", "PT10M");
    props.setProperty("robux.coordinator.loadqueuepeon.http.batchSize", "100");
  ...
  public void testKillUnusedSegmentsPeriodLessThanIndexingPeriod()
  {
    ...
    verifyCoordinatorConfigFailsWith(
        ...
        "'robux.coordinator.kill.period'[PT5S] must be greater than or equal to"
        + " 'robux.coordinator.period.indexingPeriod'[PT10S]"
    );
  }
  ...
  public void testKillUnusedSegmentsMaxSegmentsNegative()
  {
    ...
    verifyCoordinatorConfigFailsWith(
        createKillConfig().unusedSegments(killUnusedConfig).build(),
        new CoordinatorPeriodConfig(null, Duration.standardSeconds(10)),
        "'robux.coordinator.kill.maxSegments'[-5] must be a positive integer."
    );
  }
  ...
  public void testCoordinatorKillConfigDefaultValues()
  {
    final CoordinatorKillConfigs killConfigs
        = deserializeFrom(new Properties(), "robux.coordinator.kill", CoordinatorKillConfigs.class);
  ...
  {
    final Properties props = new Properties();
    props.setProperty("robux.coordinator.kill.audit.on", "false");
    props.setProperty("robux.coordinator.kill.audit.period", "PT10H");
    props.setProperty("robux.coordinator.kill.audit.durationToRetain", "PT20H");

    props.setProperty("robux.coordinator.kill.compaction.on", "false");
    props.setProperty("robux.coordinator.kill.compaction.period", "PT20H");
    props.setProperty("robux.coordinator.kill.compaction.durationToRetain", "PT30H");

    props.setProperty("robux.coordinator.kill.datasource.on", "false");
    props.setProperty("robux.coordinator.kill.datasource.period", "PT5H");
    props.setProperty("robux.coordinator.kill.datasource.durationToRetain", "PT10H");

    props.setProperty("robux.coordinator.kill.rule.on", "false");
    props.setProperty("robux.coordinator.kill.rule.period", "PT11H");
    props.setProperty("robux.coordinator.kill.rule.durationToRetain", "PT12H");

    props.setProperty("robux.coordinator.kill.supervisor.on", "false");
    props.setProperty("robux.coordinator.kill.supervisor.period", "PT1H");
    props.setProperty("robux.coordinator.kill.supervisor.durationToRetain", "PT2H");

    props.setProperty("robux.coordinator.kill.pendingSegments.on", "false");

    props.setProperty("robux.coordinator.kill.segmentSchema.on", "false");
    props.setProperty("robux.coordinator.kill.segmentSchema.period", "PT2H");
    props.setProperty("robux.coordinator.kill.segmentSchema.durationToRetain", "PT8H");

    final CoordinatorKillConfigs killConfigs
        = deserializeFrom(props, "robux.coordinator.kill", CoordinatorKillConfigs.class);
  ...
  public void testCoordinatorConfigFailsWhenCleanupPeriodIsInvalid()
  {
    ...
    verifyCoordinatorConfigFailsWith(
        ...
        periodConfig,
        "'robux.coordinator.kill.audit.period'[PT1800S] must be greater than"
        + " 'robux.coordinator.period.metadataStoreManagementPeriod'[PT3600S]"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().compaction(cleanupConfig).build(),
        periodConfig,
        "'robux.coordinator.kill.compaction.period'[PT1800S] must be greater than"
        + " 'robux.coordinator.period.metadataStoreManagementPeriod'[PT3600S]"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().datasource(cleanupConfig).build(),
        periodConfig,
        "'robux.coordinator.kill.datasource.period'[PT1800S] must be greater than"
        + " 'robux.coordinator.period.metadataStoreManagementPeriod'[PT3600S]"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().rules(cleanupConfig).build(),
        periodConfig,
        "'robux.coordinator.kill.rule.period'[PT1800S] must be greater than"
        + " 'robux.coordinator.period.metadataStoreManagementPeriod'[PT3600S]"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().supervisors(cleanupConfig).build(),
        periodConfig,
        "'robux.coordinator.kill.supervisor.period'[PT1800S] must be greater than"
        + " 'robux.coordinator.period.metadataStoreManagementPeriod'[PT3600S]"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().segmentSchema(cleanupConfig).build(),
        periodConfig,
        "'robux.coordinator.kill.segmentSchema.period'[PT1800S] must be greater than"
        + " 'robux.coordinator.period.metadataStoreManagementPeriod'[PT3600S]"
    );
  }
  ...
  public void testCoordinatorConfigFailsWhenRetainDurationIsNegative()
  {
    ...
    verifyCoordinatorConfigFailsWith(
        createKillConfig().audit(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.audit.durationToRetain'[PT-1S] must be 0 milliseconds or higher"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().compaction(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.compaction.durationToRetain'[PT-1S] must be 0 milliseconds or higher"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().datasource(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.datasource.durationToRetain'[PT-1S] must be 0 milliseconds or higher"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().rules(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.rule.durationToRetain'[PT-1S] must be 0 milliseconds or higher"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().supervisors(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.supervisor.durationToRetain'[PT-1S] must be 0 milliseconds or higher"
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().segmentSchema(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.segmentSchema.durationToRetain'[PT-1S] must be 0 milliseconds or higher"
    );
  }
  ...
  public void testCoordinatorConfigFailsWhenRetainDurationIsHigherThanCurrentTime()
  {
    ...
    verifyCoordinatorConfigFailsWith(
        createKillConfig().audit(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.audit.durationToRetain'[%s] cannot be greater than current time in milliseconds",
        futureRetainDuration
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().compaction(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.compaction.durationToRetain'[%s] cannot be greater than current time in milliseconds",
        futureRetainDuration
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().datasource(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.datasource.durationToRetain'[%s] cannot be greater than current time in milliseconds",
        futureRetainDuration
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().rules(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.rule.durationToRetain'[%s] cannot be greater than current time in milliseconds",
        futureRetainDuration
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().supervisors(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.supervisor.durationToRetain'[%s] cannot be greater than current time in milliseconds",
        futureRetainDuration
    );
    verifyCoordinatorConfigFailsWith(
        createKillConfig().segmentSchema(cleanupConfig).build(),
        defaultPeriodConfig,
        "'robux.coordinator.kill.segmentSchema.durationToRetain'[%s] cannot be"
  ...
  {
    RobuxException exception = Assert.assertThrows(
        RobuxException.class,
        () -> new RobuxCoordinatorConfig(
