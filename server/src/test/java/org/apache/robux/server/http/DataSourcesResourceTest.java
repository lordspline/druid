...
import org.apache.robux.rpc.HttpResponseException;
import org.apache.robux.rpc.indexing.OverlordClient;
import org.apache.robux.rpc.indexing.SegmentUpdateResponse;
import org.apache.robux.segment.TestDataSource;
...
import org.apache.robux.test.utils.ImmutableRobuxDataSourceTestUtils;
...
import org.apache.robux.timeline.partition.NumberedPartitionChunk;
...
public class DataSourcesResourceTest
{
  ...
  private List<RobuxDataSource> listDataSources;
  ...
  public void setUp()
  {
    ...
    server = EasyMock.niceMock(RobuxServer.class);
    ...
    listDataSources.add(
        new RobuxDataSource(TestDataSource.WIKI, new HashMap<>()).addSegment(dataSegmentList.get(0))
    );
    listDataSources.add(
        new RobuxDataSource(TestDataSource.KOALA, new HashMap<>()).addSegment(dataSegmentList.get(1))
    );
  ...
  public void testGetFullQueryableDataSources()
  {
    ...
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_ALLOW_UNSECURED_PATH)).andReturn(null).once();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED)).andReturn(null).once();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT)).andReturn(
      new AuthenticationResult("robux", "robux", null, null)
    ).atLeastOnce();
    request.setAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED, true);
    ...
    Set<ImmutableRobuxDataSource> result = (Set<ImmutableRobuxDataSource>) response.getEntity();
    Assert.assertEquals(200, response.getStatus());
    Assert.assertEquals(2, result.size());
    ImmutableRobuxDataSourceTestUtils.assertEquals(
        listDataSources.stream().map(RobuxDataSource::toImmutableRobuxDataSource).collect(Collectors.toList()),
  ...
  public void testSecuredGetFullQueryableDataSources()
  {
    ...
    ImmutableRobuxDataSourceTestUtils.assertEquals(
        listDataSources.get(0).toImmutableRobuxDataSource(),
  ...
  {
    RobuxDataSource dataSource1 = new RobuxDataSource(TestDataSource.WIKI, new HashMap<>());
    ...
    ImmutableRobuxDataSource result = (ImmutableRobuxDataSource) response.getEntity();
    Assert.assertEquals(200, response.getStatus());
    ImmutableRobuxDataSourceTestUtils.assertEquals(dataSource1.toImmutableRobuxDataSource(), result);
    EasyMock.verify(inventoryView, server);
  }
  ...
  public void testSimpleGetTheDataSourceManyTiers()
  {
    ...
    RobuxServer server2 = EasyMock.createStrictMock(RobuxServer.class);
    ...
    RobuxServer server3 = EasyMock.createStrictMock(RobuxServer.class);
  ...
  {
    server = new RobuxServer("server1", "host1", null, 1234, ServerType.HISTORICAL, "tier1", 0);
    RobuxServer server2 = new RobuxServer("server2", "host2", null, 1234, ServerType.HISTORICAL, "tier2", 0);
    RobuxServer server3 = new RobuxServer("server3", "host3", null, 1234, ServerType.HISTORICAL, "tier1", 0);
  ...
  {
    server = new RobuxServer("who", "host", null, 1234, ServerType.HISTORICAL, "tier1", 0);
    server.addDataSegment(dataSegmentList.get(0));
    server.addDataSegment(dataSegmentList.get(1));
    server.addDataSegment(dataSegmentList.get(2));
    EasyMock.expect(inventoryView.getInventory()).andReturn(ImmutableList.of(server)).atLeastOnce();
  ...
  {
    RobuxExceptionMatcher.invalidInput().assertThrowsAndMatches(
  ...
  {
    final RobuxDataSource dataSource1 = new RobuxDataSource(TestDataSource.WIKI, new HashMap<>());
  ...
  public void testGetDatasourceLoadstatusFull()
  {
    ...
    EasyMock.expect(segmentsMetadataManager.forceUpdateDataSourcesSnapshot())
            .andReturn(DataSourcesSnapshot.fromUsedSegments(segments)).once();
    RobuxCoordinator robuxCoordinator = EasyMock.createMock(RobuxCoordinator.class);
    EasyMock.expect(robuxCoordinator.getTierToDatasourceToUnderReplicatedCount(EasyMock.anyObject(), EasyMock.eq(false)))
            .andReturn(underReplicationCountsPerDataSourcePerTier).once();

    EasyMock.replay(segmentsMetadataManager, robuxCoordinator);

    DataSourcesResource dataSourcesResource =
        new DataSourcesResource(null, segmentsMetadataManager, null, null, null, robuxCoordinator, auditManager);
  ...
  public void testGetDatasourceLoadstatusFullAndComputeUsingClusterView()
  {
    ...
    EasyMock.expect(segmentsMetadataManager.forceUpdateDataSourcesSnapshot())
            .andReturn(DataSourcesSnapshot.fromUsedSegments(segments)).once();
    RobuxCoordinator robuxCoordinator = EasyMock.createMock(RobuxCoordinator.class);
    EasyMock.expect(robuxCoordinator.getTierToDatasourceToUnderReplicatedCount(EasyMock.anyObject(), EasyMock.eq(true)))
            .andReturn(underReplicationCountsPerDataSourcePerTier).once();
  ...
  private RobuxServerMetadata createRealtimeServerMetadata(String name)
  {
    return createServerMetadata(name, ServerType.REALTIME);
  }

  private RobuxServerMetadata createHistoricalServerMetadata(String name)
  {
    return createServerMetadata(name, ServerType.HISTORICAL);
  }

  private RobuxServerMetadata createServerMetadata(String name, ServerType type)
  {
    return new RobuxServerMetadata(name, name, null, 10000, type, "tier", 1);
  }
  ...
  {
    EasyMock.expect(request.getHeader(AuditManager.X_ROBUX_AUTHOR)).andReturn("author").anyTimes();
    EasyMock.expect(request.getHeader(AuditManager.X_ROBUX_COMMENT)).andReturn("comment").anyTimes();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT)).andReturn(null).anyTimes();
