...
public class SegmentStatsMonitorTest
{
  ...
  public void setUp()
  {
    robuxServerConfig = Mockito.mock(RobuxServerConfig.class);
    ...
    Mockito.when(robuxServerConfig.getTier()).thenReturn(TIER);
    Mockito.when(robuxServerConfig.getPriority()).thenReturn(PRIORITY);
  }
  ...
  public void testLazyLoadOnStartThrowsException()
  {
    ...
    new SegmentStatsMonitor(robuxServerConfig, segmentLoadDropMgr, segmentLoaderConfig);
  }
  ...
  {
    return new ServiceMetricEvent.Builder().setDimension(RobuxMetrics.DATASOURCE, DATA_SOURCE)
