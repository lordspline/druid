...
public class BalancerStrategyTest
{
  ...
  public void findNewSegmentHomeReplicatorNotEnoughSpace()
  {
    final ServerHolder serverHolder = new ServerHolder(
        new RobuxServer("server1", "host1", null, 10L, ServerType.HISTORICAL, RobuxServer.DEFAULT_TIER, 0).addDataSegment(proposedDataSegment).toImmutableRobuxServer(),
  ...
  public void findNewSegmentHomeReplicatorNotEnoughNodesForReplication()
  {
    final ServerHolder serverHolder1 = new ServerHolder(
        new RobuxServer("server1", "host1", null, 1000L, ServerType.HISTORICAL, RobuxServer.DEFAULT_TIER, 0)
            .addDataSegment(proposedDataSegment).toImmutableRobuxServer(),
        new TestLoadQueuePeon());

    final ServerHolder serverHolder2 = new ServerHolder(
        new RobuxServer("server2", "host2", null, 1000L, ServerType.HISTORICAL, RobuxServer.DEFAULT_TIER, 0)
            .addDataSegment(proposedDataSegment).toImmutableRobuxServer(),
        new TestLoadQueuePeon());
  ...
  {
    final ServerHolder serverHolder = new ServerHolder(
        new RobuxServer("server1", "host1", null, 1000L, ServerType.HISTORICAL, RobuxServer.DEFAULT_TIER, 0).toImmutableRobuxServer(),
        new TestLoadQueuePeon());
    serverHolders = new ArrayList<>();
