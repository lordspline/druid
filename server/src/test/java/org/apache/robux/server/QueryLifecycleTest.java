...
import org.apache.robux.query.QueryContextTest;
...
public class QueryLifecycleTest
{
  ...
  private final TimeseriesQuery query = Robuxs.newTimeseriesQueryBuilder()
  ...
  public void testRunSimple_withPolicyRestriction_segmentMetadataQueryRunAsInternal()
  {
    ...
    // The robux-internal gets a NoRestrictionPolicy.
    AuthorizationResult authorizationResult = AuthorizationResult.allowWithRestriction(ImmutableMap.of(
        DATASOURCE,
        Optional.of(NoRestrictionPolicy.instance())
    ));
    final SegmentMetadataQuery query = Robuxs.newSegmentMetadataQueryBuilder()
                                             .dataSource(DATASOURCE)
                                             .intervals(ImmutableList.of(Intervals.ETERNITY))
                                             .build();
    EasyMock.expect(queryConfig.getContext()).andReturn(ImmutableMap.of()).anyTimes();
  ...
  public void testRunSimple_withPolicyRestriction_segmentMetadataQueryRunAsExternal()
  {
    ...
    Assert.assertEquals(RobuxException.Persona.USER, e.getTargetPersona());
  ...
  public void testAuthorized_withPolicyRestriction_failedSecurityValidation()
  {
    ...
    RobuxException e = Assert.assertThrows(RobuxException.class, () -> lifecycle.authorize(authenticationResult));
    Assert.assertEquals(RobuxException.Category.FORBIDDEN, e.getCategory());
  ...
  public void testAuthorized_queryWithRestrictedDataSource_runWithSuperUserPermission()
  {
    ...
    Policy rowFilterPolicy = RowFilterPolicy.from(new NullFilter("some-column", null));
    // Internal robux system would get a NO_RESTRICTION on a restricted data source.
  ...
  private HttpServletRequest mockRequest()
  {
    ...
    EasyMock.expect(request.getAttribute(EasyMock.eq(AuthConfig.ROBUX_AUTHENTICATION_RESULT)))
            .andReturn(authenticationResult).anyTimes();
    EasyMock.expect(request.getAttribute(EasyMock.eq(AuthConfig.ROBUX_ALLOW_UNSECURED_PATH)))
            .andReturn(null).anyTimes();
    EasyMock.expect(request.getAttribute(EasyMock.eq(AuthConfig.ROBUX_AUTHORIZATION_CHECKED)))
            .andReturn(null).anyTimes();
