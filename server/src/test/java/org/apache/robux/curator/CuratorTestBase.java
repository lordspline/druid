...
public class CuratorTestBase
{
  ...
  protected void setupZNodeForServer(RobuxServer server, ZkPathsConfig zkPathsConfig, ObjectMapper jsonMapper)
  {
    ...
    try {
       ...
       * Address issue: https://github.com/apache/robux/issues/1512
       */
      try {
        curator.setData()
               .forPath(zkPath, jsonMapper.writeValueAsBytes(server.getMetadata()));
  ...
  )
  {
    final String segmentAnnouncementPath =
        ZKPaths.makePath(zkPathsConfig.getLiveSegmentsPath(), robuxServer.getHost(), segment.getId().toString());
  ...
  protected void unannounceSegmentForServer(RobuxServer robuxServer, DataSegment segment, ZkPathsConfig zkPathsConfig)
      throws Exception
  {
