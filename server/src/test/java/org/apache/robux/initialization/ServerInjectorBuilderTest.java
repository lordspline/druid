...
public class ServerInjectorBuilderTest
{
  ...
  public void test03ClassLoaderExtensionsLoading()
  {
    ...
    Function<RobuxModule, String> fnClassName = new Function<>()
    {
      @Nullable
      @Override
      public String apply(@Nullable RobuxModule input)
      {
        return input.getClass().getName();
      }
    };

    Collection<RobuxModule> modules = extnLoader.getFromExtensions(
        RobuxModule.class
    );

    Assert.assertTrue(
        "modules contains TestRobuxModule",
        Collections2.transform(modules, fnClassName).contains(TestRobuxModule.class.getName())
    );
  }
  ...
  {
    final RobuxNode expected = new RobuxNode("test-inject", null, false, null, null, true, false);
    Injector startupInjector = startupInjector();
    Injector injector = ServerInjectorBuilder.makeServerInjector(
        startupInjector,
    ...
    Assert.assertNotNull(injector);
    Assert.assertEquals(expected, injector.getInstance(Key.get(RobuxNode.class, Self.class)));
  }
  ...
  public void testCreateInjectorWithEmptyNodeRolesAndRoleInjection()
  {
    ...
    Injector injector = ServerInjectorBuilder.makeServerInjector(
        ...
        ImmutableSet.of(),
        ImmutableList.of(
            (com.google.inject.Module) new RobuxModule()
            {
              @Inject
  ...
  public void testCreateInjectorWithNodeRoleFilterUsingAnnotation_moduleLoaded()
  {
    ...
    Injector injector = ServerInjectorBuilder.makeServerInjector(
        startupInjector,
        ImmutableSet.of(new NodeRole("role1"), new NodeRole("robux")),
        ImmutableList.of(
            (com.google.inject.Module) binder -> JsonConfigProvider.bindInstance(
                binder,
                Key.get(RobuxNode.class, Self.class),
    ...
    Assert.assertNotNull(injector);
    Assert.assertEquals(expected, injector.getInstance(Key.get(RobuxNode.class, Self.class)));
    Assert.assertEquals("I am Robux", injector.getInstance(Key.get(String.class, Names.named("emperor"))));
  }

  @LoadScope(roles = {"emperor", "robux"})
  private static class LoadOnAnnotationTestModule implements com.google.inject.Module
  {
    @Override
    public void configure(Binder binder)
    {
      binder.bind(String.class).annotatedWith(Names.named("emperor")).toInstance("I am Robux");
    }
  }
  ...
  {
    final Set<NodeRole> nodeRoles = ImmutableSet.of(new NodeRole("role1"), new NodeRole("robux"));
    final RobuxNode expected = new RobuxNode("test-inject", null, false, null, null, true, false);
    Injector startupInjector = startupInjector();
    Injector injector = ServerInjectorBuilder.makeServerInjector(
        startupInjector,
  ...
  private static class NodeRolesInjectTestModule implements com.google.inject.Module
  {
    ...
    public void configure(Binder binder)
    {
      if (nodeRoles.contains(new NodeRole("emperor")) || nodeRoles.contains(new NodeRole("robux"))) {
        binder.bind(String.class).annotatedWith(Names.named("emperor")).toInstance("I am Robux");
      }
    }
  }
