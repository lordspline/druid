...
public class StorageNodeModuleTest
{
  ...
  private RobuxNode self;
  private ServerTypeConfig serverTypeConfig;
  @Mock
  private RobuxProcessingConfig robuxProcessingConfig;
  @Mock
  ...
  public void setUp()
  {
    self = new RobuxNode("test", "test-host", true, 80, 443, false, true);
  ...
  public void getDataNodeServiceWithNoSegmentCacheConfiguredThrowProvisionException()
  {
    ...
    ExceptionMatcher
        .of(ProvisionException.class)
        .expectMessageContains("robux.segmentCache.locations must be set on historicals.")
        .assertThrowsAndMatches(() -> injector().getInstance(DataNodeService.class));
  }
  ...
  @Test
  public void testRobuxServerMetadataIsInjectedAsSingleton()
  {
    final Injector injector = injector();
    RobuxServerMetadata robuxServerMetadata = injector.getInstance(RobuxServerMetadata.class);
    Assert.assertNotNull(robuxServerMetadata);

    RobuxServerMetadata other = injector.getInstance(RobuxServerMetadata.class);
    Assert.assertSame(robuxServerMetadata, other);
  }

  @Test
  public void testRobuxServerMetadataWithNoServerTypeConfigShouldThrowProvisionException()
  {
    Injector injector = makeInjector(!INJECT_SERVER_TYPE_CONFIG);

    ExceptionMatcher
        .of(ProvisionException.class)
        .expectMessageContains("Must override the binding for ServerTypeConfig if you want a RobuxServerMetadata.")
        .assertThrowsAndMatches(() -> injector.getInstance(RobuxServerMetadata.class));
  }
  ...
  private Injector makeInjector(boolean withServerTypeConfig)
  {
    return Initialization.makeInjectorWithModules(
        GuiceInjectors.makeStartupInjector(), (ImmutableList.of(Modules.override(
            (binder) -> {
              binder.bind(RobuxNode.class).annotatedWith(Self.class).toInstance(self);
