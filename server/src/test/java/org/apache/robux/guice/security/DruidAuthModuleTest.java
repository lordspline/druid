...
public class RobuxAuthModuleTest
{
  private Injector injector;
  private RobuxAuthModule authModule;

  @Before
  public void setup()
  {
    authModule = new RobuxAuthModule();
  ...
  {
    Properties properties = new Properties();
    properties.setProperty("robux.auth.authenticatorChain", "[\"chain\", \"of\", \"authenticators\"]");
    properties.setProperty("robux.auth.authorizers", "[\"authorizers\", \"list\"]");
    properties.setProperty("robux.auth.unsecuredPaths", "[\"path1\", \"path2\"]");
    properties.setProperty("robux.auth.allowUnauthenticatedHttpOptions", "true");
    properties.setProperty("robux.auth.authorizeQueryContextParams", "true");
