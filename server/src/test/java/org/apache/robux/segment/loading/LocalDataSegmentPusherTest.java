...
public class LocalDataSegmentPusherTest
{
  ...
  {
    configZip.storageDirectory = new File("/robux");

    // If this test fails because the path is returned as "file:/robux/", this can happen
    // when a /robux directory exists on the local filesystem.
    Assert.assertEquals(
        "file:/robux",
  ...
  public void testPathForHadoopRelative()
  {
    ...
    Assert.assertEquals(
        StringUtils.format("file:%s/robux", System.getProperty("user.dir")),
        new LocalDataSegmentPusher(configZip).getPathForHadoop()
    );
  }
}
