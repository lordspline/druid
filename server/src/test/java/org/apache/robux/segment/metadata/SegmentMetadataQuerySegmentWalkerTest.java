...
import org.apache.robux.client.CachingClusteredClientTest.ServerExpectation;
import org.apache.robux.client.CachingClusteredClientTest.ServerExpectations;
  ...
  public void setUp()
  {
    ...
    servers =
        new RobuxServer[]{
            ...
            new RobuxServer("test3", "test2", null, 10, ServerType.INDEXER_EXECUTOR, "bye", 0)
        };

    random = new Random(0);
    module = Modules.combine(
        binder -> binder.bind(CoordinatorServerView.class).toInstance(testCoordinatorServerView),
        binder -> binder.bind(RobuxHttpClientConfig.class).toInstance(httpClientConfig),
  ...
  )
  {
    ...
    for (Pair<Interval, List<Integer>> intervalAndChunk : intervalAndChunks) {
      ...
      for (int partitionNum = startNum; partitionNum <= endNum; partitionNum++) {
        ...
        RobuxServer server = servers[random.nextInt(servers.length)];
