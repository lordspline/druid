...
applications \cite{tschetter2011robux}.
...
Kafka\cite{kreps2011kafka}, Apache Samza, Apache Hadoop, and Robux
\cite{yang2014robux}, and we have found that the combination of technologies is
...
introduction of Robux as a serving layer, and our model for unifying real-time
...
Section \ref{sec:serving} describes Robux, the serving layer of the stack, and
how Robux is built for real-time and batch data ingestion, as well as
...
endpoints. Samza and Hadoop process data to load data into Robux. Robux acts as
...
Robux. Robux was designed from the ground up to provide arbitrary data
exploration, low latency aggregations, and fast data ingestion.  Robux was also
...
joined, cleaned up, and transformed before it was usable in Robux, but that was
...
into Robux. Our stream processing jobs range from simple data transformations,
such as id to name lookups, up to complex operations such as multi-stream
joins.  Pairing Robux with a stream processor enabled flexible data processing
...
\label{sec:serving}
Robux is a column-oriented data store designed for exploratory analytics and is
the serving layer in the RADStack. A Robux cluster consists of different types
...
form a fully working system. The composition of and flow of data in a Robux
cluster are shown in Figure~\ref{fig:cluster}.

\subsection{Segments}
\label{sec:segments}
Data tables in Robux (called "data sources") are collections of timestamped
...
Robux and Robux queries only understand how to scan segments. 
...
Robux nodes use one thread to scan one segment at a time, and the amount of
...
Robux segments are very self-contained for the time interval of data that they
hold. Column data is stored directly in the segment. Robux has multiple column
...
Additional indexes may be created for particular columns. For example, Robux
will by default create inverted indexes for string columns.

\subsection{Streaming Data Ingestion}
Robux real-time nodes encapsulate the functionality to ingest, query, and create
...
have collected over this small time range to other nodes in the Robux cluster
...
in-memory buffer is directly queryable and Robux behaves as a key/value store for
queries on events that exist in this JVM heap-based store. The in-memory buffer
is heavily write optimized, and given that Robux is really designed for heavy
...
Real-time ingestion in Robux is self-throttling. If a significant spike occurs
...
through the persist process, problems can still arise. Robux sets a limit on
the maximum number of persists that can occur at a time, and if this limit is
reached, Robux will begin to throttle event ingestion. In this case, the onus
...
they do occur. Robux's real-time logic does not guarantee exactly once
processing and is instead best effort. The lack of exactly once processing in
Robux is one of the motivations for requiring batch fixup in the RADStack.
...
\cite{decandia2007dynamo} or HDFS \cite{shvachko2010hadoop}, which Robux refers
...
the Robux cluster to download and serve the segment. The real-time node
continues to serve the segment until it notices that the segment is available
on Robux historical nodes, which are nodes that are dedicated to serving
...
component is shared across both real-time and batch ingestion. Robux has built
...
ingestion are directly uploaded to deep storage. Robux’s Hadoop-based batch
...
noticed by Robux coordinator nodes. Robux coordinator nodes poll the metadata
storage for what segments should be loaded on Robux historical nodes, and
...
Robux historical nodes are very simple in operation. They know how to load,
...
nodes in typical production Robux clusters are historical nodes.

To consolidate results from historical and real-time nodes, Robux has a set of
...
segments exist in the cluster and the version of those segments. Robux uses
...
interval, Robux only considers the data from the segment with the higher
...
\caption{
Robux utilizes multi-version concurrency control and reads data from segments
...
Although Robux can ingest events that are streamed in one at a time, data must
be denormalized beforehand as Robux cannot yet support
...
that Robux does not support joins at query time, so the events must be
...
data is cleaned, it is ready to be delivered to Robux for queries. The total
...
The final job of our processing pipeline is to deliver data to Robux. For high
...
as replicas of each other. The Robux broker can query for either copy of the data.
...
in preparation for the data to be loaded into Robux. The second set of jobs is
responsible for directly creating immutable Robux segments. The indexing code
for both streaming and batch ingestion in Robux is shared between the two modes
of ingestion. These segments are then uploaded to deep storage and registered
with the metadata store. Robux will proceed to load the batch generated
segments.
...
soon after batch processing completes, Robux queries begin reflecting
...
pipelines in Samza map to data sources in Robux. The second consumer reads
messages from Kafka and stores them in a distributed file system.  This file
system is the same as the one used for Robux’s deep storage, and also acts as a
...
future, we can reprocess the raw data to generate new Robux segments. 
...
Robux runs in production at several organizations, and to briefly demonstrate
...
Robux query performance can vary signficantly depending on the query being
...
To showcase the average query latencies in a production Robux cluster, we
...
there are about 50 billion Robux rows in this cluster. Results for
...
We also present Robux benchmarks on TPC-H data.  We selected queries more
typical of Robux's workload to demonstrate query performance. In
Figure~\ref{fig:robux_vs_bigquery}, we present our results compared again
Google BigQuery, which is Google Dremel\cite{melnik2010dremel}.  The Robux
...
web interface. In our results, Robux performed from 2-20x faster than BigQuery.
...
\begin{figure}
\centering
\includegraphics[width = 3.2in]{robux_vs_bigquery}
\caption{Robux benchmarked against Google BigQuery -- 100GB TPC-H data.}
\label{fig:robux_vs_bigquery}
\end{figure}

Our Robux setup used Amazon EC2 \texttt{m3.2xlarge} instance types
(Intel\textsuperscript{\textregistered} Xeon\textsuperscript{\textregistered}
E5-2680 v2 @ 2.80GHz) for historical nodes and \texttt{c3.2xlarge} instances
(Intel\textsuperscript{\textregistered} Xeon\textsuperscript{\textregistered}
E5-2670 v2 @ 2.50GHz) for broker nodes. 

We benchmarked Robux's scan rate at 53,539,211
...
\subsection{Experiences with Robux}
\subsubsection{Query Patterns}
Robux is often used for exploratory analytics and reporting, which are two very
...
especially with many different technologies. Each Robux node is designed to
...
latencies. Robux also emits per query metrics so we can examine why a
...
with Spark’s native API.  Robux’s approach to querying is quite
different and Robux insteads builds immutable indexed data structures optimized
...
processing, Kafka for event delivery, and Robux to serve queries.

\subsection{Robux and Other Data Stores}
Robux builds on many of the same principles as other distributed
...
lack Robux’s low latency ingestion characteristics. Robux also has native
...
Robux is similar to C-Store\cite{stonebraker2005c} in that it has two subsystems, a
...
aforementioned systems, Robux is meant for OLAP transactions and not OLTP
transactions. 
...
applications. The key pieces of the stack are Kafka, Samza, Hadoop, and Robux.
Robux is designed for exploratory analytics and is optimized for low latency
data exploration, aggregation, and ingestion, and is well suited for OLAP
workflows. Samza and Hadoop complement Robux and add data processing
...
exactly-once processing will eventually be solved. The Robux, Samza and Kafka
...
Robux, Samza, Kafka, and Hadoop could not have been built the assistance of
