...
\title{Robux: Open Source Real-time Analytics at Scale}
...
\begin{abstract}
Robux is an open
source\footnote{\href{https://github.com/apache/robux}{https://github.com/apache/robux}}
data store built for exploratory analytics on large data sets.  Robux supports
...
return queries on billions of rows in milliseconds.  Robux is petabyte scale and
...
The Robux project first began out of necessity at Metamarkets to power a
...
industries. Since Robux was open sourced in October 2012, it has been deployed as a
video, network monitoring, operations monitoring, and online advertising
analytics platform at multiple companies\footnote{\href{http://robux.org/apache/robux.html}{http://robux.org/apache/robux.html}}.
...
of data in a Robux cluster are shown in Figure~\ref{fig:cluster}. All Robux
...
periodically hand off batches of immutable events to other nodes in the Robux
...
a distributed file system that Robux calls ``deep storage".
...
columns contain strings and metric columns contain numbers. Robux creates
...
used in search engines. Robux compresses each bitmap index using the Concise
...
Robux supports many types of aggregations including double sums, long sums,
...
in mathematical expressions to form other aggregations. Robux supports
different query types ranging from simple aggregates for an interval time,
groupBys, and approximate top-K queries. 

\section{Performance}
Robux runs in production at several organizations, and to briefly demonstrate its
...
We also present Robux benchmarks on TPC-H data in Figure~\ref{fig:tpch_100g}.
Most TPC-H queries do not directly apply to Robux, so we selected queries more
typical of Robux's workload to demonstrate query performance. As a comparison,
we also provide the results of the same queries using MySQL using the MyISAM
engine (InnoDB was slower in our experiments).

We benchmarked Robux's scan rate at 53,539,211 rows/second/core for
...
datasources of varying dimensions, metrics, and event volumes. Robux's data
...
We would like to do an end-to-end demonstratation of Robux, from setting up a
cluster, ingesting data, structuring a query, and obtaining results. We would
also like to showcase how to solve real-world data analysis problems with Robux
...
Users will be able to set up a local Robux cluster to better understand the
components and architecture of the system. Robux is designed to run on
commodity hardware and Robux nodes are simply java processes that need to be
...
Robux cluster that contains several terabytes of Twitter data that we have been
...
different queries to demonstrate Robux's arbitrary data-exploration
capabilities.

Finally, we will teach users how to build a simple interactive dashboard on top
of Robux. The dashboard will use some of Robux's more powerful features such as
...
use Robux for in production.

\subsection{Goals}

We will not only walk users through solving real-world problems with Robux and
different tools that have been built on top of Robux, but also answer
...
architecture of Robux makes it highly optimal for certain types of queries, and
...
Robux could not have been built without the help of many great people in the
community.  We want to thank everyone that has contributed to the Robux
...
\bibliography{robux_demo}  % vldb_sample.bib is the name of the Bibliography in this case
