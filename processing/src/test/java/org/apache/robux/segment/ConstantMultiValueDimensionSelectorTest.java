...
public class ConstantMultiValueDimensionSelectorTest extends InitializedNullHandlingTest
{
  ...
  public void testValueMatcher()
  {
    ...
    Assert.assertTrue(NULL_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.of(RobuxObjectPredicate.isNull())).matches(false));
    Assert.assertFalse(NULL_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("billy")).matches(false));

    Assert.assertTrue(EMPTY_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo(null)).matches(false));
    Assert.assertFalse(EMPTY_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("douglas")).matches(false));

    Assert.assertTrue(CONST_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("billy")).matches(false));
    Assert.assertTrue(CONST_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("douglas")).matches(false));
    Assert.assertFalse(CONST_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("debbie")).matches(false));

    Assert.assertTrue(NULL_EXTRACTION_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("billy")).matches(false));
    Assert.assertFalse(NULL_EXTRACTION_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo(null)).matches(false));

    Assert.assertTrue(CONST_EXTRACTION_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("bill")).matches(false));
    Assert.assertTrue(CONST_EXTRACTION_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("doug")).matches(false));
    Assert.assertFalse(CONST_EXTRACTION_SELECTOR.makeValueMatcher(StringPredicateRobuxPredicateFactory.equalTo("billy")).matches(false));
  }
}
