...
public class SchemalessIndexTest
{
  ...
  private static final String TEST_FILE = "robux.sample.json";
  ...
  public QueryableIndex getMergedIncrementalIndex()
  {
    synchronized (log) {
      ...
      try {
        IncrementalIndex top = makeIncrementalIndex("robux.sample.json.top", METRIC_AGGS);
        IncrementalIndex bottom = makeIncrementalIndex("robux.sample.json.bottom", METRIC_AGGS);
  ...
  public QueryableIndex getMergedIncrementalIndexDiffMetrics()
  {
    return getMergedIncrementalIndex(
        Arrays.asList(
            new Pair<>("robux.sample.json.top", METRIC_AGGS_NO_UNIQ),
            new Pair<>("robux.sample.json.bottom", METRIC_AGGS)
        )
    );
  }
