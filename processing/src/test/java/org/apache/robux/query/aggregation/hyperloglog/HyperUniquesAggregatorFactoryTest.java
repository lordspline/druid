...
package org.apache.robux.query.aggregation.hyperloglog;
...
import org.apache.robux.hll.VersionZeroHyperLogLogCollector;
...
public class HyperUniquesAggregatorFactoryTest
{
  ...
  {
    Throwable exception = assertThrows(RobuxException.class, () -> AGGREGATOR_FACTORY.factorize(metricFactory));
  ...
  {
    Throwable exception = assertThrows(RobuxException.class, () -> AGGREGATOR_FACTORY.factorizeBuffered(metricFactory));
    Assert.assertEquals("Using aggregator [hyperUnique] is not supported for complex columns with type [COMPLEX<json>].", exception.getMessage());
  }
  ...
  {
    Throwable exception = assertThrows(RobuxException.class, () -> AGGREGATOR_FACTORY.factorizeVector(vectorFactory));
    Assert.assertEquals("Using aggregator [hyperUnique] is not supported for complex columns with type [COMPLEX<json>].", exception.getMessage());
  }
}
