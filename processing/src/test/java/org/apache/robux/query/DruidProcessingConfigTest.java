 ...
 *
 */
public class RobuxProcessingConfigTest
{
  ...
  public void testDefaultsMultiProcessor()
  {
    ...
    RobuxProcessingConfig config = injector.getInstance(RobuxProcessingConfig.class);
  ...
  public void testReplacements()
  {
    ...
    props.setProperty("robux.processing.buffer.sizeBytes", "1");
    props.setProperty("robux.processing.buffer.poolCacheMaxCount", "1");
    props.setProperty("robux.processing.numThreads", "256");
    props.setProperty("robux.processing.fifo", "false");
    props.setProperty("robux.processing.tmpDir", "/test/path");
  ...
  public void testInvalidSizeBytes()
  {
    ...
    ExceptionMatcher
        ...
        .assertThrowsAndMatches(() -> injector.getInstance(RobuxProcessingConfig.class));
  }
  ...
  {
    Properties props = new Properties();
    props.setProperty("robux.processing.buffer.sizeBytes", "2GiB");
    Injector injector = makeInjector(
        NUM_PROCESSORS,
        DIRECT_SIZE,
        HEAP_SIZE,
    ...
    Throwable t = Assert.assertThrows(
        ...
        () -> injector.getInstance(RobuxProcessingConfig.class)
    );

    Assert.assertTrue(t.getMessage().contains("robux.processing.buffer.sizeBytes must be less than 2GiB"));
  }
  ...
  )
  {
    ...
    return Guice.createInjector(
        startupInjector.getInstance(RobuxSecondaryModule.class),
        binder -> {
          binder.bind(RuntimeInfo.class).toInstance(new MockRuntimeInfo(numProcessors, directMemorySize, heapSize));
          JsonConfigProvider.bind(binder, "robux.processing", RobuxProcessingConfig.class);
        }
    );
  }
