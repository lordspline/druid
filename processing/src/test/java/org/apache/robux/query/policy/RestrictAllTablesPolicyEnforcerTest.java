...
import org.apache.robux.segment.ReferenceCountedObjectProvider;
...
public class RestrictAllTablesPolicyEnforcerTest
{
  ...
  public void test_serialize() throws Exception
  {
    ...
    String expected = "{\"type\":\"restrictAllTables\",\"allowedPolicies\":[\"org.apache.robux.query.policy.NoRestrictionPolicy\"]}";
  ...
  public void test_validate() throws Exception
  {
    ...
    Assert.assertFalse(policyEnforcer.validate(null));
    Assert.assertTrue(policyEnforcer.validate(policy));
    final RobuxException e2 = Assert.assertThrows(
        RobuxException.class,
        () -> policyEnforcer.validateOrElseThrow(segment, null)
    );
    Assert.assertEquals(RobuxException.Category.FORBIDDEN, e2.getCategory());
    Assert.assertEquals(RobuxException.Persona.OPERATOR, e2.getTargetPersona());
    Assert.assertEquals(
  ...
  public void test_validate_withAllowedPolicies() throws Exception
  {
    ...
    Assert.assertThrows(RobuxException.class, () -> policyEnforcer.validateOrElseThrow(table, null));
    Assert.assertThrows(
        RobuxException.class,
        () -> policyEnforcer.validateOrElseThrow(restricted.getBase(), restricted.getPolicy())
    );
    ...
    Segment segment = new SegmentForTesting("table", Intervals.ETERNITY, "1");
    Assert.assertThrows(RobuxException.class, () -> policyEnforcer.validateOrElseThrow(segment, null));
    Assert.assertThrows(RobuxException.class, () -> policyEnforcer.validateOrElseThrow(segment, policy));
