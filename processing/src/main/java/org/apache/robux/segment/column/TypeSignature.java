...
/**
 * This interface serves as a common foundation for Robuxs native type system, and provides common methods for reasoning
 * about and handling type matters. Additional type common type handling methods are provided by {@link Types} utility.
 *
 * This information is used by Robux to make decisions about how to correctly process inputs and determine output types
 * at all layers of the engine, from how to group, filter, aggregate, and transform columns up to how to best plan SQL
 * into native Robux queries.
 *
 * The native Robux type system can currently be broken down at a high level into 'primitive' types, 'array' types, and
 * 'complex' types, and this classification is defined by an enumeration which implements {@link TypeDescriptor} such
 * as {@link ValueType} for the general query engines and {@link org.apache.robux.math.expr.ExprType} for low level
 ...
 *
 * Robux 'primitive' types includes strings and numeric types. Note: multi-value string columns are still considered
 ...
 *
 * 'complex' types are Robuxs extensible types, which have a registry that allows these types to be defined and
 ...
 *
 * {@link TypeSignature} is currently manifested in 3 forms: {@link ColumnType} which is the high level 'native' Robux
 * type definitions using {@link ValueType}, and is used by row signatures and SQL schemas, used by callers as input
 * to various API methods, and most general purpose type handling. In 'robux-processing' there is an additional
 ...
 *
 * The third is {@link org.apache.robux.math.expr.ExpressionType}, which instead of {@link ValueType} uses
 * {@link org.apache.robux.math.expr.ExprType}, and is used exclusively for handling Robux native expression evaluation.
 * {@link org.apache.robux.math.expr.ExpressionType} exists because the Robux expression system does not natively
 * handle float types, so it is essentially a mapping of {@link ColumnType} where floats are coerced to double typed
 * values. Ideally at some point Robux expressions can just handle floats directly, and these two {@link TypeSignature}
...
public interface TypeSignature<Type extends TypeDescriptor>
{
   ...
   *
   * @see ValueType
   * @see org.apache.robux.math.expr.ExprType
   */
  Type getType();

  /**
   * Type name of 'complex' types ({@link ValueType#COMPLEX}, {@link org.apache.robux.math.expr.ExprType#COMPLEX}),
   ...
   * {@link org.apache.robux.math.expr.ExprType#ARRAY}).
   *
