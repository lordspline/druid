...
public class Metadata
{
  ...
  /**
   * Ordering for the segment associated with this object. Nonnull for segments written in current versions of Robux,
  ...
  public static void validateProjections(List<List<AggregateProjectionMetadata>> projectionsToMerge)
  {
    ...
    for (List<AggregateProjectionMetadata> projections : projectionsToMerge) {
      ...
      if (expectedSize < 0) {
        ...
        throw RobuxException.defensive("Unable to merge projections: mismatched projections count");
      }
      for (AggregateProjectionMetadata projection : projections) {
        ...
        if (prev != null && !prev.getSchema().equals(projection.getSchema())) {
          throw RobuxException.defensive("Unable to merge projections: mismatched projections [%s] and [%s]", prev, projection);
        }
      }
    }
    if (nullCount > 0) {
      if (nullCount != projectionsToMerge.size()) {
        throw RobuxException.defensive("Unable to merge projections: some projections were null");
      }
    }
  }
}
