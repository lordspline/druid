...
import org.apache.robux.segment.data.CompressedVariableSizedBlobColumnSerializer;
    ...
    public ProcessedValue<?> processField(ArrayList<NestedPathPart> fieldPath, @Nullable Object fieldValue)
    {
      ...
      if (writer != null) {
        try {
          ...
          throw RobuxException.defensive(e, "Failed to write field [%s], unhandled value", fieldPath);
        }
      }
      return ProcessedValue.NULL_LITERAL;
    }
      ...
      if (eval.type().isPrimitiveArray()) {
        ...
        if (writer != null) {
          try {
            ...
            throw RobuxException.defensive(e, "Failed to write field [%s] value [%s]", fieldPath, array);
          }
        }
      }
      return null;
    ...
    for (Map.Entry<String, FieldTypeInfo.MutableTypeSet> field : fields.entrySet()) {
      ...
      if (type != null) {
        if (Types.is(type, ValueType.STRING)) {
          ...
          throw RobuxException.defensive("Invalid field type [%s], how did this happen?", type);
        }
      } else {
