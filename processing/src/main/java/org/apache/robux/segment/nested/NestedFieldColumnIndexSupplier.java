...
import org.apache.robux.segment.IntListUtils;
...
import org.apache.robux.segment.index.DictionaryRangeScanningBitmapIndex;
import org.apache.robux.segment.index.DictionaryScanningBitmapIndex;
import org.apache.robux.segment.index.SimpleBitmapColumnIndex;
import org.apache.robux.segment.index.SimpleImmutableBitmapDelegatingIterableIndex;
import org.apache.robux.segment.index.SimpleImmutableBitmapIndex;
import org.apache.robux.segment.index.semantic.ArrayElementIndexes;
import org.apache.robux.segment.index.semantic.DictionaryEncodedStringValueIndex;
import org.apache.robux.segment.index.semantic.DictionaryEncodedValueIndex;
...
import org.apache.robux.segment.index.semantic.NumericRangeIndexes;
    ...
    implements ColumnIndexSupplier
{
    ...
    if (singleType != null) {
      switch (singleType.getType()) {
        case STRING:
          ...
          } else if (clazz.equals(RobuxPredicateIndexes.class)) {
  ...
  private class NestedStringLexicographicalRangeIndexes implements LexicographicalRangeIndexes
  {
    ...
    public BitmapColumnIndex forRange(
        ...
        @Nullable String endValue,
        boolean endStrict,
        RobuxObjectPredicate<String> matcher
    )
    {
      final FixedIndexed<Integer> localDictionary = localDictionarySupplier.get();
      final Indexed<ByteBuffer> stringDictionary = globalStringDictionarySupplier.get();
  ...
  private class NestedStringPredicateIndexes implements RobuxPredicateIndexes
  {
    @Override
    @Nullable
  ...
  private class NestedLongPredicateIndexes implements RobuxPredicateIndexes
  {
    @Override
    @Nullable
    public BitmapColumnIndex forPredicate(RobuxPredicateFactory matcherFactory)
    {
      ...
      return new DictionaryScanningBitmapIndex(localDictionary.size())
      {
        ...
        public Iterable<ImmutableBitmap> getBitmapIterable(boolean includeUnknown)
        {
          ...
          {
            final FixedIndexed<Long> longDictionary = globalLongDictionarySupplier.get();
            final RobuxLongPredicate longPredicate = matcherFactory.makeLongPredicate();
  ...
  private class NestedDoublePredicateIndexes implements RobuxPredicateIndexes
  {
    @Override
    @Nullable
    public BitmapColumnIndex forPredicate(RobuxPredicateFactory matcherFactory)
      ...
      return new DictionaryScanningBitmapIndex(localDictionary.size())
      {
        ...
        public Iterable<ImmutableBitmap> getBitmapIterable(boolean includeUnknown)
        {
          return () -> new Iterator<>()
          {
            final FixedIndexed<Integer> localDictionary = localDictionarySupplier.get();
            final FixedIndexed<Double> doubleDictionary = globalDoubleDictionarySupplier.get();
            final RobuxDoublePredicate doublePredicate = matcherFactory.makeDoublePredicate();
  ...
  /**
   * {@link RobuxPredicateIndexes} but for variant typed nested fields
   */
  private class NestedVariantPredicateIndexes extends NestedVariantIndexes implements RobuxPredicateIndexes
  {
    @Override
    @Nullable
    public BitmapColumnIndex forPredicate(RobuxPredicateFactory matcherFactory)
      ...
      return new DictionaryScanningBitmapIndex(localDictionary.size())
      {
        ...
        public Iterable<ImmutableBitmap> getBitmapIterable(boolean includeUnknown)
        {
          return () -> new Iterator<>()
          {
            ...
            final Supplier<RobuxObjectPredicate<Object[]>> arrayPredicateSupplier = Suppliers.memoize(
