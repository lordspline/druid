...
package org.apache.robux.query.policy;
...
/**
 * Interface for enforcing policies on data sources and segments in Robux queries.
 * <p>
...
{
  /**
   * Validates a {@link DataSource} against the policy enforcer. Prior to query execution, the {@link org.apache.robux.query.Query#getDataSource()}
   * tree is walked. This method is invoked once for each {@link org.apache.robux.query.RestrictedDataSource} and once
   * for each {@link TableDataSource} that is not wrapped inside a {@link org.apache.robux.query.RestrictedDataSource},
   * no matter where they appear within the tree.
   *
   * @param ds     the table to validate.
   * @param policy the policy attached to the table; either {@link org.apache.robux.query.RestrictedDataSource#policy} or null.
   * @throws RobuxException if the data source does not comply with the policy
   */
  default void validateOrElseThrow(TableDataSource ds, Policy policy) throws RobuxException
  {
    if (validate(policy)) {
      return;
    }
    throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                        .ofCategory(RobuxException.Category.FORBIDDEN)
   ...
   * This method is invoked once for each {@link org.apache.robux.segment.RestrictedSegment} and once for each {@link ReferenceCountedSegmentProvider}
   * that is not wrapped inside a {@link org.apache.robux.segment.RestrictedSegment}.
   * <p>
   * Direct invocation of this method is discouraged; use {@link Segment#validateOrElseThrow(PolicyEnforcer)} instead.
   *
   * @param segment the segment to validate
   * @param policy  the policy on the segment, {@link org.apache.robux.segment.RestrictedSegment#policy} or null for other
   * @throws RobuxException if the segment does not comply with the policy
   */
  default void validateOrElseThrow(Segment segment, Policy policy) throws RobuxException
  {
