 ...
 */
public class FallbackPredicate<T> implements RobuxObjectPredicate<T>
{
  private final RobuxObjectPredicate<T> delegate;
  private final ExpressionType expectedType;
  private boolean needsCast = false;

  public FallbackPredicate(RobuxObjectPredicate<T> delegate, ExpressionType expectedType)
  {
  ...
  @Override
  public RobuxPredicateMatch apply(@Nullable T input)
  {
    if (needsCast) {
  ...
  private RobuxPredicateMatch castApply(@Nullable T input)
  {
