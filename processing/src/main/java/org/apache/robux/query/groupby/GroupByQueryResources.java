 ...
 *    and the subtotals.
 * b) {@link org.apache.robux.query.groupby.epinephelinae.GroupByMergingQueryRunner} - Required for merging the results
...
public class GroupByQueryResources implements Closeable
{
  ...
  /**
   * Count the number of merge buffers required for {@link org.apache.robux.query.groupby.epinephelinae.GroupByMergingQueryRunner}
  ...
  /**
   * Returns a merge buffer associated with the {@link org.apache.robux.query.groupby.epinephelinae.GroupByMergingQueryRunner}
   */
  ...
  /**
   * Returns the number of the currently unused merge buffers reserved for {@link org.apache.robux.query.groupby.epinephelinae.GroupByMergingQueryRunner}
   */
  ...
  private static ResourceHolder<ByteBuffer> getMergeBuffer(Deque<ByteBuffer> acquiredBufferPool)
  {
    if (acquiredBufferPool.size() == 0) {
      throw RobuxException.defensive("Insufficient free merge buffers present.");
    }
