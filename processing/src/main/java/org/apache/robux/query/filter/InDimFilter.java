...
import org.apache.robux.segment.index.semantic.Utf8ValueSetIndexes;
...
public class InDimFilter extends AbstractOptimizableDimFilter implements Filter
{
  ...
  private InDimFilter(
      ...
      @Nullable final RobuxPredicateFactory predicateFactory
  )
  {
    ...
    if (predicateFactory != null) {
      ...
      this.predicateFactory = new InFilterRobuxPredicateFactory(extractionFn, this.values);
    }
  ...
  public BitmapColumnIndex getBitmapColumnIndex(ColumnIndexSelector selector)
  {
    ...
    if (extractionFn == null) {
      ...
      if (indexSupplier == null) {
        ...
        RobuxPredicateMatch match = predicateFactory.makeStringPredicate().apply(null);
  ...
  private static RobuxObjectPredicate<String> createStringPredicate(final Set<String> values)
  {
    Preconditions.checkNotNull(values, "values");
    return value -> {
      if (value == null) {
        return values.contains(null) ? RobuxPredicateMatch.TRUE : RobuxPredicateMatch.UNKNOWN;
      }
      return RobuxPredicateMatch.of(values.contains(value));
    };
  }

  private static RobuxLongPredicate createLongPredicate(final Set<String> values)
  {
    ...
    {
      @Override
      public RobuxPredicateMatch applyLong(long n)
      {
        return RobuxPredicateMatch.of(longHashSet.contains(n));
      }

      @Override
      public RobuxPredicateMatch applyNull()
      {
        return matchNull ? RobuxPredicateMatch.TRUE : RobuxPredicateMatch.UNKNOWN;
      }
    };
  }

  private static RobuxFloatPredicate createFloatPredicate(final Set<String> values)
  {
    ...
    {
      @Override
      public RobuxPredicateMatch applyFloat(float n)
      {
        return RobuxPredicateMatch.of(floatBitsHashSet.contains(Float.floatToIntBits(n)));
      }
  ...
  private static RobuxDoublePredicate createDoublePredicate(final Set<String> values)
  {
    ...
    {
      @Override
      public RobuxPredicateMatch applyDouble(double n)
      {
        return RobuxPredicateMatch.of(doubleBitsHashSet.contains(Double.doubleToLongBits(n)));
      }
  ...
  @VisibleForTesting
  public static class InFilterRobuxPredicateFactory implements RobuxPredicateFactory
  {
    private final ExtractionFn extractionFn;
    ...
    public InFilterRobuxPredicateFactory(
    ...
    {
      if (extractionFn != null) {
        final RobuxObjectPredicate<String> stringPredicate = stringPredicateSupplier.get();
    ...
    public boolean equals(Object o)
    {
      ...
      InFilterRobuxPredicateFactory that = (InFilterRobuxPredicateFactory) o;
