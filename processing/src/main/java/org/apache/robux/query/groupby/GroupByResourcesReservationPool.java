 ...
 * <p>
 * ASSUMPTIONS
 * There's an attempt to link various places where the merge buffers are acquired ({@link org.apache.robux.query.QueryToolChest#mergeResults})
 * and merge buffers are utilized ({@link org.apache.robux.query.QueryToolChest#mergeResults} and {@link GroupByQueryRunnerFactory.GroupByQueryRunner#mergeRunners}).
 * However, Robux's code doesn't provide any explicit contract between the arguments of these methods, and input to {@code mergeResults} can be any runner,
...
public class GroupByResourcesReservationPool
{
  ...
  )
  {
    ...
    if (queryResourceId == null) {
      throw RobuxException.defensive("Query resource id must be populated");
    }
    ...
    if (existingResource != null) {
      throw RobuxException.defensive("Resource with the given identifier [%s] is already present", queryResourceId);
    }
