...
public class FixedWidthGroupByColumnSelectorStrategy<T> implements GroupByColumnSelectorStrategy
{
  ...
  )
  {
    ...
    if (written < 0) {
      throw RobuxException.defensive("Unable to serialize the value [%s] to buffer", rowObj);
    }
  }
  ...
  )
  {
    ...
    if (written < 0) {
      throw RobuxException.defensive("Unable to serialize the value [%s] to buffer", value);
    }
