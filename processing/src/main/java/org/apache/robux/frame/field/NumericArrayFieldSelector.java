...
public abstract class NumericArrayFieldSelector<ElementType extends Number> implements ColumnValueSelector<Object[]>
{
   ...
   *
   * Frames read and written using {@link org.apache.robux.frame.write.FrameWriter} and
   * {@link org.apache.robux.frame.read.FrameReader} shouldn't worry about this detail, since they automatically update
  ...
  private void updateCurrentArray(final long fieldPosition, final long fieldLength)
  {
    ...
    if (!rowTerminatorSeen || curElement != numElements) {
      throw RobuxException.defensive("Unexpected end of field");
    }
  }
  ...
  {
    if (fieldSize <= 1) {
      throw RobuxException.defensive("fieldSize should be greater than 1 for non null array elements");
    }
    ...
    if (cumulativeFieldSize % getIndividualFieldSize() != 0) {
      throw RobuxException.defensive("cumulativeFieldSize should be a multiple of the individual fieldSize");
    }
