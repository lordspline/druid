...
import org.apache.robux.frame.channel.PartitionedReadableFrameChannel;
      ...
      runWorker(worker, outputRows -> {
        synchronized (runWorkersLock) {
          ...
          if (isLimited() && totalMergingLevels != UNKNOWN_LEVEL && level == totalMergingLevels - 1) {
            ...
            if (rowLimit < 0) {
              throw RobuxException.defensive("rowLimit[%d] below zero after outputRows[%d]", rowLimit, outputRows);
            }
          }
