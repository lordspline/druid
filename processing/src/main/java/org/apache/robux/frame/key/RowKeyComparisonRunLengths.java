    ...
    for (KeyColumn keyColumn : keyColumns) {
      ...
      ColumnType columnType =
          rowSignature.getColumnType(keyColumn.columnName())
                      .orElseThrow(() -> RobuxException.defensive("No type for column[%s]", keyColumn.columnName()));
  ...
  private static boolean isByteComparable(final ColumnType columnType)
  {
    if (columnType.is(ValueType.COMPLEX)) {
      if (columnType.getComplexTypeName() == null) {
        throw RobuxException.defensive("Cannot sort unknown complex types");
      }
      ...
      throw RobuxException.defensive("Nested arrays aren't supported in row based frames");
    }
    return true;
  }
