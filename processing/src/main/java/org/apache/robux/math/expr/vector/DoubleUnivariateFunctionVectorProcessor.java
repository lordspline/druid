 ...
 *
 * this one is specialized for producing double[], see {@link LongUnivariateFunctionVectorProcessor} for
 * long[] primitives.
 */
public abstract class DoubleUnivariateFunctionVectorProcessor<TInput> implements ExprVectorProcessor<double[]>
{
  ...
  public DoubleUnivariateFunctionVectorProcessor(
      ExprVectorProcessor<TInput> processor
  )
  {
    this.processor = processor;
    this.outNulls = new boolean[processor.maxVectorSize()];
    this.outValues = new double[processor.maxVectorSize()];
  }
