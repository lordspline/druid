...
package org.apache.robux.common.semantic;
    ...
    for (Method method : clazz.getMethods()) {
      if (method.isAnnotationPresent(SemanticCreator.class)) {
        if (method.getParameterCount() != 0) {
          throw RobuxException.defensive("Method [%s] annotated with SemanticCreator was not 0-argument.", method);
        }
        ...
        retVal.put(method.getReturnType(), arg -> {
          try {
            ...
            throw RobuxException.defensive().build(e, "Problem invoking method [%s]", method);
          }
        });
      }
    }
