...
package org.apache.robux.storage.local;
  ...
  public static File validateAndGetPath(String basePath, String customPath)
  {
    if (basePath == null) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          ...
                          .build(
                              "The runtime property `robux.export.storage.baseDir` must be configured for local export.");
    }
    ...
    if (!baseDir.isAbsolute()) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          ...
                          .build(
                              "The runtime property `robux.export.storage.baseDir` must be an absolute path.");
    }
    ...
    if (!exportFile.toPath().normalize().startsWith(baseDir.toPath())) {
      throw RobuxException.forPersona(RobuxException.Persona.USER)
                          .ofCategory(RobuxException.Category.INVALID_INPUT)
                          .build("The provided destination [%s] must be within the path configured by runtime property `robux.export.storage.baseDir` "
