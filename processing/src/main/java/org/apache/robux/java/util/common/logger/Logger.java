...
package org.apache.robux.java.util.common.logger;
...
/**
 * A Logger for usage inside of Robux.  Provides a layer that allows for simple changes to the logging framework
 * with minimal changes to the Robux code.
 *
...
public class Logger
{
  ...
  /**
   * Returns a copy of this Logger that does not log exception stack traces or {@link RobuxException#getContext()},
  ...
  private void logException(BiConsumer<String, Throwable> fn, Throwable t, String message)
  {
    ...
    if (logStackTrace) {
      // If logging stack traces, *also* log extra context information about RobuxExceptions.
      if (t instanceof RobuxException && !((RobuxException) t).getContext().isEmpty()) {
        message = (message == null ? "" : message + "\nRobuxException context: " + ((RobuxException) t).getContext());
      }
