...
const UNIFIED_CONSOLE_PORT = process.env['ROBUX_E2E_TEST_UNIFIED_CONSOLE_PORT'] || '8888';
...
const ROBUX_DIR = path.dirname(WEB_CONSOLE_DIR);
export const ROBUX_EXAMPLES_QUICKSTART_TUTORIAL_DIR = path.join(
  ROBUX_DIR,
  'examples',
  'quickstart',
  'tutorial',
...
export function runIndexTask(ingestionSpecPath: string, sedCommands: string[]) {
  const postIndexTask = path.join(ROBUX_DIR, 'examples', 'bin', 'post-index-task');
