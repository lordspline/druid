...
import { Column, QueryResult, SqlExpression, SqlQuery, SqlWithQuery } from 'robux-query-toolkit';
...
export function validateLastExecution(possibleLastExecution: any): LastExecution | undefined {
  if (
    ...
    !validRobuxEngine(possibleLastExecution.engine) ||
...
export class Execution {
  ...
  static fromResult(engine: RobuxEngine, result: QueryResult): Execution {
    return new Execution({
      engine,
  ...
  public changeEngine(engine: RobuxEngine): Execution {
    return new Execution({
      ...this.valueOf(),
      engine,
    });
