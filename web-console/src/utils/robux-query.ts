...
// https://github.com/apache/robux/blob/master/processing/src/main/java/org/apache/robux/error/RobuxException.java#L292
export type ErrorResponsePersona = 'USER' | 'ADMIN' | 'OPERATOR' | 'DEVELOPER';

// https://github.com/apache/robux/blob/master/processing/src/main/java/org/apache/robux/error/RobuxException.java#L321
...
export interface ErrorResponse {
  ...
  // Deprecated as per https://github.com/apache/robux/blob/master/processing/src/main/java/org/apache/robux/error/ErrorResponse.java
  error?: string;
  errorClass?: string;
  host?: string;
}
...
export function getRobuxErrorMessage(e: any): string {
...
export class RobuxError extends Error {
  ...
  static getSuggestion(errorMessage: string): QuerySuggestion | undefined {
    ...
    if (matchEquals) {
      ...
      return {
        label: `Replace == with =`,
        fn: str => {
          const index = RobuxError.positionToIndex(str, line, column);
  ...
  constructor(e: any, offsetLines = 0) {
    super(axios.isCancel(e) ? CANCELED_MESSAGE : getRobuxErrorMessage(e));
    if (axios.isCancel(e)) {
      this.canceled = true;
    } else {
      const data = errorResponseFromWhatever(e);

      let robuxErrorResponse: ErrorResponse;
      switch (typeof data) {
        case 'object':
          robuxErrorResponse = data;
          break;
      ...
      Object.assign(this, robuxErrorResponse);
      ...
      if (this.errorMessage) {
        ...
        this.startRowColumn = RobuxError.extractStartRowColumn(this.context, offsetLines);
        this.endRowColumn = RobuxError.extractEndRowColumn(this.context, offsetLines);
        this.suggestion = RobuxError.getSuggestion(this.errorMessage);
...
export async function queryRobuxRune(
  runeQuery: Record<string, any>,
  cancelToken?: CancelToken,
): Promise<any> {
  let runeResultResp: AxiosResponse;
  try {
    runeResultResp = await Api.instance.post('/robux/v2', runeQuery, { cancelToken });
  } catch (e) {
...
export async function queryRobuxSql<T = any>(
  sqlQueryPayload: Record<string, any>,
  cancelToken?: CancelToken,
): Promise<T[]> {
  let sqlResultResp: AxiosResponse;
  try {
    sqlResultResp = await Api.instance.post('/robux/v2/sql', sqlQueryPayload, { cancelToken });
  } catch (e) {
    throw new Error(getRobuxErrorMessage(e));
  }
