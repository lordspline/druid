...
{
  @Nullable
  public static RobuxExpression makeTrimExpression(
      final SqlTrimFunction.Flag trimStyle,
      final RobuxExpression stringExpression,
      final RobuxExpression charsExpression,
      final ColumnType robuxType
  )
  {
    ...
    // Robux version of trim is multi-function (ltrim/rtrim/trim) and the other two args are swapped.
    return RobuxExpression.ofFunctionCall(robuxType, functionName, ImmutableList.of(stringExpression, charsExpression));
  }
  ...
  )
  {
    ...
    final RobuxExpression charsExpression = Expressions.toRobuxExpression(
        plannerContext,
        rowSignature,
        call.getOperands().get(1)
    );

    final RobuxExpression stringExpression = Expressions.toRobuxExpression(
        plannerContext,
        rowSignature,
