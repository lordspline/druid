...
public abstract class TimeArithmeticOperatorConversion implements SqlOperatorConversion
{
  ...
  )
  {
    ...
    final RobuxExpression leftExpr = Expressions.toRobuxExpression(plannerContext, rowSignature, leftRexNode);
    final RobuxExpression rightExpr = Expressions.toRobuxExpression(plannerContext, rowSignature, rightRexNode);
    ...
    if (rightRexNode.getType().getFamily() == SqlTypeFamily.INTERVAL_YEAR_MONTH) {
      ...
      return RobuxExpression.ofExpression(
          ...
          ImmutableList.of(
              ...
              RobuxExpression.ofLiteral(ColumnType.LONG, RobuxExpression.longLiteral(direction > 0 ? 1 : -1)),
      ...
      if (call.getType().getFamily() == SqlTypeFamily.INTERVAL_YEAR_MONTH) {
        return RobuxExpression.ofExpression(
            outputType,
            RobuxExpression.functionCall("subtract_months"),
            ImmutableList.of(
                leftExpr,
                rightExpr,
                RobuxExpression.ofStringLiteral(plannerContext.getTimeZone().getID())
