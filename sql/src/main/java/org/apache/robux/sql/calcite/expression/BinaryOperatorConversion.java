...
public class BinaryOperatorConversion implements SqlOperatorConversion
{
  ...
  public BinaryOperatorConversion(final SqlOperator operator, final String robuxOperator)
  {
    this.operator = operator;
    this.joiner = Joiner.on(" " + robuxOperator + " ");
  }
  ...
  private RobuxExpression.RobuxExpressionCreator getOperatorFunction(RexNode rexNode)
  {
    return operands -> {
      if (operands.size() < 2) {
      ...
      return RobuxExpression.ofExpression(
          ...
          (args) -> StringUtils.format(
              ...
              joiner.join(
                  args.stream()
                      .map(RobuxExpression::getExpression)
