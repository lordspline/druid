...
import org.apache.robux.sql.calcite.expression.PostAggregatorVisitor;
 ...
 * store post-sorting and post-grouping projections, as well as directly in {@link RobuxQuery} to store potential
...
public class Projection
{
  ...
  )
  {
    ...
    final RobuxExpression postAggregatorExpression = Expressions.toRobuxExpression(
        plannerContext,
        inputRowSignature,
  ...
  )
  {
    ...
    if (pagg != null) {
      ...
      final RobuxExpression postAggregatorExpression = Expressions.toRobuxExpressionWithPostAggOperands(
          plannerContext,
          inputRowSignature,
          postAggregatorRexNode,
          postAggregatorVisitor
  ...
  private static void handlePostAggregatorExpression(
      ...
      final List<String> rowOrder,
      final PostAggregatorVisitor postAggregatorVisitor,
      final RobuxExpression postAggregatorExpression
  )
  {
    if (postAggregatorDirectColumnIsOk(inputRowSignature, postAggregatorExpression, postAggregatorRexNode)) {
      // Direct column access, without any type cast as far as Robux's runtime is concerned.
      ...
      final PostAggregator postAggregator = new ExpressionPostAggregator(
          ...
          null,
          postAggregatorExpression.getRobuxType(),
  ...
  {
    final List<RobuxExpression> expressions = new ArrayList<>();
    ...
    for (int i = 0; i < expressions.size(); i++) {
      final RobuxExpression expression = expressions.get(i);
  ...
  public List<PostAggregator> getPostAggregators()
  {
    ...
    // used in a post-aggregation context. This is likely a bug somewhere in RobuxQuery. See class-level Javadocs.
  ...
  public List<String> getVirtualColumns()
  {
    ...
    // used in a pre-aggregation context. This is likely a bug somewhere in RobuxQuery. See class-level Javadocs.
