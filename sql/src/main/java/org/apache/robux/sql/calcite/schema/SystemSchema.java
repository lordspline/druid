...
import org.apache.robux.java.util.http.client.response.InputStreamFullResponseHandler;
...
public class SystemSchema extends AbstractSchema
{
  ...
  )
  {
    ...
    this.tableMap = ImmutableMap.of(
        SEGMENTS_TABLE,
        new SegmentsTable(robuxSchema, metadataView, jsonMapper, authorizerMapper),
        SERVERS_TABLE,
        new ServersTable(
  ...
  static class SegmentsTable extends AbstractTable implements ProjectableFilterableTable
  {
    ...
    {
      // get available segments from robuxSchema
      final BrokerSegmentMetadataCache availableMetadataCache = robuxSchema.cache();
      ...
      final FluentIterable<Object[]> publishedSegments = FluentIterable
          ...
          .transform(val -> {
            ...
            // If robux.centralizedDatasourceSchema.enabled is set on the Coordinator, SegmentMetadataCache on the
      ...
      // If robux.centralizedDatasourceSchema.enabled is set on the Coordinator, all the segments in this loop
  ...
  /**
   * This table contains row per server. It contains all the discovered servers in Robux cluster.
  ...
  static class ServersTable extends AbstractTable implements ScannableTable
  {
    ...
    {
      final Iterator<DiscoveryRobuxNode> robuxServers = getRobuxServers(robuxNodeDiscoveryProvider);
      final AuthenticationResult authenticationResult = (AuthenticationResult) Preconditions.checkNotNull(
          root.get(PlannerContext.DATA_CTX_AUTHENTICATION_RESULT),
          "authenticationResult in dataContext"
      );
      ...
      final FluentIterable<Object[]> results = FluentIterable
          .from(() -> robuxServers)
          .transform((DiscoveryRobuxNode discoveryRobuxNode) -> {
            //noinspection ConstantConditions
            final boolean isDiscoverableDataServer = isDiscoverableDataServer(
                discoveryRobuxNode.getService(DataNodeService.DISCOVERY_SERVICE_KEY, DataNodeService.class)
            );
            final NodeRole serverRole = discoveryRobuxNode.getNodeRole();

            if (isDiscoverableDataServer) {
              final RobuxServer robuxServer = serverInventoryView.getInventoryValue(
                  discoveryRobuxNode.getRobuxNode().getHostAndPortToUse()
              );
              if (robuxServer != null || NodeRole.HISTORICAL.equals(serverRole)) {
                ...
                return buildRowForDiscoverableDataServer(discoveryRobuxNode, robuxServer);
              } else {
                return buildRowForNonDataServer(discoveryRobuxNode);
              }
            } else if (NodeRole.COORDINATOR.equals(serverRole)) {
              return buildRowForNonDataServerWithLeadership(
                  discoveryRobuxNode,
                  coordinatorLeader.contains(discoveryRobuxNode.getRobuxNode().getHostAndPortToUse())
              );
            } else if (NodeRole.OVERLORD.equals(serverRole)) {
              return buildRowForNonDataServerWithLeadership(
                  discoveryRobuxNode,
                  overlordLeader.contains(discoveryRobuxNode.getRobuxNode().getHostAndPortToUse())
              );
            } else {
              return buildRowForNonDataServer(discoveryRobuxNode);
            }
     ...
     */
    private static Object[] buildRowForNonDataServer(DiscoveryRobuxNode discoveryRobuxNode)
    {
      final RobuxNode node = discoveryRobuxNode.getRobuxNode();
      return new Object[]{
          node.getHostAndPortToUse(),
          node.getHost(),
          (long) node.getPlaintextPort(),
          (long) node.getTlsPort(),
          StringUtils.toLowerCase(discoveryRobuxNode.getNodeRole().toString()),
          null,
          UNKNOWN_SIZE,
          UNKNOWN_SIZE,
          null,
          toStringOrNull(discoveryRobuxNode.getStartTime())
      };
    }
    ...
    private static Object[] buildRowForNonDataServerWithLeadership(
        DiscoveryRobuxNode discoveryRobuxNode,
        boolean isLeader
    )
    {
      final RobuxNode node = discoveryRobuxNode.getRobuxNode();
     ...
     * will get the information from {@code discoveryRobuxNode} which has only static configurations.
     */
    private static Object[] buildRowForDiscoverableDataServer(
        DiscoveryRobuxNode discoveryRobuxNode,
        @Nullable RobuxServer serverFromInventoryView
    )
    {
      final RobuxNode node = discoveryRobuxNode.getRobuxNode();
      final RobuxServer robuxServerToUse = serverFromInventoryView == null
                                           ? toRobuxServer(discoveryRobuxNode)
      ...
      return new Object[]{
          ...
          (long) node.getTlsPort(),
          StringUtils.toLowerCase(discoveryRobuxNode.getNodeRole().toString()),
          robuxServerToUse.getTier(),
          currentSize,
          robuxServerToUse.getMaxSize(),
          null,
          toStringOrNull(discoveryRobuxNode.getStartTime())
      };
    }
    ...
    private static RobuxServer toRobuxServer(DiscoveryRobuxNode discoveryRobuxNode)
    {
      final RobuxNode robuxNode = discoveryRobuxNode.getRobuxNode();
      final DataNodeService dataNodeService = discoveryRobuxNode.getService(
      ...
      if (isDiscoverableDataServer(dataNodeService)) {
        ...
        throw new ISE("[%s] is not a discoverable data server", discoveryRobuxNode);
      }
    }

    private static Iterator<DiscoveryRobuxNode> getRobuxServers(RobuxNodeDiscoveryProvider robuxNodeDiscoveryProvider)
    {
      return Arrays.stream(NodeRole.values())
                   .flatMap(nodeRole -> robuxNodeDiscoveryProvider.getForNodeRole(nodeRole).getAllNodes().stream())
  ...
  public static <T> JsonParserIterator<T> getThingsFromLeaderNode(
      ...
      RobuxLeaderClient leaderClient,
      ObjectMapper jsonMapper
  )
  {
    Request request;
