...
/**
 * {@link RobuxLogicalNode} convention node for {@link Sort} plan node.
 */
public class RobuxSort extends Sort implements RobuxLogicalNode
{
  private RobuxSort(
  ...
  public static RobuxSort create(RelNode input, RelCollation collation, RexNode offset, RexNode fetch)
  {
    RelOptCluster cluster = input.getCluster();
    collation = RelCollationTraitDef.INSTANCE.canonize(collation);
    RelTraitSet traitSet =
        input.getTraitSet().replace(RobuxLogicalConvention.instance()).replace(collation);
    return new RobuxSort(cluster, traitSet, input, collation, offset, fetch);
  }
  ...
  {
    return new RobuxSort(getCluster(), traitSet, newInput, newCollation, offset, fetch);
  }
