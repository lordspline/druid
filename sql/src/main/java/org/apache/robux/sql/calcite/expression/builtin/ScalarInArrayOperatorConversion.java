...
import org.apache.robux.sql.calcite.expression.RobuxLiteral;
...
public class ScalarInArrayOperatorConversion extends DirectOperatorConversion
{
  ...
  )
  {
    ...
    final RobuxExpression scalarExpression = Expressions.toRobuxExpression(plannerContext, rowSignature, scalarOperand);
    ...
    if (Calcites.isLiteral(arrayOperand, true, true)) {
      ...
      for (final RexNode arrayElement : arrayElements) {
        final RobuxLiteral arrayElementEval = Expressions.calciteLiteralToRobuxLiteral(plannerContext, arrayElement);
