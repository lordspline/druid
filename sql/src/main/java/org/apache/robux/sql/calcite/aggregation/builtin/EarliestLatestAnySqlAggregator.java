...
public class EarliestLatestAnySqlAggregator implements SqlAggregator
{
  ...
  )
  {
    ...
    final List<RobuxExpression> args = Expressions.toRobuxExpressions(plannerContext, inputAccessor.getInputRowSignature(), rexNodes);
    ...
    if (outputType == null) {
      throw RobuxException.forPersona(RobuxException.Persona.ADMIN)
                          ...
                          .build(
                              "Cannot convert output SQL type[%s] to a Robux type for function [%s]",
