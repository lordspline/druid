...
import org.apache.robux.sql.calcite.planner.CalcitePlanner;
...
public class SqlQueryPlus
{
  ...
  {
    if (sqlNode == null) {
      throw RobuxException.defensive("sqlNode not set");
    }
  ...
  public SqlQueryPlus freshCopy()
  {
    return new SqlQueryPlus(
        sql,
        RobuxSqlParser.parse(sql, allowSetStatements).getMainStatement(),
        allowSetStatements,
        queryContext,
        parameters,
        authResult
