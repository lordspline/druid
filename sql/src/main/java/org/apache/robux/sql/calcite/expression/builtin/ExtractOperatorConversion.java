...
public class ExtractOperatorConversion implements SqlOperatorConversion
{
  ...
  )
  {
    ...
    final RobuxExpression input = Expressions.toRobuxExpression(plannerContext, rowSignature, arg);
    if (input == null) {
      return null;
    }

    final TimestampExtractExprMacro.Unit robuxUnit = EXTRACT_UNIT_MAP.get(calciteUnit);
    if (robuxUnit == null) {
      // Don't know how to extract this time unit.
      return null;
    }

    return TimeExtractOperatorConversion.applyTimeExtract(input, robuxUnit, plannerContext.getTimeZone());
  }
}
