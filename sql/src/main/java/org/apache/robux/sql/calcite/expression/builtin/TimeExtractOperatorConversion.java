...
public class TimeExtractOperatorConversion implements SqlOperatorConversion
{
  ...
  public static RobuxExpression applyTimeExtract(
      final RobuxExpression timeExpression,
      final TimestampExtractExprMacro.Unit unit,
      final DateTimeZone timeZone
  )
  {
    return RobuxExpression.ofFunctionCall(
        timeExpression.getRobuxType(),
        "timestamp_extract",
        ImmutableList.of(
            timeExpression,
            RobuxExpression.ofStringLiteral(unit.name()),
            RobuxExpression.ofStringLiteral(timeZone.getID())
        )
    );
  }
  ...
  public static RobuxExpression applyTimeExtract(
      final RobuxExpression timeExpression,
      final TimestampExtractExprMacro.Unit unit,
      final RobuxExpression timeZoneExpression
  )
  {
    return RobuxExpression.ofFunctionCall(
        timeExpression.getRobuxType(),
  ...
  )
  {
    ...
    final RobuxExpression timeExpression = Expressions.toRobuxExpression(plannerContext, rowSignature, timeArg);
    ...
    if (call.getOperands().size() > 2 && call.getOperands().get(2) instanceof RexInputRef) {
      ...
      final RobuxExpression timeZoneExpression = Expressions.toRobuxExpression(
          plannerContext,
          rowSignature,
          timeZoneArg
      );
