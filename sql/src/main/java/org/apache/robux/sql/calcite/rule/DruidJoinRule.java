...
public class RobuxJoinRule extends RelOptRule
{

  private final boolean enableLeftScanDirect;
  private final PlannerContext plannerContext;

  private RobuxJoinRule(final PlannerContext plannerContext)
  {
    super(
        operand(
            Join.class,
            operand(RobuxRel.class, any()),
            operand(RobuxRel.class, any())
        )
    );
  ...
  public static RobuxJoinRule instance(PlannerContext plannerContext)
  {
    return new RobuxJoinRule(plannerContext);
  }
  ...
  public boolean matches(RelOptRuleCall call)
  {
    ...
    final RobuxRel<?> left = call.rel(1);
    final RobuxRel<?> right = call.rel(2);

    // 1) Can handle the join condition as a native join.
    // 2) Left has a PartialRobuxQuery (i.e., is a real query, not top-level UNION ALL).
    // 3) Right has a PartialRobuxQuery (i.e., is a real query, not top-level UNION ALL).
    ...
    ) && left.getPartialRobuxQuery() != null && right.getPartialRobuxQuery() != null;
  }
  ...
  public void onMatch(RelOptRuleCall call)
  {
    ...
    final RobuxRel<?> newLeft;
    final RobuxRel<?> newRight;
    ...
    final boolean isLeftDirectAccessPossible = enableLeftScanDirect && (left instanceof RobuxQueryRel);

    final JoinAlgorithm joinAlgorithm = QueryUtils.getJoinAlgorithm(join, plannerContext);
    if (!joinAlgorithm.requiresSubquery()
        && left.getPartialRobuxQuery().stage() == PartialRobuxQuery.Stage.SELECT_PROJECT
        && (isLeftDirectAccessPossible || left.getPartialRobuxQuery().getWhereFilter() == null)) {
      // Swap the left-side projection above the join, so the left side is a simple scan or mapping. This helps us
      // avoid subqueries.
      final RelNode leftScan = left.getPartialRobuxQuery().getScan();
      final Project leftProject = left.getPartialRobuxQuery().getSelectProject();
      leftFilter = left.getPartialRobuxQuery().getWhereFilter();

      // Left-side projection expressions rewritten to be on top of the join.
      newProjectExprs.addAll(leftProject.getProjects());
      newLeft = left.withPartialQuery(PartialRobuxQuery.create(leftScan));
    ...
    if (!joinAlgorithm.requiresSubquery()
        && right.getPartialRobuxQuery().stage() == PartialRobuxQuery.Stage.SELECT_PROJECT
        && right.getPartialRobuxQuery().getWhereFilter() == null
        && !right.getPartialRobuxQuery().getSelectProject().isMapping()
        && conditionAnalysis.onlyUsesMappingsFromRightProject(right.getPartialRobuxQuery().getSelectProject())) {
      // Swap the right-side projection above the join, so the right side is a simple scan or mapping. This helps us
      // avoid subqueries.
      final RelNode rightScan = right.getPartialRobuxQuery().getScan();
      final Project rightProject = right.getPartialRobuxQuery().getSelectProject();
      ...
      newRight = right.withPartialQuery(PartialRobuxQuery.create(rightScan));
    ...
    // Robux join written on top of the new left and right sides.
    final RobuxJoinQueryRel robuxJoin = RobuxJoinQueryRel.create(
        join.copy(
            join.getTraitSet(),
        ...
        call.builder()
            ...
            .project(
                RexUtil.fixUp(
                    rexBuilder,
                    newProjectExprs,
                    RelOptUtil.getFieldTypeList(robuxJoin.getRowType())
                )
            );
  ...
  )
  {
    ...
    if (right != null
        && !RobuxJoinQueryRel.computeRightRequiresSubquery(plannerContext, RobuxJoinQueryRel.getSomeRobuxChild(right))
        && right instanceof RobuxQueryRel) {
      RobuxQueryRel robuxQueryRel = (RobuxQueryRel) right;
      if (robuxQueryRel.getRobuxTable().getDataSource() instanceof LookupDataSource) {
  ...
  public static class ConditionAnalysis
  {
    ...
    /**
     * Sub-conditions in join clause that cannot be handled by the RobuxJoinRule.
     */
  ...
  /**
   * Like {@link org.apache.robux.segment.join.Equality} but uses {@link RexNode} instead of
   * {@link org.apache.robux.math.expr.Expr}.
   */
  static class RexEquality
  {
    ...
    public RexNode makeCall(final RexBuilder builder)
    {
      ...
      if (kind == SqlKind.EQUALS) {
        ...
        throw RobuxException.defensive("Unexpected operator kind[%s]", kind);
      }
