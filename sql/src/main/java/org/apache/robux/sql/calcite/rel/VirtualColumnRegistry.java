 ...
 * aggregators while constructing a {@link RobuxQuery}.
 */
public class VirtualColumnRegistry
{
  ...
  /**
   * Get existing or create new {@link VirtualColumn} for a given {@link RobuxExpression} and hinted {@link ColumnType}.
   */
  public String getOrCreateVirtualColumnForExpression(
      RobuxExpression expression,
      ColumnType typeHint
  )
  {
  ...
  /**
   * Get existing or create new {@link VirtualColumn} for a given {@link RobuxExpression} and {@link RelDataType}
   */
  public String getOrCreateVirtualColumnForExpression(
      RobuxExpression expression,
  ...
  {
    if (typeHint.getSqlTypeName() == SqlTypeName.OTHER && expression.getRobuxType() != null) {
      // fall back to robux type if sql type isn't very helpful
  ...
  public VirtualColumn getVirtualColumn(String virtualColumnName)
  {
    ...
    RobuxExpression expression = registeredColumn.getExpression();
  ...
  @Nullable
  public String getVirtualColumnByExpression(RobuxExpression expression, RelDataType typeHint)
  {
   ...
   */
  public List<RobuxExpression> findVirtualColumnExpressions(List<String> allColumns)
  {
  ...
  public void visitAllSubExpressions(RobuxExpression.RobuxExpressionShuttle shuttle)
  {
    ...
    while (!toVisit.isEmpty()) {
      ...
      final List<RobuxExpression> newArgs = shuttle.visitAll(wrapped.getExpression().getArguments());
  ...
  /**
   * @deprecated use {@link #getOrCreateVirtualColumnForExpression(RobuxExpression, ColumnType)} instead
   */
  @Deprecated
  ...
  /**
   * @deprecated use {@link #getOrCreateVirtualColumnForExpression(RobuxExpression, RelDataType)} instead
   */
  @Deprecated
  public VirtualColumn getOrCreateVirtualColumnForExpression(
      PlannerContext plannerContext,
  ...
  /**
   * @deprecated use {@link #getVirtualColumnByExpression(RobuxExpression, RelDataType)} instead
   */
  @Deprecated
  @Nullable
  public VirtualColumn getVirtualColumnByExpression(String expression, RelDataType type)
  {
    ...
    ExpressionAndTypeHint wrapped = wrap(RobuxExpression.fromExpression(expression), columnType);
    return Optional.ofNullable(virtualColumnsByExpression.get(wrapped))
                   .map(this::getVirtualColumn)
                   .orElse(null);
  }

  private static ExpressionAndTypeHint wrap(RobuxExpression expression, ColumnType typeHint)
  {
    return new ExpressionAndTypeHint(expression, typeHint);
  }

  /**
   * Wrapper class for a {@link RobuxExpression} and the output {@link ColumnType} "hint" that callers can specify when
   * adding a virtual column with {@link #getOrCreateVirtualColumnForExpression(RobuxExpression, RelDataType)} or
   * {@link #getOrCreateVirtualColumnForExpression(RobuxExpression, ColumnType)}. This "hint"  will be passed into
   * {@link RobuxExpression#toVirtualColumn(String, ColumnType, ExpressionParser)}.
   *
   * The type hint might be different than {@link RobuxExpression#getRobuxType()} since that value is the captured value
   * of {@link org.apache.calcite.rex.RexNode#getType()} converted to the Robux type system, while callers might still
   * explicitly specify a different type to use for the hint. Additionally, the method used to convert Calcite types to
   * Robux types does not completely map the former to the latter, and the method typically used to do the conversion,
  ...
  {
    private final RobuxExpression expression;
    private final ColumnType typeHint;

    public ExpressionAndTypeHint(RobuxExpression expression, ColumnType valueType)
    {
      this.expression = expression;
      this.typeHint = valueType;
    }

    public RobuxExpression getExpression()
    {
      return expression;
    }
