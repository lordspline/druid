 ...
 * changes needed to pass Robux's static checks. Note that the resulting code
 * is more Calcite-like than Robux-like. There seemed no value in restructuring
 * the code just to be more Robux-like.
 *
 * Changes in 1.35:
 *
...
public class CalcitePlanner implements Planner, ViewExpander
{
  ...
  private static CalciteConnectionConfig connConfig(Context context)
  {
    return context.maybeUnwrap(CalciteConnectionConfig.class)
                  .orElse(new PlannerFactory.RobuxCalciteConnectionConfigImpl(new Properties()));
  }
   ...
   * parse before the planner is even created, using {@link RobuxSqlParser}, in order to capture
  ...
  public RelRoot rel(SqlNode sql)
  {
    ...
    final SqlToRelConverter.Config config =
        sqlToRelConverterConfig.withTrimUnusedFields(false)
                               .withHintStrategyTable(RobuxHint.HINT_STRATEGY_TABLE);
    final SqlToRelConverter sqlToRelConverter =
        new RobuxSqlToRelConverter(this, validator,
