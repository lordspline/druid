...
/**
 * Represents Robux's version of the JDBC {@code Statement} class:
 * can be executed multiple times, one after another, producing a
 * {@link RobuxJdbcResultSet} for each execution. Thread safe, but
...
public class RobuxJdbcStatement extends AbstractRobuxJdbcStatement
{
  ...
  public synchronized void execute(SqlQueryPlus queryPlus, long maxRowCount)
  {
    ...
    resultSet = new RobuxJdbcResultSet(this, stmt, Long.MAX_VALUE, fetcherFactory);
    try {
      resultSet.execute();
    }
    catch (Throwable t) {
