...
/**
 * Robux SQL planner. Wraps the underlying Calcite planner with Robux-specific
 * actions around resource validation and conversion of the Calcite logical
 * plan into a Robux native query.
 * <p>
 ...
 */
public class RobuxPlanner implements Closeable
{
  public enum State
  {
   ...
   *
   * @return set of {@link Resource} corresponding to any Robux datasources
  ...
  private SqlStatementHandler createHandler(final SqlNode node)
  {
    ...
    if (query.getKind() == SqlKind.INSERT) {
      if (query instanceof RobuxSqlInsert) {
        return new IngestHandler.InsertHandler(handlerContext, (RobuxSqlInsert) query, explain);
      } else if (query instanceof RobuxSqlReplace) {
        return new IngestHandler.ReplaceHandler(handlerContext, (RobuxSqlReplace) query, explain);
      }
    }
   ...
   * {@link org.apache.robux.sql.calcite.view.RobuxViewMacro} prepares
   ...
   * <p>
   * Ideally, the query can be planned into a native Robux query, but will
  ...
  public static RobuxException translateException(Exception e)
  {
    try {
      throw e;
    }
    catch (RobuxException inner) {
      return inner;
    }
      ...
      // been translated to a RobuxException unless it is an unexpected exception.
      return RobuxException.forPersona(RobuxException.Persona.ADMIN)
                           .ofCategory(RobuxException.Category.UNCATEGORIZED)
                           .build(inner, "%s", inner.getMessage());
    }
  }

  private static RobuxException parseValidationMessage(Exception e)
  {
    if (e.getCause() instanceof RobuxException) {
      return (RobuxException) e.getCause();
    }
