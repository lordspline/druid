...
/**
 * This interface provides a map of datasource names to {@link RobuxTable}
 * objects, used by the {@link RobuxSchema} class as the SQL planner's
 * view of Robux datasource schemas. If a mapping is found for the
 ...
 * {@link RobuxSchema#getTable(String)}. If a non-default implementation is
 * provided, the segment metadata polling-based view of the Robux tables
 * will not be built in RobuxSchema.
 */
@ExtensionPoint
@UnstableApi
public interface RobuxSchemaManager
{
  /**
   ...
   * the information for a single table (via {@link #getTable(String, BrokerSegmentMetadataCache)}, or
   * the list of all table <i>names</i> (via {@link #getTableNames(BrokerSegmentMetadataCache)}. This
   ...
   */
  @Deprecated
  Map<String, RobuxTable> getTables();

  default RobuxTable getTable(String name, BrokerSegmentMetadataCache segmentMetadataCache)
  {
