...
import org.apache.robux.query.expression.NestedDataExpressions;
import org.apache.robux.segment.column.ColumnType;
import org.apache.robux.segment.column.RowSignature;
import org.apache.robux.segment.nested.NestedPathFinder;
import org.apache.robux.segment.nested.NestedPathPart;
...
import org.apache.robux.sql.calcite.planner.convertlet.RobuxConvertletFactory;
...
{
  public static final RobuxJsonValueConvertletFactory ROBUX_JSON_VALUE_CONVERTLET_FACTORY_INSTANCE =
      new RobuxJsonValueConvertletFactory();
  ...
  public static class JsonPathsOperatorConversion implements SqlOperatorConversion
  {
    ...
    )
    {
      return OperatorConversions.convertCall(
          ...
          rowSignature,
          rexNode,
          robuxExpressions -> RobuxExpression.ofExpression(
              null,
              RobuxExpression.functionCall(FUNCTION_NAME),
              robuxExpressions
          )
      );
    }
  ...
  public static class JsonQueryOperatorConversion implements SqlOperatorConversion
  {
    ...
    )
    {
      ...
      final List<RobuxExpression> robuxExpressions = Expressions.toRobuxExpressions(
          plannerContext,
          rowSignature,
          call.getOperands().subList(0, 2)
      );

      if (robuxExpressions == null || robuxExpressions.size() != 2) {
        return null;
      }

      final Expr pathExpr = plannerContext.parseExpression(robuxExpressions.get(1).getExpression());
      if (!pathExpr.isLiteral()) {
        // if path argument is not constant, just use a pure expression
        return RobuxExpression.ofFunctionCall(ColumnType.NESTED_DATA, FUNCTION_NAME, robuxExpressions);
      }
      ...
      final RobuxExpression.ExpressionGenerator builder = (args) ->
          "json_query(" + args.get(0).getExpression() + ",'" + jsonPath + "')";
      if (robuxExpressions.get(0).isSimpleExtraction()) {

        return RobuxExpression.ofVirtualColumn(
            ColumnType.NESTED_DATA,
            builder,
            ImmutableList.of(
                RobuxExpression.ofColumn(ColumnType.NESTED_DATA, robuxExpressions.get(0).getDirectColumn())
            ),
            (name, outputType, expression, macroTable) -> new NestedFieldVirtualColumn(
                robuxExpressions.get(0).getDirectColumn(),
                name,
                outputType,
                parts,
                true,
      ...
      return RobuxExpression.ofExpression(ColumnType.NESTED_DATA, builder, robuxExpressions);
    }
  }
   ...
   * exactly matching.
   */
  public static class RobuxJsonValueConvertletFactory implements RobuxConvertletFactory
  {
    @Override
  ...
  public abstract static class JsonValueReturningTypeOperatorConversion implements SqlOperatorConversion
  {
    ...
    public JsonValueReturningTypeOperatorConversion(SqlFunction function, ColumnType robuxType)
    {
    ...
    )
    {
      ...
      if (!pathExpr.isLiteral()) {
        ...
        return RobuxExpression.ofFunctionCall(
            robuxType,
            "json_value",
            ImmutableList.<RobuxExpression>builder()
                         .addAll(robuxExpressions)
                         .add(RobuxExpression.ofStringLiteral(robuxType.asTypeString()))
                         .build()
        );
      }
      // pre-normalize path so that the same expressions with different json path syntax are collapsed
      ...
      final String jsonPath = NestedPathFinder.toNormalizedJsonPath(parts);
      final RobuxExpression.ExpressionGenerator builder = (args) ->
          "json_value(" + args.get(0).getExpression() + ",'" + jsonPath + "', '" + robuxType.asTypeString() + "')";
      ...
      return RobuxExpression.ofExpression(robuxType, builder, robuxExpressions);
    }
  ...
  public abstract static class JsonValueReturningArrayTypeOperatorConversion implements SqlOperatorConversion
  {
    ...
    public JsonValueReturningArrayTypeOperatorConversion(SqlFunction function, ColumnType robuxType)
    {
      this.robuxType = robuxType;
      this.function = function;
    }
  ...
  public static class JsonValueAnyOperatorConversion implements SqlOperatorConversion
  {
    ...
    )
    {
      ...
      return RobuxExpression.ofExpression(columnType, builder, robuxExpressions);
    }
  }
  ...
  public static class JsonObjectOperatorConversion implements SqlOperatorConversion
  {
    ...
    @Nullable
    @Override
    public RobuxExpression toRobuxExpression(PlannerContext plannerContext, RowSignature rowSignature, RexNode rexNode)
    {
      final RobuxExpression.RobuxExpressionCreator expressionFunction = robuxExpressions ->
      ...
      if (robuxExpressions == null) {
        return null;
      }

      return expressionFunction.create(robuxExpressions);
    }
  }
  ...
  public static class JsonMergeOperatorConversion implements SqlOperatorConversion
  {
    ...
    )
    {
      return OperatorConversions.convertCall(
          ...
          robuxExpressions -> RobuxExpression.ofExpression(
              ColumnType.NESTED_DATA,
              RobuxExpression.functionCall("json_merge"),
              robuxExpressions
          )
      );
    }
