...
import org.apache.robux.sql.calcite.planner.RobuxConformance;
...
/**
 * Contains the utility function {@link #parse(String, boolean)}, for parsing Robux SQL statements.
 */
public class RobuxSqlParser
{
  public static final SqlParser.Config PARSER_CONFIG = SqlParser
      ...
      .withConformance(RobuxConformance.instance())
      .withParserFactory(new RobuxSqlParserImplFactory());

  private static final Joiner SPACE_JOINER = Joiner.on(" ");
  private static final Joiner COMMA_JOINER = Joiner.on(", ");

  private RobuxSqlParser()
  {
    // No instantiation.
  }
  ...
  )
  {
    if (root instanceof SqlNodeList) {
      ...
      boolean isMissingRobuxStatementNode = true;
      ...
      for (int i = 0; i < nodeList.size(); i++) {
        ...
        if (sqlNode instanceof SqlSetOption) {
          ...
          // last SqlNode
          root = sqlNode;
          isMissingRobuxStatementNode = false;
        }
      }
      if (isMissingRobuxStatementNode) {
  ...
  /**
   * Constructs a user-friendly {@link RobuxException} from a Calcite {@link SqlParseException}.
   */
  private static RobuxException translateParseException(SqlParseException e)
  {
    final Throwable cause = e.getCause();
    if (cause instanceof RobuxException) {
      return (RobuxException) cause;
    }
    ...
    if (cause instanceof ParseException) {
      ...
      // wrapped in a RobuxException with invalid input
