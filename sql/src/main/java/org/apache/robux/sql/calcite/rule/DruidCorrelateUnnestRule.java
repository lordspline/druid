...
import org.apache.robux.sql.calcite.rel.RobuxCorrelateUnnestRel;
 ...
 *   6:LogicalTableScan(subset=[rel#74:Subset#0.NONE.[]], table=[[robux, numfoo]])
 ...
 * </pre>
 * <p>
 * {@link RobuxUnnestRule} takes care of the Uncollect(last 3 lines) to generate a {@link RobuxUnnestRel}
 * thereby reducing the logical plan to:
 * <pre>
 *        LogicalCorrelate
 *           /       \
 *      RobuxRel    RobuxUnnestDataSourceRel
 * </pre>
 * This forms the premise of this rule. The goal is to transform the above-mentioned structure in the tree
 * with a new rel {@link RobuxCorrelateUnnestRel} which shall be created here.
 */
public class RobuxCorrelateUnnestRule extends RelOptRule
{
  private final PlannerContext plannerContext;

  public RobuxCorrelateUnnestRule(final PlannerContext plannerContext)
  {
    super(
        operand(
            Correlate.class,
            operand(RobuxRel.class, any()),
            operand(RobuxUnnestRel.class, any())
        )
    );
  ...
  public boolean matches(RelOptRuleCall call)
  {
    ...
    return left.getPartialRobuxQuery() != null;
  }
  ...
  public void onMatch(RelOptRuleCall call)
  {
    ...
    final RobuxRel<?> left = call.rel(1);
    final RobuxUnnestRel right = call.rel(2);
    ...
    if (left.getPartialRobuxQuery().stage() == PartialRobuxQuery.Stage.SELECT_PROJECT
      ...
      newLeft = left.withPartialQuery(PartialRobuxQuery.create(leftScan).withWhereFilter(leftFilter));
    ...
    // Build the new Correlate rel and a RobuxCorrelateUnnestRel wrapper.
    final RobuxCorrelateUnnestRel robuxCorrelateUnnest = RobuxCorrelateUnnestRel.create(
        correlate.copy(
    ...
    final RelBuilder relBuilder =
        call.builder()
            .push(robuxCorrelateUnnest)
            .project(
                RexUtil.fixUp(
                    rexBuilder,
                    pulledUpProjects,
                    RelOptUtil.getFieldTypeList(robuxCorrelateUnnest.getRowType())
                )
            );
