...
import org.apache.robux.catalog.model.table.ExternalTableSpec;
import org.apache.robux.catalog.model.table.TableFunction;
...
import org.apache.robux.sql.calcite.table.ExternalTable;
...
public class Externals
{
  ...
  {
    final RelDataTypeFactory typeFactory = RobuxTypeSystem.TYPE_FACTORY;
  ...
  public static List<RelDataType> dataTypes(List<FunctionParameter> parameters)
  {
    return parameters
        .stream()
        .map(parameter -> parameter.getType(RobuxTypeSystem.TYPE_FACTORY))
   ...
   * Robux-specific EXTEND syntax added to the parser.
   */
  ...
  /**
   * Define the Robux input schema from a name provided in the EXTEND
   * clause. Calcite allows any form of name: a.b.c, say. But, Robux
   ...
   * EXTEND clause. Calcite allows any form of type. But, Robux
   * requires only the Robux supported types (and their aliases.)
   * <p>
   * Robux has its own rules for nullability. We ignore any nullability
