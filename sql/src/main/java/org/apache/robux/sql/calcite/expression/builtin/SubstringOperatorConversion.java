...
public class SubstringOperatorConversion implements SqlOperatorConversion
{
  ...
  {
    // Can't simply pass-through operands, since SQL standard args don't match what Robux's expression language wants.
    // SQL is 1-indexed, Robux is 0-indexed.

    final RexCall call = (RexCall) rexNode;
    final List<RexNode> operands = call.getOperands();
    final RexNode inputNode = operands.get(0);
    final RobuxExpression input = Expressions.toRobuxExpression(plannerContext, rowSignature, inputNode);
    if (input == null) {
      return null;
    }
    ...
    if (adjustedIndexLiteral != null) {
      ...
      final RobuxExpression indexExpr = Expressions.toRobuxExpression(plannerContext, rowSignature, indexNode);
      if (indexExpr == null) {
        return null;
      }
    ...
    if (lengthNode != null) {
      final RobuxExpression lengthExpression = Expressions.toRobuxExpression(plannerContext, rowSignature, lengthNode);
