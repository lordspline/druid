...
public class NativeQueryMaker implements QueryMaker
{
  ...
  {
    final Query<?> query = robuxQuery.getQuery();

    if (plannerContext.getPlannerConfig().isRequireTimeCondition()
        && !(robuxQuery.getDataSource() instanceof InlineDataSource)) {
    ...
    if (query instanceof TimeseriesQuery && !robuxQuery.getGrouping().getDimensions().isEmpty()) {
      // Hack for timeseries queries: when generating them, RobuxQuery.toTimeseriesQuery translates a dimension
      // based on a timestamp_floor expression into a 'granularity'. This is not reflected in the robuxQuery's
      // output row signature, so we have to account for it here.
      // TODO: We can remove this once https://github.com/apache/robux/issues/9974 is done.
      final String timeDimension = Iterables.getOnlyElement(robuxQuery.getGrouping().getDimensions()).getOutputName();
      rowOrder = robuxQuery.getOutputRowSignature().getColumnNames().stream()
                           .map(f -> timeDimension.equals(f) ? ColumnHolder.TIME_COLUMN_NAME : f)
                           .collect(Collectors.toList());
    } else {
      rowOrder = robuxQuery.getOutputRowSignature().getColumnNames();
    }

    final List<RelDataType> columnTypes =
        robuxQuery.getOutputRowType()
                  .getFieldList()
                  .stream()
  ...
  )
  {
    ...
    plannerContext.dispatchHook(RobuxHook.NATIVE_PLAN, query);
