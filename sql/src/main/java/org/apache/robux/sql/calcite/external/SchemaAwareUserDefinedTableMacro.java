...
/**
 * Table macro designed for use with the Robux EXTEND operator. Example:
 * <code><pre>
 * INSERT INTO dst
 * SELECT *
 * FROM TABLE(staged(
 ...
 * </pre></code>
 * <p>
 * For Robux, we want the above form: extend a table function, not a
 ...
 * <li>When asked to produce a table ({@code apply(.)}), we call a Robux-specific
 * version that passes along the schema saved previously.</li>
 * <li>The extended {@code RobuxTableMacro} uses the schema to define the
 * input source.</li>
 * <li>Care is taken that the same {@code RobuxTableMacro} can be used without
    ...
    extends BaseUserDefinedTableMacro implements AuthorizableOperator
{
   ...
   * actual work to build the Robux table. This macro also caches the
