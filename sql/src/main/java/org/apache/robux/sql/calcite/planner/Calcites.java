...
public class Calcites
{
   ...
   * {@code robux.expressions.allowArrayToStringCast}.
   *
  ...
  /**
   * Like RelDataTypeFactory.createSqlType, but creates types that align best with how Robux represents them.
   */
  ...
  /**
   * Like RelDataTypeFactory.createSqlTypeWithNullability, but creates types that align best with how Robux
   * represents them.
   */
  ...
  )
  {
    ...
    switch (typeName) {
      ...
      case TIMESTAMP_WITH_LOCAL_TIME_ZONE:
        ...
        dataType = typeFactory.createSqlType(typeName, RobuxTypeSystem.DEFAULT_TIMESTAMP_PRECISION);
        break;
      case CHAR:
      case VARCHAR:
  ...
  public static DateTime calciteDateTimeLiteralToJoda(final RexNode literal, final DateTimeZone timeZone)
  {
    ...
    if (literal.getKind() != SqlKind.LITERAL) {
      throw RobuxException.defensive("Expected literal but got[%s]", literal.getKind());
    }
    ...
    if (typeName == SqlTypeName.TIMESTAMP) {
      ...
      throw RobuxException.defensive("Expected TIMESTAMP or DATE but got[%s]", typeName);
    }
  }
