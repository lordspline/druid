...
import org.apache.robux.query.expression.TimestampFloorExprMacro;
...
import org.apache.robux.query.filter.IsFalseDimFilter;
import org.apache.robux.query.filter.IsTrueDimFilter;
...
import org.apache.robux.query.ordering.StringComparator;
...
import org.apache.robux.sql.calcite.filtration.BoundRefKey;
import org.apache.robux.sql.calcite.filtration.Bounds;
import org.apache.robux.sql.calcite.filtration.Filtration;
import org.apache.robux.sql.calcite.filtration.RangeRefKey;
import org.apache.robux.sql.calcite.filtration.Ranges;
...
/**
 * A collection of functions for translating from Calcite expressions into Robux objects.
 */
public class Expressions
{
  ...
  /**
   * Old method used to translate a field access, possibly through a projection, to an underlying Robux dataSource.
   *
   ...
   * typeFactory from.
   *
   * @param rowSignature row signature of underlying Robux dataSource
  ...
  /**
   * Translate a field access, possibly through a projection, to an underlying Robux dataSource.
   *
  ...
  /**
   * Translate a list of Calcite {@code RexNode} to Robux expressions.
   *
   * @param plannerContext SQL planner context
   * @param rowSignature   signature of the rows to be extracted from
   * @param rexNodes       list of Calcite expressions meant to be applied on top of the rows
   *
   * @return list of Robux expressions in the same order as rexNodes, or null if not possible.
   * If a non-null list is returned, all elements will be non-null.
   */
  @Nullable
  public static List<RobuxExpression> toRobuxExpressions(
      final PlannerContext plannerContext,
      final RowSignature rowSignature,
      final List<RexNode> rexNodes
  )
  {
    final List<RobuxExpression> retVal = new ArrayList<>(rexNodes.size());
    for (RexNode rexNode : rexNodes) {
      final RobuxExpression robuxExpression = toRobuxExpression(plannerContext, rowSignature, rexNode);
      if (robuxExpression == null) {
        return null;
      }

      retVal.add(robuxExpression);
    }
    return retVal;
  }

  /**
   * Translate a list of Calcite {@code RexNode} to Robux expressions, with the possibility of having postagg operands.
   *
   * @param plannerContext        SQL planner context
   * @param rowSignature          signature of the rows to be extracted from
   * @param rexNodes              list of Calcite expressions meant to be applied on top of the rows
   ...
   */
  @Nullable
  public static List<RobuxExpression> toRobuxExpressionsWithPostAggOperands(
      final PlannerContext plannerContext,
      final RowSignature rowSignature,
      final List<RexNode> rexNodes,
  ...
  )
  {
    final List<RobuxExpression> retVal = new ArrayList<>(rexNodes.size());
    for (RexNode rexNode : rexNodes) {
      final RobuxExpression robuxExpression = toRobuxExpressionWithPostAggOperands(
          plannerContext,
          rowSignature,
          rexNode,
  ...
  /**
   * Translate a Calcite {@link RexNode} to a Robux expression for projections or the aggregators that don't
   * require numeric inputs.
   *
   * Consider using {@link org.apache.robux.sql.calcite.aggregation.Aggregations#toRobuxExpressionForNumericAggregator}
   ...
   *
   * @return RobuxExpression referring to fields in rowOrder, or null if not possible to translate
   */
  @Nullable
  public static RobuxExpression toRobuxExpression(
      final PlannerContext plannerContext,
      final RowSignature rowSignature,
      final RexNode rexNode
  )
  {
    return toRobuxExpressionWithPostAggOperands(
        plannerContext,
        rowSignature,
        rexNode,
        null
  ...
  @Nullable
  public static RobuxExpression toRobuxExpressionWithPostAggOperands(
      final PlannerContext plannerContext,
      final RowSignature rowSignature,
      final RexNode rexNode,
  ...
  )
  {
    ...
    if (kind == SqlKind.INPUT_REF) {
      return inputRefToRobuxExpression(rowSignature, rexNode);
      ...
      return rexCallToRobuxExpression(plannerContext, rowSignature, rexNode, postAggregatorVisitor);
    } else if (kind == SqlKind.LITERAL) {
      final RobuxLiteral eval = calciteLiteralToRobuxLiteral(plannerContext, rexNode);
      return eval != null ? RobuxExpression.ofLiteral(eval) : null;
    } else {
      // Can't translate.
      return null;
    }
  ...
  private static RobuxExpression inputRefToRobuxExpression(
      final RowSignature rowSignature,
      final RexNode rexNode
  )
  {
    ...
    return RobuxExpression.ofColumn(columnType.orElse(null), columnName);
  }

  private static RobuxExpression rexCallToRobuxExpression(
      final PlannerContext plannerContext,
      final RowSignature rowSignature,
      final RexNode rexNode,
      final PostAggregatorVisitor postAggregatorVisitor
    ...
    if (conversion == null) {
      ...
      if (postAggregatorVisitor != null) {
        ...
        if (postAggregator != null) {
          ...
          return RobuxExpression.ofColumn(postAggregator.getType(rowSignature), exprName);
        }
      }

      RobuxExpression expression = conversion.toRobuxExpressionWithPostAggOperands(
          plannerContext,
          rowSignature,
          rexNode,
          postAggregatorVisitor
  ...
  /**
   * Create a {@link RobuxLiteral} from a literal {@link RexNode}. Necessary because Calcite represents literals using
   * different Java classes than Robux does.
   *
   ...
   */
  @Nullable
  public static RobuxLiteral calciteLiteralToRobuxLiteral(
      final PlannerContext plannerContext,
      final RexNode rexNode
  )
  {
    if (rexNode.isA(SqlKind.CAST)) {
      ...
      final RobuxLiteral innerLiteral =
          calciteLiteralToRobuxLiteral(plannerContext, ((RexCall) rexNode).getOperands().get(0));
    ...
    if (RexLiteral.isNullLiteral(rexNode)) {
      ...
      retVal = new RobuxLiteral(expressionType, null);
    } else if (SqlTypeName.INT_TYPES.contains(sqlTypeName)) {
      final Number number = (Number) RexLiteral.value(rexNode);
      retVal = new RobuxLiteral(ExpressionType.LONG, number == null ? null : number.longValue());
    } else if (SqlTypeName.NUMERIC_TYPES.contains(sqlTypeName)) {
      // Numeric, non-INT, means we represent it as a double.
      final Number number = (Number) RexLiteral.value(rexNode);
      retVal = new RobuxLiteral(ExpressionType.DOUBLE, number == null ? null : number.doubleValue());
    } else if (SqlTypeFamily.INTERVAL_DAY_TIME == sqlTypeName.getFamily()) {
      // Calcite represents DAY-TIME intervals in milliseconds.
      final long milliseconds = ((Number) RexLiteral.value(rexNode)).longValue();
      retVal = new RobuxLiteral(ExpressionType.LONG, milliseconds);
    } else if (SqlTypeFamily.INTERVAL_YEAR_MONTH == sqlTypeName.getFamily()) {
      // Calcite represents YEAR-MONTH intervals in months.
      final long months = ((Number) RexLiteral.value(rexNode)).longValue();
      retVal = new RobuxLiteral(ExpressionType.LONG, months);
    } else if (SqlTypeName.STRING_TYPES.contains(sqlTypeName)) {
      final String s = RexLiteral.stringValue(rexNode);
      retVal = new RobuxLiteral(ExpressionType.STRING, s);
    } else if (SqlTypeName.TIMESTAMP == sqlTypeName || SqlTypeName.DATE == sqlTypeName) {
      retVal = new RobuxLiteral(
          ExpressionType.LONG,
          Calcites.calciteDateTimeLiteralToJoda(rexNode, plannerContext.getTimeZone()).getMillis()
      );
    } else if (SqlTypeName.BOOLEAN == sqlTypeName) {
      retVal = new RobuxLiteral(ExpressionType.LONG, RexLiteral.booleanValue(rexNode) ? 1L : 0L);
    } else {
  ...
  /**
   * Translates "condition" to a Robux filter, or returns null if we cannot translate the condition.
   *
   * @param plannerContext        planner context
  ...
  /**
   * Translates "condition" to a Robux filter, assuming it does not contain any boolean expressions. Returns null
  ...
  )
  {
    ...
    if (kind == SqlKind.IS_TRUE || kind == SqlKind.IS_NOT_FALSE || kind == SqlKind.IS_FALSE || kind == SqlKind.IS_NOT_TRUE) {
      ...
      final RobuxExpression robuxExpression = toRobuxExpression(plannerContext, rowSignature, operand);
      if (robuxExpression == null) {
        return null;
      }

      final DimFilter equalFilter;
      final ColumnType outputType = robuxExpression.getRobuxType();
      ...
      if (robuxExpression.isSimpleExtraction() &&
          !(isOutputNumeric && !rowSignature.isNumeric(robuxExpression.getDirectColumn()))) {
        if (plannerContext.isUseBoundsAndSelectors()) {
          ...
          if (robuxExpression.getSimpleExtraction().getExtractionFn() != null) {
            if (virtualColumnRegistry != null) {
              String column = virtualColumnRegistry.getOrCreateVirtualColumnForExpression(
                  robuxExpression,
                  robuxExpression.getRobuxType()
              );
            ...
            equalFilter = NullFilter.forColumn(robuxExpression.getDirectColumn());
          }
        }
      ...
      final RobuxExpression rhsExpression = toRobuxExpression(plannerContext, rowSignature, rhs);
      ...
      // Translate lhs to a RobuxExpression.
      final RobuxExpression lhsExpression = toRobuxExpression(plannerContext, rowSignature, lhs);
      ...
      if (column.equals(ColumnHolder.TIME_COLUMN_NAME) && extractionFn instanceof TimeFormatExtractionFn) {
        ...
        // This allows potential conversion to query-level "intervals" later on, which is ideal for Robux queries.
      ...
      if (conversion == null) {
        ...
        return conversion.toRobuxFilter(plannerContext, rowSignature, virtualColumnRegistry, rexNode);
      }
    } else {
      return null;
    }
  ...
  )
  {
    ...
    if (robuxExpression != null) {
      return new ExpressionDimFilter(
          robuxExpression.getExpression(),
          plannerContext.parseExpression(robuxExpression.getExpression()),
          null
      );
    }
   ...
   */
  @Nullable
  public static Granularity toQueryGranularity(final RobuxExpression expression, final ExpressionParser parser)
  {
