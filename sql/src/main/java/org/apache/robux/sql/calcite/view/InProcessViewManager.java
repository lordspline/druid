...
{
  private final ConcurrentMap<String, RobuxViewMacro> views;
  private final RobuxViewMacroFactory robuxViewMacroFactory;

  @Inject
  @VisibleForTesting
  public InProcessViewManager(final RobuxViewMacroFactory robuxViewMacroFactory)
  {
    this.views = new ConcurrentHashMap<>();
    this.robuxViewMacroFactory = robuxViewMacroFactory;
  }

  @Override
  public void createView(final PlannerFactory plannerFactory, final String viewName, final String viewSql)
  {
    final TableMacro oldValue = views.putIfAbsent(viewName, robuxViewMacroFactory.create(plannerFactory, viewSql));
    if (oldValue != null) {
  ...
  {
    final TableMacro oldValue = views.replace(viewName, robuxViewMacroFactory.create(plannerFactory, viewSql));
    if (oldValue != null) {
