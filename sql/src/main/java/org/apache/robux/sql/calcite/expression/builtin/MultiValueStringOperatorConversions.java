...
public class MultiValueStringOperatorConversions
{
  ...
  private abstract static class ListFilter implements SqlOperatorConversion
  {
    ...
    )
    {
      ...
      Expr expr = plannerContext.parseExpression(robuxExpressions.get(1).getExpression());
      if (robuxExpressions.get(0).isSimpleExtraction() && expr.isLiteral()) {
        ...
        RobuxExpression robuxExpression = RobuxExpression.ofVirtualColumn(
            Calcites.getColumnTypeForRelDataType(rexNode.getType()),
            builder,
            robuxExpressions,
            (name, outputType, expression, macroTable) -> new ListFilteredVirtualColumn(
                name,
                robuxExpressions.get(0)
                                .getSimpleExtraction()
                                .toDimensionSpec(robuxExpressions.get(0).getDirectColumn(), outputType),
                literals,
                isAllowList()
            )
        );
        ...
        if (plannerContext.getJoinExpressionVirtualColumnRegistry() != null) {
          ...
          return RobuxExpression.ofColumn(ColumnType.STRING, virtualColumnName);
        }

        return robuxExpression;
      }

      return RobuxExpression.ofExpression(ColumnType.STRING, builder, robuxExpressions);
    }
  }
