...
public class InformationSchema extends AbstractSchema
{
  ...
  private static final String CATALOG_NAME = "robux";
  ...
  private final RobuxSchemaCatalog rootSchema;
  private final Map<String, Table> tableMap;
  private final AuthorizerMapper authorizerMapper;

  @Inject
  public InformationSchema(
      @Named(RobuxCalciteSchemaModule.INCOMPLETE_SCHEMA) final RobuxSchemaCatalog rootSchema,
                      ...
                      Iterables.concat(
                          FluentIterable.from(authorizedTableNames).transform(
                              tableName -> {
                                ...
                                if (table instanceof RobuxTable) {
                                  RobuxTable robuxTable = (RobuxTable) table;
                                  isJoinable = robuxTable.isJoinable();
                                  isBroadcast = robuxTable.isBroadcast();
                                } else {
                                  isJoinable = false;
                                  isBroadcast = false;
                                }
              ...
              new Function<>()
              {
                ...
                public Object[] apply(final RelDataTypeField field)
                {
                  ...
                  return new Object[]{
                      ...
                      (long) type.getSqlTypeName().getJdbcOrdinal() // JDBC_TYPE (Robux extension)
                  };
                }
              }
          );
  ...
  static class RoutinesTable extends AbstractTable implements ScannableTable
  {
    ...
    private final RobuxOperatorTable operatorTable;

    public RoutinesTable(
        RobuxOperatorTable operatorTable
    )
    {
    ...
    public Enumerable<Object[]> scan(DataContext root)
    {
      ...
      for (SqlOperator sqlOperator : operatorList) {
        ...
        if (!RobuxOperatorTable.isFunctionSyntax(sqlOperator.getSyntax())) {
          continue;
        }
