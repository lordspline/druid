 ...
 * translated as those are currently unsupported in Robux.
 */
public class LogicalUnnestRule extends RelOptRule implements SubstitutionRule
{
  ...
  public void onMatch(RelOptRuleCall call)
  {
    ...
    if (unnestConfig == null) {
      throw RobuxException.defensive("Couldn't process possible unnest for reltree: \n%s", RelOptUtil.toString(cor));
    }

    unnestConfig = unnestConfig.withExpr(
        new RobuxCorrelateUnnestRel.CorrelatedFieldAccessToInputRef(cor.getCorrelationId())
