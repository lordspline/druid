 ...
 * - caching disabled because Robux's caching is better.
 *
 * All the provided schema are added to the rootSchema.
 */
public class RootSchemaProvider implements Provider<RobuxSchemaCatalog>
{
  ...
  @Override
  public RobuxSchemaCatalog get()
  {
    final SchemaPlus rootSchema = CalciteSchema.createRootSchema(false, false).plus();
    for (NamedSchema schema : namedSchemas) {
      rootSchema.add(schema.getSchemaName(), schema.getSchema());
    }
    return new RobuxSchemaCatalog(rootSchema, ImmutableMap.copyOf(schemasByName));
  }
}
