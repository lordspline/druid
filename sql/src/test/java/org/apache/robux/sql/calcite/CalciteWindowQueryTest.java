...
import org.apache.robux.sql.calcite.CalciteWindowQueryTest.WindowQueryTestInputClass.TestType;
import org.apache.robux.sql.calcite.QueryTestRunner.QueryResults;
import org.apache.robux.sql.calcite.QueryVerification.QueryResultsVerifier;
...
public class CalciteWindowQueryTest extends BaseCalciteQueryTest
{
  ...
  public static final boolean DUMP_ACTUAL_RESULTS = Boolean.parseBoolean(
      System.getProperty("robux.tests.sql.dumpActualResults")
  ...
  public void testFailure_partitionByMVD()
  {
    ...
    final RobuxException e1 = Assert.assertThrows(
        RobuxException.class,
        () -> testBuilder()
            .sql("select cityName, countryName, array_to_mv(array[1,length(cityName)]),\n"
                 + "row_number() over (partition by  array_to_mv(array[1,length(cityName)]) order by countryName, cityName)\n"
