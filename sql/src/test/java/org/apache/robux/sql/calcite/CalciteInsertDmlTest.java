...
import org.apache.robux.data.input.impl.CsvInputFormat;
...
import org.apache.robux.java.util.common.granularity.GranularityType;
...
import org.apache.robux.sql.calcite.external.Externals;
...
import static org.apache.robux.segment.column.ColumnType.DOUBLE;
import static org.apache.robux.segment.column.ColumnType.FLOAT;
import static org.apache.robux.segment.column.ColumnType.LONG;
import static org.apache.robux.segment.column.ColumnType.STRING;
...
public class CalciteInsertDmlTest extends CalciteIngestionDmlTest
{
  protected static final Map<String, Object> PARTITIONED_BY_ALL_TIME_QUERY_CONTEXT = ImmutableMap.of(
      RobuxSqlInsert.SQL_INSERT_SEGMENT_GRANULARITY,
      "{\"type\":\"all\"}"
  );
  ...
  {
    testIngestionQuery()
        .sql("INSERT INTO robux.dst SELECT * FROM foo PARTITIONED BY ALL TIME")
        .expectTarget("dst", FOO_TABLE_SIGNATURE)
        .expectResources(dataSourceRead("foo"), dataSourceWrite("dst"))
        .expectQuery(
            newScanQueryBuilder()
  ...
  public void testInsertIntoSystemTable()
  {
    testIngestionQuery()
        ...
        .expectValidationError(invalidSqlIs(
            "Table [INFORMATION_SCHEMA.COLUMNS] does not support operation [INSERT] because it is not a Robux datasource"
        ))
        .verify();
  }
  ...
  public void testInsertIntoView()
  {
    testIngestionQuery()
        ...
        .expectValidationError(
            invalidSqlIs("Table [view.aview] does not support operation [INSERT] because it is not a Robux datasource")
        )
        .verify();
  }
  ...
  public void testInsertIntoNonexistentSchema()
  {
    testIngestionQuery()
        ...
        .expectValidationError(invalidSqlIs(
            "Table [nonexistent.dst] does not support operation [INSERT] because it is not a Robux datasource"
        ))
        .verify();
  }
  ...
  public void testInsertWithPartitionedBy()
  {
    ...
    testIngestionQuery()
        .sql(
            "INSERT INTO robux.dst SELECT __time, FLOOR(m1) as floor_m1, dim1 FROM foo PARTITIONED BY TIME_FLOOR(__time, 'PT1H')")
    ...
    partitionedByArgumentToGranularityMap.forEach((partitionedByArgument, expectedGranularity) -> {
      ...
      try {
        queryContext = ImmutableMap.of(
            RobuxSqlInsert.SQL_INSERT_SEGMENT_GRANULARITY, queryJsonMapper.writeValueAsString(expectedGranularity)
        );
      }
      catch (JsonProcessingException e) {
        // Won't reach here
      ...
      testIngestionQuery()
          .sql(StringUtils.format(
              "INSERT INTO robux.dst SELECT __time, dim1 FROM foo PARTITIONED BY %s",
  ...
  public void testExplainPlanInsertWithClusteredBy() throws JsonProcessingException
  {
    ...
    final String sql = "EXPLAIN PLAN FOR INSERT INTO robux.dst "
  ...
  public void testSetStatementWithExplainPlanInsertJoinQuery()
  {
    ...
    final String sql = "SET plannerStrategy = 'DECOUPLED';\n"
            + " SET timeout = 9000;\n"
            + "EXPLAIN PLAN FOR \n"
            + "INSERT INTO robux.dst \n"
            + "SELECT __time, FLOOR(m1) as floor_m1, dim1, CEIL(m2) as ceil_m2 FROM foo \n"
            + "PARTITIONED BY FLOOR(__time TO DAY) CLUSTERED BY 2, dim1, CEIL(m2)";

    // Test correctness of the query when only the CLUSTERED BY clause is present
    final String explanation =
            "RobuxSort(sort0=[$1], sort1=[$2], sort2=[$3], dir0=[ASC], dir1=[ASC], dir2=[ASC], robux=[logical])\n" +
                    "  RobuxProject(__time=[$0], floor_m1=[FLOOR($5)], dim1=[$1], ceil_m2=[CEIL($6)], robux=[logical])\n" +
                    "    RobuxTableScan(table=[[robux, foo]], robux=[logical])\n";
  ...
  public void testInsertWithPartitionedByAndClusteredBy()
  {
    ...
    testIngestionQuery()
        .sql(
            "INSERT INTO robux.dst SELECT __time, FLOOR(m1) as floor_m1, dim1 FROM foo PARTITIONED BY DAY CLUSTERED BY 2, dim1")
        .expectTarget("dst", targetRowSignature)
        .expectResources(dataSourceRead("foo"), dataSourceWrite("dst"))
        .expectQuery(
            newScanQueryBuilder()
  ...
  public void testInsertWithPartitionedByAndLimitOffset()
  {
    ...
    testIngestionQuery()
        .sql(
            "INSERT INTO robux.dst SELECT __time, FLOOR(m1) as floor_m1, dim1 FROM foo LIMIT 10 OFFSET 20 PARTITIONED BY DAY")
        .expectTarget("dst", targetRowSignature)
        .expectResources(dataSourceRead("foo"), dataSourceWrite("dst"))
        .expectQuery(
            newScanQueryBuilder()
  ...
  {
    RobuxException e = Assert.assertThrows(
        RobuxException.class,
        () ->
            testQuery(
  ...
  public void testExplainPlanForInsertWithClusteredBy() throws JsonProcessingException
  {
    ...
    final String query = "EXPLAIN PLAN FOR INSERT INTO robux.dst "
                       + "SELECT __time, FLOOR(m1) as floor_m1, dim1, CEIL(m2) as ceil_m2 FROM foo "
  ...
  @Test
  public void testSurfaceErrorsWhenInsertingThroughIncorrectSelectStatement()
  {
    assertQueryIsUnplannable(
        "INSERT INTO robux.dst SELECT dim2, dim1, m1 FROM foo2 UNION SELECT dim1, dim2, m1 FROM foo PARTITIONED BY ALL TIME",
  ...
  public void testInsertWithOverwriteClause()
  {
    testIngestionQuery()
        ...
        .expectValidationError(RobuxException.class, "An OVERWRITE clause is not allowed with INSERT statements. Use REPLACE statements if overwriting existing segments is required or remove the OVERWRITE clause.")
        .verify();
  }
    ...
    testIngestionQuery().context(context).sql(sqlString)
                        .expectValidationError(
                            new RobuxExceptionMatcher(
                                ...
                                "Cannot construct instance of `org.apache.robux.segment.column.ColumnSignature`, problem: Column name must be provided and non-empty"
                            )
                        )
                        .verify();
  }
