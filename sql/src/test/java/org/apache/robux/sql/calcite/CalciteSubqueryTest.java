...
import org.apache.robux.query.filter.TypedInFilter;
...
/**
 * Calcite tests which involve subqueries and materializing the intermediate results on {@link org.apache.robux.server.ClientQuerySegmentWalker}
...
public class CalciteSubqueryTest extends BaseCalciteQueryTest
{
  ...
  public void testExactCountDistinctUsingSubqueryWithWhereToOuterFilter(String testName, Map<String, Object> queryContext)
  {
    ...
    testQuery(
        ...
        + "  SUM(cnt),\n"
        + "  COUNT(*)\n"
        + "FROM (SELECT dim2, SUM(cnt) AS cnt FROM robux.foo GROUP BY dim2 LIMIT 1)\n"
        + "WHERE cnt > 0",
  ...
  public void testExactCountDistinctOfSemiJoinResult(String testName, Map<String, Object> queryContext)
  {
    ...
    testQuery(
        ...
        + "    SELECT SUBSTRING(dim1, 1, 1) FROM robux.foo WHERE dim1 <> ''\n"
  ...
  public void testTwoExactCountDistincts(String testName, Map<String, Object> queryContext)
  {
    testQuery(
        ...
        queryContext,
        "SELECT COUNT(distinct dim1), COUNT(distinct dim2) FROM robux.foo",
  ...
  public void testViewAndJoin(String testName, Map<String, Object> queryContext)
  {
    ...
    testQuery(
        "SELECT COUNT(*) FROM view.cview as a INNER JOIN robux.foo d on d.dim2 = a.dim2 WHERE a.dim1_firstchar <> 'z' ",
  ...
  {
    // Regression test for https://github.com/apache/robux/issues/4208

    testQuery(
        "SELECT dim1, dim2 FROM robux.foo\n"
        + " WHERE dim2 IN (\n"
        + "   SELECT dim2\n"
        + "   FROM robux.foo\n"
        + "   GROUP BY dim2\n"
            ...
            newScanQueryBuilder()
                .dataSource(
                    join(
                        ...
                        "j0.",
                        equalsCondition(RobuxExpression.ofColumn(ColumnType.STRING, "dim2"), RobuxExpression.ofColumn(ColumnType.STRING, "j0.d0")),
                        JoinType.INNER
                    )
                )
                .intervals(querySegmentSpec(Filtration.eternity()))
  ...
  {
    testQuery(
        "SELECT __time, cnt, dim1, dim2 FROM robux.foo "
        + " WHERE (dim1, dim2) IN ("
            ...
            newScanQueryBuilder()
                .dataSource(
                    join(
                        ...
                        StringUtils.format(
                            "(%s && %s)",
                            equalsCondition(RobuxExpression.ofColumn(ColumnType.STRING, "dim1"), RobuxExpression.ofColumn(ColumnType.STRING, "j0.d0")),
                            equalsCondition(RobuxExpression.ofColumn(ColumnType.STRING, "dim2"), RobuxExpression.ofColumn(ColumnType.STRING, "j0.p0"))
                        ),
                        JoinType.INNER
                    )
                )
  ...
  private void testMaxSubqueryRowsWithoutMemoryLimit(String testName, Map<String, Object> queryContext)
  {
    ...
    testQueryThrows(
        "SELECT\n"
            + "  SUM(cnt),\n"
            + "  COUNT(*)\n"
            + "FROM (SELECT dim2, SUM(cnt) AS cnt FROM robux.foo GROUP BY dim2 LIMIT 2) \n"
            + "WHERE cnt > 0",
  ...
  public void testJoinWithTimeDimension(String testName, Map<String, Object> queryContext)
  {
    testQuery(
        PLANNER_CONFIG_DEFAULT,
        queryContext,
        "SELECT count(*) FROM robux.foo t1 inner join robux.foo t2 on t1.__time = t2.__time",
        CalciteTests.REGULAR_USER_AUTH_RESULT,
  ...
  public void testUsingSubqueryWithLimit(String testName, Map<String, Object> queryContext)
  {
    ...
    testQuery(
        "SELECT COUNT(*) AS cnt FROM ( SELECT * FROM robux.foo LIMIT 10 ) tmpA",
        queryContext,
        ImmutableList.of(
            GroupByQuery.builder()
                        .setDataSource(
  ...
  public void testSelfJoin(String testName, Map<String, Object> queryContext)
  {
    ...
    testQuery(
        "SELECT COUNT(*) FROM robux.foo x, robux.foo y\n",
        queryContext,
        ImmutableList.of(
            Robuxs.newTimeseriesQueryBuilder()
                  .dataSource(
  ...
  public void testSingleValueStringMultipleRowsAgg(String testName, Map<String, Object> queryContext)
  {
    ...
    testQueryThrows(
        ...
        "java.util.concurrent.ExecutionException: org.apache.robux.error.RobuxException: Subquery expression returned more than one row"
    );
  }
    ...
    testQuery(
        ...
        ImmutableList.of(
            GroupByQuery.builder()
                        .setDataSource(join(
                            new TableDataSource(CalciteTests.WIKIPEDIA),
                            new QueryDataSource(Robuxs.newTimeseriesQueryBuilder()
                                                      .dataSource(CalciteTests.WIKIPEDIA)
                                                      .intervals(querySegmentSpec(Filtration.eternity()))
                                                      .granularity(Granularities.ALL)
  ...
  {
    cannotVectorizeUnlessFallback();
    DimFilter filter = new TypedInFilter("v0", ColumnType.LONG, null, ImmutableList.of(1, 17), null);
    testQuery(
