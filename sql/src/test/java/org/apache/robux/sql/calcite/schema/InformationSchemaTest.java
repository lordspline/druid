...
public class InformationSchemaTest extends BaseCalciteQueryTest
{
  ...
  {
    RobuxOperatorTable operatorTable = qf.operatorTable();
    ...
    for (Object[] row : rows) {
      ...
      Assert.assertEquals("robux", row[0]);
  ...
  public void testScanRoutinesTableWithCustomOperators()
  {
    ...
    RobuxOperatorTable mockOperatorTable = Mockito.mock(RobuxOperatorTable.class);
    ...
    Assert.assertEquals("There should be exactly 2 rows; any non-function syntax operator should get filtered out",
                        2, rows.size());
    Object[] expectedRow1 = {"robux", "INFORMATION_SCHEMA", "FOO", "FUNCTION", "NO", "'FOO([<ANY>])'"};
    Assert.assertTrue(rows.stream().anyMatch(row -> Arrays.equals(row, expectedRow1)));

    Object[] expectedRow2 = {"robux", "INFORMATION_SCHEMA", "BAR", "FUNCTION", "NO", "'BAR(<INTEGER>, <INTEGER>)'"};
