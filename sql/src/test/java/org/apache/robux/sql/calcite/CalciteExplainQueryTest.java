...
public class CalciteExplainQueryTest extends BaseCalciteQueryTest
{
  ...
  public void testExplainInformationSchemaColumns()
  {
    final String explanation =
        ...
        + "  BindableFilter(condition=[AND(=($1, 'robux'), =($2, 'foo'))])\n"
  ...
  public void testExplainExactCountDistinctOfSemiJoinResult()
  {
    ...
    final String query = "EXPLAIN PLAN FOR SELECT COUNT(*)\n"
                         ...
                         + "    SELECT SUBSTRING(dim1, 1, 1) FROM robux.foo WHERE dim1 IS NOT NULL\n"
                         + "  )\n"
                         + ")";
  ...
  public void testSetStatementWithExplainSanity()
  {
    ...
    final String explanation = "RobuxAggregate(group=[{}], EXPR$0=[COUNT()], robux=[logical])\n" +
            "  RobuxAggregate(group=[{0}], robux=[logical])\n" +
            "    RobuxJoin(condition=[=($1, $2)], joinType=[inner])\n" +
            "      RobuxProject(dim2=[$2], $f1=[SUBSTRING($2, 1, 1)], robux=[logical])\n" +
            "        RobuxTableScan(table=[[robux, foo]], robux=[logical])\n" +
            "      RobuxAggregate(group=[{0}], robux=[logical])\n" +
            "        RobuxProject(EXPR$0=[SUBSTRING($1, 1, 1)], robux=[logical])\n" +
            "          RobuxFilter(condition=[IS NOT NULL($1)])\n" +
            "            RobuxTableScan(table=[[robux, foo]], robux=[logical])\n";
    final String resources = "[{\"name\":\"foo\",\"type\":\"DATASOURCE\"}]";
    final String attributes = "{\"statementType\":\"SELECT\"}";
  ...
  public void testExplainSelectStar()
  {
    ...
    String sql = "EXPLAIN PLAN FOR SELECT * FROM robux.foo";
  ...
  public void testExplainMultipleTopLevelUnionAllQueries()
  {
    ...
    final String query = "EXPLAIN PLAN FOR SELECT dim1 FROM robux.foo\n"
                         + "UNION ALL (SELECT dim1 FROM robux.foo WHERE dim1 = '42'\n"
                         + "UNION ALL SELECT dim1 FROM robux.foo WHERE dim1 = '44')";
