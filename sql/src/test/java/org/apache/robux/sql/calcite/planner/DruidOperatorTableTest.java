...
import org.apache.robux.sql.calcite.rel.Windowing;
...
public class RobuxOperatorTableTest
{
  @Test
  public void testBuiltInOperatorTable()
  {
    RobuxOperatorTable operatorTable = new RobuxOperatorTable(ImmutableSet.of(), ImmutableSet.of());
  ...
  {
    Assert.assertTrue(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.FUNCTION));
    Assert.assertTrue(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.FUNCTION_STAR));
    Assert.assertTrue(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.FUNCTION_ID));
    Assert.assertTrue(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.SPECIAL));
    Assert.assertTrue(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.INTERNAL));

    Assert.assertFalse(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.BINARY));
    Assert.assertFalse(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.PREFIX));
    Assert.assertFalse(RobuxOperatorTable.isFunctionSyntax(SqlSyntax.POSTFIX));
  }
  ...
  public void testCustomOperatorTable()
  {
    ...
    RobuxOperatorTable operatorTable = new RobuxOperatorTable(ImmutableSet.of(), extractionOperators);
    List<SqlOperator> operatorList = operatorTable.getOperatorList();
    Assert.assertNotNull(operatorList);
    ...
    Assert.assertTrue(RobuxOperatorTable.isFunctionSyntax(operator1.getSyntax()));
    Assert.assertFalse(RobuxOperatorTable.isFunctionSyntax(operator2.getSyntax()));
  }
  ...
  public void testBuiltinWindowOperatorsSupportFramingAsExpected()
  {
    ...
    for (SqlOperator operator : windowOps) {
      switch (operator.kind) {
        ...
        case LAST_VALUE:
        case NTILE:
          // These are handled with RobuxSqlValidator and a rewrite rule.
          continue;
        default:
          assertFalse(
