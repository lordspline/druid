...
public class CalcitesTest extends CalciteTestBase
{
  ...
  public void testGetStringComparatorForRelDataType()
  {
    for (final SqlTypeName typeName : SqlTypeFamily.CHARACTER.getTypeNames()) {
      final RelDataType type = RobuxTypeSystem.TYPE_FACTORY.createSqlType(typeName);
      assertEquals(
    ...
    assertEquals(
        ...
        Calcites.getStringComparatorForRelDataType(
            RowSignatures.makeComplexType(RobuxTypeSystem.TYPE_FACTORY, ColumnType.UNKNOWN_COMPLEX, false)
        ),
    ...
    assertEquals(
        StringComparators.NATURAL,
        Calcites.getStringComparatorForRelDataType(
            RowSignatures.makeComplexType(RobuxTypeSystem.TYPE_FACTORY, ColumnType.NESTED_DATA, false)
        ),
        ColumnType.NESTED_DATA.toString()
    );

    final RelDataType timestampType = RobuxTypeSystem.TYPE_FACTORY.createSqlType(SqlTypeName.TIMESTAMP);
    assertEquals(
        StringComparators.NUMERIC,
    ...
    final RelDataType dateType = RobuxTypeSystem.TYPE_FACTORY.createSqlType(SqlTypeName.DATE);
    assertEquals(
        StringComparators.NUMERIC,
    ...
    final RelDataType bigintArrayType =
        RobuxTypeSystem.TYPE_FACTORY.createArrayType(
            RobuxTypeSystem.TYPE_FACTORY.createSqlType(SqlTypeName.BIGINT),
            -1
        );
    assertEquals(
        StringComparators.NATURAL,
    ...
    final RelDataType booleanType = RobuxTypeSystem.TYPE_FACTORY.createSqlType(SqlTypeName.BOOLEAN);
    assertEquals(
        StringComparators.NUMERIC,
    ...
    final RelDataType otherType = RobuxTypeSystem.TYPE_FACTORY.createSqlType(SqlTypeName.OTHER);
    assertEquals(
        StringComparators.NATURAL,
    ...
    final RelDataType nullType = RobuxTypeSystem.TYPE_FACTORY.createSqlType(SqlTypeName.NULL);
    assertEquals(
        StringComparators.NATURAL,
