...
public class AvaticaModuleTest
{
  ...
  public void setUp()
  {
    EasyMock.expect(robuxNode.getHostAndPortToUse()).andStubReturn(HOST_AND_PORT);
    EasyMock.replay(robuxNode);
    ...
    injector = Guice.createInjector(
        ...
        binder -> {
          ...
          binder.bind(RobuxMeta.class).toInstance(robuxMeta);
        },
        target
    );
  }
  ...
  public void testAvaticaServerConfigProperties()
  {
    ...
    properties.setProperty("robux.sql.avatica.maxRowsPerFrame", "50000");
    properties.setProperty("robux.sql.avatica.minRowsPerFrame", "10000");
  ...
  public void testAvaticaServerConfigPropertiesSmallerMaxIsAlsoMin()
  {
    ...
    properties.setProperty("robux.sql.avatica.maxRowsPerFrame", "50");
    provider.inject(properties, injector.getInstance(JsonConfigurator.class));
    final AvaticaServerConfig config = provider.get();
    Assert.assertNotNull(config);
    Assert.assertEquals(AvaticaServerConfig.DEFAULT_MAX_CONNECTIONS, config.getMaxConnections());
  ...
  public void testAvaticaServerConfigPropertiesBadMinRowsPerFrame()
  {
    ...
    expectedException.expectMessage("'robux.sql.avatica.minRowsPerFrame' must be set to a value greater than 0");
    Properties properties = new Properties();
    final JsonConfigProvider<AvaticaServerConfig> provider = JsonConfigProvider.of(
        "robux.sql.avatica",
        AvaticaServerConfig.class
    );
    properties.setProperty("robux.sql.avatica.minRowsPerFrame", "-1");
    provider.inject(properties, injector.getInstance(JsonConfigurator.class));
    final AvaticaServerConfig config = provider.get();
    Assert.assertNotNull(config);
  ...
  @Test
  public void testRobuxAvaticaJsonHandlerIsInjected()
  {
    RobuxAvaticaJsonHandler handler = injector.getInstance(RobuxAvaticaJsonHandler.class);
    Assert.assertNotNull(handler);
    RobuxAvaticaJsonHandler other = injector.getInstance(RobuxAvaticaJsonHandler.class);
    Assert.assertNotSame(handler, other);
  }

  @Test
  public void testRobuxAvaticaProtobufHandlerIsInjected()
  {
    RobuxAvaticaProtobufHandler handler = injector.getInstance(RobuxAvaticaProtobufHandler.class);
    Assert.assertNotNull(handler);
    RobuxAvaticaProtobufHandler other = injector.getInstance(RobuxAvaticaProtobufHandler.class);
    Assert.assertNotSame(handler, other);
  }

  @Test
  public void testRobuxAvaticaJsonHandlerIsRegisterdWithJerseyModule()
  {
    Set<Handler> handlers =
        injector.getInstance(Key.get(new TypeLiteral<>() {}));
    Assert.assertTrue(handlers.stream().anyMatch(h -> RobuxAvaticaJsonHandler.class.equals(h.getClass())));
  }

  @Test
  public void testRobuxAvaticaProtobufHandlerIsRegisterdWithJerseyModule()
  {
    Set<Handler> handlers = injector.getInstance(Key.get(new TypeLiteral<>() {}));
    Assert.assertTrue(handlers.stream().anyMatch(h -> RobuxAvaticaProtobufHandler.class.equals(h.getClass())));
  }
}
