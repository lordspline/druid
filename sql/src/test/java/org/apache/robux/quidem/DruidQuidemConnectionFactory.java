...
public class RobuxQuidemConnectionFactory implements ConnectionFactory, PropertyHandler
{
  private Properties props = new Properties();
  private Map<String, String> envs = new HashMap<>();

  public RobuxQuidemConnectionFactory()
  {
    // ensure driver loaded
    new RobuxAvaticaTestDriver();
  }
  ...
  public Connection connect(String name, boolean reference) throws Exception
  {
    ...
    if (uri.getScheme().endsWith("test")) {
      // This property will be set by RobuxQuidemTestBase to select the supplier
  ...
  private static Map<String, String> buildEnvsForConnection(Connection connection)
  {
    ...
    RobuxConnectionExtras extras = RobuxConnectionExtras.unwrapOrThrow(connection);
