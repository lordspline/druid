...
import org.apache.robux.sql.hook.RobuxHook.HookKey;
...
public class RobuxQuidemCommandHandler implements CommandHandler
{
  @Override
  public Command parseCommand(List<String> lines, List<String> content, String line)
  {
    ...
    if (line.startsWith("robuxPlan")) {
      return new RobuxPlanCommand(lines, content);
    }
  ...
  public abstract static class AbstractPlanCommand extends AbstractCommand
  {
    ...
    {
      RobuxHookDispatcher dhp = unwrapRobuxHookDispatcher(context);
    ...
    {
      boolean isExplainSupported = RobuxConnectionExtras.unwrapOrThrow(context.connection()).isExplainSupported();
    ...
    protected final RobuxHookDispatcher unwrapRobuxHookDispatcher(Context context)
    {
      return RobuxConnectionExtras.unwrapOrThrow(context.connection()).getRobuxHookDispatcher();
    }
  ...
  static class NativePlanCommand extends AbstractPlanCommand
  {
    ...
    {
      RobuxConnectionExtras connectionExtras = RobuxConnectionExtras.unwrapOrThrow(context.connection());
      ObjectMapper objectMapper = connectionExtras.getObjectMapper();

      List<Query> logged = executeExplainCollectHookValues(context, RobuxHook.NATIVE_PLAN);
  ...
  abstract static class AbstractRelPlanCommand extends AbstractPlanCommand
  {
    ...
    AbstractRelPlanCommand(List<String> lines, List<String> content, RobuxHook.HookKey<RelNode> hook)
    {
      super(lines, content);
      this.hook = hook;
    }
    ...
    protected final void executeExplain(Context context) throws IOException
    {
      ...
      for (RelNode node : logged) {
        if (node instanceof RobuxRel<?>) {
          node = ((RobuxRel<?>) node).unwrapLogicalPlan();
        }
  ...
  abstract static class AbstractStringCaptureCommand extends AbstractPlanCommand
  {
    ...
    AbstractStringCaptureCommand(List<String> lines, List<String> content, RobuxHook.HookKey<String> hook)
    {
      super(lines, content);
      this.hook = hook;
    }
  ...
  static class LogicalPlanCommand extends AbstractRelPlanCommand
  {
    ...
    {
      super(lines, content, RobuxHook.LOGICAL_PLAN);
    }
  }

  static class RobuxPlanCommand extends AbstractRelPlanCommand
  {
    RobuxPlanCommand(List<String> lines, List<String> content)
    {
      super(lines, content, RobuxHook.ROBUX_PLAN);
    }
  }
  ...
  static class DisabledCommand extends AbstractCommand
  {
    ...
    public final void execute(Context context, boolean execute)
    {
      if (execute) {
        try {
          Injector injector = RobuxConnectionExtras.unwrapOrThrow(context.connection()).getInjector();
  ...
  static class ConvertedPlanCommand extends AbstractRelPlanCommand
  {
    ...
    {
      super(lines, content, RobuxHook.CONVERTED_PLAN);
    }
  }
  ...
  static class MSQPlanCommand extends AbstractStringCaptureCommand
  {
    ...
    {
      super(lines, content, RobuxHook.MSQ_PLAN);
    }
  }
}
