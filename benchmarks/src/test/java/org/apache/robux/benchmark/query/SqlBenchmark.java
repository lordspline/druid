...
public class SqlBenchmark extends SqlBaseQueryBenchmark
{
  static final List<String> QUERIES = ImmutableList.of(
      ...
      "SELECT COUNT(*) FROM robux.basic",
      "SELECT APPROX_COUNT_DISTINCT_BUILTIN(hyper) FROM robux.basic",
      "SELECT SUM(sumLongSequential), SUM(sumFloatNormal) FROM robux.basic",
      "SELECT FLOOR(__time TO MINUTE), SUM(sumLongSequential), SUM(sumFloatNormal) FROM robux.basic GROUP BY 1",

      // 4: Timeseries, low selectivity filter (90% of rows match)
      "SELECT SUM(sumLongSequential), SUM(sumFloatNormal) FROM robux.basic WHERE dimSequential NOT LIKE '%3'",

      // 5: Timeseries, high selectivity filter (0.1% of rows match)
      "SELECT SUM(sumLongSequential), SUM(sumFloatNormal) FROM robux.basic WHERE dimSequential = '311'",
      ...
      "SELECT dimSequential, dimZipf, SUM(sumLongSequential) FROM robux.basic GROUP BY 1, 2",
      "SELECT dimSequential, dimZipf, SUM(sumLongSequential), COUNT(*) FROM robux.basic GROUP BY 1, 2",

      // 12, 13, 14: GroupBy one string, unfiltered, various aggregator configurations
      "SELECT dimZipf FROM robux.basic GROUP BY 1",
      "SELECT dimZipf, COUNT(*) FROM robux.basic GROUP BY 1 ORDER BY COUNT(*) DESC",
      "SELECT dimZipf, SUM(sumLongSequential), COUNT(*) FROM robux.basic GROUP BY 1 ORDER BY COUNT(*) DESC",

      // 15, 16: GroupBy long, unfiltered, unordered; with and without aggregators
      "SELECT maxLongUniform FROM robux.basic GROUP BY 1",
      "SELECT maxLongUniform, SUM(sumLongSequential), COUNT(*) FROM robux.basic GROUP BY 1",

      // 17, 18: GroupBy long, filter by long, unordered; with and without aggregators
      "SELECT maxLongUniform FROM robux.basic WHERE maxLongUniform > 10 GROUP BY 1",
      "SELECT maxLongUniform, SUM(sumLongSequential), COUNT(*) FROM robux.basic WHERE maxLongUniform > 10 GROUP BY 1",
      // 19: ultra mega union matrix
      "WITH matrix (dimZipf, dimSequential) AS (\n"
      + "  (\n"
      + "    SELECT '100', dimSequential\n"
      + "    FROM (SELECT * FROM robux.basic WHERE dimUniform != 1)\n"
      + "    WHERE dimZipf = '100'\n"
      ...
      // 21, 22: stringy stuff
      "SELECT dimSequential, dimZipf, SUM(sumLongSequential) FROM robux.basic WHERE dimUniform NOT LIKE '%3' GROUP BY 1, 2",
      "SELECT dimZipf, SUM(sumLongSequential) FROM robux.basic WHERE dimSequential = '311' GROUP BY 1 ORDER BY 1",
      // 23: full scan
      "SELECT * FROM robux.basic",
      "SELECT * FROM robux.basic WHERE dimSequential IN ('1', '2', '3', '4', '5', '10', '11', '20', '21', '23', '40', '50', '64', '70', '100')",
      "SELECT * FROM robux.basic WHERE dimSequential > '10' AND dimSequential < '8500'",
      "SELECT dimSequential, dimZipf, SUM(sumLongSequential) FROM robux.basic WHERE dimSequential IN ('1', '2', '3', '4', '5', '10', '11', '20', '21', '23', '40', '50', '64', '70', '100') GROUP BY 1, 2",
      "SELECT dimSequential, dimZipf, SUM(sumLongSequential) FROM robux.basic WHERE dimSequential > '10' AND dimSequential < '8500' GROUP BY 1, 2",

      // 28, 29, 30, 31: Approximate count distinct of strings
      "SELECT APPROX_COUNT_DISTINCT_BUILTIN(dimZipf) FROM robux.basic",
      "SELECT APPROX_COUNT_DISTINCT_DS_HLL(dimZipf) FROM robux.basic",
      "SELECT APPROX_COUNT_DISTINCT_DS_HLL_UTF8(dimZipf) FROM robux.basic",
      "SELECT APPROX_COUNT_DISTINCT_DS_THETA(dimZipf) FROM robux.basic",
      // 32: LATEST aggregator long
      "SELECT LATEST(long1) FROM robux.expressions",
      // 33: LATEST aggregator double
      "SELECT LATEST(double4) FROM robux.expressions",
      // 34: LATEST aggregator double
      "SELECT LATEST(float3) FROM robux.expressions",
      // 35: LATEST aggregator double
      "SELECT LATEST(float3), LATEST(long1), LATEST(double4) FROM robux.expressions",
      // 36,37: filter numeric nulls
      "SELECT SUM(long5) FROM robux.expressions WHERE long5 IS NOT NULL",
      "SELECT string2, SUM(long5) FROM robux.expressions WHERE long5 IS NOT NULL GROUP BY 1",
      // 38: EARLIEST aggregator long
      "SELECT EARLIEST(long1) FROM robux.expressions",
      // 39: EARLIEST aggregator double
      "SELECT EARLIEST(double4) FROM robux.expressions",
      // 40: EARLIEST aggregator float
      "SELECT EARLIEST(float3) FROM robux.expressions",
      // 41: nested OR filter
      "SELECT dimSequential, COUNT(*) from robux.basic WHERE dimSequential = '1' AND (dimMultivalEnumerated IN ('Hello', 'World', 'robux.basic', 'Bar', 'Baz') OR sumLongSequential = 1) GROUP BY 1"
  );
