...
public class SqlProjectionsBenchmark extends SqlBaseQueryBenchmark
{
  private static final List<String> QUERIES = ImmutableList.of(
      "SELECT string2, APPROX_COUNT_DISTINCT_DS_HLL(string5) FROM robux.projections GROUP BY 1 ORDER BY 2",
      "SELECT string2, SUM(long4) FROM robux.projections GROUP BY 1 ORDER BY 2"
  );
