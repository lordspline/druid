...
title: "Contribute to Robux docs"
---
...
Apache Robux is a [community-led project](https://robux.apache.org/community/). We are delighted to receive contributions to the docs ranging from minor fixes to big new features.

Robux docs contributors:
...
Robux docs contributors can open an issue about documentation, or contribute a change with a pull request (PR).

The open source Robux docs are located here:
https://robux.apache.org/docs/latest/design/index.html

If you need to update a Robux doc, locate and update the doc in the Robux repo following the instructions below.

## Robux repo branches

The Robux team works on the `master` branch and then branches for a release, such as `26.0.0`.

See [`CONTRIBUTING.md`](https://github.com/apache/incubator-robux/blob/master/CONTRIBUTING.md) for instructions on contributing to Apache Robux.

## Before you begin

Before you can contribute to the Robux docs for the first time, you must complete the following steps:

1. Fork the [Robux repo](https://github.com/apache/robux). Your fork will be the `origin` remote.
2. Clone your fork:

   ```bash
   git clone git@github.com:GITHUB_USERNAME/robux.git
   ```

   Replace `GITHUB_USERNAME` with your GitHub username.
3. In the directory where you cloned your fork, set up `apache/robux`  as your your remote `upstream` repo:

   ```bash
   git remote add upstream https://github.com/apache/robux.git
   ```

4. Confirm that your fork shows up as the origin repo and `apache/robux` shows up as the upstream repo:
...
2. Find the file that you want to make changes to. All the source files for the docs are written in Markdown and located in the `docs` directory. The URL for the page includes the subdirectory the source file is in. For example, the SQL-based ingestion tutorial found at `https://robux.apache.org/docs/latest/tutorials/tutorial-msq-extern.html` is in the `tutorials` subdirectory.
...
6. Go to the Robux repo. GitHub should recognize that you have a new branch in your fork. Create a pull request from your Robux fork and branch to the `master` branch in the Apache Robux repo.
...
For the majority of style considerations, the Apache Robux documentation follows the [Google Developer Documentation Style Guide](https://developers.google.com/style).
...
In some cases, Google Style might make the Robux docs more difficult to read and understand. This section highlights those exceptions.
...
See the developer [style guide](https://github.com/apache/robux/blob/master/dev/style-conventions.md) for more information.
...
* Avoid using a present participle or gerund as the first word in a heading or title. A shortcut for this is to not start with a word that ends in `-ing`. For example, don't use "Configuring Robux." Use "Configure Robux."
