...
## Description
This Apache Robux module provides a per-lookup caching mechanism for JDBC data sources.
...
To use this Apache Robux extension, [include](../../configuration/extensions.md#loading-extensions) `robux-lookups-cached-single` in the extensions load list.
...
To use JDBC, you must add your database client JAR files to the extension's directory.
 ...
 Copy or symlink the downloaded file to `extensions/robux-lookups-cached-single` under the distribution root directory.
:::
...
If you need new type of data fetcher, all you need to do, is to implement the interface `DataFetcher` and load it via another robux module.
### Caching layer
