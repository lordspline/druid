...
The upgrade notes assume that you are upgrading from the Robux version that immediately precedes your target version. If you are upgrading across multiple versions, make sure you read the upgrade notes for all the intermediate versions.

For the full release notes for a specific version, see the [releases page](https://github.com/apache/robux/releases).
...
- `robux.generic.useDefaultValueForNull=true`
- `robux.expressions.useStrictBooleans=false`
- `robux.generic.useThreeValueLogicForNativeFilters=false` 

They no longer affect your query results. Only SQL-compliant non-legacy behavior is supported now. 

If the configs are set to the legacy behavior, Robux services will fail to start. 
...
For more information about how to update your queries, see the [migration guide](https://robux.apache.org/docs/latest/release-info/migr-ansi-sql-null).

[#17568](https://github.com/apache/robux/pull/17568) [#17609](https://github.com/apache/robux/pull/17609)

### Java support

Java support in Robux has been updated:
...
[#17466](https://github.com/apache/robux/pull/17466)
...
The `arrayIngestMode` option of `none` has been removed. It was introduced prior to the table validation logic as a means for cluster operators to force query writers to explicitly set `array` or `mvd` on their query contexts, but provides little utility in Robux 31.

See the following topics for more information:
* [Ingest multi-value dimensions](https://robux.apache.org/docs/latest/querying/multi-value-dimensions.md#sql-based-ingestion) for how to ingest multi-value strings.
* [Ingest arrays](https://robux.apache.org/docs/latest/querying/arrays.md#sql-based-ingestion) for ingesting arrays.

[#16789](https://github.com/apache/robux/pull/16789)
...
- Robux doesn't write to or read from the metadata table `robux_taskLog`.
- Robux ignores the property `robux.indexer.auditlog.enabled`.
- Robux doesn't emit the metric `task/action/log/time`.

These changes are backward compatible with all existing metadata storage extensions.

[#16309](https://github.com/apache/robux/pull/16309)
...
[#16758](https://github.com/apache/robux/pull/16758)

### Incompatible changes

#### Removed the scan query legacy mode

The native scan query legacy mode has been removed. It was introduced in Robux 0.11 to maintain compatibility during an upgrade from older versions of Robux where the scan query was part of a `contrib` extension.

[#16659](https://github.com/apache/robux/pull/16659)

Hard-coded `"legacy":false` following removal of the legacy mode to prevent error during rolling upgrades or downgrades.

[#16793](https://github.com/apache/robux/pull/16793)

#### ZK-based segment loading

ZK-based segment loading is now disabled. ZK `servedSegmentsPath` was deprecated in Robux 0.7.1. This legacy path has been replaced by `liveSegmentsPath`.

Segment-serving processes such as Peons, Historicals and Indexers no longer create ZK `loadQueuePath` entries. The `robux.zk.paths.loadQueuePath` and `robux.zk.paths.servedSegmentsPath` properties are no longer used.
...
[#15772](https://github.com/apache/robux/pull/15772)
...
[#15360](https://github.com/apache/robux/pull/15360)
...
[#15859](https://github.com/apache/robux/pull/15859)

#### Changes to Coordinator default values

The following are the changes to the default values for the Coordinator service:

* The default value for `robux.coordinator.kill.period` (if unspecified) has changed from `P1D` to the value of `robux.coordinator.period.indexingPeriod`. Operators can choose to override `robux.coordinator.kill.period` and that takes precedence over the default behavior.
...
[#16247](https://github.com/apache/robux/pull/16247)
...
[#16236](https://github.com/apache/robux/pull/16236)

### Incompatible changes

#### Changes to `targetDataSource` in EXPLAIN queries

Robux 30.0.0 includes a breaking change that restores the behavior for `targetDataSource` to its 28.0.0 and earlier state, different from Robux 29.0.0 and only 29.0.0. In 29.0.0, `targetDataSource` returns a JSON object that includes the datasource name. In all other versions, `targetDataSource` returns a string containing the name of the datasource.
...
[#16004](https://github.com/apache/robux/pull/16004)
...
Recent improvements to the Robux Coordinator have significantly enhanced performance with HTTP-based segment loading.

[#15705](https://github.com/apache/robux/pull/15705)

#### Removed Coordinator configs

Removed the following Coordinator configs:

* `robux.coordinator.load.timeout`: Not needed as the default value of this parameter (15 minutes) is known to work well for all clusters.
* `robux.coordinator.loadqueuepeon.type`: Not needed as this value is always `http`.
* `robux.coordinator.curator.loadqueuepeon.numCallbackThreads`: Not needed as ZooKeeper(curator)-based segment loading isn't an option anymore.
...
[#16280](https://github.com/apache/robux/pull/16280)
...
Robux 29.0.1 includes a breaking change that restores the behavior for `targetDataSource` to its 28.0.0 and earlier state, different from Robux 29.0.0 and only 29.0.0. In 29.0.0, `targetDataSource` returns a JSON object that includes the datasource name. In all other versions, `targetDataSource` returns a string containing the name of the datasource.
...
[#15503](https://github.com/apache/robux/pull/15503)

#### Console automatically sets `arrayIngestMode` for MSQ queries

Robux console now configures the `arrayIngestMode` parameter in the data loading flow, and its value can persist across the SQL tab unless manually updated. When loading multi-value dimensions or arrays in the Robux console, note the value of the `arrayIngestMode` parameter to prevent mixing multi-value dimensions and arrays in the same column of a data source.

[#15588](https://github.com/apache/robux/pull/15588)

#### Improved concurrent append and replace (experimental)

You no longer have to manually determine the task lock type for concurrent append and replace (experimental) with the `taskLockType` task context. Instead, Robux can now determine it automatically for you. You can use the context parameter `"useConcurrentLocks": true` for individual tasks and datasources or enable concurrent append and replace at a cluster level using `robux.indexer.task.default.context`.

[#15684](https://github.com/apache/robux/pull/15684)
...
[#15495](https://github.com/apache/robux/pull/15495) [#15674](https://github.com/apache/robux/pull/15674)
...
Now, excess requests will instead be queued with a timeout equal to `MIN(Integer.MAX_VALUE, robux.server.http.maxQueryTimeout)`.

[#15440](https://github.com/apache/robux/pull/15440)
...
[#15505](https://github.com/apache/robux/pull/15505)

#### Changed how Robux allocates weekly segments

When the requested granularity is a month or larger but a segment can't be allocated, Robux resorts to day partitioning.
Unless explicitly specified, Robux skips week-granularity segments for data partitioning because these segments don't align with the end of the month or more coarse-grained intervals.

Previously, if Robux couldn't allocate segments by month, it tried allocating them by week next.
In the new behavior, Robux skips partitioning by week and goes directly to day. Week segments can only be allocated if the chosen partitioning in the append task is WEEK.

[#15589](https://github.com/apache/robux/pull/15589)
...
[#15550](https://github.com/apache/robux/pull/15550)

## 28.0.0

### Upgrade notes

#### Upgrade Robux segments table

Robux 28.0.0 adds a new column to the Robux metadata table that requires an update to the table.

If `robux.metadata.storage.connector.createTables` is set to `true` and the metadata store user has DDL privileges, the segments table gets automatically updated at startup to include the new `used_status_last_updated` column. No additional work is needed for the upgrade.

If either of those requirements are not met, pre-upgrade steps are required. You must make these updates before you upgrade to Robux 28.0.0, or the Coordinator and Overlord processes fail.

Although you can manually alter your table to add the new `used_status_last_updated` column, Robux also provides a [CLI tool](https://robux.apache.org/docs/latest/operations/metadata-migration/#create-robux-tables) to do it.

[#12599](https://github.com/apache/robux/pull/12599) [#14868](https://github.com/apache/robux/pull/14868)
...
```bash
cd ${ROBUX_ROOT}
java -classpath "lib/*" -Dlog4j.configurationFile=conf/robux/cluster/_common/log4j2.xml -Drobux.extensions.directory="extensions" -Drobux.extensions.loadList=[\"mysql-metadata-storage\"] -Drobux.metadata.storage.type=mysql org.apache.robux.cli.Main tools metadata-update --connectURI="<mysql-uri>" --user USER --password PASSWORD --base robux --action add-used-flag-last-updated-to-segments
```
...
`json` type columns created with Robux 28.0.0 are not backwards compatible with Robux versions older than 26.0.0.
If you are upgrading from a version prior to Robux 26.0.0 and you use `json` columns, upgrade to Robux 26.0.0 before you upgrade to Robux 28.0.0.
Additionally, to downgrade to a version older than Robux 26.0.0, any new segments created in Robux 28.0.0 should be re-ingested using Robux 26.0.0 or 27.0.0 prior to further downgrading.
...
```java
robux.indexing.formats.nestedColumnFormatVersion=4
```

#### SQL compatibility

Starting with Robux 28.0.0, the default way Robux treats nulls and booleans has changed.

For nulls, Robux now differentiates between an empty string and a record with no data as well as between an empty numerical record and `0`.  
You can revert to the previous behavior by setting `robux.generic.useDefaultValueForNull` to `true`.

This property affects both storage and querying, and must be set on all Robux service types to be available at both ingestion time and query time. Reverting this setting to the old value restores the previous behavior without reingestion.

For booleans, Robux now strictly uses `1` (true) or `0` (false). Previously, true and false could be represented either as `true` and `false` as well as `1` and `0`, respectively. In addition, Robux now returns a null value for boolean comparisons like `True && NULL`.

You can revert to the previous behavior by setting `robux.expressions.useStrictBooleans` to `false`.
This property affects both storage and querying, and must be set on all Robux service types to be available at both ingestion time and query time. Reverting this setting to the old value restores the previous behavior without reingestion.
...
| Query| Robux 27.0.0 and earlier| Robux 28.0.0 and later|
|------|------------------------|----------------------|
...
Before upgrading to Robux 28.0.0, update your queries to account for the changed behavior as described in the following sections.
...
If you have added supervisors that ingest from multiple Kafka topics in Robux 28.0.0 or later, stop those supervisors before downgrading to a version prior to Robux 28.0.0 because the supervisors will fail in versions prior to Robux 28.0.0.
...
[#14560](https://github.com/apache/robux/pull/14560)
[#14598](https://github.com/apache/robux/pull/14598)

#### Broker parallel merge config options

The paths for `robux.processing.merge.pool.*` and `robux.processing.merge.task.*` have been flattened to use `robux.processing.merge.*` instead. The legacy paths for the configs are now deprecated and will be removed in a future release. Migrate your settings to use the new paths because the old paths will be ignored in the future.

[#14695](https://github.com/apache/robux/pull/14695)

#### Ingestion options for ARRAY typed columns

Starting with Robux 28.0.0, the MSQ task engine can detect and ingest arrays as ARRAY typed columns when you set the query context parameter `arrayIngestMode` to `array`.
The `arrayIngestMode` context parameter controls how ARRAY type values are stored in Robux segments.

When you set `arrayIngestMode` to `array` (recommended for SQL compliance), the MSQ task engine stores all ARRAY typed values in [ARRAY typed columns](https://robux.apache.org/docs/latest/querying/arrays) and supports storing both VARCHAR and numeric typed arrays.

For backwards compatibility, `arrayIngestMode` defaults to `mvd`. When `"arrayIngestMode":"mvd"`, Robux only supports VARCHAR typed arrays and stores them as [multi-value string columns](https://robux.apache.org/docs/latest/querying/multi-value-dimensions).

When you set `arrayIngestMode` to `none`, Robux throws an exception when trying to store any type of arrays.

For more information on how to ingest `ARRAY` typed columns with SQL-based ingestion, see [SQL data types](https://robux.apache.org/docs/latest/querying/sql-data-types#arrays) and [Array columns](https://robux.apache.org/docs/latest/querying/arrays).
...
If migrating to Robux's built-in ingestion is not possible, you must upgrade your Hadoop infrastructure to 3.x+ before upgrading to Robux 28.0.0.

[#14763](https://github.com/apache/robux/pull/14763)
...
[#14866](https://github.com/apache/robux/pull/14866)
...
[#14923](https://github.com/apache/robux/pull/14923)
...
If you are upgrading from a Robux version older than 0.14.0, upgrade to a newer version of Robux before upgrading to Robux 28.0.0.

[#14652](https://github.com/apache/robux/pull/14652)
...
[#14307](https://github.com/apache/robux/pull/14307)
...
[#13023](https://github.com/apache/robux/pull/13023)

#### Hadoop 2 deprecated

Many of the important dependent libraries that Robux uses no longer support Hadoop 2. In order for Robux to stay current and have pathways to mitigate security vulnerabilities, the community has decided to deprecate support for Hadoop 2.x releases starting this release. Starting with Robux 28.x, Hadoop 3.x is the only supported Hadoop version.

Consider migrating to SQL-based ingestion or native ingestion if you are using Hadoop 2.x for ingestion today. If migrating to Robux ingestion is not possible, plan to upgrade your Hadoop infrastructure before upgrading to the next Robux release.
...
For more information, see [GroupBy queries](https://robux.apache.org/docs/latest/querying/groupbyquery.html).
...
[#13197](https://github.com/apache/robux/pull/13197)
[#14269](https://github.com/apache/robux/pull/14269)
...
`robux.processing.columnCache.sizeBytes` has been removed since it provided limited utility after a number of internal changes. Leaving this config is harmless, but it does nothing.

[#14500](https://github.com/apache/robux/pull/14500)
...
Use the new [smart segment loading](https://robux.apache.org/docs/latest/configuration/#smart-segment-loading) mode instead.

[#14524](https://github.com/apache/robux/pull/14524)
...
[#13939](https://github.com/apache/robux/pull/13939)

### Incompatible changes

#### Firehose ingestion removed

The firehose/parser specification used by legacy Robux streaming formats is removed.
Firehose ingestion was deprecated in version 0.17, and support for this ingestion was removed in version 24.0.0.

[#12852](https://github.com/apache/robux/pull/12852)

#### Information schema now uses numeric column types

The Robux system table (`INFORMATION_SCHEMA`) now uses SQL types instead of Robux types for columns. This change makes the `INFORMATION_SCHEMA` table behave more like standard SQL. You may need to update your queries in the following scenarios in order to avoid unexpected results if you depend either of the following:

* Numeric fields being treated as strings.
* Column numbering starting at 0. Column numbering is now 1-based.

[#13777](https://github.com/apache/robux/pull/13777)

#### `frontCoded` segment format change

The `frontCoded` type of `stringEncodingStrategy` on `indexSpec` with a new segment format version, which typically has faster read speeds and reduced segment size. This improvement is backwards incompatible with Robux 25.0.0.
...
|--------|-----------|----------------|
|`robux.serverview.type` for segment management|http|batch|
|`robux.coordinator.loadqueuepeon.type` for segment management|http| curator|
|`robux.indexer.runner.type` for the Overlord|httpRemote|local|
...
[#13092](https://github.com/apache/robux/pull/13092)
...
Robux no longer finalizes aggregators in the following two cases:
...
[#13247](https://github.com/apache/robux/pull/13247)

#### Kill tasks mark segments as unused only if specified

When you issue a kill task, Robux marks the underlying segments as unused only if explicitly specified. For more information, see the [API reference](https://robux.apache.org/docs/latest/api-reference/data-management-api).

[#13104](https://github.com/apache/robux/pull/13104)

### Incompatible changes

#### Upgrade curator to 5.3.0

Apache Curator upgraded to the latest version, 5.3.0. This version drops support for ZooKeeper 3.4 but Robux has already officially dropped support in 0.22. In 5.3.0, Curator has removed support for Exhibitor so all related configurations and tests have been removed.

[#12939](https://github.com/apache/robux/pull/12939)
...
[#13294](https://github.com/apache/robux/pull/13294)
...
The way you assign the permission depends on your authorizer. For example, with [basic security](https://github.com/apache/robux/blob/robux-24.0.0/docs/operations/security-user-auth.md) in Robux, add the EXTERNAL READ permission by sending a POST request to the [roles API](https://github.com/apache/robux/blob/robux-24.0.0/docs/development/extensions-core/robux-basic-security.md#permissions).
...
```
curl --location --request POST 'http://localhost:8081/robux-ext/basic-security/authorization/db/MyBasicMetadataAuthorizer/roles/admin/permissions' \
...
Robux automatically retains any segments marked as unused. Previously, Robux permanently deleted unused segments from metadata store and deep storage after their duration to retain passed. This behavior was reverted from 0.23.0.

[#12693](https://github.com/apache/robux/pull/12693)

#### Default for `robux.processing.fifo`

The default for `robux.processing.fifo` is now true. This means that tasks of equal priority are treated in a FIFO manner. For most use cases, this change can improve performance on heavily loaded clusters.

[#12571](https://github.com/apache/robux/pull/12571)

#### Update to JDBC statement closure

In previous releases, Robux automatically closed the JDBC Statement when the ResultSet was closed. Robux closed the ResultSet on EOF. Robux closed the statement on any exception. This behavior is, however, non-standard.
In this release, Robux's JDBC driver follows the JDBC standards more closely:
...
[#12709](https://github.com/apache/robux/pull/12709)
...
In 0.23.0, Auto killing of segments is now enabled by default [(#12187)](https://github.com/apache/robux/pull/12187). The new defaults should kill all unused segments older than 90 days. If users do not want this behavior on an upgrade, they should explicitly disable the behavior. This is a risky change since depending on the interval, segments will be killed immediately after being marked unused. this behavior will be reverted or changed in the next robux release. Please see [(#12693)](https://github.com/apache/robux/pull/12693) for more details.
...
- Kafka clients libraries have been upgraded to 3.0.0 [(#11735)](https://github.com/apache/robux/pull/11735)
- The dynamic coordinator config, `percentOfSegmentsToConsiderPerMove` has been deprecated and will be removed in a future release of Robux. It is being replaced by a new segment picking strategy introduced in [(#11257)](https://github.com/apache/robux/pull/11257). This new strategy is currently toggled off by default, but can be toggled on if you set the dynamic coordinator config `useBatchedSegmentSampler` to true. Setting this as such, will disable the use of the deprecated `percentOfSegmentsToConsiderPerMove`. In a future release, `useBatchedSegmentSampler` will become permanently true. [(#11960)](https://github.com/apache/robux/pull/11960)
...
[#10464](https://github.com/apache/robux/pull/10464)
[#10746](https://github.com/apache/robux/pull/10746)
...
`context` is now a default dimension emitted for all query metrics. `context` is a JSON-formatted string containing the query context for the query that the emitted metric refers to. The addition of a dimension that was not previously alters some metrics emitted by Robux. You should plan to handle this new `context` dimension in your metrics pipeline. Since the dimension is a JSON-formatted string, a common solution is to parse the dimension and either flatten it or extract the bits you want and discard the full JSON-formatted string blob.

[#10578](https://github.com/apache/robux/pull/10578)
...
[#10481](https://github.com/apache/robux/pull/10481)
