...
In Apache Robux 32.0.0, legacy configurations which were incompatible with the ANSI SQL standard were removed. 

These configurations were:
* `robux.generic.useDefaultValueForNull`
* `robux.expressions.useStrictBooleans`
* `robux.generic.useThreeValueLogicForNativeFilters`

This guide provides strategies for Robux operators who rely on legacy Robux null handling behavior in their applications to transition to Robux 32.0.0 or later.

## SQL compliant null handling

As of Robux 28.0.0, Robux writes segments in an ANSI SQL compatible null handling mode by default, and in Robux 32.0.0 this is no longer configurable.
This is a change of legacy behavior and means that Robux stores null values distinctly from empty strings for string dimensions and distinctly from 0 for numeric dimensions.
...
Follow the [Null handling tutorial](../tutorials/tutorial-sql-null.md) to learn how null handling works in Robux.

## Legacy null handling and two-valued filter logic

Prior to Robux 28.0.0, Robux defaulted to a legacy mode which stored default values instead of nulls.
In this mode, Robux created segments with the following characteristics at ingestion time:

- String columns couldn't distinguish an empty string, `''`, from null.
    Therefore, Robux treated both values as interchangeable.
- Numeric columns couldn't represent null valued rows.
    Therefore, Robux stored `0` instead of `null`.

## Migrate to SQL compliant mode

If your business logic relies on the behavior of legacy mode, you have the following options to operate Robux in an ANSI SQL compatible null handling mode:
...
- Modify incoming data to either [avoid nulls](#replace-null-values-at-ingestion-time) or [avoid empty strings](#coerce-empty-strings-to-null-at-ingestion-time) to achieve the same query behavior as legacy mode. This means modifying your ingestion SQL queries and ingestion specs to handle nulls or empty strings.
    ...
    However, it means that your existing queries should operate as if Robux were in legacy mode.
...
If you don't need to preserve null values within Robux, you can use a transform at ingestion time to replace nulls with other values.
...
The following example illustrates how to use COALESCE and NVL at ingestion time to avoid null values in Robux:
...
Robux ingests the data with no null values as follows:
...
In legacy mode, Robux recognized empty strings as nulls for equality comparison.
...
In legacy mode, Robux wrote an empty string for the third record.
...
In SQL compliant mode, Robux differentiates between empty strings and nulls, so the same query would return 1.
...
Robux ingests the data with no empty strings as follows:
...
If you want to maintain null values in your data within Robux, you can use the following ANSI SQL compliant querying strategies to achieve the same results as legacy null handling:
...
Consider the following Robux datasource `null_example`:
...
Robux excludes null strings from equality comparisons. For example:
...
Robux returns 1 because null is considered unknown: neither equal nor unequal to the value.
...
Robux returns the following because null + any value is null for the ANSI SQL standard:
...
See the following topics for more information:
 - [Null handling tutorial](../tutorials/tutorial-sql-null.md) to learn how the default null handling works in Robux.
 - [Null values](../querying/sql-data-types.md#null-values) for a description of Robux's null values.
 - [Handling null values](../design/segments.md#handling-null-values) for details about how Robux stores null values.