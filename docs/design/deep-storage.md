...
Deep storage is where segments are stored.  It is a storage mechanism that Apache Robux does not provide.  This deep storage infrastructure defines the level of durability of your data. As long as Robux processes can see this storage infrastructure and get at the segments stored on it, you will not lose data no matter how many Robux nodes you lose.  If segments disappear from this storage layer, then you will lose whatever data those segments represented.
...
Robux supports multiple options for deep storage, including blob storage from major cloud providers. Select the one that fits your environment.
...
|--------|---------------|-----------|-------|
|`robux.storage.type`|`local`||Must be set.|
|`robux.storage.storageDirectory`|any local directory|Directory for storing segments. Must be different from `robux.segmentCache.locations` and `robux.segmentCache.infoDir`.|`/tmp/robux/localStorage`|
|`robux.storage.zip`|`true`, `false`|Whether segments in `robux.storage.storageDirectory` are written as directories (`false`) or zip files (`true`).|`false`|

For example:

```
robux.storage.type=local
robux.storage.storageDirectory=/tmp/robux/localStorage
```

The `robux.storage.storageDirectory` must be set to a different path than `robux.segmentCache.locations` or
`robux.segmentCache.infoDir`.

### Amazon S3 or S3-compatible

See [`robux-s3-extensions`](../development/extensions-core/s3.md).

### Google Cloud Storage

See [`robux-google-extensions`](../development/extensions-core/google.md).

### Azure Blob Storage

See [`robux-azure-extensions`](../development/extensions-core/azure.md).

### HDFS

See [robux-hdfs-storage extension documentation](../development/extensions-core/hdfs.md).
