---
id: mixed-workloads
title: Configure Robux for mixed workloads
...
If you frequently run concurrent, heterogeneous workloads on your Apache Robux cluster, configure Robux to properly allocate cluster resources to optimize your overall query performance.

Each Robux query consumes a certain amount of cluster resources, such as processing threads, memory buffers for intermediate query results, and HTTP threads for communicating between Brokers and data servers.
...
For example, if you run both sets of queries in the same Robux cluster, heavy queries may employ all available HTTP threads.
...
Robux provides the following strategies to isolate resources and improve query concurrency:
...
You can profile Robux queries using normal performance profiling techniques such as Robux query metrics analysis, thread dumps of JVM, or flame graphs to identify what resources are affected by mixed workloads.
...
Query lanes are analogous to carpool and normal lanes on the freeway. With query laning, Robux sets apart prioritized lanes from other general lanes.
Robux restricts low priority queries to the general lanes and allows high priority queries to run wherever possible, whether in a VIP or general lane.

In Robux, query lanes reserve resources for Broker HTTP threads. Each Robux query requires one Broker thread. The number of threads on a Broker is defined by the `robux.server.http.numThreads` parameter. Broker threads may be occupied by tasks other than queries, such as health checks. You can use query laning to limit the number of HTTP threads designated for resource-intensive queries, leaving other threads available for short-running queries and other tasks.

### General properties

Set the following query laning properties in the `broker/runtime.properties` file.

* `robux.query.scheduler.laning.strategy` – The strategy used to assign queries to lanes.
...
* `robux.query.scheduler.numThreads` – The total number of queries that can be served per Broker. We recommend setting this value to 1-2 less than `robux.server.http.numThreads`.

The query scheduler by default does not limit the number of queries that a Broker can serve. Setting this property to a bounded number limits the thread count. If the allocated threads are all occupied, any incoming query, including interactive queries, will be queued on the broker and will timeout after the request stays in the queue for more than the configured timeout. This configured timeout is equal to `MIN(Integer.MAX_VALUE, robux.server.http.maxQueryTimeout)`. If the value of `robux.server.http.maxQueryTimeout` is negative, the request is queued forever. 

### Lane-specific properties

If you use the __high/low laning strategy__, set the following:

* `robux.query.scheduler.laning.maxLowPercent` – The maximum percent of query threads to handle low priority queries. The remaining query threads are dedicated to high priority queries.
...
* `robux.query.scheduler.laning.lanes.{name}` – The limit for how many queries can run in the `name` lane. Define as many named lanes as needed.
* `robux.query.scheduler.laning.isLimitPercent` – Whether to treat the lane limit as an exact number or a percent of the minimum of `robux.server.http.numThreads` or `robux.query.scheduler.numThreads`.
...
```
# Laning strategy
robux.query.scheduler.laning.strategy=hilo
robux.query.scheduler.laning.maxLowPercent=20

# Limit the number of HTTP threads for query processing
# This value should be less than robux.server.http.numThreads
robux.query.scheduler.numThreads=40
```
...
Historical-only tiering is not sufficient to meet the demands of mixed workloads on a Robux cluster.
...
For the most recent month of data, Robux loads three replicas in the hot tier and one replica in the default cold tier.
...
See [Load rules](rule-configuration.md#load-rules) for more information on segment load rules. Visit [Tutorial: Configuring data retention](../tutorials/tutorial-retention.md) for an example of setting retention rules from the Robux web console.
...
```
robux.server.tier=hot
robux.server.priority=1
```

Example Historical in the cold tier:

```
robux.server.tier=_default_tier
robux.server.priority=0
```
...
```
robux.service=robux:broker-hot
```

Example config for a Broker in the cold tier:
```
robux.service=robux:broker-cold
```
...
```
robux.broker.select.tier=highestPriority
```
...
```
robux.router.defaultBrokerServiceName=robux:broker-cold
robux.router.tierToBrokerMap={"hot":"robux:broker-hot","_default_tier":"robux:broker-cold"}
```

If you plan to run Robux SQL queries, also [enable routing of SQL queries](../design/router.md#routing-of-sql-queries-using-strategies) by setting the following:
```
robux.router.sql.enable=true
```
