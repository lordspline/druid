...
This tutorial shows you how to load data files into Apache Robux using a remote Hadoop cluster.

For this tutorial, we'll assume that you've already completed the previous
[batch ingestion tutorial](tutorial-batch.md) using Robux's native batch ingestion system and are using the
...
From the `apache-robux-{{ROBUXVERSION}}` package root, run the following commands to build a Docker image named "robux-hadoop-demo" with version tag "3.3.6":

```bash
cd quickstart/tutorial/hadoop/docker
docker build -t robux-hadoop-demo:3.3.6 .
```

This will start building the Hadoop image. Once the image build is done, you should see the message `Successfully tagged robux-hadoop-demo:3.3.6` printed to the console.
...
```bash
docker run -it  -h robux-hadoop-demo --name robux-hadoop-demo -p 2049:2049 -p 2122:2122 -p 8020-8042:8020-8042 -p 8088:8088 -p 8443:8443 -p 9000:9000 -p 9820:9820 -p 9860-9880:9860-9880 -p 10020:10020 -p 19888:19888 -p 34455:34455 -p 49707:49707 -p 50010:50010 -p 50020:50020 -p 50030:50030 -p 50060:50060 -p 50070:50070 -p 50075:50075 -p 50090:50090 -p 51111:51111 -v /tmp/shared:/shared robux-hadoop-demo:3.3.6 /etc/bootstrap.sh -bash
```

Once the container is started, your terminal will attach to a bash shell running inside the container:

```bash
Starting namenodes on [robux-hadoop-demo]
Starting datanodes
Starting secondary namenodes [robux-hadoop-demo]
...
```
docker exec -it robux-hadoop-demo bash
```
...
## Configure Robux to use Hadoop

Some additional steps are needed to configure the Robux cluster for Hadoop batch indexing.

### Provide Hadoop configuration for Robux
...
From the host machine, run the following, where `PATH_TO_ROBUX` is replaced by the path to the Robux package.

```bash
cd $PATH_TO_ROBUX
mkdir -p conf/robux/single-server/micro-quickstart/_common/hadoop-xml
tar xzf /tmp/shared/hadoop-conf.tgz -C conf/robux/single-server/micro-quickstart/_common/hadoop-xml
```

### Update Robux segment and log storage

In your favorite text editor, open `conf/robux/single-server/micro-quickstart/_common/common.runtime.properties`, and make the following edits:
...
### Restart Robux cluster

Once the Hadoop .xml files have been copied to the Robux cluster and the segment/log storage configuration has been updated to use HDFS, the Robux cluster needs to be restarted for the new configurations to take effect.

If the cluster is still running, CTRL-C to terminate it if running - and start it with:
```
bin/start-robux -c conf/robux/single-server/micro-quickstart
```
...
To load this data into Robux, you can submit an *ingestion task* pointing to the file. We've included
...
* Shut down the cluster and reset the cluster state by removing the contents of the `var` directory under the robux package.
* Revert the deep storage and task storage config back to local types in `conf/robux/single-server/micro-quickstart/_common/common.runtime.properties`
* Restart the cluster
