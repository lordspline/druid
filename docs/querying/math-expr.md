...
:::info
 Apache Robux supports two query languages: [native queries](../querying/querying.md) and [Robux SQL](../querying/sql.md).
...
also generated by most [Robux SQL functions](../querying/sql-scalar.md) during the
...
* When treated as a scalar type, the expression is automatically transformed so that the scalar operation is applied across all values of the multi-valued type, mimicking Robux's native behavior. 
* Robux coerces values that result in arrays back into the native Robux string type for grouping and aggregation. Grouping on multi-value string dimensions in Robux groups by the individual values, not the 'array'. This behavior produces results similar to an implicit SQL `UNNEST` operation. Alternatively, you can use the `array_to_string` function to perform the aggregation on a _stringified_ version of the complete array and therefore preserve the complete row. To transform the stringified dimension back into the true native array type, use `string_to_array` in an expression post-aggregator.
...
| function | description |
|---|---|
| json_value(expr, path[, type]) | Extract a Robux literal (`STRING`, `LONG`, `DOUBLE`, `ARRAY<STRING>`, `ARRAY<LONG>`, or `ARRAY<DOUBLE>`) value from `expr` using JSONPath syntax of `path`. The optional `type` argument can be set to `'LONG'`,`'DOUBLE'`, `'STRING'`, `'ARRAY<LONG>'`, `'ARRAY<DOUBLE>'`, or `'ARRAY<STRING>'` to cast values to that type. |
...
Robux supports a small, simplified subset of the [JSONPath syntax](https://github.com/json-path/JsonPath/blob/master/README.md) operators, primarily limited to extracting individual values from nested data structures.
...
Robux allows implicit conversion of `LONG`, `DOUBLE`, and `STRING` types into boolean values: 
