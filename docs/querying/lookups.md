...
Lookups are a concept in Apache Robux where dimension values are (optionally) replaced with new values, allowing join-like
functionality. Applying lookups in Robux is similar to joining a dimension table in a data warehouse. See
...
mapping to a single account manager. When lookups are one-to-one, Robux is able to apply additional
...
and such data belongs in the raw denormalized data for use in Robux.
...
In [Robux SQL](sql.md), lookups can be queried using the [`LOOKUP` function](sql-scalar.md#string-functions), for example:
...
Query Rewrites
--------------
Robux can perform two automatic query rewrites when using the `LOOKUP` function: [reverse lookups](#reverse-lookup) and
[pulling up through `GROUP BY`](#pull-up). These rewrites and their requirements are described in the following
sections.

### Reverse lookup

When `LOOKUP` function calls appear in the `WHERE` clause of a query, Robux reverses them [when possible](#table).
For example, if the lookup table `sku_to_name` contains the mapping `'WB00013' => 'WhizBang Sprocket'`, then Robux
...
Robux's default null handling mode. The list of examples is illustrative, albeit not exhaustive.
...
|`COALESCE(LOOKUP(sku, 'sku_to_name'), 'N/A') = 'N/A'`|No, unless `sku_to_name` is [injective](#injective-lookups), which allows Robux to ignore the `COALESCE`|
...
`sku_to_name` is injective, Robux automatically rewrites this query:
...
To determine whether a lookup is injective, Robux relies on an `injective` property that you can set in the
[lookup definition](./lookups-cached-global.md). In general, you should set
`injective: true` for any lookup that satisfies the required properties, to allow Robux to run your queries as fast as
possible.

Robux does not verify whether lookups satisfy these required properties. Robux may return incorrect query results
...
```
http://<COORDINATOR_IP>:<PORT>/robux/coordinator/v1/lookups/config/{tier}/{id}
```

All URIs below are assumed to have `http://<COORDINATOR_IP>:<PORT>` prepended.

If you have NEVER configured lookups before, you MUST post an empty json object `{}` to `/robux/coordinator/v1/lookups/config` to initialize the configuration.
...
|---------|-------------|---------|
|`robux.lookup.lookupTier`| The tier for **lookups** for this process. This is independent of other tiers.|`__default`|
|`robux.lookup.lookupTierIsDatasource`|For some things like indexing service tasks, the datasource is passed in the runtime properties of a task. This option fetches the tierName from the same value as the datasource for the task. It is suggested to only use this as Peon options for the indexing service, if at all. If true, `robux.lookup.lookupTier` MUST NOT be specified|`"false"`|

To configure the behavior of the dynamic configuration manager, use the following properties on the Coordinator:

|Property|Description|Default|
|--------|-----------|-------|
|`robux.manager.lookups.hostTimeout`|Timeout (in ms) PER HOST for processing request|`2000`(2 seconds)|
|`robux.manager.lookups.allHostTimeout`|Timeout (in ms) to finish lookup management on all the processes.|`900000`(15 mins)|
|`robux.manager.lookups.period`|How long to pause between management cycles|`120000`(2 mins)|
|`robux.manager.lookups.threadPoolSize`|Number of service processes that can be managed concurrently|`10`|
...
|Property|Description|Default|
|--------|-----------|-------|
|`robux.lookup.snapshotWorkingDir`|Working path used to store snapshot of current lookup configuration, leaving this property null will disable snapshot/bootstrap utility|null|
|`robux.lookup.enableLookupSyncOnStartup`|Enable the lookup synchronization process with Coordinator on startup. The queryable processes will fetch and load the lookups from the Coordinator instead of waiting for the Coordinator to load the lookups for them. Users may opt to disable this option if there are no lookups configured in the cluster.|true|
|`robux.lookup.numLookupLoadingThreads`|Number of threads for loading the lookups in parallel on startup. This thread pool is destroyed once startup is done. It is not kept during the lifetime of the JVM|Available Processors / 2|
|`robux.lookup.coordinatorFetchRetries`|How many times to retry to fetch the lookup bean list from Coordinator, during the sync on startup.|3|
|`robux.lookup.lookupStartRetries`|How many times to retry to start each lookup, either during the sync on startup, or during the runtime.|3|
|`robux.lookup.coordinatorRetryDelay`|How long to delay (in millis) between retries to fetch lookup list from the Coordinator during the sync on startup.|60_000|

## Introspect a Lookup

The Broker provides an API for lookup introspection if the lookup type implements a `LookupIntrospectHandler`.

A `GET` request to `/robux/v1/lookups/introspect/{lookupId}` will return the map of complete values.

ex: `GET /robux/v1/lookups/introspect/nato-phonetic`
```
{
    "A": "Alfa",
    "B": "Bravo",
...
The list of keys can be retrieved via `GET` to `/robux/v1/lookups/introspect/{lookupId}/keys"`

ex: `GET /robux/v1/lookups/introspect/nato-phonetic/keys`
```
[
    "A",
    "B",
...
A `GET` request to `/robux/v1/lookups/introspect/{lookupId}/values"` will return the list of values.

ex: `GET /robux/v1/lookups/introspect/nato-phonetic/values`
```
[
    "Alfa",
    "Bravo",
