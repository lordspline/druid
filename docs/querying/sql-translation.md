...
:::info
 ...
 This document describes the Robux SQL language.
:::

Robux uses [Apache Calcite](https://calcite.apache.org/) to parse and plan SQL queries.
Robux translates SQL statements into its [native JSON-based query language](querying.md).
In general, the slight overhead of translating SQL on the Broker is the only minor performance penalty to using Robux SQL compared to native queries.
...
translating Robux SQL queries to native queries.
...
3. Currently, Robux does not support pushing down predicates (condition and filter) past a Join (i.e. into 
...
[raise an issue on GitHub](https://github.com/apache/robux/issues/new/choose). A reproducible test case is always
appreciated.
...
- a `PLAN` column that contains a JSON array of native queries that Robux will run
...
We can see this for ourselves using Robux's [request logging](../configuration/index.md#request-logging) feature. After
...
Robux SQL uses four different native query types.
...
have aggregation functions but no GROUP BY. This query type takes advantage of the fact that Robux segments are sorted
by time.
...
- [GroupBy](groupbyquery.md) is used for all other aggregations, including any nested aggregation queries. Robux's
...
possible, which allows Robux to use its global time index to quickly prune the set of data that must be scanned.
...
2. If a join cannot be handled directly by a native [join datasource](datasource.md#join) as written, Robux SQL
...
3. Robux SQL does not currently reorder joins to optimize queries.
...
Robux SQL will use approximate algorithms in some situations:
...
algorithm. Robux SQL will switch to exact distinct counts if you set "useApproximateCountDistinct" to "false", either
...
approximate algorithm. Robux SQL will switch to an exact grouping algorithm if you set "useApproximateTopN" to "false",
...
See [GitHub issue 11544](https://github.com/apache/robux/issues/11544) for more details.
...
Robux does not support all SQL features. In particular, the following features are not supported.
...
- DDL and DML.
- Using Robux-specific functions like `TIME_PARSE` and `APPROX_QUANTILE_DS` on [system tables](sql-metadata-tables.md).

Additionally, some Robux native query features are not supported by the SQL language. Some unsupported Robux features
include:

- [Inline datasources](datasource.md#inline).
- [Spatial filters](geo.md).
- [Multi-value dimensions](sql-data-types.md#multi-value-strings) are only partially implemented in Robux SQL. There are known
