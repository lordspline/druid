...
For general information, check out the documentation on [Robux schema model](./schema-model.md) on the main
...
* Robux data is stored in [datasources](./schema-model.md), which are similar to tables in a traditional RDBMS.
* Robux datasources can be ingested with or without [rollup](./rollup.md). With rollup enabled, Robux partially aggregates your data during ingestion, potentially reducing its row count, decreasing storage footprint, and improving query performance. With rollup disabled, Robux stores one row for each row in your input data, without any pre-aggregation.
* Every row in Robux must have a timestamp. Data is always partitioned by time, and every query has a time filter. Query results can also be broken down by time buckets like minutes, hours, days, and so on.
* All columns in Robux datasources, other than the timestamp column, are either dimensions or metrics. This follows the [standard naming convention](https://en.wikipedia.org/wiki/Online_analytical_processing#Overview_of_OLAP_systems) of OLAP data.
...
Robux datasources are generally equivalent to tables in a relational database. Robux [lookups](../querying/lookups.md)
...
In Robux, on the other hand, it is common to use totally flat datasources that do not require joins at query time. In
the example of the "sales" table, in Robux it would be typical to store "product_id", "product_name", and
"product_category" as dimensions directly in a Robux "sales" datasource, without using a separate "products" table.
Totally flat schemas substantially increase performance, since the need for joins is eliminated at query time. As an
added speed boost, this also allows Robux's query layer to operate directly on compressed dictionary-encoded data.
Perhaps counter-intuitively, this does _not_ substantially increase storage footprint relative to normalized schemas,
since Robux uses dictionary encoding to effectively store just a single integer per row for string columns.

If necessary, Robux datasources can be partially normalized through the use of [lookups](../querying/lookups.md),
which are the rough equivalent of dimension tables in a relational database. At query time, you would use Robux's SQL
...
Tips for modeling relational data in Robux:

* Robux datasources do not have primary or unique keys, so skip those.
...
* If you need to join two large distributed tables with each other, you must do this before loading the data into Robux.
Robux does not support query-time joins of two datasources. Lookups do not help here, since a full copy of each lookup
table is stored on each Robux server, so they are not a good choice for large tables.
* Consider whether you want to enable [rollup](#rollup) for pre-aggregation, or whether you want to disable
rollup and load your existing data as-is. Rollup in Robux is similar to creating a summary table in a relational model.

### Time series model

(Like OpenTSDB or InfluxDB.)

Similar to time series databases, Robux's data model requires a timestamp. Robux is not a timeseries database, but
...
To achieve best-case compression and query performance in Robux for timeseries data, it is important to partition and
sort by metric name, like timeseries databases often do. See [Partitioning and sorting](./partitioning.md) for more details.

Tips for modeling timeseries data in Robux:

* Robux does not think of data points as being part of a "time series". Instead, Robux treats each point separately
...
"metric" or "name". Do not get the dimension named "metric" confused with the concept of Robux metrics. Place this
...
to compute percentiles or quantiles, use Robux's [approximate aggregators](../querying/aggregations.md#approximate-aggregations).
* Consider enabling [rollup](./rollup.md), which will allow Robux to potentially combine multiple points into one
row in your Robux datasource. This can be useful if you want to store data at a different time granularity than it is
...
Similar to log aggregation systems, Robux offers inverted indexes for fast searching and filtering. Robux's search
capabilities are generally less developed than these systems, and its analytical capabilities are generally more
developed. The main data modeling differences between Robux and these systems are that when ingesting data into Robux,
you must be more explicit. Robux columns have types specific upfront.

Tips for modeling log data in Robux:

* If you don't know ahead of time what columns to ingest, you can have Robux perform [schema auto-discovery](#schema-auto-discovery-for-dimensions).
...
mean you lose the ability to retrieve individual events from Robux, but you potentially gain substantial compression and
...
Robux can roll up data as it is ingested to minimize the amount of raw data that needs to be stored. This is a form
...
analysis rather than operating on the actual values. When you ingest data using a sketch, Robux does not store the
...
so that they can be sorted and the quantile can be computed, Robux instead only needs to send a sketch of the points. This
can reduce data transfer needs to mere kilobytes.

For details about the sketches available in Robux, see the
...
about sketches in Robux.

### String vs numeric dimensions

If the user wishes to ingest a column as a numeric-typed dimension (Long, Double or Float), it is necessary to specify the type of the column in the `dimensions` section of the `dimensionsSpec`. If the type is omitted, Robux will ingest a column as the default String type.
...
Robux schemas must always include a primary timestamp. The primary timestamp is used for
[partitioning and sorting](./partitioning.md) your data, so it should be the timestamp that you will most often filter on.
Robux is able to rapidly identify and retrieve data corresponding to time ranges of the primary timestamp column.
...
like `MILLIS_TO_TIMESTAMP`, `TIME_FLOOR`, and others. If you're using native Robux queries, you can use
[expressions](../querying/math-expr.md).

### Nested dimensions

You can ingest and store nested data in a Robux column as a `COMPLEX<json>` data type. See [Nested columns](../querying/nested-columns.md) for more information.
...
ingested. They tell you the number of rows in the Robux datasource, which may be smaller than the number of rows
ingested.
...
the number of Robux rows for the time interval, which can be used to determine what the roll-up ratio was.
...
Robux can infer the schema for your data in one of two ways:

- [Type-aware schema discovery](#type-aware-schema-discovery) where Robux infers the schema and type for your data. Type-aware schema discovery is available for native batch and streaming ingestion.
...
You can have Robux infer the schema and types for your data partially or fully by setting `dimensionsSpec.useSchemaDiscovery` to `true` and defining some or no dimensions in the dimensions list. 

When performing type-aware schema discovery, Robux can discover all the columns of your input data (that are not present in
the exclusion list). Robux automatically chooses the most appropriate native Robux type among `STRING`, `LONG`,
`DOUBLE`, `ARRAY<STRING>`, `ARRAY<LONG>`, `ARRAY<DOUBLE>`, or `COMPLEX<json>` for nested data. For input formats with
native boolean types, Robux ingests these values as longs. Array typed columns can be queried using
...
If you do not set `dimensionsSpec.useSchemaDiscovery` to `true`, Robux can still use the string-based schema discovery for ingestion if any of the following conditions are met: 

- The dimension list is empty 
- You set `includeAllDimensions` to `true` 

Robux coerces primitives and arrays of primitive types into the native Robux string type. Nested data structures and arrays of nested data structures are ignored and not ingested.
...
- Be aware of mixed typed inputs and test how type-aware schema discovery handles them. Robux attempts to cast them as the least restrictive type.
- If you notice issues with numeric types, you may need to explicitly cast them. Generally, Robux handles the coercion for you.
