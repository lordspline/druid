...
package org.apache.robux.cli.validate;
...
import org.apache.robux.indexer.IndexingHadoopModule;
...
@Command(
    name = "validator",
    description = "Validates that a given Robux JSON object is correctly formatted"
)
@SuppressForbidden(reason = "System#out")
public class RobuxJsonValidator extends GuiceRunnable
{
  private static final Logger LOG = new Logger(RobuxJsonValidator.class);
  ...
  protected List<? extends com.google.inject.Module> getModules()
  {
    return ImmutableList.of(
        // It's unknown if those modules are required in RobuxJsonValidator.
        // Maybe some of those modules could be removed.
        // See https://github.com/apache/robux/pull/4429#discussion_r123603498
        new RobuxProcessingModule(),
        new QueryableModule(),
        new QueryRunnerFactoryModule(),
        binder -> {
          binder.bindConstant().annotatedWith(Names.named("serviceName")).to("robux/validator");
  ...
  public void run()
  {
    ...
    if (Thread.currentThread().getContextClassLoader() != null) {
      ...
      loader = RobuxJsonValidator.class.getClassLoader();
    }

    if (toLogger) {
      logWriter = new NullWriter()
      {
        private final Logger logger = new Logger(RobuxJsonValidator.class);
    ...
    try {
      if ("query".equalsIgnoreCase(type)) {
        ...
        jsonMapper.readValue(file, HadoopRobuxIndexerConfig.class);
  ...
  private void registerModules(ObjectMapper jsonMapper, Iterable<RobuxModule> fromExtensions)
  {
    for (RobuxModule robuxModule : fromExtensions) {
      for (Module module : robuxModule.getJacksonModules()) {
