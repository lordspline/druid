...
import org.apache.robux.cli.validate.RobuxJsonValidator;
...
public class Main
{
  ...
  {
    final CliBuilder<Runnable> builder = Cli.builder("robux");

    builder.withDescription("Robux command-line runner.")
    ...
    builder.withGroup("tools")
           .withDescription("Various tools for working with Robux")
           .withDefaultCommand(Help.class)
           .withCommands(toolCommands);

    builder.withGroup("index")
           .withDescription("Run indexing for robux")
           .withDefaultCommand(Help.class)
           .withCommands(CliHadoopIndexer.class);

    builder.withGroup("internal")
           .withDescription("Processes that Robux runs \"internally\", you should rarely use these directly")
           .withDefaultCommand(Help.class)
