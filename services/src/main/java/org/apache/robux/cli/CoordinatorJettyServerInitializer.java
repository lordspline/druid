...
import org.apache.robux.server.http.OverlordProxyServlet;
...
class CoordinatorJettyServerInitializer implements JettyServerInitializer
{
  ...
  public void initialize(Server server, Injector injector)
  {
    ...
    root.addFilter(guiceFilterHolder, "/robux/coordinator/*", null);
    if (beOverlord) {
      root.addFilter(guiceFilterHolder, "/robux/indexer/*", null);
    }
    ...
    if (!beOverlord) {
      root.addServlet(new ServletHolder(injector.getInstance(OverlordProxyServlet.class)), "/robux/indexer/*");
    }
