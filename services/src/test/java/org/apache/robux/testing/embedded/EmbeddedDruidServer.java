...
import org.apache.robux.cli.ServerRunnable;
...
/**
 * An embedded Robux server used in embedded tests.
 ...
 */
public abstract class EmbeddedRobuxServer<T extends EmbeddedRobuxServer<T>> implements EmbeddedResource
{
  private static final Logger log = new Logger(EmbeddedRobuxServer.class);
  ...
  EmbeddedRobuxServer()
  {
    ...
    beforeStartHooks.add(
        (cluster, self) -> {
          ...
          self.addProperty("robux.host", "localhost");
          self.addProperty("robux.indexer.task.baseDir", taskDirectory);
          self.addProperty("robux.indexer.logs.directory", logsDirectory);
          self.addProperty("robux.storage.storageDirectory", storageDirectory);

          // Add properties for Zookeeper
          if (cluster.getZookeeper() != null) {
            self.addProperty("robux.zk.service.host", cluster.getZookeeper().getConnectString());
          }
  ...
  @Override
  public void beforeStart(EmbeddedRobuxCluster cluster)
  {
   ...
   * {@code runtime.properties} file in a real Robux cluster, and override the
   * common properties specified via {@link EmbeddedRobuxCluster#addCommonProperty}.
   */
  ...
  /**
   * Adds a {@link BeforeStart} to run as part of {@link #beforeStart(EmbeddedRobuxCluster)}
   */
   ...
   */
  public final EmbeddedRobuxServer<T> setServerMemory(long serverMemory)
  {
   ...
   */
  public final EmbeddedRobuxServer<T> setServerDirectMemory(long serverDirectMemory)
  {
  ...
  /**
   * Called from {@link EmbeddedRobuxCluster#addServer(EmbeddedRobuxServer)} to
  ...
  /**
   * Creates a {@link ServerRunnable} corresponding to a specific Robux service.
   * Implementations of this class MUST NOT return a {@link ServerRunnable} that
   * overrides any default Robux behaviour so that the embedded cluster closely
   * replicates a real cluster. If an override is needed, it must be done using
   * extensions and Robux properties, which are visible to the unit test so that
   * there is no hidden config.
   *
   * @see EmbeddedRobuxCluster#addExtension
   * @see EmbeddedRobuxCluster#addCommonProperty
   * @see EmbeddedRobuxServer#addProperty
   */
   ...
   * the Robux server have been initialized.
   */
   ...
   * All implementations of {@link EmbeddedRobuxServer} must use this binding in
  ...
  interface LifecycleInitHandler
  {
     ...
     * All implementations of {@link EmbeddedRobuxServer} must call this method
  ...
  {
    /**
     * Allows a {@link EmbeddedRobuxServer} to perform additional initialization before starting
     *
     * @param cluster - the {@link EmbeddedRobuxCluster} the {@link EmbeddedRobuxServer} is part of
     * @param self    - the {@link EmbeddedRobuxServer} to perform initialization on
     */
    void run(EmbeddedRobuxCluster cluster, EmbeddedRobuxServer<?> self);
  }
}
