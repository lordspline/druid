...
import org.apache.robux.query.http.ClientSqlQuery;
...
/**
 * Contains various utility methods to interact with an {@link EmbeddedRobuxCluster}.
 *
...
{
  private final EmbeddedRobuxCluster cluster;

  EmbeddedClusterApis(EmbeddedRobuxCluster cluster)
  {
  ...
  public void waitForTaskToSucceed(String taskId, EmbeddedOverlord overlord)
  {
    overlord.latchableEmitter().waitForEvent(
        event -> event.hasMetricName(TaskMetrics.RUN_DURATION)
                      .hasDimension(RobuxMetrics.TASK_ID, taskId)
    );
   ...
   * post JSON payloads to a Robux API. Using a generic map allows the client
