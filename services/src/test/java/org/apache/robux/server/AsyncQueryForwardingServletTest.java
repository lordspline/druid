...
import org.apache.robux.common.utils.SocketUtil;
...
import org.apache.robux.server.router.RendezvousHashAvaticaConnectionBalancer;
import org.apache.robux.server.security.AllowAllAuthorizer;
...
public class AsyncQueryForwardingServletTest extends BaseJettyTest
{
  ...
  public void setup() throws Exception
  {
    ...
    final RobuxNode node = injector.getInstance(Key.get(RobuxNode.class, Self.class));
            ...
            new Module()
            {
              ...
              public void configure(Binder binder)
              {
                JsonConfigProvider.bindInstance(
                    ...
                    new RobuxNode("test", "localhost", false, null, null, true, false)
                );
  ...
  public void testDeleteBroadcast() throws Exception
  {
    ...
    final URL url = new URL("http://localhost:" + port + "/robux/v2/abc123");
    final HttpURLConnection post = (HttpURLConnection) url.openConnection();
  ...
  public void testSqlQueryProxy() throws Exception
  {
    ...
    properties.setProperty("robux.router.sql.enable", "true");
  ...
  {
    final TimeseriesQuery query = Robuxs.newTimeseriesQueryBuilder()
                                        .dataSource("foo")
                                        .intervals("2000/P1D")
  ...
  ) throws Exception
  {
    ...
    requestMock.setAttribute("org.apache.robux.proxy.objectMapper", jsonMapper);
    EasyMock.expectLastCall();
    EasyMock.expect(requestMock.getRequestURI())
            .andReturn(isNativeSql ? "/robux/v2/sql" : (isJDBCSql ? "/robux/v2/sql/avatica" : "/robux/v2/"));
    ...
    requestMock.setAttribute(
        "org.apache.robux.proxy." + (isNativeSql ? "sqlQuery" : (isJDBCSql ? "avaticaQuery" : "query")),
    ...
    EasyMock.expect(requestMock.getAttribute("org.apache.robux.proxy.avaticaQuery"))
            .andReturn(isJDBCSql ? query : null);
    EasyMock.expect(requestMock.getAttribute("org.apache.robux.proxy.query"))
            .andReturn(isJDBCSql ? null : (isNativeSql ? null : query));
    EasyMock.expect(requestMock.getAttribute("org.apache.robux.proxy.sqlQuery"))
    ...
    requestMock.setAttribute("org.apache.robux.proxy.to.host", "1.2.3.4:9999");
    EasyMock.expectLastCall();
    requestMock.setAttribute("org.apache.robux.proxy.to.host.scheme", "http");
    EasyMock.expectLastCall();
    EasyMock.expect(requestMock.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT)).andReturn(new AuthenticationResult("userA", "basic", "basic", null));
    if (isFailure) {
    ...
    HttpResponse response = new HttpResponse(proxyRequestMock, ImmutableList.of())
    {
      ...
      public HttpFields getHeaders()
      {
        ...
        if (isJDBCSql) {
          httpFields.add(new HttpField("X-Robux-SQL-Query-Id", "jdbcDummy"));
        } else if (isNativeSql) {
          httpFields.add(new HttpField("X-Robux-SQL-Query-Id", "dummy"));
        }
        return httpFields;
      }
    };
  ...
  public static class ProxyJettyServerInit implements JettyServerInitializer
  {
    ...
    @Inject
    public ProxyJettyServerInit(@Self RobuxNode node)
    {
      this.node = node;
    }
    ...
    public void initialize(Server server, Injector injector)
    {
      ...
      {
        @Override
        public org.apache.robux.client.selector.Server pickServer(Query query)
        {
          return new TestServer("http", "localhost", node.getPlaintextPort());
        }

        @Override
        public org.apache.robux.client.selector.Server pickDefaultServer()
        {
          return new TestServer("http", "localhost", node.getPlaintextPort());
        }

        @Override
        public Collection<org.apache.robux.client.selector.Server> getAllServers()
        {
      ...
      ServletHolder holder = new ServletHolder(
          new AsyncQueryForwardingServlet(
              ...
              injector.getInstance(RobuxHttpClientConfig.class),
          ...
          )
          {
            ...
            protected String rewriteURI(HttpServletRequest request, String scheme, String host)
            {
              ...
              if (uri.contains("/robux/v2")) {
                return URI.create(StringUtils.replace(uri, "/robux/v2", "/default")).toString();
              }
      ...
      root.addServlet(holder, "/robux/v2/*");
  ...
  public void testGetAvaticaConnectionId() throws JsonProcessingException
  {
    ...
    final String query = "SELECT someColumn FROM robux.someTable WHERE someColumn IS NOT NULL";
    ...
    final List<? extends Service.Request> jsonRequests = ImmutableList.of(
        ...
        new Service.SchemasRequest(connectionId, "robux", null),
        new Service.TablesRequest(connectionId, "robux", "robux", null, null),
        new Service.ColumnsRequest(connectionId, "robux", "robux", "someTable", null),
  ...
  private static class TestServer implements org.apache.robux.client.selector.Server
  {
  ...
  private ArgumentCaptor<Exception> captureExceptionHandledByServlet(ServerConfig serverConfig, ServletTestAction action) throws Exception
  {
    ...
    Mockito.when(request.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT)).thenReturn(new AuthenticationResult("userA", "basic", "basic", null));
