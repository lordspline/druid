...
public class JavaScriptTieredBrokerSelectorStrategyTest
{
  ...
  public void testGetBrokerServiceName()
  {
    ...
    tierBrokerMap.put("fast", "robux/broker");
    tierBrokerMap.put("slow", "robux/slowBroker");
    ...
    Assert.assertEquals(
        ...
        STRATEGY.getBrokerServiceName(
            tieredBrokerConfig,
            Robuxs.newTimeBoundaryQueryBuilder().dataSource("test").bound("maxTime").build()
        )
    );

    Assert.assertEquals(
        Optional.of("robux/slowBroker"),
        STRATEGY.getBrokerServiceName(
            tieredBrokerConfig,
