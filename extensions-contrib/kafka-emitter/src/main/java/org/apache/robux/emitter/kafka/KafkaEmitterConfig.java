...
package org.apache.robux.emitter.kafka;
...
import org.apache.robux.metadata.MapStringDynamicConfigProvider;
  ...
  )
  {
    ...
    if (bootstrapServers == null) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          ...
                          .build("robux.emitter.kafka.bootstrap.servers must be specified.");
    }

    if (this.eventTypes.contains(EventType.METRICS) && metricTopic == null) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          .ofCategory(RobuxException.Category.NOT_FOUND)
                          .build("robux.emitter.kafka.metric.topic must be specified"
                                 + " if robux.emitter.kafka.event.types contains %s.", EventType.METRICS);
    }
    if (this.eventTypes.contains(EventType.ALERTS) && alertTopic == null) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          .ofCategory(RobuxException.Category.NOT_FOUND)
                          .build("robux.emitter.kafka.alert.topic must be specified"
                                 + " if robux.emitter.kafka.event.types contains %s.", EventType.ALERTS);
    }
    if (this.eventTypes.contains(EventType.REQUESTS) && requestTopic == null) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          .ofCategory(RobuxException.Category.NOT_FOUND)
                          .build("robux.emitter.kafka.request.topic must be specified"
                                 + " if robux.emitter.kafka.event.types contains %s.", EventType.REQUESTS);
    }
    if (this.eventTypes.contains(EventType.SEGMENT_METADATA) && segmentMetadataTopic == null) {
      throw RobuxException.forPersona(RobuxException.Persona.OPERATOR)
                          .ofCategory(RobuxException.Category.NOT_FOUND)
                          .build("robux.emitter.kafka.segmentMetadata.topic must be specified"
                                 + " if robux.emitter.kafka.event.types contains %s.", EventType.SEGMENT_METADATA);
    }
