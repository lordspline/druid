...
public interface RobuxToGraphiteEventConverter
{
  /**
   * This function acts as a filter. It returns <tt>null</tt> if the event is not supposed to be emitted to Graphite
   * Also This function will define the mapping between the robux event dimension's values and Graphite metric Path
   *
   * @param serviceMetricEvent Robux event ot type {@link ServiceMetricEvent}
   *
   * @return {@link GraphiteEvent} or <tt>null</tt>
   */
  GraphiteEvent robuxEventToGraphite(ServiceMetricEvent serviceMetricEvent);
}
