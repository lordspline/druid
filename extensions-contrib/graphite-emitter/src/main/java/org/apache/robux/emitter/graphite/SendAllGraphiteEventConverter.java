 ...
 * <namespacePrefix>.[<robux service name>].[<robux hostname>].<dimensions values ordered by dimension's name>.<metric>
 * <p>
 * Note that this path will be sanitized by replacing all the `.` or `space` to `_` {@link GraphiteEmitter#sanitize(String)}
 */

@JsonTypeName("all")
public class SendAllGraphiteEventConverter implements RobuxToGraphiteEventConverter
{
  @JsonProperty
