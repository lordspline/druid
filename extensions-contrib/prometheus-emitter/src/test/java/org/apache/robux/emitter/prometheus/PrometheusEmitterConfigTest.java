...
public class PrometheusEmitterConfigTest
{
  ...
  public void testEmitterConfigWithBadExtraLabels()
  {
    ...
    Exception exception = Assert.assertThrows(RobuxException.class, () -> {
  ...
  public void testDefaultConstructor()
  {
    ...
    Assert.assertEquals("robux", config.getNamespace());
  ...
  {
    PrometheusEmitterConfig config = new PrometheusEmitterConfig(PrometheusEmitterConfig.Strategy.exporter, "robux", null, 8080, null, true, true, null, null, null, null);
    Assert.assertEquals(PrometheusEmitterConfig.Strategy.exporter, config.getStrategy());
    Assert.assertEquals("robux", config.getNamespace());
  ...
  {
    PrometheusEmitterConfig config = new PrometheusEmitterConfig(PrometheusEmitterConfig.Strategy.pushgateway, "robux", null, null, "localhost:9091", false, false, 30, null, true, 5000L);
  ...
  {
    RobuxException robuxException = Assert.assertThrows(RobuxException.class, () -> {
      Map<String, String> extraLabels = new HashMap<>();
    ...
    Assert.assertEquals("Invalid metric label name [invalid label]. Label names must conform to the pattern [[a-zA-Z_:][a-zA-Z0-9_:]*].", robuxException.getMessage());
  }
  ...
  public void testNegativeWaitForShutdownDelay()
  {
    ...
    Assert.assertEquals("Invalid value for waitForShutdownDelay[-1] specified, waitForShutdownDelay must be >= 0.", robuxException.getMessage());
  }
}
