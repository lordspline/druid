...
public class OssDataSegmentMover implements DataSegmentMover
{
  ...
  ) throws IOException, SegmentLoadingException
  {
    if (srcBucket.equals(dstBucket) && srcPath.equals(dstPath)) {
      log.info("No need to move file[%s://%s/%s] onto itself", OssStorageRobuxModule.SCHEME, srcBucket, srcPath);
      return;
    }
    ...
    if (client.doesObjectExist(srcBucket, srcPath)) {
      ...
      if (listResult.getObjectSummaries().size() == 0) {
        // should never happen
        throw new ISE("Unable to list object [%s://%s/%s]", OssStorageRobuxModule.SCHEME, srcBucket, srcPath);
      }
  ...
  private void deleteWithRetriesSilent(final String bucket, final String path)
  {
    try {
      ...
      log.error(e, "Failed to delete file [%s://%s/%s], giving up", OssStorageRobuxModule.SCHEME, bucket, path);
    }
  }
    ...
    RetryUtils.retry(
        () -> {
          try {
            ...
            log.info(e, "Error while trying to delete [%s://%s/%s]", OssStorageRobuxModule.SCHEME, bucket, path);
            throw e;
          }
        },
        OssUtils.RETRYABLE,
