...
{
  private static final String SCHEME = OssStorageRobuxModule.SCHEME;
  ...
  {
    if (uri.getScheme().equalsIgnoreCase(OssStorageRobuxModule.SCHEME_ZIP)) {
      uri = URI.create(SCHEME + uri.toString().substring(OssStorageRobuxModule.SCHEME_ZIP.length()));
    }
