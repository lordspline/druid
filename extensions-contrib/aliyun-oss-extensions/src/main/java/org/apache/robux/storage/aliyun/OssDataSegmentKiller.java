...
public class OssDataSegmentKiller implements DataSegmentKiller
{
  ...
  public void kill(DataSegment segment) throws SegmentLoadingException
  {
    try {
      ...
      if (client.doesObjectExist(bucket, path)) {
        log.info("Removing index file[%s://%s/%s] from aliyun OSS!", OssStorageRobuxModule.SCHEME, bucket, path);
  ...
  public void killAll() throws IOException
  {
    if (segmentPusherConfig.getBucket() == null || segmentPusherConfig.getPrefix() == null) {
      throw new ISE(
          "Cannot delete all segment from aliyun OSS Deep Storage since robux.storage.bucket and robux.storage.baseKey are not both set.");
    }
