...
package org.apache.robux.storage.cloudfiles;
...
/**
 */
public class CloudFilesStorageRobuxModule implements RobuxModule
{

  private static final Logger log = new Logger(CloudFilesStorageRobuxModule.class);
  ...
  {
    log.info("Configuring CloudFilesStorageRobuxModule...");
    JsonConfigProvider.bind(binder, "robux.storage", CloudFilesDataSegmentPusherConfig.class);
    JsonConfigProvider.bind(binder, "robux.cloudfiles", CloudFilesAccountConfig.class);

    Binders.dataSegmentPusherBinder(binder).addBinding(SCHEME).to(CloudFilesDataSegmentPusher.class)
           .in(LazySingleton.class);

    log.info("Configured CloudFilesStorageRobuxModule.");
  }
