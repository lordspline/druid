...
public class InfluxdbEmitter implements Emitter
{
  ...
  {
    // split Robux metric on slashes and join middle parts (if any) with "_"
    ...
    // measurement
    StringBuilder payload = new StringBuilder("robux_");
    payload.append(parts[0]);

    // tags
    StringBuilder tag = new StringBuilder(",service=");
    tag.append(getValue("service", event));
    String metricTag = parts.length == 2 ? "" : ",metric=robux_" + metric;
    ...
    // fields
    payload.append(StringUtils.format(" robux_%1$s=%2$s", parts[parts.length - 1], getValue("value", event)));
