...
package org.apache.robux.emitter.opentsdb;
...
public class EventConverterTest
{
  ...
  public void setUp()
  {
    converterWithNamespacePrefix = new EventConverter(new ObjectMapper(), null, "robux");
    converterWithNamespacePrefixContainingSpace = new EventConverter(new ObjectMapper(), null, "legendary robux");
  ...
  public void testConvertWithNamespacePrefix()
  {
    ...
    ServiceMetricEvent configuredEvent = new ServiceMetricEvent.Builder()
        ...
        .build("robux:broker", "127.0.0.1:8080");

    Map<String, Object> expectedTags = new HashMap<>();
    expectedTags.put("service", "robux_broker");
    ...
    Assert.assertEquals("robux.query.time", opentsdbEvent.getMetric());
  ...
  public void testConvertWithNamespacePrefixContainingSpace()
  {
    ...
    Assert.assertEquals("legendary_robux.query.time", opentsdbEvent.getMetric());
    Assert.assertEquals(dateTime.getMillis() / 1000L, opentsdbEvent.getTimestamp());
    Assert.assertEquals(10, opentsdbEvent.getValue());
    Assert.assertEquals(expectedTags, opentsdbEvent.getTags());
