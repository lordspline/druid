...
package org.apache.robux.emitter.statsd;
...
public class StatsDEmitter implements Emitter
{
  ...
  private static final char ROBUX_METRIC_SEPARATOR = '/';
  private static final String ROBUX_DEFAULT_PREFIX = "robux";
  ...
  private static final String TAG_SERVICE = "robux_service";
  ...
  void emitMetric(ServiceMetricEvent metricEvent)
  {
    ...
    if (config.isDogstatsd() && config.isDogstatsdServiceAsTag()) {
      ...
      nameBuilder.add(ROBUX_DEFAULT_PREFIX);
    } else {
    ...
    if (statsDMetric != null) {
      ...
      fullName = StringUtils.replaceChar(fullName, ROBUX_METRIC_SEPARATOR, config.getSeparator());
