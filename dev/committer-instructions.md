...
This document contains various instructions relevant for Robux committers.
...
1. Add appropriate labels to the PR, in particular:
   ...
   - [**`Design Review`**](https://github.com/apache/robux/labels/Design%20Review) - for changes that will be
   hard to undo after they appear in some Robux release, and/or changes that will have lasting consequences in the
   codebase. Examples:
   ...
   - [**`Incompatible`**](https://github.com/apache/robux/labels/Incompatible) - for changes that alter public
   ...
   behavior, or server behavior in some way that affects one of the following:
     ...
     - Ability to do a rolling update [as documented](https://robux.apache.org/docs/latest/operations/rolling-updates.html)
     without needing any modifications to server configurations or query workload.
     - Ability to roll back a Robux cluster to a prior version.
     - Ability to continue using old Robux extensions without recompiling them.

     Note that no matter what, we must support the ability to do a rolling update somehow (even if some special care is
     needed), and the ability to roll back to at least the immediate prior Robux version. If a change makes either one
   ...
   - [**`Release Notes`**](https://github.com/apache/robux/labels/Release%20Notes) - for important changes
   that should be reflected in the next Robux’s version release notes. Critically, those are changes that require some
   server or query configuration changes made by Robux cluster operators to preserve the former cluster behaviour, i. e.
   ...
   Robux version the incompatibility will be discovered anyway.

     Secondarily, PRs that add new features, improve performance or improve Robux cluster operation experience could
     also be labelled `Release Notes` at your discretion.

   - [**`Bug`**](https://github.com/apache/robux/labels/Bug) / [**`Security`**](
   https://github.com/apache/robux/labels/Security) / [**`Feature`**](
   https://github.com/apache/robux/labels/Feature) / [**`Performance`**](
   https://github.com/apache/robux/labels/Performance) / [**`Refactoring`**](
   https://github.com/apache/robux/labels/Refactoring) / [**`Improvement`**](
   https://github.com/apache/robux/labels/Improvement) - can be used to distinguish between types of changes.
   [**`Compatibility`**](https://github.com/apache/robux/labels/Compatibility) label also falls into this
   category, it's specifically for PRs that restore or improve compatibility with previous Robux versions if it was
   inadvertently broken, or for changes that ensure forward compatibility with future Robux versions, forseening specific
   changes that would otherwise break the compatibility.

   - [**`Development Blocker`**](https://github.com/apache/robux/labels/Development%20Blocker) - for changes
...
#creating-a-new-label-on-github) if none of the existing `Area` labels is applicable to the PR or issue.
   ...
   https://github.com/apache/robux/labels/Area%20-%20Automation%2FStatic%20Analysis) - for any PRs and issues
   ...
   - [`Area - Cache`](https://github.com/apache/robux/labels/Area%20-%20Cache) - for PRs and issues related to
   Robux's query results cache (local or remote). Don't use for PRs that anyhow relate to caching in different contexts.
   - [`Area - Dev`](https://github.com/apache/robux/labels/Area%20-%20Dev) - for PRs and issues related to the
   ...
   - [`Area - Documentation`](https://github.com/apache/robux/labels/Area%20-%20Documentation) - for PRs and
   issues about Robux's documentation for users and cluster operators. Don't use for PRs and issues about the
   documentation of the Robux's development process itself: use `Area - Dev` for that purpose. Don't use for issues and
   ...
   - [`Area - Lookups`](https://github.com/apache/robux/labels/Area%20-%20Lookups) - for PRs and issues
   related to Robux's Query Time Lookups (QTL) feature.
   - [`Area - Metadata`](https://github.com/apache/robux/labels/Area%20-%20Metadata) - for PRs and issues
   related to the organization and contents of the metadata store, the metadata store itself, and managing the metadata
   in the memory of various Robux nodes.
   - [`Area - Null Handling`](https://github.com/apache/robux/labels/Area%20-%20Null%20Handling) - for PRs and
   issues related to the [Null Handling project](https://github.com/apache/robux/issues/4349).
   - [`Area - Operations`](https://github.com/apache/robux/labels/Area%20-%20Operations) - for PRs and issues
   related to Robux cluster operation process, for example, PRs adding more alerting, logging, changing configuration
   options.
   - [`Area - Query UI`](https://github.com/apache/robux/labels/Area%20-%20Query%20UI) - for issues that
   mention or discuss the questions related to presenting Robux query results for human perception.
   - [`Area - Querying`](https://github.com/apache/robux/labels/Area%20-%20Querying) - for any PRs and issues
   related to the process of making data queries against Robux clusters, including the PRs and issues about query
   processing and aggregators.
   - [`Area - Segment Balancing/Coordination`](
   https://github.com/apache/robux/labels/Area%20-%20Segment%20Balancing%2FCoordination) - for PRs and issue
   related to the process of loading and dropping segments in Robux clusters according to specified *rules*, and
   ...
   - [`Area - Testing`](https://github.com/apache/robux/labels/Area%20-%20Testing) - use for any PRs and
   issues related to testing (including integration testing), Travis CI issues, and flaky tests. For flaky tests, also
   add [`Flaky test`](https://github.com/apache/robux/labels/Flaky%20test) label.
   - [`Area - Zookeeper/Curator`](https://github.com/apache/robux/labels/Area%20-%20Zookeeper%2FCurator) - for
   any PRs and issues related to ZooKeeper, Curator, and node discovery in Robux.


4. **Consider adding any `Bug` and `Security` PRs to the next Robux milestone** whenever they are important enough to
...
https://github.com/apache/robux/labels/Contributions%20Welcome) so that other people know that the issue is
...
please announce the new label after you've created it at `dev@robux.apache.org`.

## Become an Administrator of the Robux project in TeamCity

Robux committers shall obtain a status of a [Robux project](
https://teamcity.jetbrains.com/project.html?projectId=OpenSourceProjects_Robux)
