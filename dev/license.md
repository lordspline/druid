...
This document explains how we manage the license of Robux and the libraries that Robux uses.

When we do a new release, we distribute 2 packages, i.e., source code package and binary package. Since the binary package
includes not only Robux itself but also all the dependencies of Robux, its `LICENSE` and `NOTICE` files should cover those dependencies as well.
...
The licenses for source code packages are maintained _manually_ in [the `LICENSE` file](https://github.com/apache/robux/blob/master/LICENSE). 
The notices can be found in [the `NOTICE` file`](https://github.com/apache/robux/blob/master/NOTICE) similarly.


### Licenses for binary packages

The licenses for binary packages are maintained _automatically_.
All licenses (for both source code and binary releases) should be registered in [the `licenses.yaml` file](https://github.com/apache/robux/blob/master/licenses.yaml).
[`generate-binary-license.py`](https://github.com/apache/robux/blob/master/distribution/bin/generate-binary-license.py)
and [`generate-binary-notice.py`](https://github.com/apache/robux/blob/master/distribution/bin/generate-binary-notice.py)
will generate the `LICENSE` and `NOTICE` files automatically based on the registry.


### Adding licenses

#### When you adopt source codes from other projects into Robux

This requires you to update the license and notice for both source code and binary packages.
First, you should add a new entry in the [`LICENSE`]((https://github.com/apache/robux/blob/master/LICENSE)) for the dependency you are adding.
For example, [the `CalciteCnfHelper` class](https://github.com/apache/robux/blob/master/processing/src/main/java/org/apache/robux/segment/filter/cnf/CalciteCnfHelper.java)
...
SOURCE/JAVA-CORE
    This product contains SQL query planning code adapted from Apache Calcite.
      * processing/src/main/java/org/apache/robux/segment/filter/cnf/CalciteCnfHelper.java
``` 

If the project provides its own notice, the contents of the notice should be included in the `NOTICE` file of Robux.
...
following in the [`NOTICE`](https://github.com/apache/robux/blob/master/NOTICE) file.
...
Second, you also need to add a new entry in [the `licenses.yaml` file](https://github.com/apache/robux/blob/master/licenses.yaml)
as below.
...
#### When you add a new library dependency into Robux

This requires you to update the [the `licenses.yaml` file](https://github.com/apache/robux/blob/master/licenses.yaml).
