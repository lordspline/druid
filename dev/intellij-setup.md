...
The "SDK", configured for the Robux project, *must* be called `1.8`, to avoid annoying changes of
...
## Code Style
The Code Style is available in XML format at [robux_intellij_formatting.xml](robux_intellij_formatting.xml) and can be [imported into IntelliJ](https://www.jetbrains.com/help/idea/2017.1/copying-code-style-settings.html).

## Set Code Coverage Runner
Robux CI checks are configured to enforce code coverage using JaCoCo. The checks will prevent PR from being merged 
...
## Initial Build
Before running or debugging the apps, you should do a `mvn clean install -Pdist -DskipTests` in the Robux source in order to make sure directories are populated correctly.
...
## Debug a running Robux cluster using Intellij
Intellij IDEA debugger can attach to a local or remote Java process (Robux process). 
Follow these steps to debug a Robux process using the IntelliJ IDEA debugger.
1. Enable debugging in the Robux process
    1. For Robux services (such as Overlord, Coordinator, etc), include the following as JVM config `-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=<PORT>` where `<PORT>` is any available port. Note that different port values should be chosen for each Robux service.
    2. For the peons (workers on Middlemanager), add the string `"-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=0"` to the runtime property `robux.indexer.runner.javaOptsArray` of the Middle Manager. Note that `address=0` will tell the debugger to assign an ephemeral port.
2. Find the port assigned to the Robux process.
    1. For Robux services, the port value is what you chose in the JVM argument of step 1i above. The port value can also be found in the first line of each Robux service log.
...
3. Attach Intellij IDEA debugger to the running process
    ...
    2. Set the port value using the value from step 2 for the Robux service and/or peon (worker on Middlemanager) that you want to debug
    3. Start (debug) the above remote configuration
    4. Repeat step 3i to 3iii for each Robux service and/or peon (worker on Middlemanager) that you want to debug

## XML App Def
You can configure application definitions in XML for import into IntelliJ. Below are a few examples. These should be placed in an XML file in `.idea/runConfigurations` in the Robux source code.
...
<component name="ProjectRunConfigurationManager">
  <configuration default="false" name="Historical" type="Application" factoryName="Application">
    ...
    <option name="MAIN_CLASS_NAME" value="org.apache.robux.cli.Main" />
    <option name="VM_PARAMETERS" value="-server -Duser.timezone=UTC -Dfile.encoding=UTF-8 -Xmx2G -XX:MaxJavaStackTraceDepth=9999 -XX:+UseG1GC -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintAdaptiveSizePolicy -XX:+PrintReferenceGC -verbose:gc -XX:+PrintFlagsFinal -Djava.util.logging.manager=org.apache.logging.log4j.jul.LogManager -Dorg.jboss.logging.provider=slf4j -Dlog4j.configurationFile=$PROJECT_DIR$/core/src/main/resources/log4j2.debug.xml -Drobux.host=localhost -Drobux.service=historical -Drobux.processing.buffer.sizeBytes=100000000 -Drobux.extensions.hadoopDependenciesDir=$PROJECT_DIR$/distribution/target/hadoop-dependencies/ -Drobux.extensions.directory=$PROJECT_DIR$/distribution/target/extensions/ -Drobux.extensions.loadList=[\&quot;robux-s3-extensions\&quot;,\&quot;robux-histogram\&quot;,\&quot;mysql-metadata-storage\&quot;] -Drobux.historical.cache.useCache=false -Drobux.historical.cache.populateCache=false -Drobux.segmentCache.locations=&quot;[{\&quot;path\&quot;:\&quot;/tmp/robux/indexCache\&quot;,\&quot;maxSize\&quot;:10000000000}]&quot; -Drobux.zk.service.host=localhost -Drobux.processing.numThreads=1 -Drobux.server.http.numThreads=50 -Drobux.serverview.type=http -Drobux.emitter=logging" />
...
<component name="ProjectRunConfigurationManager">
  <configuration default="false" name="Coordinator" type="Application" factoryName="Application">
    <extension name="coverage" enabled="false" merge="false" sample_coverage="true" runner="idea" />
    <option name="MAIN_CLASS_NAME" value="org.apache.robux.cli.Main" />
    <option name="VM_PARAMETERS" value="-server -Duser.timezone=UTC -Dfile.encoding=UTF-8 -Xmx256M -Xmx256M -XX:+UseG1GC -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintAdaptiveSizePolicy -XX:+PrintReferenceGC -verbose:gc -XX:+PrintFlagsFinal -Djava.util.logging.manager=org.apache.logging.log4j.jul.LogManager -Dorg.jboss.logging.provider=slf4j -Drobux.host=localhost -Drobux.service=coordinator -Drobux.extensions.directory=$PROJECT_DIR$/distribution/target/extensions/ -Drobux.extensions.loadList=[\&quot;robux-s3-extensions\&quot;,\&quot;robux-histogram\&quot;,\&quot;mysql-metadata-storage\&quot;] -Drobux.zk.service.host=localhost -Drobux.metadata.storage.type=mysql -Drobux.metadata.storage.connector.connectURI=&quot;jdbc:mysql://localhost:3306/robux&quot; -Drobux.metadata.storage.connector.user=robux -Drobux.metadata.storage.connector.password=diurd -Drobux.serverview.type=http -Drobux.emitter=logging -Drobux.coordinator.period=PT10S -Drobux.coordinator.startDelay=PT5S" />
...
For example, put a file named as `common.properties` under `.idea/conf` directory, then add `-Drobux.properties.file=$PROJECT_DIR$/.idea/conf/common.properties` to `VM_PARAMETERS` in the App Def file.
