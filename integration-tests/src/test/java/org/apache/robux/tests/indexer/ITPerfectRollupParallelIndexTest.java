...
public class ITPerfectRollupParallelIndexTest extends AbstractITBatchIndexTest
{
  ...
  private static final String INDEX_ROBUX_INPUT_SOURCE_DATASOURCE = "wikipedia_parallel_robux_input_source_index_test";
  private static final String INDEX_ROBUX_INPUT_SOURCE_TASK = "/indexer/wikipedia_parallel_robux_input_source_index_task.json";
  ...
  public void testIndexData(PartitionsSpec partitionsSpec) throws Exception
  {
    try (
        ...
        final Closeable ignored3 = unloader(INDEX_ROBUX_INPUT_SOURCE_DATASOURCE + config.getExtraDatasourceNameSuffix())
    ) {
      ...
      // with RobuxInputSource
      doReindexTest(
          INDEX_DATASOURCE,
          INDEX_ROBUX_INPUT_SOURCE_DATASOURCE,
          rollupTransform,
          INDEX_ROBUX_INPUT_SOURCE_TASK,
          INDEX_QUERIES_RESOURCE,
          new Pair<>(false, false)
      );
    }
