...
public class ITSqlQueryTest
{
  ...
  )
  {
    ...
    executeWithRetry(StringUtils.format("%s/robux/v2/sql/", config.getBrokerUrl()), contentType, executable);

    // Send query to router to execute
    executeWithRetry(StringUtils.format("%s/robux/v2/sql/", config.getRouterUrl()), contentType, executable);
  }
