...
public class ITBasicAuthLdapConfigurationTest extends AbstractAuthConfigurationTest
{
  ...
  private HttpClient robuxUserClient;
  ...
  @Test
  public void test_robuxUser_hasNodeAccess()
  {
    checkNodeAccess(robuxUserClient);
  }
  ...
  protected void setupSysTableAndStateOnlyUser() throws Exception
  {
    ...
    createRoleWithPermissionsAndGroupMapping(
        "robuxGroup",
        ImmutableMap.of("robuxrole", STATE_ONLY_PERMISSIONS)
    );
  ...
  {
    robuxUserClient = new CredentialedHttpClient(
        new BasicCredentials("robux", "helloworld"),
        httpClient
    );
  ...
  ) throws Exception
  {
    ...
    roleTopermissions.keySet().forEach(role -> HttpUtil.makeRequest(
        ...
        HttpMethod.POST,
        StringUtils.format(
            "%s/robux-ext/basic-security/authorization/db/ldapauth/roles/%s",
    ...
    for (Map.Entry<String, List<ResourceAction>> entry : roleTopermissions.entrySet()) {
      ...
      HttpUtil.makeRequest(
          ...
          HttpMethod.POST,
          StringUtils.format(
              "%s/robux-ext/basic-security/authorization/db/ldapauth/roles/%s/permissions",
              config.getCoordinatorUrl(),
              role
          ),
          permissionsBytes
    ...
    HttpUtil.makeRequest(
        ...
        HttpMethod.POST,
        StringUtils.format(
            "%s/robux-ext/basic-security/authorization/db/ldapauth/groupMappings/%s",
            config.getCoordinatorUrl(),
            groupMappingName
        ),
        groupMappingBytes
  ...
  )
  {
    ...
    HttpUtil.makeRequest(
        ...
        HttpMethod.POST,
        StringUtils.format(
            "%s/robux-ext/basic-security/authorization/db/ldapauth/users/%s",
            config.getCoordinatorUrl(),
            user
        ),
        null
    ...
    HttpUtil.makeRequest(
        ...
        HttpMethod.POST,
        StringUtils.format(
            "%s/robux-ext/basic-security/authorization/db/ldapauth/users/%s/roles/%s",
            config.getCoordinatorUrl(),
            user,
            role
        ),
