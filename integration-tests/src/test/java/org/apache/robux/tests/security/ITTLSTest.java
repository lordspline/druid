...
public class ITTLSTest
{
  ...
  public void checkAccessWithCustomCertificateChecks()
  {
    ...
    checkFailedAccess(
        wrongHostnameClient,
        HttpMethod.POST,
        config.getCustomCertCheckRouterTLSUrl() + "/robux/v2",
        "Custom cert check",
        ISE.class,
        "Error while making request to url[https://127.0.0.1:9091/robux/v2] status[400 Bad Request] content[{\"error\":\"Unknown exception\",\"errorMessage\":\"No content to map due to end-of-input",
        true
    );

    makeRequest(wrongHostnameClient, HttpMethod.GET, config.getCustomCertCheckRouterTLSUrl() + "/robux/coordinator/v1/leader", null);
  }
