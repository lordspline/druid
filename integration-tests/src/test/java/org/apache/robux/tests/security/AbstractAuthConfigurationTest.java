...
import org.apache.robux.query.http.SqlTaskStatus;
...
import org.apache.robux.testing.utils.MsqTestQueryHelper;
import org.apache.robux.testing.utils.TestQueryHelper;
...
public abstract class AbstractAuthConfigurationTest
{
  ...
  protected enum User
  {
    ...
    INTERNAL_SYSTEM("robux_system", "warlock");
  ...
  public void test_systemSchemaAccess_datasourceOnlyUser() throws Exception
  {
    ...
    HttpUtil.makeRequest(
        ...
        HttpMethod.GET,
        config.getBrokerUrl() + "/robux/v2/datasources/auth_test",
        null
    );
  ...
  {
    HttpUtil.makeRequest(client, HttpMethod.GET, config.getCoordinatorUrl() + "/robux/coordinator/v1/loadqueue", null);
  }
  ...
  protected void checkLoadStatusSingle(HttpClient httpClient, String baseUrl) throws Exception
  {
    StatusResponseHolder holder = HttpUtil.makeRequest(
        httpClient,
        HttpMethod.GET,
        baseUrl + "/robux-ext/basic-security/authentication/loadStatus",
        null
    );
    ...
    holder = HttpUtil.makeRequest(
        httpClient,
        HttpMethod.GET,
        baseUrl + "/robux-ext/basic-security/authorization/loadStatus",
        null
    );
  ...
  {
    return makeSQLQueryRequest(httpClient, query, "/robux/v2/sql", ImmutableMap.of(), expectedStatus);
  }
  ...
  {
    return makeSQLQueryRequest(httpClient, query, "/robux/v2/sql", context, expectedStatus);
  }
  ...
  {
    return makeSQLQueryRequest(httpClient, query, "/robux/v2/sql/task", context, expectedStatus);
  }
  ...
  ) throws Exception
  {
    ...
    return makeSQLQueryRequest(httpClient, query, "/robux/v2/sql", dartContext, expectedStatus);
  }
  ...
  {
    return "jdbc:avatica:remote:url=" + config.getBrokerUrl() + RobuxAvaticaJsonHandler.AVATICA_PATH;
  }

  protected String getRouterAvacticaUrl()
  {
    return "jdbc:avatica:remote:url=" + config.getRouterUrl() + RobuxAvaticaJsonHandler.AVATICA_PATH;
  }
  ...
  protected void verifyAuthenticationInvalidAuthNameFails()
  {
    verifyInvalidAuthNameFails(StringUtils.format(
        "%s/robux-ext/basic-security/authentication/listen/%s",
  ...
  {
    verifyInvalidAuthNameFails(StringUtils.format(
        "%s/robux-ext/basic-security/authorization/listen/users/%s",
        config.getCoordinatorUrl(),
        INVALID_NAME
    ));
  }
  ...
  {
    verifyInvalidAuthNameFails(StringUtils.format(
        "%s/robux-ext/basic-security/authorization/listen/groupMappings/%s",
        config.getCoordinatorUrl(),
        INVALID_NAME
    ));
  }
