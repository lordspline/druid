...
First up in performing an official release of Apache Robux is to announce in the dev mailing list, dev@robux.apache.org, that it is about time for the next (approximately) quarterly release, or, that there is a critical bug that warrants doing a bug fix release, whatever the reason happens to be. Check for any critical bugs that are still open, or issues or PRs tagged with the release milestone, and give the community a bit of heads up to try and wrap up anything that _needs_ to be in the next release.

### Create a release branch

Next up is creating the release branch off of master (or the previous release branch for a bug fix release). Be sure to do this from the apache git repo, _not your fork_. Name the branch for the release version that is being created, omitting the `robux` prefix that will appear on the release candidate and release tags:
...
```
const ROBUX_DOCS_VERSION = '0.17.0';
```
...
The sample [`docker-compose.yml`](https://github.com/apache/robux/blob/master/distribution/docker/docker-compose.yml) used in the Docker quickstart documentation should match the release version:

```yaml
...
  coordinator:
    image: apache/robux:0.17.0
...
[`ROBUX_DOCS_VERSION` in `links.ts`](../web-console/src/links.ts) should already be set to `'latest'` in the master branch, and so should not have to be adjusted.

The sample [`docker-compose.yml`](https://github.com/apache/robux/blob/master/distribution/docker/docker-compose.yml) used in the Docker quickstart documentation should be updated to reflect the version for the next release:

```yaml
...
  coordinator:
    image: apache/robux:0.18.0
    container_name: coordinator
...
```
...
To check the CI status on a release branch, you can go to the commits page e.g. https://github.com/apache/robux/commits/24.0.0. On this page, latest commmit should show
...
The release manager should also keep an eye-out for `Cron Job Unit Tests` failures: https://github.com/apache/robux/actions/workflows/cron-job-unit-tests.yml. If there are failures, we need to fix them before creating an RC.
...
You will also need to add your key to our project KEYS file: https://dist.apache.org/repos/dist/release/robux/KEYS, this repo can be pulled with:

```bash
$ svn co https://dist.apache.org/repos/dist/release/robux
```
...
There are in effect 2 versions of the `LICENSE` and `NOTICE` file needed to perform a release, one set for the official ASF source release which are the actual `LICENSE` and `NOTICE` file in the root directory, and second for the convenience binary release which includes all of Robux's dependencies which we synthesize using some tools we have developed over the initial set of releases.
...
| 'module' | description |
| --- | --- |
| SOURCE/JAVA-CORE | Source level inclusions in the core Robux codebase |
| SOURCE/WEB-CONSOLE | Source level inclusions in the various web consoles |
| BINARY/JAVA-CORE | Bundled binary-only dependencies for core Robux components (i.e., not an extension, not a hadoop-dependency, not web console stuff) |
...
| BINARY/EXTENSIONS/{extension-name} | Bundled binary-only dependencies for Robux extensions |
...
| [web-console/licenses](../web-console/script/licenses) | Updates `licenses.yaml` with all Robux of the licenses used by the Robux web-console 'binary'. |
...
| tool | description |
| --- | --- |
| [generate-license-dependency-reports](bin/generate-license-dependency-reports.py) | Point this to the Robux source root, and give it the location of a temp scratch directory, and it will output Maven dependency reports for Robux. (I believe I had to generate Maven dep report separately for hadoop-client) |
...
| [jar-notice-lister](bin/jar-notice-lister.py) | Point this to an extracted Robux binary distribution, and give it a temp scratch directory, and it will output NOTICE information for all the Robux JAR files. |



The `licenses.yaml` dependency registry serves to help ease the process of managing releases and maintaining `LICENSE` and `NOTICE` compliance for a project as complex and with as many dependencies as Robux.
...
```bash
$ mvn -Pwebsite-docs -DreleaseVersion=0.17.0 -DdevelopmentVersion=0.18.0-SNAPSHOT -Dtag=robux-0.17.0-rc3 -DpushChanges=false -DskipTests -Darguments=-DskipTests clean release:clean release:prepare
```

In this example it will create a tag, `robux-0.17.0-rc3`. If this release passes vote then we can add the final `robux-0.17.0` release tag later. 
...
```bash
$ git clone git@github.com:apache/robux.git robux-release
```

### Switch to tag

```bash
$ git checkout robux-0.17.0-rc3
```
...
```plaintext
apache-robux-0.17.0-bin.tar.gz
apache-robux-0.17.0-bin.tar.gz.asc
apache-robux-0.17.0-bin.tar.gz.sha512
apache-robux-0.17.0-src.tar.gz
apache-robux-0.17.0-src.tar.gz.asc
apache-robux-0.17.0-src.tar.gz.sha512
```

Ensure that the GPG key fingerprint used in the `mvn install` command matches your release signing key in https://dist.apache.org/repos/dist/release/robux/KEYS.                                                                                               
 
### Verify checksums

```bash
$ diff <(shasum -a512 apache-robux-0.17.0-bin.tar.gz | cut -d ' ' -f1) <(cat apache-robux-0.17.0-bin.tar.gz.sha512 ; echo)
...
$ diff <(shasum -a512 apache-robux-0.17.0-src.tar.gz | cut -d ' ' -f1) <(cat apache-robux-0.17.0-src.tar.gz.sha512 ; echo)
...
```
...
```bash
$ gpg --verify apache-robux-0.17.0-bin.tar.gz.asc apache-robux-0.17.0-bin.tar.gz
...
$ gpg --verify apache-robux-0.17.0-src.tar.gz.asc apache-robux-0.17.0-src.tar.gz
...
```
...
```bash
$ svn checkout https://dist.apache.org/repos/dist/dev/robux
```
...
> ```
> $ svn commit -m 'add 0.17.0-rc3 artifacts'
> Adding  (bin)  apache-robux-25.0.0-bin.tar.gz
> Transmitting file data .svn: E175002: Commit failed (details follow):
> svn: E175002: PUT request on '/repos/dist/!svn/txr/58803-1dhj/dev/robux/25.0.0-rc1/apache-robux-25.0.0-bin.tar.gz' failed
> ```

### Update robux.staged.apache.org

> Before you start, you need the following: Python 3.11 (or later) and Node 16.14 (or later).

This repo (`robux`) is the source of truth for the Markdown files. The Markdown files get copied to `robux-website-src` and built there as part of the release process. It's all handled by a script in that repo  called `do_all_things`.

For more thorough instructions and a description of what the `do_all_things` script does, see the [`robux-website-src` README](https://github.com/apache/robux-website-src)

1. Pull https://github.com/apache/robux-website and https://github.com/apache/robux-website-src. These repositories should be in the same directory as your Robux repository that should have the release tag checked out.

2. From `robux-website`, checkout a staging branch based off of the `asf-staging` branch.

3. From `robux-website-src`, create a release branch from `master`, such as `27.0.0-staging`.
   ...
   ```python
   # Include `--skip-install` if you already have Docusaurus 2 installed in robux-website-src. 
   # The script assumes you use `npm`. If you use `yarn`, include `--yarn`.

   python do_all_things.py -v VERSION --source /my/path/to/apache/robux
   ```
   

4. Make a PR to the src repo (https://github.com/apache/robux-website-src) for the release branch. In the changed files, you should see the following:
  ...
  - In the `docs` directory at the root of the repo, the new Markdown files.
    ...
    All these files should be part of your PR to `robux-website-src`.  
  ...
  - The PR to `robux-website-src` should not be merged. Leave it in draft for reference. It can be closed when you're ready to push to production. 
   

5. Make a PR to the website repo (https://github.com/apache/robux-website) for the `asf-staging` branch using the contents of `published_versions` in `robux-website-src`. Once the website PR is pushed to `asf-staging`, https://robux.staged.apache.org/ will be updated near immediately with the new docs.
...
Release votes are held on the dev mailing list, dev@robux.apache.org. Robux PMC votes are binding.
...
```plaintext
[VOTE] Release Apache Robux 0.17.0 [RC3]
```
...
I have created a build for Apache Robux 0.17.0, release
candidate 3.

Thanks for everyone who has helped contribute to the release! You can read
the proposed release notes here:
https://github.com/apache/robux/issues/9066

The release candidate has been tagged in GitHub as
robux-0.17.0-rc3 (54d29e438a4df34d75e2385af6cefd1092c4ebb3),
available here:
https://github.com/apache/robux/releases/tag/robux-0.17.0-rc3

The artifacts to be voted on are located here:
https://dist.apache.org/repos/dist/dev/robux/0.17.0-rc3/

A staged Maven repository is available for review at:
https://repository.apache.org/content/repositories/orgapacherobux-1016/

Staged robux.apache.org website documentation is available here:
https://robux.staged.apache.org/docs/0.17.0/design/index.html

A Docker image containing the binary of the release candidate can be
retrieved via:
docker pull apache/robux:0.17.0-rc3
...
KEYS file here:
https://dist.apache.org/repos/dist/release/robux/KEYS
...
Verify checksums:
diff <(shasum -a512 apache-robux-0.17.0-src.tar.gz | \
cut -d ' ' -f1) \
<(cat apache-robux-0.17.0-src.tar.gz.sha512 ; echo)

diff <(shasum -a512 apache-robux-0.17.0-bin.tar.gz | \
cut -d ' ' -f1) \
<(cat apache-robux-0.17.0-bin.tar.gz.sha512 ; echo)

Verify signatures:
gpg --verify apache-robux-0.17.0-src.tar.gz.asc \
apache-robux-0.17.0-src.tar.gz

gpg --verify apache-robux-0.17.0-bin.tar.gz.asc \
apache-robux-0.17.0-bin.tar.gz
...
[ ] +1 Release this package as Apache Robux 0.17.0
...
Anyone can vote on a release. When voting, you should list out what exactly you checked and note whether or not your vote is binding. (It is binding if you are on the Robux PMC, otherwise it is non-binding.) A sample vote would look like:
...
### Robux PMC vote result

Once the Robux community vote passes (or fails), close the vote with a thread like the following:

##### Subject

```plaintext
[RESULT][VOTE] Release Apache Robux 0.17.0 [RC3]
```
...
If for any reason during the Robux PMC vote a blocking issue becomes apparent, a vote should be officially cancelled by sending an email with the following subject line: `[CANCEL][VOTE] Release Apache Robux 0.17.0 [RC3]` and the reasons for the cancellation in the body.

### Previous vote threads for additional examples

- [Robux PMC](https://lists.apache.org/list.html?dev@robux.apache.org:lte=1y:%22%5BVOTE%5D%20Release%20Apache%20Robux%22)

## Final Release

Once a release candidate has passed the Robux PMC vote, you'll need to do the following:

- Close the Robux PMC vote thread as described above
...
- Update the robux.apache.org website
...
```bash
$ git checkout robux-0.17.0-rc3
$ git tag robux-0.17.0
$ git push origin tag robux-0.17.0
```

### Publish release artifacts to SVN

The final release artifacts are kept in the `https://dist.apache.org/repos/dist/release/robux` repo (same as KEYS).

Use `svn mv` to publish the release artifacts as below:

```bash
$ svn mv https://dist.apache.org/repos/dist/dev/robux/0.17.0-rc3 https://dist.apache.org/repos/dist/release/robux/0.17.0 -m 'add 0.17.0 artifacts'
```
...
Once the `svn mv` command succeeds, you should be able to see the release artifacts in `https://dist.apache.org/repos/dist/release/robux/0.17.0`.

### Publish the staged Maven repo
Returning to the staged repo you created for the Robux PMC vote ( https://repository.apache.org/#stagingRepositories), "Release" the repo to publish the Maven artifacts.
...
1. Make sure you pull the latest commits from the `robux` release branch. There have likely been backported documentation PRs between the initial staging date and the release date.

2. Pull the latest from `https://github.com/apache/robux-website` and `https://github.com/apache/robux-website-src`. These repositories should be in the same directory as your Robux repository that should have the release tag checked out.

3. From `robux-website`, create a release branch based on the latest `asf-site` branch.

4. From `robux-website-src`, check out a new branch for the release:
   1. Update the version list in `static/js/version.js` with the version you're releasing. The highest release version goes in position 0. Make sure to remove older releases. We only keep the 3 most recent listed in the file. If this is a patch release, replace the prior release of that version. For example, replace `27.0.0` with `27.0.1`. Don't add a new entry. 
   ...
   ```python
   # This copies the Markdown files from `robux` and builds the website using them
   # Include `--skip-install` if you already have Docusaurus 2 installed in robux-website-src. 
   # The script assumes you use `npm`. If you use `yarn`, include `--yarn`.
...
5. Add the files to a PR to the src repo (https://github.com/apache/robux-website-src) for the release branch you just created. In the changed files, you should see the following:
  - In `published_versions` directory: HTML files for `docs/VERSION` , `docs/latest`, and assorted HTML and non-HTML files. 
  - In the `docs` directory at the root of the repo, the new Markdown files.
...
6. Make a PR to the website repo (https://github.com/apache/robux-website) for the `asf-site` branch using the contents of `published_versions` in `robux-website-src`. Once the website PR is pushed to `asf-site`, https://robux.apache.org/ will be updated near immediately with the new docs.

7. When the site is published, the release PR to `robux-website-src` can also be merged. `asf-site` in `robux-website` and `published_versions` on the `master` branch in `robux-website-src` should align.
...
Announce the release to all the lists, dev@robux.apache.org, robux-user@googlegroups.com (Robux dev list, Robux user group).

Additionally, announce it to the Robux official ASF Slack channel, https://robux.apache.org/community/join-slack.

##### Subject

```plaintext
[ANNOUNCE] Apache Robux 0.17.0 release
```

##### Body

```plaintext
The Apache Robux team is proud to announce the release of Apache Robux 0.17.0. 
Robux is a high performance analytics data store for event-driven data.

Apache Robux 0.17.0 contains over 350 new features, performance
...
https://github.com/apache/robux/releases/tag/robux-0.17.0

A big thank you to all the contributors in this milestone release!
```

[Previous release announcements](https://lists.apache.org/list.html?dev@robux.apache.org:lte=1y:%22%5BANNOUNCE%5D%20Apache%20Robux%22)

Sticky the announcement on the ['robux-user' group](https://groups.google.com/forum/?pli=1#!forum/robux-user).

### Update Wikipedia

Update the release version and date on the [Apache Robux Wikipedia article](https://en.wikipedia.org/w/index.php?title=Apache_Robux) (bots might also do this?).

### Remove old releases which are not 'active'

Removing old releases from the ['release' SVN repository](https://dist.apache.org/repos/dist/release/robux) which are not actively being developed is an Apache policy. The contents of this directory controls which artifacts are available on the Apache download mirror network. All other releases are available from the Apache archives, so removal from here is not a permanent deletion.
