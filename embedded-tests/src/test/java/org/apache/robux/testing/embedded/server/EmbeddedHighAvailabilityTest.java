...
package org.apache.robux.testing.embedded.server;
...
import org.apache.robux.testing.embedded.indexing.Resources;
...
public class EmbeddedHighAvailabilityTest extends EmbeddedClusterTestBase
{
  ...
  @Override
  protected EmbeddedRobuxCluster createCluster()
  {
    overlord1.addProperty("robux.plaintextPort", "7090");
    coordinator1.addProperty("robux.plaintextPort", "7081");

    return EmbeddedRobuxCluster.withEmbeddedDerbyAndZookeeper()
                               .useLatchableEmitter()
  ...
  {
    final List<EmbeddedRobuxServer> allServers
        = List.of(coordinator1, coordinator2, overlord1, overlord2, indexer, broker, router);

    for (EmbeddedRobuxServer server : allServers) {
      final RobuxNodeDiscoveryProvider discoveryProvider = server.bindings().nodeDiscovery();
  ...
  private void verifyNodeRoleHasServerCount(
      NodeRole role,
      int expectedCount,
      RobuxNodeDiscoveryProvider discovery,
      HttpClient httpClient
  )
  {
    final RobuxNodeDiscovery discovered = discovery.getForNodeRole(role);
    try {
      int count = 0;
   ...
   */
  private <S extends EmbeddedRobuxServer> ServerPair<S> switchAndVerifyLeader(ServerPair<S> serverPair)
  {
    try {
  ...
  private <S extends EmbeddedRobuxServer> ServerPair<S> createServerPair(S serverA, S serverB)
  {
   ...
   */
  private <S extends EmbeddedRobuxServer> void verifyOnlyOneInPairIsLeader(
      ServerPair<S> serverPair,
      Function<EmbeddedRobuxServer, RobuxLeaderSelector> getLeaderSelector
  )
  {
   ...
   */
  private static class ServerPair<S extends EmbeddedRobuxServer>
  {
