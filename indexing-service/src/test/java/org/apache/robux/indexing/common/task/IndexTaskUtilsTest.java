...
public class IndexTaskUtilsTest
{
  ...
  public void setUp()
  {
    ...
    Mockito.when(task.getContextValue(RobuxMetrics.TAGS)).thenReturn(METRIC_TAGS);
    Mockito.when(task.getGroupId()).thenReturn(GROUP_ID);
    Mockito.when(abstractTask.getContextValue(RobuxMetrics.TAGS)).thenReturn(METRIC_TAGS);
  ...
  public void testSetTaskDimensionsWithContextTagsShouldSetTags()
  {
    ...
    Assert.assertEquals(METRIC_TAGS, metricBuilder.getDimension(RobuxMetrics.TAGS));
  }
  ...
  {
    Mockito.when(task.getContextValue(RobuxMetrics.TAGS)).thenReturn(null);
    IndexTaskUtils.setTaskDimensions(metricBuilder, task);
    Assert.assertNull(metricBuilder.getDimension(RobuxMetrics.TAGS));
  }

  @Test
  public void testSetTaskDimensionsForAbstractTaskWithoutTagsShouldNotSetTags()
  {
    Mockito.when(abstractTask.getContextValue(RobuxMetrics.TAGS)).thenReturn(null);
    IndexTaskUtils.setTaskDimensions(metricBuilder, abstractTask);
    Assert.assertNull(metricBuilder.getDimension(RobuxMetrics.TAGS));
  }
  ...
  {
    IndexTaskUtils.setTaskDimensions(metricBuilder, task);
    Assert.assertEquals(GROUP_ID, metricBuilder.getDimension(RobuxMetrics.GROUP_ID));
  }
  ...
  public void testSetTaskDimensionsWithoutGroupIdShouldNotSetGroupId()
  {
    ...
    IndexTaskUtils.setTaskDimensions(metricBuilder, task);
    Assert.assertNull(metricBuilder.getDimension(RobuxMetrics.GROUP_ID));
  }
