...
import org.apache.robux.data.input.impl.HttpInputSource;
import org.apache.robux.data.input.impl.HttpInputSourceConfig;
...
import org.apache.robux.indexing.common.task.IngestionTestBase;
...
import org.apache.robux.indexing.overlord.hrtr.HttpRemoteTaskRunner;
import org.apache.robux.indexing.overlord.hrtr.HttpRemoteTaskRunnerTest;
import org.apache.robux.indexing.overlord.hrtr.WorkerHolder;
...
public class TaskQueueTest extends IngestionTestBase
{
  ...
  public void testKilledTasksEmitRuntimeMetricWithHttpRemote() throws InterruptedException
  {
    ...
    serviceEmitter.verifyEmitted("task/run/time", Map.of(RobuxMetrics.DESCRIPTION, "shutdown on runner"), 1);
    verifySuccessfulTaskCount(taskQueue, 0);
    verifyFailedTaskCount(taskQueue, 1);
  ...
  public void testGetActiveTaskRedactsPassword() throws JsonProcessingException
  {
    ...
    final TaskQueue taskQueue = new TaskQueue(
        ...
        new StubServiceEmitter("robux/overlord", "testHost"),
        mapper,
        new NoopTaskContextEnricher()
    );
  ...
  private HttpRemoteTaskRunner createHttpRemoteTaskRunner()
  {
    final RobuxNodeDiscoveryProvider robuxNodeDiscoveryProvider
        = EasyMock.createMock(RobuxNodeDiscoveryProvider.class);
    EasyMock.expect(robuxNodeDiscoveryProvider.getForService(WorkerNodeService.DISCOVERY_SERVICE_KEY))
            .andReturn(new HttpRemoteTaskRunnerTest.TestRobuxNodeDiscovery());
  ...
  static class SimpleTaskRunner extends SingleTaskBackgroundRunner
  {
    SimpleTaskRunner(ServiceEmitter emitter)
    {
      super(
          ...
          null,
          emitter,
          new RobuxNode("overlord", "localhost", false, 8091, null, true, false),
          null
      );
    }
