...
import org.apache.robux.indexing.overlord.supervisor.CompactionSupervisorManager;
import org.apache.robux.indexing.overlord.supervisor.SupervisorManager;
import org.apache.robux.indexing.overlord.supervisor.VersionedSupervisorSpec;
...
import org.apache.robux.server.coordinator.DataSourceCompactionConfigAuditEntry;
...
public class OverlordCompactionResourceTest
{
  ...
  public void setUp()
  {
    ...
    EasyMock.expect(authorizerMapper.getAuthorizer("robux")).andReturn(new AllowAllAuthorizer()).anyTimes();
  ...
  private void verifyInvalidInputResponse(Response response, String message)
  {
    ...
    MatcherAssert.assertThat(
        ...
        RobuxExceptionMatcher.invalidInput().expectMessageIs(message)
    );
  }

  private void setupMockRequestForUser(String user)
  {
    EasyMock.expect(httpRequest.getAttribute(AuthConfig.ROBUX_ALLOW_UNSECURED_PATH)).andReturn(null).atLeastOnce();
    EasyMock.expect(httpRequest.getAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED)).andReturn(null).atLeastOnce();
    EasyMock.expect(httpRequest.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT))
            .andReturn(new AuthenticationResult(user, "robux", null, null))
            .atLeastOnce();
    httpRequest.setAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED, true);
    EasyMock.expectLastCall().anyTimes();
  }

  private void setupMockRequestForAudit()
  {
    EasyMock.expect(httpRequest.getHeader(AuditManager.X_ROBUX_AUTHOR)).andReturn("author").once();
    EasyMock.expect(httpRequest.getHeader(AuditManager.X_ROBUX_COMMENT)).andReturn("comment").once();

    EasyMock.expect(httpRequest.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT))
            .andReturn(new AuthenticationResult("robux", "robux", null, null))
            .atLeastOnce();
