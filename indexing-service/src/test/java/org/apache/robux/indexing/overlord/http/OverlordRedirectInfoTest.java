...
public class OverlordRedirectInfoTest
{
  ...
  public void testDoLocalWhenNotLeading()
  {
    ...
    Assert.assertTrue(redirectInfo.doLocal("/robux/indexer/v1/leader"));
    Assert.assertTrue(redirectInfo.doLocal("/robux/indexer/v1/isLeader"));
    Assert.assertFalse(redirectInfo.doLocal("/robux/indexer/v1/other/path"));
    EasyMock.verify(overlord);
  }
  ...
  {
    String host = "http://localhost";
    String request = "/robux/indexer/v1/task/" + URLEncoder.encode(
    ...
    Assert.assertEquals(
        "http://localhost/robux/indexer/v1/task/index_hadoop_datasource_2017-07-12T07%3A43%3A01.495Z/status",
        url.toString()
    );
    EasyMock.verify(overlord);
  }
