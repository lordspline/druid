...
package org.apache.robux.indexing.overlord.supervisor;
...
import org.apache.robux.indexing.seekablestream.TestSeekableStreamDataSourceMetadata;
...
public class SupervisorResourceTest extends EasyMockSupport
{
        ...
        new AuthorizerMapper(null)
        {
          ...
          public Authorizer getAuthorizer(String name)
          {
            return (authenticationResult, resource, action) -> {
              if (authenticationResult.getIdentity().equals("robux")) {
                return Access.OK;
              } else {
  ...
  public void testSpecPostWithInputSourceSecurityEnabledUnauthorized()
  {
    ...
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_ALLOW_UNSECURED_PATH)).andReturn(null).atLeastOnce();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED)).andReturn(null).atLeastOnce();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT)).andReturn(
        new AuthenticationResult("notrobux", "robux", null, null)
    ).atLeastOnce();
    request.setAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED, false);
  ...
  public void testSuspendAllWithPartialAuthorization()
  {
    ...
    setupMockRequestForUser("notRobux");
    replayAll();
  ...
  private void setupMockRequestForUser(String user)
  {
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_ALLOW_UNSECURED_PATH)).andReturn(null).atLeastOnce();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED)).andReturn(null).atLeastOnce();
    EasyMock.expect(request.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT))
  ...
  {
    EasyMock.expect(request.getHeader(AuditManager.X_ROBUX_AUTHOR)).andReturn("author").once();
    EasyMock.expect(request.getHeader(AuditManager.X_ROBUX_COMMENT)).andReturn("comment").once();
