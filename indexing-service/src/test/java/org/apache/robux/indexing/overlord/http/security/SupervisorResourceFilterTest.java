...
package org.apache.robux.indexing.overlord.http.security;
...
public class SupervisorResourceFilterTest
{
  ...
  {
    setExpectations("/robux/indexer/v1/supervisor/datasource1", "GET", "datasource1", Action.READ, true);
  ...
  {
    setExpectations("/robux/indexer/v1/supervisor/datasource1", "GET", "datasource1", Action.READ, false);
  ...
  {
    setExpectations("/robux/indexer/v1/supervisor/datasource1", "POST", "datasource1", Action.WRITE, true);
    ContainerRequest filteredRequest = resourceFilter.filter(containerRequest);
    Assert.assertNotNull(filteredRequest);
    verifyMocks();
  }
  ...
  {
    setExpectations("/robux/indexer/v1/supervisor/datasource1", "POST", "datasource1", Action.WRITE, false);
  ...
  public void testSupervisorNotFound()
  {
    ...
    expect(containerRequest.getPathSegments())
        .andReturn(getPathSegments("/robux/indexer/v1/supervisor/" + dataSource))
        .anyTimes();
  ...
  )
  {
    ...
    expect(servletRequest.getAttribute(AuthConfig.ROBUX_ALLOW_UNSECURED_PATH))
        .andReturn(null).anyTimes();
    expect(servletRequest.getAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED))
        .andReturn(null).anyTimes();
    ...
    expect(servletRequest.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT))
