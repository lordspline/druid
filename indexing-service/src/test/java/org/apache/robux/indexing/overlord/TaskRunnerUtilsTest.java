...
public class TaskRunnerUtilsTest
{
  ...
  public void testMakeWorkerURL()
  {
    final URL url = TaskRunnerUtils.makeWorkerURL(
        ...
        "/robux/worker/v1/task/%s/log",
        "foo bar&"
    );
    Assert.assertEquals("https://1.2.3.4:8290/robux/worker/v1/task/foo%20bar%26/log", url.toString());
    Assert.assertEquals("1.2.3.4:8290", url.getAuthority());
    Assert.assertEquals("/robux/worker/v1/task/foo%20bar%26/log", url.getPath());
  }
