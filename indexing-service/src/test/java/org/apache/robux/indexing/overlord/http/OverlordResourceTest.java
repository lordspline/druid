...
package org.apache.robux.indexing.overlord.http;
...
import org.apache.robux.indexer.TaskInfo;
...
import org.apache.robux.indexing.common.TimeChunkLock;
...
import org.apache.robux.indexing.overlord.WorkerTaskRunnerQueryAdapter;
...
import org.apache.robux.metadata.TaskLookup;
import org.apache.robux.metadata.TaskLookup.ActiveTaskLookup;
import org.apache.robux.metadata.TaskLookup.CompleteTaskLookup;
import org.apache.robux.metadata.TaskLookup.TaskLookupType;
...
public class OverlordResourceTest
{
  ...
  public void setUp()
  {
    ...
    overlord = EasyMock.createStrictMock(RobuxOverlord.class);
  ...
  public void testKillTaskIsAudited()
  {
    ...
    final String username = Users.ROBUX;
    ...
    EasyMock.expect(req.getHeader(AuditManager.X_ROBUX_AUTHOR)).andReturn(username).once();
    EasyMock.expect(req.getHeader(AuditManager.X_ROBUX_COMMENT)).andReturn("killing segments").once();
    EasyMock.expect(req.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT))
            .andReturn(new AuthenticationResult(username, "robux", null, null))
            .once();
    ...
    Assert.assertEquals("robux", auditEntry.getAuditInfo().getIdentity());
  ...
  @Test
  public void testKillPendingSegmentsThrowsInvalidInputRobuxException()
  {
    expectAuthorizationTokenCheck();
  ...
  @Test
  public void testKillPendingSegmentsThrowsDefensiveRobuxException()
  {
    expectAuthorizationTokenCheck();
    ...
    EasyMock
        ...
        .andThrow(RobuxException.defensive(exceptionMsg))
        .once();
  ...
  public void testGetTaskPayload() throws Exception
  {
    ...
    // This should be fixed in https://github.com/apache/robux/issues/6685.
    // expectAuthorizationTokenCheck();
  ...
  {
    expectAuthorizationTokenCheck(Users.ROBUX);
  }

  private void expectAuthorizationTokenCheck(String username)
  {
    AuthenticationResult authenticationResult = new AuthenticationResult(username, "robux", null, null);
    EasyMock.expect(req.getAttribute(AuthConfig.ROBUX_ALLOW_UNSECURED_PATH)).andReturn(null).anyTimes();
    EasyMock.expect(req.getAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED)).andReturn(null).atLeastOnce();
    EasyMock.expect(req.getAttribute(AuthConfig.ROBUX_AUTHENTICATION_RESULT))
            .andReturn(authenticationResult)
            .atLeastOnce();

    req.setAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED, false);
    EasyMock.expectLastCall().anyTimes();

    req.setAttribute(AuthConfig.ROBUX_AUTHORIZATION_CHECKED, true);
    EasyMock.expectLastCall().anyTimes();
  }
  ...
  {
    private static final String ROBUX = "robux";
