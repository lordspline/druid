...
import org.apache.robux.indexing.overlord.http.security.SupervisorResourceFilter;
 ...
 */
@Path("/robux/indexer/v1/supervisor")
            ...
            List<SupervisorStatus> allStates = authorizedSupervisorIds
                ...
                .map(x -> {
                  ...
                  if (theSpec.isPresent()) {
                    ...
                    if (includeSystem) {
                      try {
                        // serializing SupervisorSpec here, so that callers of `robux/indexer/v1/supervisor?system`
