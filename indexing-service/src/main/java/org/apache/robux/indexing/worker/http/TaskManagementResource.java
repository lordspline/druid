...
import org.apache.robux.indexing.worker.WorkerHistoryItem;
...
public class TaskManagementResource
{
   ...
   *
   * (1) Client sends first request /robux/internal/v1/worker?counter=-1&timeout=<timeout>
   * Server responds with current list of running/completed tasks and metadata. And, a <counter,hash> pair.
   *
   * (2) Client sends subsequent requests /robux/internal/v1/worker?counter=<counter>&hash=<hash>&timeout=<timeout>
  ...
  public Response assignTask(Task task)
  {
    ...
    // Ref: https://github.com/apache/robux/pull/15412.
    try {
